/* =========================================================================
   NEXA MESSENGER — client script
   ========================================================================= */

let currentUser = null;
let selectedUser = null;
let messageRepliedTo = null;
let editingMessageId = null;
let deferredPrompt = window.deferredPrompt || null;
window.addEventListener('beforeinstallprompt', (e) => {
  e.preventDefault();
  deferredPrompt = e;
  window.deferredPrompt = e;
  console.log("📱 Nexa PWA automatic install prompt captured");
});

let profileSetupRequired = false;

const CLOUD_NAME = "doasf3d1u";
const UPLOAD_PRESET = "NeuroStack";
const BACKEND_URL = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1"
  ? "http://localhost:3000"
  : "https://nexa-backend-e6pq.onrender.com";

let selectedMusicData = null;
let musicDuration = 30;
let statusTab = 'image';
let statusFile = null;
let storyAudioElement = null;

/// Story pause/resume handling
let typingResumeTimer = null;
function pauseStory() {
  if (window.storySlideTimer) clearInterval(window.storySlideTimer);
  if (storyAudioElement && !storyAudioElement.paused) storyAudioElement.pause();
}
function resumeStory() {
  if (window.storySlideTimer) {
    clearInterval(window.storySlideTimer);
    window.storySlideTimer = setInterval(nextStorySlide, 5000);
  }
  if (storyAudioElement && storyAudioElement.paused) storyAudioElement.play().catch(() => {});
}
function handleStoryPauseOnTyping() {
  pauseStory();
}
function handleStoryResumeOnTyping() {
  clearTimeout(typingResumeTimer);
  typingResumeTimer = setTimeout(resumeStory, 800);
}
// Profile picture modal
function openProfilePic(src) {
  const modal = document.getElementById('profilePicModal');
  const img = document.getElementById('modalProfileImg');
  img.src = src;
  modal.classList.add('active');
}
function closeProfilePicModal() {
  document.getElementById('profilePicModal').classList.remove('active');
}

console.log('🚀 Nexa Messenger Starting...');

firebase.initializeApp({
  apiKey: "AIzaSyBczCrV1vpJB_0DTsbBDupTXQuEV7l5IDg",
  authDomain: "mel-odix.firebaseapp.com",
  projectId: "mel-odix",
  storageBucket: "mel-odix.firebasestorage.app",
  messagingSenderId: "217595352090",
  appId: "1:217595352090:web:56696fd53ae5bb59d2eda3"
});

const auth = firebase.auth();
const db = firebase.firestore();

// Expose globals for external modules (nexa-voice-room.js) and leaderboard.
window.db = db;
window.auth = auth;
window.isUserOnline = isUserOnline;
window.allUsersData = [];
window.userPresenceCache = {};
// Expose FieldValue so nexa-voice-room.js can use arrayUnion/arrayRemove without
// importing firebase directly (it only has access to window.db).
window.db.fieldValue = firebase.firestore.FieldValue;

function escapeHtml(str) {
  if (!str) return "";
  return String(str).replace(/[&<>"']/g, c => ({
    '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;'
  }[c]));
}

// Sanitize a media URL before interpolating it into HTML markup (src="…",
// style url("…"), inline handlers). Only http(s) and local blob:/data: media
// URLs are allowed — javascript:/vbscript: are dropped, and quote/backslash
// characters are stripped so the URL can't break out of its attribute.
function safeMediaUrl(url) {
  if (!url || typeof url !== "string") return "";
  const u = url.trim();
  if (/^(https?:|blob:|data:(image|video|audio)\/)/i.test(u)) {
    return u.replace(/["'`\\]/g, "");
  }
  return "";
}

function linkify(text) {
  if (!text) return "";
  // Matches, in order: full URLs (http/https), bare www. links, and bare
  // domain-with-known-TLD links (with optional subdomains + path). Operates on
  // RAW text so & < > are escaped once, at insertion time — no double-escape.
  // Quotes are excluded from matches so they can never break out of href.
  const urlRe = /((?:https?:\/\/|www\.)[^\s<>"']+|[\w-]+(?:\.[\w-]+){0,3}\.(?:com|net|org|io|co|me|dev|app|link|xyz|info|biz|tv)(?:\.[a-z]{2,3})?(?:\/[^\s<>"']*)?)/gi;

  let out = "";
  let last = 0;
  let m;
  let guard = 0;
  while ((m = urlRe.exec(text)) !== null && guard++ < 500) {
    const pre = text.slice(last, m.index);
    const raw = m[0];

    // Skip emails (foo@example.com) and mid-word matches (e.g. "abc.example"):
    // keep the original text verbatim (don't drop the prefix or double-link).
    const prevChar = m.index > 0 ? text[m.index - 1] : "";
    if (prevChar === "@" || prevChar === "." || /[\w-]/.test(prevChar)) {
      out += escapeHtml(pre + raw);
      last = m.index + raw.length;
      continue;
    }

    let href = raw;
    if (!/^https?:\/\//i.test(raw)) href = "https://" + raw;

    // Strip trailing punctuation / stray closing chars from href + label.
    const cleaned = href.replace(/[),.;:!?]+$/g, "");

    out += escapeHtml(pre);
    out += `<a class="nexa-link" href="${escapeHtml(cleaned)}" target="_blank" rel="noopener noreferrer">${escapeHtml(cleaned.replace(/^https?:\/\//i, "").replace(/^www\./i, ""))}</a>`;
    last = m.index + raw.length;
  }
  out += escapeHtml(text.slice(last));
  return out;
}

// Human-readable file size for attachments.
function formatFileSize(bytes) {
  if (!bytes && bytes !== 0) return "";
  if (bytes < 1024) return bytes + " B";
  if (bytes < 1024 * 1024) return (bytes / 1024).toFixed(1) + " KB";
  if (bytes < 1024 * 1024 * 1024) return (bytes / (1024 * 1024)).toFixed(1) + " MB";
  return (bytes / (1024 * 1024 * 1024)).toFixed(1) + " GB";
}

// Pick an emoji icon for an attachment based on its extension.
function fileIconFor(name) {
  const ext = (name.split(".").pop() || "").toLowerCase();
  if (["jpg", "jpeg", "png", "gif", "webp", "bmp", "svg"].includes(ext)) return "🖼";
  if (["mp4", "mov", "webm", "mkv", "avi"].includes(ext)) return "🎥";
  if (["mp3", "wav", "ogg", "m4a", "aac"].includes(ext)) return "🎵";
  if (ext === "pdf") return "📕";
  if (["doc", "docx", "txt", "rtf"].includes(ext)) return "📄";
  if (["xls", "xlsx", "csv"].includes(ext)) return "📊";
  if (["ppt", "pptx"].includes(ext)) return "📽";
  if (["zip", "rar", "7z", "tar", "gz"].includes(ext)) return "🗜";
  return "📎";
}



async function showProfileSetupModal() {
  document.getElementById("profileSetupOverlay").classList.add("active");
  document.getElementById("setupNameInput").focus();
}

async function completeProfileSetup() {
  const nameInput = document.getElementById("setupNameInput");
  const displayName = nameInput.value.trim();

  if (!displayName || displayName.length < 2) {
    showNotifToast("Please enter a valid display name", "error");
    return;
  }

  if (!currentUser) {
    showNotifToast("User not authenticated", "error");
    return;
  }

  try {
    await currentUser.updateProfile({ displayName: displayName });
    await db.collection("users").doc(currentUser.uid).set({
      uid: currentUser.uid,
      email: currentUser.email,
      displayName: displayName,
      photoUrl: currentUser.photoURL || "https://i.imgur.com/HeIi0wU.png",
      createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      profileSetupComplete: true
    }, { merge: true });

    document.getElementById("myName").textContent = displayName;
    document.getElementById("profileSetupOverlay").classList.remove("active");
    profileSetupRequired = false;
    showNotifToast("✓ Profile setup complete!", "success");
  } catch (error) {
    console.error("Error completing profile setup:", error);
    showNotifToast("Error saving profile: " + error.message, "error");
  }
}

async function loadProfile() {
  if (!currentUser) return;

  try {
    const doc = await db.collection("users").doc(currentUser.uid).get();
    const data = doc.data();

    let displayName = null;
    if (data?.displayName && data.displayName.trim() && data.displayName !== "User") {
      displayName = data.displayName;
    } else if (currentUser.displayName && currentUser.displayName.trim()) {
      displayName = currentUser.displayName;
    } else {
      profileSetupRequired = true;
      showProfileSetupModal();
      return;
    }

    const photoUrl = data?.photoUrl || currentUser.photoURL || "https://i.imgur.com/HeIi0wU.png";

    if (!data || !data.displayName || data.displayName === "User") {
      await db.collection("users").doc(currentUser.uid).set({
        uid: currentUser.uid,
        email: currentUser.email,
        displayName: displayName,
        photoUrl: photoUrl,
        createdAt: firebase.firestore.FieldValue.serverTimestamp(),
        profileSetupComplete: true
      }, { merge: true });
    }

    document.getElementById("myName").textContent = displayName;
    document.getElementById("myPic").src = photoUrl;
    renderProfileTab();

    // Cache own profile so the next open paints the header before auth resolves.
    try {
      localStorage.setItem("nexa_my_profile_" + currentUser.uid,
        JSON.stringify({ name: displayName, photo: photoUrl }));
    } catch (e) {}

    console.log("✓ Profile loaded:", displayName);
  } catch (err) {
    console.error("Error loading profile:", err);
    showNotifToast("Error loading profile", "error");
  }

  loadPrefs();
  startStatusListener();
}

let notificationSettings = {
  messages: true,
  stories: true,
  calls: true,
  reactions: true
};

function loadNotificationSettings() {
  try {
    const saved = localStorage.getItem("notificationSettings");
    if (saved) {
      notificationSettings = JSON.parse(saved);
    }
  } catch (e) {}
  updateNotifUI();
}

function updateNotifUI() {
  Object.keys(notificationSettings).forEach(key => {
    const toggle = document.getElementById(`notif${key.charAt(0).toUpperCase() + key.slice(1)}`);
    if (toggle) {
      toggle.classList.toggle("active", notificationSettings[key]);
    }
  });
}

function toggleNotif(key) {
  notificationSettings[key] = !notificationSettings[key];
  localStorage.setItem("notificationSettings", JSON.stringify(notificationSettings));
  updateNotifUI();
  showNotifToast(`${key} ${notificationSettings[key] ? 'enabled' : 'disabled'}`, 'success');
}

function openNotifSettings() {
  loadNotificationSettings();
  document.getElementById("notifSettingsModal").classList.add("active");
}

function closeNotifSettings() {
  document.getElementById("notifSettingsModal").classList.remove("active");
}

function handleNotifSettingsOverlayClick(e) {
  if (e.target === document.getElementById("notifSettingsModal")) {
    closeNotifSettings();
  }
}

document.getElementById("myPic").addEventListener("click", () => {
  document.getElementById("profileUpload").click();
});

document.getElementById("profileUpload").addEventListener("change", async (e) => {
  const file = e.target.files[0];
  if (!file || !currentUser) return;

  try {
    const url = await uploadFile(file);
    if (url) {
      await currentUser.updateProfile({ photoURL: url });
      await db.collection("users").doc(currentUser.uid).set({
        photoUrl: url
      }, { merge: true });

      document.getElementById("myPic").src = url;
      document.getElementById("chatPic").src = url;
      showNotifToast("✓ Profile picture updated!", "success");
    }
  } catch (err) {
    showNotifToast("Failed to upload: " + err.message, "error");
  }
  e.target.value = "";
});

function saveLatestMsgState() {
  try {
    localStorage.setItem("nexa_latest_msg_time", JSON.stringify(latestMsgTime));
    localStorage.setItem("nexa_latest_msg_text", JSON.stringify(latestMsgText));
  } catch (e) {}
}

function loadLatestMsgState() {
  try {
    const savedTime = localStorage.getItem("nexa_latest_msg_time");
    const savedText = localStorage.getItem("nexa_latest_msg_text");
    if (savedTime) latestMsgTime = { ...JSON.parse(savedTime), ...latestMsgTime };
    if (savedText) latestMsgText = { ...JSON.parse(savedText), ...latestMsgText };
  } catch (e) {}
}

function saveCachedUsers() {
  if (!currentUser || !allUsersData.length) return;
  try {
    localStorage.setItem("nexa_cached_users_" + currentUser.uid, JSON.stringify(allUsersData));
  } catch (e) {}
}

function loadCachedUsers() {
  if (!currentUser) return;
  try {
    const saved = localStorage.getItem("nexa_cached_users_" + currentUser.uid);
    if (saved) {
      allUsersData = JSON.parse(saved);
      // Re-sanitize cached photo URLs — the cache may predate sanitization.
      allUsersData.forEach(u => {
        u.photo = safeMediaUrl(u.photo) || "https://i.imgur.com/HeIi0wU.png";
      });
      renderUsers();
    }
  } catch (e) {}
}

async function loadInitialChatTimestamps() {
  if (!currentUser) return;
  loadLatestMsgState(); // Instantly restore cached chat order from localStorage

  try {
    const [sentSnap, recvSnap] = await Promise.all([
      db.collection("chats")
        .where("from", "==", currentUser.uid)
        .orderBy("createdAt", "desc")
        .limit(50)
        .get(),
      db.collection("chats")
        .where("to", "==", currentUser.uid)
        .orderBy("createdAt", "desc")
        .limit(50)
        .get()
    ]);

    sentSnap.forEach(doc => {
      const d = doc.data();
      if (d.to && d.createdAt) {
        if (!latestMsgTime[d.to] || d.createdAt > latestMsgTime[d.to]) {
          latestMsgTime[d.to] = d.createdAt;
          latestMsgText[d.to] = d.text || (d.image ? "📷 Photo" : d.video ? "🎥 Video" : d.audio ? "🎤 Voice" : "Message");
        }
      }
    });

    recvSnap.forEach(doc => {
      const d = doc.data();
      if (d.from && d.createdAt) {
        if (!latestMsgTime[d.from] || d.createdAt > latestMsgTime[d.from]) {
          latestMsgTime[d.from] = d.createdAt;
          latestMsgText[d.from] = d.text || (d.image ? "📷 Photo" : d.video ? "🎥 Video" : d.audio ? "🎤 Voice" : "Message");
        }
      }
    });

    saveLatestMsgState();
    renderUsers();
  } catch (err) {
    console.warn("Initial chat timestamp fetch:", err.message);
  }
}

let accountStatusUnsub = null;
function listenCurrentAccountStatus(uid) {
  if (accountStatusUnsub) accountStatusUnsub();
  let hasLoadedOnce = false;

  accountStatusUnsub = db.collection("users").doc(uid).onSnapshot(doc => {
    if (!doc.exists) {
      if (hasLoadedOnce) {
        alert("❌ Your account has been deleted by an administrator.");
        auth.signOut().then(() => {
          window.location.href = "login.html?deleted=1";
        });
      }
      return;
    }

    hasLoadedOnce = true;
    const data = doc.data();
    if (data && data.banned === true) {
      alert("🚫 Your account has been banned by an administrator.");
      auth.signOut().then(() => {
        window.location.href = "login.html?banned=1";
      });
      return;
    }
  });
}


auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL).catch(err => {
  console.warn("Dashboard persistence error:", err);
});

auth.onAuthStateChanged(async (user) => {
  if (!user) {
    if (accountStatusUnsub) accountStatusUnsub();
    try { localStorage.removeItem("nexa_signed_in"); } catch (e) {}
    window.location.href = "login.html";
    return;
  }

  currentUser = user;
  window.currentUser = user;
  // The voice-room module only has window.*, and its invite listener binds to
  // the real uid — tell it to (re)bind now that the real user is known so
  // cross-device voice invites actually arrive.
  if (window.NexaVoiceRoom && typeof window.NexaVoiceRoom.bindInviteListener === 'function') {
    try { window.NexaVoiceRoom.bindInviteListener(); } catch (e) {}
  }
  // Mark this device as signed-in so index.html skips the landing page and
  // bounces straight here on the next app open (WhatsApp-style).
  try {
    localStorage.setItem("nexa_signed_in", "1");
    localStorage.setItem("nexa_last_uid", user.uid);
  } catch (e) {}
  console.log("👤 User:", user.email);
  listenCurrentAccountStatus(user.uid);
  showNotifToast("Welcome to Nexa Messenger", "info");
  // If this account was created via a referral link, greet them by their
  // inviter's username so the referral feels personal.
  db.collection("users").doc(user.uid).get().then(d => {
    const rb = d.exists && d.data().referredBy;
    if (rb) showNotifToast(`🎉 Welcome! You joined via @${rb}'s invite`, "success");
  }).catch(() => {});
  loadLatestMsgState();
  loadDeletedChats();
  await initFCM(user.uid);

  initPeerJS();

  await loadProfile();

  if (!profileSetupRequired) {
    startUsersListener();
    startSharedPresenceListener();
    loadInitialChatTimestamps();
    setupHeartbeat();
    loadUnreadCounts();
    listenIncoming();
    initPWA();
    await updateOnline(true);
    maybeReopenLastChat();
    listenMyGroups();
    listenMyChannels();
    listenDiscoverChannels();
    checkGroupInviteUrlParam();
    checkChannelInviteUrlParam();
    console.log("✅ App fully ready");
    revealNexaApp();
    // Re-engagement nudge: pops a fun welcome-back popup when the user
    // returns after being offline for ~2.5+ days (once per 30 days).
    checkReturningUserWelcome();
  }
});

/* ── Splash → app reveal ───────────────────────────────────────────────
   The branded splash (first rendered element) fades out once the app
   signals it's ready so the underlying UI is fully initialized behind it.
   A hard timeout guarantees the interface is never hidden indefinitely,
   even if an upstream init call hangs. */
let _nexaSplashShown = false;
function revealNexaApp() {
  if (_nexaSplashShown) return;
  _nexaSplashShown = true;
  const splash = document.getElementById('nexaSplash');
  if (!splash) return;
  // Give the browser one frame to paint the ready app before fading.
  requestAnimationFrame(() => {
    splash.classList.add('nexa-done');
    setTimeout(() => {
      splash.style.display = 'none';
    }, 650);
  });
}
// Hard timeout: never trap the user behind the splash (8s, well past init).
(function () {
  setTimeout(() => {
    try {
      const splash = document.getElementById('nexaSplash');
      if (splash && !_nexaSplashShown) revealNexaApp();
    } catch (e) {}
  }, 8000);
})();

/* Play the send button's "sent" pulse once — a tiny tactile confirmation
   that the message left the composer. No-op when the button isn't visible. */
function pulseSendButton() {
  const btn = document.querySelector('.btn-send');
  if (!btn) return;
  btn.classList.remove('sent');
  // Force reflow so a rapid double-send replays the animation.
  void btn.offsetWidth;
  btn.classList.add('sent');
  setTimeout(() => btn.classList.remove('sent'), 600);
}

window.showNotifToast = function(message, type = 'info') {
  const toast = document.createElement('div');
  toast.className = `notif-toast ${type}`;
  toast.textContent = message;
  document.body.appendChild(toast);
  setTimeout(() => toast.remove(), 3000);
};

// ── Notification history (in-app) ──────────────────────────────────────────
// Each user's own notifHistory/{uid} doc keeps the last N notifications that
// ARRIVED to THIS device/app so they can browse them later (WhatsApp/Telegram
// style). Only self-writes are allowed by the rules.
const NEXA_NOTIF_HISTORY_MAX = 50;

function notifHistoryIconFor(title) {
  const t = String(title || '').toLowerCase();
  if (!t) return '🔔';
  if (t.includes('call')) return '📞';
  if (t.includes('react')) return '❤️';
  if (t.includes('story')) return '📸';
  if (t.includes('poll')) return '📊';
  if (t.includes('group')) return '👥';
  if (t.includes('channel') || t.includes('broadcast')) return '📣';
  if (t.includes('follow')) return '➕';
  return '💬';
}

async function recordNotificationHistory({ icon, title, body, at }) {
  try {
    if (!currentUser) return;
    const ref = db.collection('notifHistory').doc(currentUser.uid);
    const entry = {
      icon: icon || notifHistoryIconFor(title),
      title: String(title || 'Notification').slice(0, 120),
      body: String(body || '').slice(0, 240),
      at: at || Date.now()
    };
    // Read-modify-write own doc, capped to NEXA_NOTIF_HISTORY_MAX items
    // (newest first). Missed concurrent writes are fine — history is a cache.
    try {
      const cur = await ref.get();
      let items = (cur.exists && Array.isArray(cur.data().items)) ? cur.data().items.slice() : [];
      // Dedupe consecutive identical entries (FCM + chat-listener may both
      // report the same message).
      const last = items[0];
      if (last && last.title === entry.title && last.body === entry.body) {
        last.at = entry.at;
        await ref.set({ items, updatedAt: Date.now() }, { merge: true });
        return;
      }
      items.unshift(entry);
      items = items.slice(0, NEXA_NOTIF_HISTORY_MAX);
      await ref.set({ items, updatedAt: Date.now() }, { merge: true });
    } catch (e) {
      // First write (doc may not exist yet).
      await ref.set({ items: [entry], updatedAt: Date.now() }, { merge: true });
    }
  } catch (e) {
    console.warn('recordNotificationHistory error:', e);
  }
}

window.openNotificationHistory = async function () {
  const modal = document.getElementById('notifHistoryModal');
  if (!modal) return;
  modal.classList.add('active');
  if (window.lucide) lucide.createIcons();

  const list = document.getElementById('notifHistoryList');
  if (!list) return;
  list.innerHTML = '<div style="text-align:center; padding:40px 0; color:var(--text-3); font-size:13px;">Loading notifications…</div>';

  if (!currentUser) return;
  updateNotificationHistoryBadge();
  try {
    const doc = await db.collection('notifHistory').doc(currentUser.uid).get();
    const items = (doc.exists && Array.isArray(doc.data().items)) ? doc.data().items : [];
    if (!items.length) {
      list.innerHTML = `
        <div style="text-align:center; padding:44px 20px;">
          <div style="font-size:38px; margin-bottom:10px;">🔕</div>
          <div style="font-size:15px; font-weight:600; color:var(--text-1);">No notifications yet</div>
          <div style="font-size:12.5px; color:var(--text-3); margin-top:4px;">Notifications you receive will show up here.</div>
        </div>`;
      return;
    }
    list.innerHTML = items.map(it => `
      <div class="nh-item">
        <div class="nh-item-icon">${escapeHtml(it.icon || '🔔')}</div>
        <div class="nh-item-main">
          <div class="nh-item-title">${escapeHtml(it.title || '')}</div>
          ${it.body ? `<div class="nh-item-body">${escapeHtml(it.body)}</div>` : ''}
        </div>
        <div class="nh-item-time">${relTime(it.at)}</div>
      </div>`).join('');
  } catch (e) {
    list.innerHTML = '<div style="text-align:center; padding:40px 0; color:var(--text-3); font-size:13px;">Could not load history.</div>';
  }
};

window.closeNotificationHistory = function () {
  const modal = document.getElementById('notifHistoryModal');
  if (modal) modal.classList.remove('active');
};

function relTime(ts) {
  if (!ts) return '';
  const diff = Date.now() - ts;
  const m = Math.floor(diff / 60000);
  if (m < 1) return 'now';
  if (m < 60) return m + 'm';
  const h = Math.floor(m / 60);
  if (h < 24) return h + 'h';
  const d = Math.floor(h / 24);
  if (d < 7) return d + 'd';
  return new Date(ts).toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
}

// Fill the small "N" count pill next to "Notification History" in the profile
// tab without reading the whole doc (stored counts on the notifHistory doc).
window.updateNotificationHistoryBadge = async function () {
  const el = document.getElementById('notifHistoryCount');
  if (!el || !currentUser) return;
  try {
    const doc = await db.collection('notifHistory').doc(currentUser.uid).get();
    const n = (doc.exists && Array.isArray(doc.data().items)) ? doc.data().items.length : 0;
    el.textContent = n ? String(n) : '';
    el.style.display = n ? '' : 'none';
  } catch (e) {
    el.style.display = 'none';
  }
};

function sendPushNotification(title, body, targetUidOverride = null, extraData = {}) {
  const targetUid = targetUidOverride || (selectedUser ? selectedUser.uid : null);
  if (!targetUid) return;

  const isCall = title.includes("Call") || extraData.isCall === "true";
  const isReaction = title.toLowerCase().includes("react");

  if (isCall && !notificationSettings.calls) return;
  if (isReaction && !notificationSettings.reactions) return;
  if (!isCall && !isReaction && !notificationSettings.messages) return;

  // A single stable id for THIS notification. The receiver's service worker
  // uses it as the notification tag so that if the backend delivers the same
  // message to several of the user's FCM tokens, the resulting notifications
  // COLLAPSE into one instead of stacking (2-3 per message).
  const notifId = extraData.messageId || extraData.msgId || ('n_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8));

  fetch(`${BACKEND_URL}/api/send-notification`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      targetUid: targetUid,
      title: title,
      body: body,
      icon: '/icon-192.png',
      data: {
        senderUid: currentUser ? currentUser.uid : null,
        timestamp: String(Date.now()),
        isCall: isCall ? "true" : "false",
        messageId: notifId,
        ...extraData
      }
    })
  }).then(res => {
    if (res.ok) console.log('✅ Instant push notification dispatched');
  }).catch(err => console.warn('Push notification error:', err.message));
}


const vapidKey = "BGD7zwejdkH_EnGAfFgi872tkBWDM8-k0It7Px7QzitJ_abkqyK8d1ZEfQFslGS4TY6JXY1mylpSIH5QNDfhCRI";

async function initFCM(uid) {
  if (!('serviceWorker' in navigator) || !('Notification' in window)) return null;
  try {
    const registration = await navigator.serviceWorker.register('/firebase-messaging-sw.js');
    const permission = await Notification.requestPermission();
    if (permission !== 'granted') {
      console.warn('🔔 Notification permission not granted');
      return null;
    }
    if (firebase.messaging.isSupported && await firebase.messaging.isSupported()) {
      const messaging = firebase.messaging();
      const token = await messaging.getToken({ vapidKey, serviceWorkerRegistration: registration });
      if (token) {
        db.collection("users").doc(uid).set({
          fcmToken: token,
          fcmTokens: firebase.firestore.FieldValue.arrayUnion(token)
        }, { merge: true }).catch(err => console.warn("Client FCM token save error:", err));

        await fetch(`${BACKEND_URL}/api/save-token`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ uid, token })
        }).catch(e => console.warn('Failed to save FCM token via backend:', e));
        console.log('🔔 FCM token saved:', token);
      }

      // ── Foreground message handler ──
      // Since we use data-only messages (no `notification` key),
      // FCM SDK won't auto-display anything when the app is in
      // the foreground. We handle it manually here.
      messaging.onMessage((payload) => {
        console.log('[FCM] Foreground message received:', payload);
        const data = payload.data || {};
        const title = data.title || 'Nexa Messenger';
        const body = data.body || 'You have a new message';

        // Record into the in-app notification history (self doc, capped).
        recordNotificationHistory({ icon: notifHistoryIconFor(title), title, body, at: Number(data.timestamp) || Date.now() });

        // Show an in-app toast
        if (typeof showNotifToast === 'function') {
          showNotifToast(`${title}: ${body}`, 'info');
        }

        // Only show a SYSTEM notification when the app is NOT focused on the
        // relevant chat. Showing one while the user is already reading that
        // chat created a duplicate (toast + system) for the same message.
        const isCall = title.includes('Call') || data.isCall === 'true';
        const sameChatOpen =
          !!selectedUser &&
          data.senderUid === selectedUser.uid &&
          document.visibilityState === 'visible' &&
          document.hasFocus();

        if (Notification.permission === 'granted' && registration && !sameChatOpen) {
          // Stable tag per message so duplicate pushes collapse into one
          // notification (e.g. backend sent to several of the user's tokens).
          const msgId = data.messageId || data.msgId || (data.senderUid ? (data.senderUid + '_' + (data.timestamp || Date.now())) : null);
          const tag = isCall ? 'nexa-incoming-call' : (data.tag || (msgId ? ('nexa-msg-' + msgId) : ('nexa-foreground-' + Date.now())));
          registration.showNotification(title, {
            body: body,
            icon: data.icon || '/icon-192.png',
            badge: data.badge || '/icon-192.png',
            tag: tag,
            data: data,
            requireInteraction: false
          });
          // Auto-dismiss after 10s (WhatsApp-style). Calls are not dismissed.
          if (!isCall) {
            setTimeout(() => {
              registration.getNotifications({ tag: tag }).then(notifs => {
                notifs.forEach(n => n.close());
              }).catch(() => {});
            }, 10000);
          }
        }
      });

      return token;
    }
  } catch (e) {
    console.warn('FCM init warning:', e.message);
  }
  return null;
}


function isStandaloneApp() {
  return window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true;
}

function initPWA() {
  window.addEventListener('beforeinstallprompt', (e) => {
    e.preventDefault();
    deferredPrompt = e;
    window.deferredPrompt = e;
    if (!isStandaloneApp()) {
      showPWAPopup();
    }
  });

  window.addEventListener('appinstalled', () => {
    deferredPrompt = null;
    window.deferredPrompt = null;
    dismissPWAPopup();
    showNotifToast('🎉 Nexa Messenger was successfully installed!', 'success');
  });

  setTimeout(() => {
    if (!isStandaloneApp() && !localStorage.getItem('pwaPopupDismissed')) {
      showPWAPopup();
    }
  }, 2500);

  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('/firebase-messaging-sw.js')
      .then(reg => {
        console.log('✅ Unified Service Worker registered');
      })
      .catch(e => {
        console.warn('⚠ SW registration failed:', e.message);
      });
  }

  loadNotificationSettings();
}

function showPWABanner() {
  showPWAPopup();
}

function dismissPWABanner() {
  dismissPWAPopup();
}

function showPWAPopup() {
  const modal = document.getElementById('pwaModalPopup');
  if (modal) {
    modal.classList.add('active');
    if (window.lucide) lucide.createIcons();
  }
}

function dismissPWAPopup() {
  const modal = document.getElementById('pwaModalPopup');
  if (modal) modal.classList.remove('active');
  localStorage.setItem('pwaPopupDismissed', 'true');
}

async function triggerPWAInstall() {
  let promptEvent = deferredPrompt || window.deferredPrompt;
  
  if (!promptEvent) {
    // Wait briefly in case event was just dispatched
    await new Promise(r => setTimeout(r, 300));
    promptEvent = deferredPrompt || window.deferredPrompt;
  }

  if (promptEvent) {
    try {
      promptEvent.prompt();
      const choice = await promptEvent.userChoice;
      if (choice && choice.outcome === 'accepted') {
        showNotifToast('✅ Installing Nexa Messenger...', 'success');
      }
    } catch (err) {
      console.warn("PWA prompt error:", err);
    }
    deferredPrompt = null;
    window.deferredPrompt = null;
    dismissPWAPopup();
    return;
  }

  const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
  if (isIOS) {
    showNotifToast("📱 To install on iOS Safari: Tap Share button and select 'Add to Home Screen'", "info");
  } else {
    showNotifToast("📱 Tap Install or look for the Install icon in your browser address bar", "info");
  }
  dismissPWAPopup();
}

async function installPWA() {
  triggerPWAInstall();
}

function handleLogout() {
  if (confirm('Logout?')) {
    try { localStorage.removeItem("nexa_signed_in"); } catch (e) {}
    auth.signOut();
  }
}

let allUsersData = window.allUsersData;
// Mirror my own users.doc into window.profileObj. It is refreshed whenever
// the users snapshot lands.
function _refreshMyProfile() {
  if (!currentUser) return;
  try {
    const me = allUsersData.find(u => u.uid === currentUser.uid);
    if (me) window.profileObj = me;
  } catch (e) {}
}
let unreadMessages = {};
let mutedChats = {};
let favoritedChats = {};
let blockedUsers = {};
let deletedChats = {};
let latestMsgTime = {};
let latestMsgText = {};
let userPresenceCache = window.userPresenceCache;
let userStatus = "online";
let presenceUnsubs = {};
let userToDelete = null;

function loadDeletedChats() {
  if (!currentUser) return;
  try {
    const saved = localStorage.getItem("nexa_deleted_chats_" + currentUser.uid);
    if (saved) deletedChats = JSON.parse(saved);
    else deletedChats = {};
  } catch (e) {
    deletedChats = {};
  }
}

function saveDeletedChats() {
  if (!currentUser) return;
  try {
    localStorage.setItem("nexa_deleted_chats_" + currentUser.uid, JSON.stringify(deletedChats));
  } catch (e) {}
}

function openDeleteChatModal(user) {
  if (!user) return;
  userToDelete = user;
  const name = user.displayName || user.name || "User";
  const nameEl = document.getElementById("deleteChatTargetName");
  if (nameEl) nameEl.textContent = name;
  const modal = document.getElementById("deleteChatModal");
  if (modal) modal.classList.add("active");
}

function closeDeleteChatModal() {
  userToDelete = null;
  const modal = document.getElementById("deleteChatModal");
  if (modal) modal.classList.remove("active");
}

function confirmDeleteChat() {
  if (!userToDelete || !currentUser) return;
  const uid = userToDelete.uid;
  deletedChats[uid] = true;
  saveDeletedChats();

  // Clear this user's unread state so it stops counting toward the Chats-tab
  // total badge (and isn't left lingering as read:false in Firestore).
  delete unreadMessages[uid];
  updateTotalUnreadBadge();
  markRead(uid);

  if (selectedUser && selectedUser.uid === uid) {
    selectedUser = null;
    setLastOpenChat(null);
    clearChatMsgCache(uid);
    const chatNameEl = document.getElementById("chatName");
    if (chatNameEl) chatNameEl.textContent = "Select a conversation";
    setChatHandle("");
    const msgBox = document.getElementById("messages");
    if (msgBox) msgBox.innerHTML = '<div class="empty-chat"><div class="empty-chat-icon">💬</div><div style="font-size: 16px; font-weight: 600;">Nexa Messenger</div><div style="font-size: 13px; color: var(--text-3);">Select a user to start chatting</div></div>';
    if (window.innerWidth <= 768) {
      closeMobileChat();
    }
  }

  renderUsers();
  closeDeleteChatModal();
  showNotifToast("Chat removed from user list", "info");
}

const PRESENCE_TIMEOUT_MS = 90000;

// Online = the user's app is actively heartbeating RIGHT NOW. The heartbeat
// writes lastSeen every ~30s while the app/tab is alive, so a user is online
// iff lastSeen is fresher than PRESENCE_TIMEOUT_MS. The 90s window gives a
// comfortable margin around the 30s heartbeat (3x) so a single missed tick
// doesn't flicker a user offline, while a truly closed app goes offline
// within ~90s even if beforeunload didn't fire (mobile beforeunload/pagehide
// is unreliable, so a stale lastSeen is the real signal). An explicit offline
// flag wins immediately.
// Do NOT short-circuit on `online === true` alone — that flag stays true
// forever when a mobile tab is swiped away without firing beforeunload.
function isUserOnline(pdata) {
  if (!pdata) return false;
  if (pdata.online === false || pdata.status === "offline") return false;
  const lastSeen = pdata.lastSeen || pdata.last_seen || 0;
  if (!lastSeen) return false;
  return (Date.now() - lastSeen) < PRESENCE_TIMEOUT_MS;
}

function updateUserOnlineDot(uid, pdata) {
  const dot = document.getElementById(`dot-${uid}`);
  const online = isUserOnline(pdata);
  if (dot) dot.classList.toggle("online", online);
}

function startUsersListener() {
  loadCachedUsers();

  db.collection("users").onSnapshot(snap => {
    allUsersData = [];
    const seenUids = new Set();

    snap.forEach(doc => {
      if (doc.id !== currentUser.uid) {
        const uid = doc.id;
        seenUids.add(uid);
        const d = doc.data();
        let displayName = null;

        if (d.displayName && d.displayName.trim() && d.displayName !== "User") {
          displayName = d.displayName;
        } else if (d.name && d.name.trim()) {
          displayName = d.name;
        } else {
          displayName = d.email ? d.email.split('@')[0] : "User";
        }

        allUsersData.push({
          uid: uid,
          ...d,
          displayName: displayName,
          photo: safeMediaUrl(d.photoUrl || d.photo) || "https://i.imgur.com/HeIi0wU.png"
        });
      }
    });

    // Keep window.allUsersData in sync (same reference) for external modules.
    window.allUsersData = allUsersData;
    _refreshMyProfile();

    saveCachedUsers();
    renderUsers();
    renderActiveNowBar();
  });
}

// Single shared presence listener — feeds userPresenceCache for the whole app
// (online dots, Active Now bar, chat-header last seen, voice-room invite list).
// Do NOT re-introduce per-user presence/{uid} doc listeners OR a second
// full-collection presence listener — each duplicates reads and races with
// this cache.
let sharedPresenceUnsub = null;
function startSharedPresenceListener() {
  if (sharedPresenceUnsub || !db) return;
  sharedPresenceUnsub = db.collection("presence").onSnapshot(snap => {
    snap.docChanges().forEach(change => {
      const uid = change.doc.id;
      const pdata = change.doc.data() || {};
      userPresenceCache[uid] = pdata;
      updateUserOnlineDot(uid, pdata);
    });
    // Coalesce the re-render: a single presence snapshot can carry many
    // docChanges (a heartbeat wave), so render once per frame instead of
    // once per change. The chat-header update is cheap and immediate.
    if (selectedUser) updateChatHeaderPresence(userPresenceCache[selectedUser.uid]);
    scheduleRender();
  }, err => console.error("Shared presence listener error:", err));
}

// Facebook-style "Active Now" horizontal avatar strip. Reads from the shared
// userPresenceCache so it stays in sync with the single presence listener.
function renderActiveNowBar() {
  const scroller = document.getElementById("activeNowScroller");
  if (!scroller) return;

  const onlineUsers = allUsersData
    .filter(u => !blockedUsers[u.uid] && isUserOnline(userPresenceCache[u.uid]))
    .sort((a, b) => (a.displayName || "").localeCompare(b.displayName || ""));

  if (!onlineUsers.length) {
    if (!scroller.querySelector(".active-now-empty")) {
      scroller.innerHTML = '<div class="active-now-empty">No friends online right now</div>';
    }
    return;
  }

  // Avoid full re-render when the online set hasn't changed (preserves scroll).
  const key = onlineUsers.map(u => u.uid).join(",");
  if (scroller.dataset.key === key) return;
  scroller.dataset.key = key;

  scroller.innerHTML = onlineUsers.map(u => {
    const photo = u.photo || "https://i.imgur.com/HeIi0wU.png";
    const name = escapeHtml(u.displayName || "User");
    return `
      <button class="active-now-item" data-uid="${u.uid}" onclick="selectUserFromActiveNow('${u.uid}')" title="${name}">
        <span class="active-now-avatar-wrap">
          <img src="${photo}" class="active-now-avatar" loading="lazy" decoding="async" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
          <span class="active-now-dot"></span>
        </span>
        <span class="active-now-name">${name.split(' ')[0]}</span>
      </button>`;
  }).join("");
}

function selectUserFromActiveNow(uid) {
  const u = allUsersData.find(x => x.uid === uid);
  if (u) {
    const el = document.querySelector(`.user-item[data-uid="${uid}"]`);
    selectChat(u, el);
  }
}

let lastRenderedUserOrder = "";

// Render coalescer: collapses a burst of renderUsers() calls (e.g. when many
// presence docs change at once during a heartbeat wave) into ONE render per
// animation frame. This is the single biggest "feels instant, no jank" win —
// without it, 50 presence changes fire 50 synchronous renderUsers() calls,
// each re-sorting + re-querying the whole user list, causing visible lag.
// Only data-change paths use this; imperative callers that need an immediate
// update (selectChat, toggling favorites) call renderUsers() directly.
let _renderScheduled = false;
function scheduleRender() {
  if (_renderScheduled) return;
  _renderScheduled = true;
  requestAnimationFrame(() => {
    _renderScheduled = false;
    renderUsers();
    renderActiveNowBar();
  });
}

function setChatHandle(text) {
  const el = document.getElementById("chatHandle");
  if (el) {
    if (text) {
      el.textContent = text;
      el.style.display = "block";
    } else {
      el.textContent = "";
      el.style.display = "none";
    }
  }
}

function renderUsers() {
  const box = document.getElementById("users");
  if (!box) return;

  const q = document.getElementById("userSearch")?.value.trim().toLowerCase() || "";
  let list = allUsersData.filter(u => !blockedUsers[u.uid]);
  if (!q) {
    list = list.filter(u => !deletedChats[u.uid]);
  } else {
    list = list.filter(u =>
      (u.displayName || "").toLowerCase().includes(q) ||
      ((u.username || "").toLowerCase().includes(q) && q !== "") ||
      ((u.phone || "").replace(/\D/g, "").includes(q.replace(/\D/g, "")) && q.replace(/\D/g, "") !== "")
  );
  }

  const userMetrics = new Map();
  list.forEach(u => {
    const time = latestMsgTime[u.uid] || 0;
    userMetrics.set(u.uid, {
      fav: favoritedChats[u.uid] ? 1 : 0,
      time: time,
      hasMsg: time > 0 ? 1 : 0,
      online: isUserOnline(userPresenceCache[u.uid]) ? 1 : 0,
      name: (u.displayName || "").toLowerCase()
    });
  });

  list.sort((a, b) => {
    const mA = userMetrics.get(a.uid);
    const mB = userMetrics.get(b.uid);
    if (mA.fav !== mB.fav) return mB.fav - mA.fav;
    if (mA.hasMsg !== mB.hasMsg) return mB.hasMsg - mA.hasMsg;
    if (mA.time !== mB.time) return mB.time - mA.time;
    if (mA.online !== mB.online) return mB.online - mA.online;
    return mA.name.localeCompare(mB.name);
  });

  // WhatsApp-style "Message yourself" pinned self-contact, always at the very
  // top, only when there is no active search query (so searching still works).
  const selfContact = (!q && currentUser) ? getSelfContact() : null;

  if (!list.length && !selfContact) { 
    box.innerHTML = '<div class="no-users">👋 No users found</div>'; 
    lastRenderedUserOrder = "";
    return; 
  }

  const fullList = selfContact ? [selfContact, ...list] : list;
  const currentOrder = fullList.map(u => u.uid).join(",");
  const orderChanged = currentOrder !== lastRenderedUserOrder;
  lastRenderedUserOrder = currentOrder;

  // Clear loading/empty state placeholder if users are found
  const noUsersEl = box.querySelector(".no-users");
  if (noUsersEl) {
    noUsersEl.remove();
  }

  const validUids = new Set(fullList.map(u => u.uid));
  box.querySelectorAll(".user-item").forEach(item => {
    if (!validUids.has(item.dataset.uid)) {
      item.remove();
    }
  });

  fullList.forEach((user, idx) => {
    const name = escapeHtml(user.displayName || "User");
    const photo = user.photo || "https://i.imgur.com/HeIi0wU.png";
    const unread = unreadMessages[user.uid] || 0;
    const t = latestMsgTime[user.uid];
    const timeStr = t ? formatTime(t) : "";
    const pdata = userPresenceCache[user.uid];
    const online = isUserOnline(pdata);
    const isSelf = !!user.isSelf;
    const prevText = isSelf
      ? (latestMsgText[user.uid] || "Message yourself")
      : (latestMsgText[user.uid] || "Tap to chat");
    const isSelected = selectedUser && selectedUser.uid === user.uid;

    let el = box.querySelector(`.user-item[data-uid="${user.uid}"]`);

    if (!el) {
      el = document.createElement("div");
      el.className = "user-item" + (isSelected ? " active" : "") + (isSelf ? " self-chat-item" : "") + (unread > 0 ? " has-unread" : "");
      el.dataset.uid = user.uid;
      el.innerHTML = `
        <div class="user-avatar-wrap">
          <img src="${photo}" class="user-avatar" loading="lazy" decoding="async" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
          <div class="status-dot ${online ? 'online' : ''}" id="dot-${user.uid}"></div>
        </div>
        <div class="user-info">
          <div class="user-name">${isSelf ? '📝 ' : (favoritedChats[user.uid] ? '⭐ ' : '')}${name}</div>
          <div class="user-handle" id="handle-${user.uid}" ${user.username && user.username !== (user.displayName || '').toLowerCase() ? '' : 'style="display:none"'}>${user.username && user.username !== (user.displayName || '').toLowerCase() ? '@' + escapeHtml(user.username) : ''}</div>
          <div class="user-preview" id="prev-${user.uid}">${escapeHtml(prevText)}</div>
        </div>
        <div class="user-meta">
          <div class="user-time">${timeStr}</div>
          ${unread > 0 ? `<div class="unread-badge">${unread > 99 ? '99+' : unread}</div>` : ''}
        </div>
      `;
    } else {
      el.className = "user-item" + (isSelected ? " active" : "") + (isSelf ? " self-chat-item" : "");
      
      const dot = el.querySelector(`.status-dot`);
      if (dot) dot.className = `status-dot ${online ? 'online' : ''}`;
      
      const nameEl = el.querySelector(`.user-name`);
      if (nameEl) nameEl.innerHTML = `${isSelf ? '📝 ' : (favoritedChats[user.uid] ? '⭐ ' : '')}${name}`;

      const handleEl = el.querySelector(`.user-handle`);
      if (handleEl) {
        const h = (user.username && user.username !== (user.displayName || '').toLowerCase()) ? '@' + user.username : '';
        if (handleEl.textContent !== h) handleEl.textContent = h;
        handleEl.style.display = h ? '' : 'none';
      }
      
      const prevEl = el.querySelector(`.user-preview`);
      if (prevEl && prevEl.textContent !== prevText) prevEl.textContent = prevText;
      
      const timeEl = el.querySelector(`.user-time`);
      if (timeEl && timeEl.textContent !== timeStr) timeEl.textContent = timeStr;
      
      let badge = el.querySelector(`.unread-badge`);
      if (unread > 0) {
        if (!badge) {
          badge = document.createElement("div");
          badge.className = "unread-badge";
          const metaBox = el.querySelector(".user-meta");
          if (metaBox) metaBox.appendChild(badge);
        }
        if (badge) badge.textContent = unread > 99 ? '99+' : unread;
      } else if (badge) {
        badge.remove();
      }
      el.classList.toggle("has-unread", unread > 0);
    }

    el._userData = user;

    if (!el.dataset.boundLongPress) {
      el.dataset.boundLongPress = "true";
      let pressTimer = null;
      let isLongPress = false;

      el.addEventListener("touchstart", (e) => {
        if (el._userData && el._userData.isSelf) return; // no delete on self-chat
        isLongPress = false;
        clearTimeout(pressTimer);
        pressTimer = setTimeout(() => {
          isLongPress = true;
          if (navigator.vibrate) navigator.vibrate(40);
          if (el._userData) openDeleteChatModal(el._userData);
        }, 500);
      }, { passive: true });

      el.addEventListener("touchend", () => clearTimeout(pressTimer));
      el.addEventListener("touchmove", () => clearTimeout(pressTimer));
      el.addEventListener("touchcancel", () => clearTimeout(pressTimer));

      el.addEventListener("contextmenu", (e) => {
        if (el._userData && el._userData.isSelf) return; // no delete on self-chat
        e.preventDefault();
        e.stopPropagation();
        if (el._userData) openDeleteChatModal(el._userData);
      });

      el.addEventListener("click", (e) => {
        if (isLongPress) {
          e.preventDefault();
          e.stopPropagation();
          isLongPress = false;
          return;
        }
        if (el._userData) selectChat(el._userData, el);
      });
    }

    const currentChild = box.children[idx];
    if (currentChild !== el) {
      box.insertBefore(el, currentChild || null);
    }
  });
}

let searchUsersTimer = null;
function searchUsers() {
  clearTimeout(searchUsersTimer);
  searchUsersTimer = setTimeout(() => {
    renderUsers();
  }, 100);
}

/* =========================================================================
   TAB & NAVIGATION CONTROL (Chats, Stories, Settings, Profile)
   ========================================================================= */
let currentTab = 'chats';

function switchTab(tabName) {
  closeStoryViewer();
  currentTab = tabName;

  // Community-tab marker so voice/video call buttons can be hard-hidden via CSS
  document.body.classList.toggle('community-tab-active', tabName === 'community');

  // 1. Update sidebar top tab icons
  document.querySelectorAll('.s-tab-btn').forEach(btn => btn.classList.remove('active'));
  const sBtn = document.getElementById(`sTab${tabName.charAt(0).toUpperCase() + tabName.slice(1)}`);
  if (sBtn) sBtn.classList.add('active');

  // 2. Update mobile bottom nav items
  document.querySelectorAll('.nav-item').forEach(item => item.classList.remove('active'));
  const nBtn = document.getElementById(`navBtn${tabName.charAt(0).toUpperCase() + tabName.slice(1)}`);
  if (nBtn) nBtn.classList.add('active');

  // 3. Update tab panes display
  document.querySelectorAll('.tab-pane').forEach(pane => pane.classList.remove('active'));
  const pane = document.getElementById(`tabPane${tabName.charAt(0).toUpperCase() + tabName.slice(1)}`);
  if (pane) pane.classList.add('active');

  // 4. On mobile, if a chat is currently open full screen, close it when user clicks bottom tabs
  if (window.innerWidth <= 768) {
    closeMobileChat();
  }

  // 5. Trigger tab-specific renders
  if (tabName === 'stories') {
    renderStoriesTab();
    updateStoryTabDot();
  } else if (tabName === 'settings') {
    renderSettingsTab();
  } else if (tabName === 'profile') {
    renderProfileTab();
  } else if (tabName === 'community') {
    renderCommunityTab();
  }

  // Strictly enforce that call buttons NEVER show in Community tab
  const voiceBtn = document.getElementById('headerVoiceCallBtn');
  const videoBtn = document.getElementById('headerVideoCallBtn');
  if (tabName === 'community' || currentChatMode !== 'direct' || !selectedUser) {
    if (voiceBtn) voiceBtn.style.display = 'none';
    if (videoBtn) videoBtn.style.display = 'none';
  } else if (tabName === 'chats' && currentChatMode === 'direct' && selectedUser) {
    if (voiceBtn) voiceBtn.style.display = 'flex';
    if (videoBtn) videoBtn.style.display = 'flex';
  }
  updatePollButtonVisibility();

  if (window.lucide) lucide.createIcons();
}

function closeMobileChat() {
  const app = document.getElementById("app");
  if (app) app.classList.remove("chat-open-mobile");
  document.body.classList.remove("chat-open-mobile");
  const bNav = document.getElementById("mobileBottomNav");
  if (bNav) bNav.classList.remove("hidden");
}

function openMyStoryOrAdd() {
  const myStatuses = userStatuses[currentUser?.uid] || [];
  if (myStatuses.length > 0) {
    openStoryViewer(currentUser.uid);
  } else {
    openStatusModal();
  }
}

function renderStoriesTab() {
  const feedList = document.getElementById("storiesFeedList");
  const myPic = document.getElementById("myPic")?.src || "https://i.imgur.com/HeIi0wU.png";
  const myStatuses = userStatuses[currentUser?.uid] || [];
  
  // Sync My Story Card
  const tabMyPic = document.getElementById("storyTabMyPic");
  if (tabMyPic) tabMyPic.src = myPic;
  const tabMySub = document.getElementById("storyTabMySub");
  if (tabMySub) {
    tabMySub.textContent = myStatuses.length 
      ? `${myStatuses.length} active story${myStatuses.length > 1 ? 's' : ''} (Tap to view)` 
      : "Tap to add a new story";
  }

  if (!feedList) return;
  feedList.innerHTML = "";

  const seenMap = getSeenStories();
  let count = 0;

  allUsersData.forEach(user => {
    const statuses = userStatuses[user.uid];
    if (!statuses || !statuses.length) return;
    count++;

    const allSeen = statuses.every(s => seenMap[s.id]);
    const photo = user.photo || "https://i.imgur.com/HeIi0wU.png";
    const name = escapeHtml(user.displayName || "User");
    const latest = statuses[statuses.length - 1];
    const timeStr = latest ? formatTime(latest.createdAt) : "";

    const card = document.createElement("div");
    card.className = "story-feed-card";
    card.onclick = () => openStoryViewer(user.uid);

    card.innerHTML = `
      <div class="story-feed-ring ${allSeen ? 'seen' : 'unseen'}">
        <img src="${photo}" class="story-feed-avatar" loading="lazy" decoding="async" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
      </div>
      <div class="story-feed-info">
        <div class="story-feed-name">${name}</div>
        <div class="story-feed-time">${timeStr} • ${statuses.length} update${statuses.length > 1 ? 's' : ''}</div>
      </div>
      <div class="story-feed-arrow">›</div>
    `;

    feedList.appendChild(card);
  });

  if (count === 0) {
    feedList.innerHTML = `
      <div class="no-stories-card">
        <div style="font-size: 28px; margin-bottom: 8px;">⭕</div>
        <div style="font-size: 14px; font-weight: 600; margin-bottom: 4px;">No Recent Stories</div>
        <div style="font-size: 12px; color: var(--text-3);">Be the first to share a story with your contacts!</div>
      </div>
    `;
  }
}

/* =========================================================================
   REFERRAL / INVITE FRIENDS
   Each user's personal invite link uses their username as the ref code:
   <app-origin>/?ref=<username>. signup.html records referredBy on the new
   account and increments the inviter's referralCount. We show the count live.
   ========================================================================= */
let _myUsername = null; // cached after first load (from the users doc)

// Resolve the current user's username (from their Firestore users doc) for the
// referral code. Firebase auth's currentUser has no username field.
async function getMyUsername() {
  if (_myUsername) return _myUsername;
  if (!currentUser) return "user";
  try {
    const snap = await db.collection("users").doc(currentUser.uid).get();
    const d = snap.exists ? snap.data() : {};
    _myUsername = (d.username || d.displayName || currentUser.email?.split("@")[0] || "user")
      .toLowerCase().replace(/[^a-z0-9_]/g, "");
  } catch (e) {
    _myUsername = (currentUser.email?.split("@")[0] || "user").toLowerCase().replace(/[^a-z0-9_]/g, "");
  }
  return _myUsername;
}

function getReferralLink(username) {
  const u = username || _myUsername || "user";
  return `${window.location.origin}/?ref=${encodeURIComponent(u)}`;
}

async function initReferralCard() {
  const username = await getMyUsername();
  const link = getReferralLink(username);
  const input = document.getElementById("referralLinkInput");
  if (input) input.value = link;
  const text = document.getElementById("referralLinkText");
  if (text) text.textContent = link;
  loadReferralCount();
}

async function loadReferralCount() {
  const el = document.getElementById("referralCount");
  const listEl = document.getElementById("referralList");
  if (!currentUser) return;
  try {
    const snap = await db.collection("users").doc(currentUser.uid).get();
    const d = snap.exists ? snap.data() : {};
    // Count from the referrals array when present (most reliable), else the counter.
    const refs = Array.isArray(d.referrals) ? d.referrals : [];
    const count = refs.length || (d.referralCount || 0);
    if (el) el.textContent = count;

    // Show the list of people you invited (their usernames).
    if (listEl) {
      if (refs.length) {
        const items = refs.map(r => {
          const un = (typeof r === "string") ? "" : (r.username || r.displayName || "");
          return un ? `<span class="referral-chip">@${escapeHtml(un)}</span>` : "";
        }).filter(Boolean).join("");
        if (items) {
          listEl.innerHTML = `<div class="referral-list-title">People you invited</div><div class="referral-chips">${items}</div>`;
          listEl.style.display = "block";
        } else {
          listEl.style.display = "none";
        }
      } else {
        listEl.style.display = "none";
      }
    }
  } catch (e) {
    console.warn("Referral count load failed:", e);
  }
}

function copyReferralLink() {
  const link = getReferralLink();
  const done = () => showNotifToast("🔗 Invite link copied!", "success");
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(link).then(done).catch(() => fallbackCopy(link, done));
  } else {
    fallbackCopy(link, done);
  }
}
function fallbackCopy(text, done) {
  const ta = document.createElement("textarea");
  ta.value = text;
  ta.style.position = "fixed";
  ta.style.opacity = "0";
  document.body.appendChild(ta);
  ta.select();
  try { document.execCommand("copy"); done(); } catch (e) { showNotifToast("Copy failed", "error"); }
  document.body.removeChild(ta);
}

function shareReferralLink() {
  const link = getReferralLink();
  const text = "Join me on Nexa Messenger — fast, private chat with voice notes, stories & more. Sign up with my link:";
  if (navigator.share) {
    navigator.share({ title: "Nexa Messenger", text, url: link }).catch(() => {});
  } else {
    copyReferralLink();
  }
}

/* =========================================================================
   RETURNING-USER WELCOME POPUP (re-engagement nudge)
   When a signed-in user opens the app after being offline for ~2-3+ days,
   we show a fun "you disappear o" popup once (then once per 30 days per
   device, tracked in localStorage) that nudges them to Invite their friends
   via the existing referral link. It is non-blocking: it appears AFTER the
   app is fully revealed so the main UI stays usable behind it. Absence is
   computed from the user's OWN presence doc's lastSeen freshness (falling
   back to last_login when no presence doc exists yet).
   ========================================================================= */
const RETURN_WELCOME_MIN_GAP_MS = 2.5 * 24 * 60 * 60 * 1000; // offline for 2.5+ days
const RETURN_WELCOME_COOLDOWN_MS = 30 * 24 * 60 * 60 * 1000; // once per 30 days

function showReturningWelcome() {
  const modal = document.getElementById("welcomeBackModal");
  if (modal) {
    modal.classList.add("active");
    modal.setAttribute("aria-hidden", "false");
    try { localStorage.setItem("nexa_return_welcome_seen", String(Date.now())); } catch (e) {}
  }
}

function dismissReturningWelcome() {
  const modal = document.getElementById("welcomeBackModal");
  if (modal) {
    modal.classList.remove("active");
    modal.setAttribute("aria-hidden", "true");
  }
}

function returningWelcomeInvite() {
  dismissReturningWelcome();
  // Native share sheet on mobile (WhatsApp-style, friendlier than a bare copy).
  // Falls back to copying the referral link when Web Share isn't available.

  const link = getReferralLink();
  const doShare = async () => {
    await getMyUsername();
    if (navigator.share) {
      try {
        await navigator.share({ title: "Join me on Nexa Messenger", text: "Yo! Come and gist with me on Nexa Messenger — " + link });
        return;
      } catch (e) {
        if (e.name === "AbortError") return;
        // Fall through to clipboard copy on failure/unsupported.
 
      }
    }
    copyReferralLink();
  };
  doShare().catch(() => { try { copyReferralLink(); } catch (e) {} });
}

async function checkReturningUserWelcome() {
  if (!currentUser) return;
  try {
    let seen = 0;
    try { seen = parseInt(localStorage.getItem("nexa_return_welcome_seen") || "0", 10) || 0; } catch (e) {}
    if (seen && Date.now() - seen < RETURN_WELCOME_COOLDOWN_MS) return;
    // Absence signal: the user's OWN presence doc. lastSeen (heartbeat by
    // this device while open) is the freshest last-online timestamp; if there's
    // no presence doc yet, fall back to the users doc's last_login.
    const presenceSnap = await db.collection("presence").doc(currentUser.uid).get();
    let lastActive = presenceSnap.exists ? (presenceSnap.data().lastSeen || presenceSnap.data().last_seen || 0) : 0;
    if (!lastActive) {
      const userSnap = await db.collection("users").doc(currentUser.uid).get();
      const ud = userSnap.exists ? userSnap.data() : {};
      lastActive = ud.last_login || ud.lastLogin || ud.createdAt || 0;
      if (typeof lastActive === "object" && lastActive && lastActive.toMillis) lastActive = lastActive.toMillis();
    }
    if (!lastActive) return;
    if (Date.now() - lastActive < RETURN_WELCOME_MIN_GAP_MS) return;
    // Wait a beat so it pops as the user lands, after the app is revealed.

    setTimeout(showReturningWelcome, 1200);
  } catch (e) {
    console.warn("Returning-welcome check skipped:", e);
  }
}

window.showReturningWelcome = showReturningWelcome;
window.dismissReturningWelcome = dismissReturningWelcome;

function renderSettingsTab() {
  loadNotificationSettings();
  initReferralCard();
  const msgToggle = document.getElementById("tabNotifMessages");
  if (msgToggle) msgToggle.classList.toggle("active", notificationSettings.messages);
  const stToggle = document.getElementById("tabNotifStories");
  if (stToggle) stToggle.classList.toggle("active", notificationSettings.stories);
  const clToggle = document.getElementById("tabNotifCalls");
  if (clToggle) clToggle.classList.toggle("active", notificationSettings.calls);

  const grid = document.getElementById("settingsThemeGrid");
  if (!grid) return;
  grid.innerHTML = "";

  const themes = [
    { id: 'light', name: 'Nexa White', color: '#eef2f7', icon: 'sun' },
    { id: 'nexa', name: 'Nexa Deep Blue', color: '#2b8fff', icon: 'moon' }
  ];

  themes.forEach(t => {
    const card = document.createElement("div");
    card.className = "theme-card-option" + (currentTheme === t.id ? " active" : "");
    card.onclick = () => {
      setTheme(t.id);
      renderSettingsTab();
      showNotifToast(`Theme set to ${t.name}`, 'success');
    };

    card.innerHTML = `
      <div class="theme-swatch" style="background: ${t.color}"></div>
      <div class="theme-option-name"><i data-lucide="${t.icon}" style="width: 13px; height: 13px;"></i> ${t.name}</div>
      ${currentTheme === t.id ? '<div class="theme-check"><i data-lucide="check" style="width: 12px; height: 12px;"></i></div>' : ''}
    `;
    grid.appendChild(card);
  });
  if (window.lucide) lucide.createIcons();
}

function renderProfileTab() {
  if (!currentUser) return;
  updateNotificationHistoryBadge();
  const myPic = document.getElementById("myPic")?.src || "https://i.imgur.com/HeIi0wU.png";
  const myName = document.getElementById("myName")?.textContent || currentUser.displayName || (currentUser.email ? currentUser.email.split('@')[0] : "User");

  const pPic = document.getElementById("profileTabPic");
  if (pPic) pPic.src = myPic;

  const pNameDisplay = document.getElementById("profileTabNameDisplay");
  if (pNameDisplay) pNameDisplay.textContent = myName === "Loading…" ? (currentUser.displayName || currentUser.email?.split('@')[0] || "User") : myName;

  const pUsername = document.getElementById("profileTabUsername");
  if (pUsername) {
    const meDoc = allUsersData.find(u => u.uid === currentUser.uid);
    const myUsername = (meDoc && (meDoc.username || meDoc.usernameField)) || _myUsername || (currentUser.email ? currentUser.email.split('@')[0] : '');
    pUsername.textContent = myUsername ? '@' + myUsername : '@—';
  }

  const pEmail = document.getElementById("profileTabEmail");
  if (pEmail) pEmail.textContent = currentUser.email || "No email";

  loadProfileNote();
  refreshMyUsernameInfo();
}


const NEXA_USERNAME_RE = /^[a-z0-9_]{3,20}$/;

// One-time username handler: users with a username AND users without one may
// set/change it exactly once (users doc flag usernameChangeUsed). After that
// the button is hidden and the handle is locked.
async function refreshMyUsernameInfo() {
  if (!currentUser || !window.db) return;
  try {
    const snap = await db.collection("users").doc(currentUser.uid).get();
    if (!snap.exists) return;
    const d = snap.data();
    const myUsername = String(d.username || "").trim();
    _myUsername = myUsername ? myUsername.toLowerCase().replace(/[^a-z0-9_]/g, "") : _myUsername;
    const used = !!d.usernameChangeUsed;

    const pUsername = document.getElementById("profileTabUsername");
    if (pUsername) {
      pUsername.textContent = myUsername
        ? '@' + myUsername
        : (currentUser.email ? '@' + currentUser.email.split('@')[0] : '@\u2014');
    }

    const btn = document.getElementById("changeUsernameBtn");
    if (btn) btn.style.display = used ? "none" : "inline-flex";

    const hint = document.getElementById("profileUsernameHint");
    if (hint) {
      hint.innerHTML = used
        ? '\uD83D\uDD12 Your @username is locked and cannot be changed.<span style="display:block;margin-top:2px;">Other people find you by this handle \u2014 share it anywhere.</span>'
        : '\uD83D\uDD8A You can change your @username <strong>once</strong>.<span style="display:block;margin-top:2px;">Other people find you by this handle \u2014 share it anywhere.</span>';
    }

    const refInput = document.getElementById("referralLinkInput");
    if (refInput && typeof initReferralCard === "function") initReferralCard();
  } catch (e) {
    console.warn("refreshMyUsernameInfo failed:", e);
  }
}

async function openChangeUsernameModal() {
  if (!currentUser) return;
  const modal = document.getElementById("changeUsernameModal");
  if (!modal) return;
  const input = document.getElementById("changeUsernameInput");
  const err = document.getElementById("changeUsernameError");
  const saveBtn = document.getElementById("changeUsernameSaveBtn");
  if (err) err.textContent = "";
  if (saveBtn) {
    saveBtn.disabled = false;
    saveBtn.textContent = "Save Username";
  }
  let current = "";
  try {
    const snap = await db.collection("users").doc(currentUser.uid).get();
    if (snap.exists) {
      const d = snap.data();
      if (d.usernameChangeUsed) {
        showNotifToast("You've already used your one-time username change", "info");
        refreshMyUsernameInfo();
        return;
      }
      current = String(d.username || "").trim();
    }
  } catch (e) {
    console.warn("openChangeUsernameModal read failed:", e);
  }
  if (input) {
    input.value = current;
    input.disabled = false;
  }
  modal.classList.add("active");
  setTimeout(() => { if (input) input.focus(); }, 60);
}

function closeChangeUsernameModal() {
  const modal = document.getElementById("changeUsernameModal");
  if (modal) modal.classList.remove("active");
}

function handleChangeUsernameOverlayClick(e) {
  if (e.target === document.getElementById("changeUsernameModal")) closeChangeUsernameModal();
}

async function saveUsernameChange() {
  if (!currentUser) return;
  const input = document.getElementById("changeUsernameInput");
  const err = document.getElementById("changeUsernameError");
  const saveBtn = document.getElementById("changeUsernameSaveBtn");
  const newUsername = (input ? input.value : "").trim().toLowerCase();

  if (err) err.textContent = "";
  if (!newUsername) {
    if (err) err.textContent = "Username cannot be empty.";
    return;
  }
  if (!NEXA_USERNAME_RE.test(newUsername)) {
    if (err) err.textContent = "Use 3\u201320 letters, numbers or underscores (no spaces).";
    return;
  }
  if (saveBtn) {
    saveBtn.disabled = true;
    saveBtn.textContent = "Saving\u2026";
  }

  try {
    const me = await db.collection("users").doc(currentUser.uid).get();
    if (me.exists && me.data().usernameChangeUsed) {
      if (err) err.textContent = "You've already used your one-time username change.";
      if (saveBtn) { saveBtn.disabled = false; saveBtn.textContent = "Save Username"; }
      refreshMyUsernameInfo();
      return;
    }

    // Uniqueness check (exact match; stored usernames are lowercased).
    const q = await db.collection("users").where("username", "==", newUsername).limit(2).get();
    const takenByOther = q.docs.some(doc_ => doc_.id !== currentUser.uid);
    if (takenByOther) {
      if (err) err.textContent = "That username is already taken. Try another.";
      if (saveBtn) { saveBtn.disabled = false; saveBtn.textContent = "Save Username"; }
      return;
    }

    await db.collection("users").doc(currentUser.uid).update({
      username: newUsername,
      usernameChangeUsed: true
    });

    _myUsername = newUsername;
    closeChangeUsernameModal();
    renderProfileTab();
    const refInput = document.getElementById("referralLinkInput");
    if (refInput && typeof initReferralCard === "function") initReferralCard();
    showNotifToast('\u2713 Username changed to @' + newUsername, "success");
  } catch (e) {
    console.error("Username change failed:", e);
    if (err) err.textContent = "Couldn't save: " + (e.message || "please try again");
    if (saveBtn) { saveBtn.disabled = false; saveBtn.textContent = "Save Username"; }
  }
}


/* =========================================================================
   PROFILE NOTES — 24-hour note shown on the user's profile (WhatsApp-style)
   Stored in Firestore: profile_notes/{uid} = { text, createdAt, updatedAt }
   Auto-expires after 24h (enforced on read + a cleanup tick).
   ========================================================================= */
let profileNoteUnsub = null;

function loadProfileNote() {
  if (!currentUser || !window.db) return;
  if (profileNoteUnsub) { profileNoteUnsub(); profileNoteUnsub = null; }
  profileNoteUnsub = window.db.collection("profile_notes").doc(currentUser.uid)
    .onSnapshot(doc => {
      const display = document.getElementById("profileNoteDisplay");
      const input = document.getElementById("profileNoteInput");
      if (!display) return;
      const data = doc.data();
      const now = Date.now();
      const expired = data && data.createdAt && (now - data.createdAt > 24 * 60 * 60 * 1000);
      if (!data || !data.text || expired) {
        display.innerHTML = '<div class="note-empty">No active note. Add one above!</div>';
        if (input) input.value = "";
        return;
      }
      const remaining = 24 * 60 * 60 * 1000 - (now - data.createdAt);
      display.innerHTML = `
        <div class="note-active-card">
          <div class="note-active-text">${linkify(data.text)}</div>
          <div class="note-active-meta"><i data-lucide="clock" style="width: 12px; height: 12px; vertical-align: -1px;"></i> Expires in ${formatTimeRemaining(remaining)} • <a href="#" onclick="deleteProfileNote(); return false;" style="color: var(--danger, #ef4444);">Remove</a></div>
        </div>`;
      if (window.lucide) lucide.createIcons();
      if (input) input.value = data.text;
    }, err => console.warn("Profile note listener error:", err));
}

async function saveProfileNote() {
  if (!currentUser) return;
  const input = document.getElementById("profileNoteInput");
  const text = input?.value.trim();
  if (!text) { showNotifToast("Please write something first", "error"); return; }
  const btn = document.getElementById("noteSaveBtn");
  if (btn) { btn.disabled = true; btn.textContent = "Posting..."; }
  try {
    await window.db.collection("profile_notes").doc(currentUser.uid).set({
      text: text,
      uid: currentUser.uid,
      createdAt: Date.now(),
      updatedAt: Date.now()
    }, { merge: true });
    showNotifToast("✓ Note posted to your profile!", "success");
    if (input) input.value = "";
  } catch (err) {
    showNotifToast("Failed to save note: " + err.message, "error");
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = "Post"; }
  }
}

async function deleteProfileNote() {
  if (!currentUser) return;
  try {
    await window.db.collection("profile_notes").doc(currentUser.uid).delete();
    showNotifToast("Note removed", "info");
  } catch (err) {
    showNotifToast("Failed to remove note: " + err.message, "error");
  }
}

function formatTimeRemaining(ms) {
  if (ms <= 0) return "soon";
  const h = Math.floor(ms / (60 * 60 * 1000));
  const m = Math.floor((ms % (60 * 60 * 1000)) / (60 * 1000));
  if (h > 0) return `${h}h ${m}m`;
  return `${m}m`;
}

// Fetch another user's active note for the profile modal (one-time read).
// Logs permission errors distinctly so an undeployed firestore.rules (the
// usual cause of "the note doesn't show on their profile") is visible in the
// console instead of silently looking like "no note set".
async function fetchUserNote(uid) {
  if (!uid) return null;
  try {
    const doc = await window.db.collection("profile_notes").doc(uid).get();
    const data = doc.data();
    if (!data || !data.text) return null;
    if (data.createdAt && (Date.now() - data.createdAt > 24 * 60 * 60 * 1000)) return null;
    return data.text;
  } catch (e) {
    console.warn("[profile_notes] read failed for", uid, "— if this is a permission-denied error, deploy firestore.rules (`firebase deploy --only firestore:rules`) so cross-user reads are allowed.", e && e.code ? "(" + e.code + ")" : "", e);
    return null;
  }
}

function triggerProfilePhotoUpload() {
  const input = document.getElementById("profileUpload");
  if (input) input.click();
}

function clearAppCache() {
  if (confirm("Clear local cache and saved preferences?")) {
    localStorage.clear();
    showNotifToast("✓ Cache cleared", "success");
    setTimeout(() => window.location.reload(), 1000);
  }
}

/* =========================================================================
   CHAT SELECTION
   ========================================================================= */
let unsubMessagesA = null;
let unsubMessagesB = null;
let unsubChatPresence = null;
let unsubTyping = null;
let unsubDisappearing = null;
let disappearingSettings = {};

function getChatKey(u1, u2) {
  if (!u1 || !u2) return "";
  return [u1, u2].sort().join("_");
}

function parseDisappearingMs(val) {
  if (val === '5m') return 5 * 60 * 1000;
  if (val === '24h') return 24 * 60 * 60 * 1000;
  if (val === '7d') return 7 * 24 * 60 * 60 * 1000;
  if (val === '90d') return 90 * 24 * 60 * 60 * 1000;
  return 0;
}

function formatDisappearingLabel(val) {
  if (val === '5m') return '5 Minutes';
  if (val === '24h') return '24 Hours';
  if (val === '7d') return '7 Days';
  if (val === '90d') return '90 Days';
  return 'Off';
}

function listenDisappearingSettings() {
  if (unsubDisappearing) { unsubDisappearing(); unsubDisappearing = null; }
  if (!selectedUser || !currentUser) return;
  const key = getChatKey(currentUser.uid, selectedUser.uid);
  unsubDisappearing = db.collection("disappearingSettings").doc(key).onSnapshot(doc => {
    const data = doc.data();
    const duration = data?.duration || 'off';
    disappearingSettings[key] = duration;

    const select = document.getElementById("disappearingSelect");
    if (select) select.value = duration;

    updateDisappearingBadge(duration);
    renderMessageList();
  });
}

function updateDisappearingBadge(duration) {
  const banner = document.getElementById("disappearingNotice");
  const bannerText = document.getElementById("disappearingBannerText");

  const isActive = duration && duration !== 'off';

  if (banner) {
    banner.style.display = isActive ? 'flex' : 'none';
  }
  if (bannerText && isActive) {
    bannerText.textContent = `Disappearing messages · ${formatDisappearingLabel(duration)}`;
  }
}

async function changeDisappearingSetting(value) {
  if (!selectedUser || !currentUser) return;
  const key = getChatKey(currentUser.uid, selectedUser.uid);
  disappearingSettings[key] = value;
  updateDisappearingBadge(value);

  try {
    await db.collection("disappearingSettings").doc(key).set({
      duration: value,
      updatedAt: Date.now(),
      updatedBy: currentUser.uid
    }, { merge: true });

    showNotifToast("Disappearing messages set to " + formatDisappearingLabel(value), "success");
    renderMessageList();
  } catch (err) {
    showNotifToast("Error setting disappearing messages: " + err.message, "error");
  }
}

function selectChat(user, el) {
  if (currentChatMode !== 'direct') resetCommunityChatMode();
  currentChatMode = 'direct';
  selectedGroup = null;
  selectedChannel = null;

  // Header buttons for 1v1 direct chat
  const voiceBtn = document.getElementById('headerVoiceCallBtn');
  const videoBtn = document.getElementById('headerVideoCallBtn');
  const infoBtn = document.getElementById('headerInfoBtn');
  const showCallButtons = currentTab !== 'community'; // never show voice/video while on the Community tab
  if (voiceBtn) voiceBtn.style.display = showCallButtons ? 'flex' : 'none';
  if (videoBtn) videoBtn.style.display = showCallButtons ? 'flex' : 'none';
  if (infoBtn) infoBtn.style.display = 'flex';
  updatePollButtonVisibility();
  // Persist the chat we're LEAVING so it reopens instantly next app-open,
  // then remember the newly selected chat as the last-open one.
  if (selectedUser && selectedUser.uid !== user.uid) saveChatMsgCache(selectedUser.uid);
  selectedUser = user;
  setLastOpenChat(user.uid);
  applyCurrentWallpaper();
  if (deletedChats[user.uid]) {
    delete deletedChats[user.uid];
    saveDeletedChats();
  }
  messageRepliedTo = null;
  clearReply();
  document.getElementById("chatName").textContent = user.displayName || "User";
  const handleText = (user.username && user.username !== (user.displayName || "").toLowerCase()) ? "@" + user.username : "";
  setChatHandle(handleText);
  document.getElementById("chatPic").src = user.photo || "https://i.imgur.com/HeIi0wU.png";
  document.querySelectorAll(".user-item").forEach(e => e.classList.remove("active"));
  if (el) el.classList.add("active");
  if (!isSelfChat()) markRead(user.uid);
  delete unreadMessages[user.uid];
  updateTotalUnreadBadge();
  renderUsers(); // Move selected user to the top immediately
  loadMessages();
  // Typing + presence don't apply to the self-chat — skip those listeners.
  if (isSelfChat()) {
    if (unsubTyping) { unsubTyping(); unsubTyping = null; }
    if (unsubChatPresence) { unsubChatPresence(); unsubChatPresence = null; }
  } else {
    listenPresence();
    listenTyping();
  }
  // Reset the typing-write guard on chat switch so the new conversation's
  // first keystroke writes typing:true (not skipped as "already active").
  typingCurrentlyActive = false;
  if (typingTimeout) { clearTimeout(typingTimeout); typingTimeout = null; }
  listenDisappearingSettings();
  document.getElementById("typingBar").innerHTML = "";
  closeChatInfo();
  scrollDownNewCount = 0;
  updateScrollDownBtn();

  if (window.innerWidth <= 768) {
    const app = document.getElementById("app");
    if (app) app.classList.add("chat-open-mobile");
    document.body.classList.add("chat-open-mobile");
    const bNav = document.getElementById("mobileBottomNav");
    if (bNav) bNav.classList.add("hidden");
  }
}

function markRead(fromUid) {
  // Batch all unread-from updates into a single write to avoid N individual
  // update calls (one Firestore write per unread message) when opening a chat.
  db.collection("chats")
    .where("to", "==", currentUser.uid).where("from", "==", fromUid).where("read", "==", false)
    .get().then(snap => {
      if (snap.empty) return;
      const batch = db.batch();
      const now = Date.now();
      snap.forEach(d => batch.update(d.ref, { read: true, readAt: now }));
      batch.commit().catch(e => console.error("markRead batch error:", e));
    });
}

function toggleSidebar() {
  const sidebar = document.getElementById("sidebar");
  sidebar.classList.toggle("show");
  if (sidebar.classList.contains("show")) {
    setTimeout(() => document.addEventListener("click", _sidebarOutsideClick), 50);
  } else {
    document.removeEventListener("click", _sidebarOutsideClick);
  }
}

function closeSidebarFn() {
  const sidebar = document.getElementById("sidebar");
  sidebar.classList.remove("show");
  document.removeEventListener("click", _sidebarOutsideClick);
}

function _sidebarOutsideClick(e) {
  const sidebar = document.getElementById("sidebar");
  const btn = document.getElementById("mobileMenuBtn");
  if (sidebar && !sidebar.contains(e.target) && (!btn || !btn.contains(e.target))) closeSidebarFn();
}

function updateChatHeaderPresence(data) {
  if (!selectedUser) return;
  data = data || userPresenceCache[selectedUser.uid] || {};
  const online = isUserOnline(data);
  const status = data.status || (online ? "online" : "offline");
  const lastSeen = data.lastSeen || 0;
  const el = document.getElementById("status");
  const dot = document.getElementById("chatStatusDot");
  if (online && status === "online") {
    if (el) { el.textContent = "🟢 online"; el.className = "online-status"; }
    if (dot) dot.classList.add("online");
  } else if (online && status === "away") {
    if (el) { el.textContent = "🟡 away"; el.className = ""; }
    if (dot) dot.classList.remove("online");
  } else {
    if (el) { el.textContent = `⚪ last seen ${formatLastSeen(lastSeen)}`; el.className = ""; }
    if (dot) dot.classList.remove("online");
  }
}

function listenPresence() {
  // Do NOT open a per-user presence listener — the shared listener
  // (startSharedPresenceListener) already feeds userPresenceCache and
  // updates the chat header. Just refresh from the cache.
  if (!selectedUser) return;
  if (unsubChatPresence) { unsubChatPresence(); unsubChatPresence = null; }
  updateChatHeaderPresence(userPresenceCache[selectedUser.uid]);
}

function formatLastSeen(ts) {
  if (!ts) return "a while ago";
  const diff = Date.now() - ts, m = Math.floor(diff / 60000), h = Math.floor(m / 60), d = Math.floor(h / 24);
  if (m < 1) return "just now";
  if (m < 60) return `${m}m ago`;
  if (h < 24) return `${h}h ago`;
  if (d === 1) return "yesterday";
  return `${d}d ago`;
}

let typingTimeout;
let typingCurrentlyActive = false; // guard: only write typing:true once per typing session

document.addEventListener("input", function(e) {
  if (e.target.id !== "text" || !selectedUser) return;
  const ta = document.getElementById("text");
  ta.style.height = "auto";
  ta.style.height = Math.min(ta.scrollHeight, 100) + "px";
  // Only write typing:true ONCE per typing session (transition from idle→typing),
  // not on every keystroke. Previously this wrote on every input event, which
  // was many writes/second during fast typing.
  if (!typingCurrentlyActive) {
    typingCurrentlyActive = true;
    db.collection("typing").doc(currentUser.uid).set({ typing: true, to: selectedUser.uid, timestamp: Date.now() }, { merge: true });
  }
  clearTimeout(typingTimeout);
  typingTimeout = setTimeout(() => {
    typingCurrentlyActive = false;
    db.collection("typing").doc(currentUser.uid).set({ typing: false }, { merge: true });
  }, 1200);
});

function listenTyping() {
  if (unsubTyping) { unsubTyping(); unsubTyping = null; }
  if (!selectedUser) return;
  unsubTyping = db.collection("typing").doc(selectedUser.uid).onSnapshot(doc => {
    const bar = document.getElementById("typingBar");
    const d = doc.data();
    if (d?.typing && d.to === currentUser.uid) {
      bar.innerHTML = '<div class="t-dots"><div class="t-dot"></div><div class="t-dot"></div><div class="t-dot"></div></div> typing…';
    } else {
      bar.innerHTML = "";
    }
  });
}

function loadUnreadCounts() {
  // Scope the listener to UNREAD messages only (.where("read","==",false)).
  // Previously this listened to the ENTIRE incoming inbox (every message ever
  // received) and re-read all of it on every change — a major quota burner for
  // users with long histories. The all-equality query (to==me, read==false)
  // needs no composite index (Firestore zigzag-merges single-field indexes),
  // and the unread set is small, so we simply rebuild per-sender counts from
  // the (small) snapshot each event instead of maintaining an incremental
  // counter that could drift on read-flag flips.
  if (!currentUser) return;
  db.collection("chats")
    .where("to", "==", currentUser.uid)
    .where("read", "==", false)
    .onSnapshot(snap => {
      const counts = {};
      snap.forEach(d => {
        const m = d.data();
        if (m.from && m.from !== currentUser.uid) {
          counts[m.from] = (counts[m.from] || 0) + 1;
        }
      });
      unreadMessages = counts;
      try { localStorage.setItem("nexa_unread_" + currentUser.uid, JSON.stringify(counts)); } catch (e) {}
      scheduleRender();
      updateTotalUnreadBadge();
    }, err => console.error("loadUnreadCounts listener error:", err));
}

// WhatsApp-style total unread badge on the Chats nav button (mobile bottom nav
// + any desktop chat tab). Sums unreadMessages across all chats.
function updateTotalUnreadBadge() {
  let total = 0;
  for (const uid in unreadMessages) total += unreadMessages[uid] || 0;
  const label = total > 99 ? '99+' : String(total);
  const navBtn = document.getElementById("navBtnChats");
  if (!navBtn) return;
  let badge = navBtn.querySelector(".nav-unread-badge");
  if (total > 0) {
    if (!badge) {
      badge = document.createElement("span");
      badge.className = "nav-unread-badge";
      navBtn.appendChild(badge);
    }
    badge.textContent = label;
  } else if (badge) {
    badge.remove();
  }
}

async function updateOnline(on) {
  if (!currentUser) return;
  const isOnlineState = !!on;
  // Going online preserves "away" if the user is currently idle (the inactivity
  // timer sets userStatus="away"); going online from a fresh interaction resets
  // to "online". Going offline always sets status "offline".
  if (!isOnlineState) {
    userStatus = "offline";
  } else if (userStatus === "offline") {
    userStatus = "online";
  }
  try {
    await db.collection("presence").doc(currentUser.uid).set(
      { online: isOnlineState, status: userStatus, last_seen: Date.now(), lastSeen: Date.now(), uid: currentUser.uid },
      { merge: true }
    );
  } catch (err) {
    console.error("Error updating presence:", err);
  }
}

// Lightweight heartbeat: refreshes lastSeen (and re-asserts online) WITHOUT
// clobbering an "away" status the inactivity timer may have set. This is what
// the 10s interval calls so a backgrounded-but-open app keeps showing online
// while idle-away is preserved. lastSeen freshness is what isUserOnline() keys
// on, so this is the single source of truth for "in the app right now".
async function heartbeat() {
  if (!currentUser) return;
  if (userStatus === "offline") return;
  try {
    await db.collection("presence").doc(currentUser.uid).set(
      { online: true, status: userStatus, last_seen: Date.now(), lastSeen: Date.now(), uid: currentUser.uid },
      { merge: true }
    );
  } catch (err) {
    console.error("Error heartbeat presence:", err);
  }
}

// Browsers block autoplay + suspend AudioContext until a user gesture and on
// backgrounding. This re-triggers .play() on the 1:1 call audio/video elements,
// all voice-room remote audio elements, and resumes the voice room's AudioContext.
// Wired to window focus, visibilitychange, and one-shot touchstart/click above.
function resumeAllAudio() {
  try {
    const els = [
      document.getElementById("remoteCallAudio"),
      document.getElementById("remoteVideo"),
      ...document.querySelectorAll('audio[id^="audio_"]')
    ];
    els.forEach(el => {
      if (el && el.srcObject && el.paused) {
        el.play().catch(() => {});
      }
    });
    if (window.NexaVoiceRoom && NexaVoiceRoom.audioCtx && NexaVoiceRoom.audioCtx.state === "suspended") {
      NexaVoiceRoom.audioCtx.resume().catch(() => {});
    }
  } catch (e) {
    console.warn("resumeAllAudio error:", e);
  }
}

function setupHeartbeat() {
  // Heartbeat every 30 seconds while the app/tab is ALIVE (not gated on
  // visibilityState). A backgrounded-but-open tab is still "in the app", so it
  // keeps heartbeating and stays online. Online status is decided purely by
  // lastSeen freshness in isUserOnline(), so a truly closed/killed app stops
  // heartbeating and goes offline within the stale window even if beforeunload
  // didn't fire (mobile reliably fails to fire beforeunload on swipe-away).
  // 30s (not 10s) keeps lastSeen well within the online threshold while
  // cutting presence writes ~3x and reducing presence-listener re-fires.
  setInterval(() => {
    heartbeat();
  }, 30000);

  // Periodic ticker every 10 seconds to re-evaluate active presence timeouts
  // (a user whose heartbeat went stale should flip to offline here, not wait
  // for a Firestore write from them).
  setInterval(() => {
    if (allUsersData && allUsersData.length) {
      allUsersData.forEach(user => {
        updateUserOnlineDot(user.uid, userPresenceCache[user.uid]);
      });
    }
    if (selectedUser) {
      updateChatHeaderPresence(userPresenceCache[selectedUser.uid]);
    }
    renderActiveNowBar();
  }, 10000);

  // Backgrounding/foregrounding: do NOT write offline when hidden — the tab is
  // still alive and the heartbeat keeps it online (matches "in the app alone").
  // On becoming visible again, re-assert online immediately + resume audio.
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") {
      updateOnline(true);
      resumeAllAudio();
    }
  });

  // Only a TRUE close goes offline. These fire on tab close / app kill when the
  // browser cooperates; if they don't (mobile), the stale-heartbeat timeout in
  // isUserOnline() handles it.
  window.addEventListener("beforeunload", () => updateOnline(false));
  window.addEventListener("pagehide", () => updateOnline(false));
  window.addEventListener("focus", resumeAllAudio);

  // One-shot gesture listeners so autoplay-blocked audio resumes on first
  // user interaction (mobile especially blocks until a gesture happens).
  const resumeOnGesture = () => {
    resumeAllAudio();
    document.removeEventListener("touchstart", resumeOnGesture);
    document.removeEventListener("click", resumeOnGesture);
  };
  document.addEventListener("touchstart", resumeOnGesture, { once: true, passive: true });
  document.addEventListener("click", resumeOnGesture, { once: true });

  let inactivityTimer = null;

  function resetInactivity() {
    clearTimeout(inactivityTimer);
    // User interacted again — if they were "away", move them back to "online".
    if (userStatus === "away") {
      userStatus = "online";
      heartbeat();
    }
    inactivityTimer = setTimeout(async () => {
      if (!currentUser) return;
      userStatus = "away";
      heartbeat();
    }, 180000);
  }

  document.addEventListener("click", resetInactivity);
  document.addEventListener("keypress", resetInactivity);
  document.addEventListener("mousemove", resetInactivity);
  resetInactivity();
}

/* =========================================================================
   MESSAGES (fix #1: scoped to the open conversation only, via two
   from/to listeners merged client-side, instead of the entire global
   chats collection. fix #4: scroll position preserved unless near bottom.)
   ========================================================================= */
let allMessages = [];
let _msgsA = [];
let _msgsB = [];
let _olderMsgsA = []; // fetched-on-demand history, kept separate so the live
let _olderMsgsB = []; // windowed listeners never overwrite it
let chatJustOpened = false;

// WhatsApp-style message pagination: only the newest MSG_PAGE_SIZE messages
// are live-listened; older history loads in MSG_OLDER_PAGE_SIZE chunks as the
// user scrolls to the top (maybeLoadOlderMessages → loadOlderMessages).
const MSG_PAGE_SIZE = 12;
const MSG_OLDER_PAGE_SIZE = 25;
let msgPaging = {
  oldestA: null, oldestB: null,
  windowFullA: false, windowFullB: false,
  exhaustedA: false, exhaustedB: false,
  seamA: [], seamB: [], // timestamp gaps left by bulk window slides
  loading: false
};
let _loadingOlderRender = false; // renderMessageList must not auto-scroll on older loads

function resetMsgPaging() {
  msgPaging = {
    oldestA: null, oldestB: null,
    windowFullA: false, windowFullB: false,
    exhaustedA: false, exhaustedB: false,
    seamA: [], seamB: [],
    loading: false
  };
  _olderMsgsA = [];
  _olderMsgsB = [];
  _loadingOlderRender = false;
}

function msgPagingHasMore() {
  return (msgPaging.windowFullA && !msgPaging.exhaustedA)
      || (msgPaging.windowFullB && !msgPaging.exhaustedB)
      || msgPaging.seamA.length > 0
      || msgPaging.seamB.length > 0;
}

/* =========================================================================
   OPEN-CHAT INSTANT CACHE (WhatsApp-style): remember the newest messages of
   the chat the user last had open. On the next app open we auto-reopen that
   chat and paint these cached bubbles instantly — before the live from/to
   listeners return. The live snapshots then overwrite/replace them (they
   dedupe by id and carry authoritative read/edited/media state), so this is
   purely a perceived-speed optimization, never a source of truth.
   ========================================================================= */
const CHAT_MSG_CACHE_MAX = 24;
function chatMsgCacheKey(uid) {
  const me = (currentUser && currentUser.uid) || localStorage.getItem("nexa_last_uid");
  return me ? ("nexa_chat_msgs_" + me + "_" + uid) : null;
}
function saveChatMsgCache(uid) {
  try {
    if (!currentUser) return;
    const k = chatMsgCacheKey(uid);
    if (!k) return;
    const merged = _msgsA.concat(_msgsB)
      .filter(m => !m.isViewOnce) // never cache view-once (self-destructing)
      .sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));
    const slice = merged.slice(-CHAT_MSG_CACHE_MAX);
    if (!slice.length) return;
    localStorage.setItem(k, JSON.stringify(slice));
  } catch (e) {}
}
function loadChatMsgCache(uid) {
  try {
    const k = chatMsgCacheKey(uid);
    if (!k) return [];
    const arr = JSON.parse(localStorage.getItem(k) || "[]");
    return Array.isArray(arr) ? arr : [];
  } catch (e) { return []; }
}
function clearChatMsgCache(uid) {
  try { const k = chatMsgCacheKey(uid); if (k) localStorage.removeItem(k); } catch (e) {}
}
function setLastOpenChat(uid) {
  try {
    if (!currentUser) return;
    if (uid) localStorage.setItem("nexa_last_open_chat_" + currentUser.uid, uid);
    else localStorage.removeItem("nexa_last_open_chat_" + currentUser.uid);
  } catch (e) {}
}
function getLastOpenChat() {
  try {
    return (currentUser && localStorage.getItem("nexa_last_open_chat_" + currentUser.uid)) || null;
  } catch (e) { return null; }
}

// WhatsApp-style: on app open, jump straight back into the conversation you
// last had open. Cached bubbles paint instantly (loadChatMsgCache), then the
// live listeners fill in anything new. On mobile the chat slides in full-
// screen via the existing chat-open-mobile class inside selectChat().
function maybeReopenLastChat() {
  try {
    const uid = getLastOpenChat();
    if (!uid || selectedUser) return;
    let user = null;
    if (currentUser && uid === currentUser.uid) {
      user = getSelfContact(); // the pinned "Message yourself" contact
    } else {
      user = (allUsersData || []).find(u => u.uid === uid);
    }
    if (!user) return; // contact no longer in the list — stay on the chat list
    selectChat(user, null);
  } catch (e) {}
}

// Find a message across both the live window and the fetched older pages.
function findLocalMsg(id) {
  return _msgsA.find(m => m.id === id)
      || _msgsB.find(m => m.id === id)
      || _olderMsgsA.find(m => m.id === id)
      || _olderMsgsB.find(m => m.id === id)
      || null;
}

// Detect a bulk slide seam: if the previously FULL window's newest message is
// older than the new window's oldest, messages between them never appeared in
// any snapshot (a many-messages-at-once slide). Record the range; it gets
// healed lazily on scroll-up via a startAfter/endBefore fetch.
function recordSeam(prevArr, newMsgs, seamArr) {
  if (prevArr.length < MSG_PAGE_SIZE || !newMsgs.length) return;
  let prevMaxTs = null;
  prevArr.forEach(m => {
    if (typeof m.createdAt === "number" && (prevMaxTs == null || m.createdAt > prevMaxTs)) prevMaxTs = m.createdAt;
  });
  let newMinTs = null;
  newMsgs.forEach(m => {
    if (typeof m.createdAt === "number" && (newMinTs == null || m.createdAt < newMinTs)) newMinTs = m.createdAt;
  });
  if (prevMaxTs == null || newMinTs == null) return;
  if (prevMaxTs < newMinTs) seamArr.push({ after: prevMaxTs, before: newMinTs });
}

// Messages that slide out of the newest-N live window as new messages arrive
// are retained as "history" so scrolling up never hits a gap. A slide-out
// only ever ejects from the OLDEST end of the window, so a message removed
// from a newer position (a real remote delete) is not retained. (Known edge:
// if the remote party deletes exactly the window's oldest message while you
// have history loaded, it lingers until you re-open the chat.)
function retainSlidOut(prevArr, newMsgs, olderArr) {
  if (!prevArr.length) return;
  const newIds = new Set(newMsgs.map(m => m.id));
  let newOldestTs = null;
  newMsgs.forEach(m => {
    if (typeof m.createdAt === "number" && (newOldestTs == null || m.createdAt < newOldestTs)) {
      newOldestTs = m.createdAt;
    }
  });
  prevArr.forEach(m => {
    if (newIds.has(m.id)) return;
    if (typeof m.id === "string" && m.id.startsWith("temp_")) return;
    if (newOldestTs == null || typeof m.createdAt !== "number" || m.createdAt >= newOldestTs) return;
    if (!olderArr.some(o => o.id === m.id)) olderArr.push(m);
  });
}

function loadMessages() {
  if (unsubMessagesA) { unsubMessagesA(); unsubMessagesA = null; }
  if (unsubMessagesB) { unsubMessagesB(); unsubMessagesB = null; }
  _msgsA = [];
  _msgsB = [];
  allMessages = [];
  chatJustOpened = true;
  resetMsgPaging();

  const box = document.getElementById("messages");

  if (!selectedUser) {
    if (box) box.innerHTML = '<div class="empty-chat"><div class="empty-chat-icon">💬</div><div style="font-size: 14px; font-weight: 500; color: var(--text-3);">Select a conversation</div></div>';
    return;
  }

  // WhatsApp-style instant open: paint this chat's cached newest messages
  // immediately so the conversation appears before the live listeners return.
  // Cached bubbles go into the "older" buckets (which the windowed listeners
  // never overwrite); the live snapshots then replace them by id.
  const cached = loadChatMsgCache(selectedUser.uid);
  if (cached.length) {
    _olderMsgsA = cached.filter(m => m.from === currentUser.uid);
    _olderMsgsB = cached.filter(m => m.from !== currentUser.uid);
    scheduleMessageRender();
  } else if (box) {
    box.innerHTML = '<div class="empty-chat"><div class="empty-chat-icon">💬</div><div style="font-size: 14px; font-weight: 500; color: var(--text-3);">Loading conversation...</div></div>';
  }

  const otherUid = selectedUser.uid;

  // Track the oldest timestamp ever loaded per direction — older pages are
  // fetched with endBefore(that timestamp). Live snapshots only ever LOWER
  // it (a fresh window's oldest is newer than any loaded history).
  const trackWindow = (side, docs) => {
    docs.forEach(d => {
      const ts = d.data().createdAt;
      if (typeof ts === "number") {
        const k = side === "A" ? "oldestA" : "oldestB";
        if (msgPaging[k] == null || ts < msgPaging[k]) msgPaging[k] = ts;
      }
    });
    msgPaging[side === "A" ? "windowFullA" : "windowFullB"] = docs.length >= MSG_PAGE_SIZE;
  };

  // WhatsApp-style "chat yourself": both directions are the same query
  // (from==me && to==me), so use a SINGLE listener to avoid duplicate
  // message rendering.
  if (isSelfChat()) {
    unsubMessagesA = db.collection("chats")
      .where("from", "==", currentUser.uid)
      .where("to", "==", currentUser.uid)
      .orderBy("createdAt", "desc")
      .limit(MSG_PAGE_SIZE)
      .onSnapshot(snap => {
        const newMsgs = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        recordSeam(_msgsA, newMsgs, msgPaging.seamA);
        retainSlidOut(_msgsA, newMsgs, _olderMsgsA);
        _msgsA = newMsgs;
        _msgsB = [];
        trackWindow("A", snap.docs);
        scheduleMessageRender();
      }, err => console.error("messages(self) listener error:", err));
    return;
  }

  unsubMessagesA = db.collection("chats")
    .where("from", "==", currentUser.uid)
    .where("to", "==", otherUid)
    .orderBy("createdAt", "desc")
    .limit(MSG_PAGE_SIZE)
    .onSnapshot(snap => {
      const newMsgs = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      recordSeam(_msgsA, newMsgs, msgPaging.seamA);
      retainSlidOut(_msgsA, newMsgs, _olderMsgsA);
      _msgsA = newMsgs;
      trackWindow("A", snap.docs);
      scheduleMessageRender();
    }, err => console.error("messages(A) listener error:", err));

  unsubMessagesB = db.collection("chats")
    .where("from", "==", otherUid)
    .where("to", "==", currentUser.uid)
    .orderBy("createdAt", "desc")
    .limit(MSG_PAGE_SIZE)
    .onSnapshot(snap => {
      const newMsgs = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      recordSeam(_msgsB, newMsgs, msgPaging.seamB);
      retainSlidOut(_msgsB, newMsgs, _olderMsgsB);
      _msgsB = newMsgs;
      trackWindow("B", snap.docs);

      // Mark newly-arrived, unread messages as read — batched into a single
      // write so opening a chat with many unread messages is one request, not
      // one update per message.
      const unread = snap.docs.filter(d => !(d.data().read));
      if (unread.length) {
        const batch = db.batch();
        const now = Date.now();
        unread.forEach(d => batch.update(d.ref, { read: true, readAt: now }));
        batch.commit().catch(() => {});
      }

      scheduleMessageRender();
    }, err => console.error("messages(B) listener error:", err));
}

// Fires on every messages-area scroll; kicks off an older-page fetch when the
// user reaches the top of the loaded history.
function maybeLoadOlderMessages() {
  const box = document.getElementById("messages");
  if (!box || !selectedUser || chatJustOpened || msgPaging.loading) return;
  if (!msgPagingHasMore()) return;
  if (box.scrollHeight <= box.clientHeight) return; // nothing scrollable yet
  if (box.scrollTop < 80) loadOlderMessages();
}

async function loadOlderMessages() {
  if (!selectedUser || !currentUser || msgPaging.loading) return;
  if (!msgPagingHasMore()) return;
  msgPaging.loading = true;

  const spinner = document.getElementById("olderMsgsSpinner");
  if (spinner) spinner.style.display = "flex";

  const box = document.getElementById("messages");
  const prevHeight = box ? box.scrollHeight : 0;
  const prevTop = box ? box.scrollTop : 0;

  try {
    const me = currentUser.uid;
    const other = selectedUser.uid;
    const olderQuery = (fromUid, toUid, before) =>
      db.collection("chats")
        .where("from", "==", fromUid)
        .where("to", "==", toUid)
        .orderBy("createdAt", "desc")
        .endBefore(before)
        .limit(MSG_OLDER_PAGE_SIZE)
        .get();

    const fetches = [];
    if (msgPaging.windowFullA && !msgPaging.exhaustedA && msgPaging.oldestA != null)
      fetches.push(olderQuery(me, other, msgPaging.oldestA).then(snap => ({ side: "A", kind: "page", snap })));
    if (!isSelfChat() && msgPaging.windowFullB && !msgPaging.exhaustedB && msgPaging.oldestB != null)
      fetches.push(olderQuery(other, me, msgPaging.oldestB).then(snap => ({ side: "B", kind: "page", snap })));

    // Heal bulk-slide seams (messages that never appeared in any snapshot).
    const seamQuery = (fromUid, toUid, range) =>
      db.collection("chats")
        .where("from", "==", fromUid)
        .where("to", "==", toUid)
        .orderBy("createdAt", "asc")
        .startAfter(range.after)
        .endBefore(range.before)
        .limit(200)
        .get();
    ["A", "B"].forEach(side => {
      if (side === "B" && isSelfChat()) return;
      const seamArr = side === "A" ? msgPaging.seamA : msgPaging.seamB;
      const fromUid = side === "A" ? me : other;
      const toUid = side === "A" ? other : me;
      seamArr.forEach(range => {
        fetches.push(seamQuery(fromUid, toUid, range).then(snap => ({ side, kind: "seam", range, snap })));
      });
    });

    const results = await Promise.all(fetches);

    results.forEach(({ side, kind, range, snap }) => {
      const msgs = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      const arr = side === "A" ? _olderMsgsA : _olderMsgsB;
      // Dedupe against both the older pages and the live window (they can
      // overlap when messages arrive while paging up).
      const live = side === "A" ? _msgsA : _msgsB;
      const existing = new Set([...arr, ...live].map(m => m.id));
      let minTs = null;
      msgs.forEach(m => {
        if (existing.has(m.id)) return;
        arr.push(m);
        if (typeof m.createdAt === "number" && (minTs == null || m.createdAt < minTs)) minTs = m.createdAt;
      });
      if (kind === "seam") {
        // Consume the healed range.
        const seamArr = side === "A" ? msgPaging.seamA : msgPaging.seamB;
        const ri = seamArr.indexOf(range);
        if (ri > -1) seamArr.splice(ri, 1);
        return;
      }
      const oldestK = side === "A" ? "oldestA" : "oldestB";
      if (minTs != null && (msgPaging[oldestK] == null || minTs < msgPaging[oldestK])) {
        msgPaging[oldestK] = minTs;
      }
      // A short page means history is exhausted in this direction.
      if (snap.size < MSG_OLDER_PAGE_SIZE) {
        msgPaging[side === "A" ? "exhaustedA" : "exhaustedB"] = true;
      }
    });

    // Render with scroll preserved so the view stays on the same message
    // instead of jumping to the bottom or top.
    _loadingOlderRender = true;
    renderMessageList();
    if (box) {
      requestAnimationFrame(() => {
        box.scrollTop = box.scrollHeight - prevHeight + prevTop;
        _loadingOlderRender = false;
      });
    } else {
      _loadingOlderRender = false;
    }
  } catch (err) {
    console.warn("loadOlderMessages error:", err);
    _loadingOlderRender = false;
  } finally {
    msgPaging.loading = false;
    if (spinner) spinner.style.display = "none";
  }
}

// WhatsApp-style "Message yourself" — selectedUser is a synthetic self
// contact whose uid equals currentUser.uid.
function isSelfChat() {
  return !!selectedUser && !!currentUser && selectedUser.uid === currentUser.uid;
}

// Build the synthetic self-contact shown as a pinned "You (Message yourself)"
// entry at the top of the chat list.
function getSelfContact() {
  if (!currentUser) return null;
  const myName = document.getElementById('myName')?.textContent || currentUser.displayName || 'You';
  const myPhoto = document.getElementById('myPic')?.src || currentUser.photoURL || 'https://i.imgur.com/HeIi0wU.png';
  return {
    uid: currentUser.uid,
    displayName: myName + ' (You)',
    photo: myPhoto,
    isSelf: true
  };
}

let activeAudioPlayer = null;

function renderVoiceNotePlayer(msgId, audioUrl, durationMs, fromMe) {
  const container = document.createElement("div");
  container.className = "nexa-vn-player";
  container.dataset.vnId = msgId;

  const barCount = 18;
  let seed = 0;
  for (let i = 0; i < (msgId || "").length; i++) seed += msgId.charCodeAt(i);
  
  let barsHtml = "";
  for (let i = 0; i < barCount; i++) {
    const h = 25 + Math.abs(Math.sin(seed + i * 0.8) * 70);
    barsHtml += `<div class="vn-bar" data-bar-idx="${i}" style="height: ${h.toFixed(1)}%;"></div>`;
  }

  container.innerHTML = `
    <button type="button" class="vn-btn-play" title="Play voice note">
      <svg class="vn-icon-play" viewBox="0 0 24 24"><polygon points="5 3 19 12 5 21 5 3"/></svg>
      <svg class="vn-icon-pause" viewBox="0 0 24 24" style="display: none;"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>
    </button>
    <div class="vn-body">
      <div class="vn-waveform-wrap">${barsHtml}</div>
      <div class="vn-meta">
        <span class="vn-time">0:00</span>
        <button type="button" class="vn-speed-btn" title="Playback speed">1x</button>
      </div>
    </div>
  `;

  const audio = new Audio(audioUrl);
  // Preload the whole clip so the FIRST tap plays instantly — no wait for the
  // stream to buffer. Voice notes are small, so eager loading is cheap.
  audio.preload = "auto";
  try { audio.load(); } catch (e) {}
  let playbackRate = 1.0;

  const playBtn = container.querySelector(".vn-btn-play");
  const playIcon = container.querySelector(".vn-icon-play");
  const pauseIcon = container.querySelector(".vn-icon-pause");
  const timeEl = container.querySelector(".vn-time");
  const speedBtn = container.querySelector(".vn-speed-btn");
  const bars = container.querySelectorAll(".vn-bar");
  const waveWrap = container.querySelector(".vn-waveform-wrap");

  function fmtVnTime(sec) {
    if (isNaN(sec) || !isFinite(sec)) return "0:00";
    const m = Math.floor(sec / 60);
    const s = Math.floor(sec % 60);
    return `${m}:${s.toString().padStart(2, "0")}`;
  }

  if (durationMs && durationMs > 0 && durationMs < 3600000) {
    timeEl.textContent = fmtVnTime(durationMs / 1000);
  }

  audio.addEventListener("loadedmetadata", () => {
    timeEl.textContent = fmtVnTime(audio.duration);
  });

  audio.addEventListener("timeupdate", () => {
    if (!audio.duration) return;
    const progress = audio.currentTime / audio.duration;
    timeEl.textContent = `${fmtVnTime(audio.currentTime)} / ${fmtVnTime(audio.duration)}`;
    
    const playedBars = Math.floor(progress * barCount);
    bars.forEach((b, idx) => {
      if (idx <= playedBars) {
        b.classList.add("played");
      } else {
        b.classList.remove("played");
      }
    });
  });

  audio.addEventListener("pause", () => {
    playIcon.style.display = "block";
    pauseIcon.style.display = "none";
  });

  window.addEventListener("nexa-audio-pause", (e) => {
    if (e.detail?.audio !== audio && !audio.paused) {
      audio.pause();
    }
  });

  audio.addEventListener("ended", () => {
    playIcon.style.display = "block";
    pauseIcon.style.display = "none";
    bars.forEach(b => b.classList.remove("played"));
    if (audio.duration) timeEl.textContent = fmtVnTime(audio.duration);
    if (activeAudioPlayer === audio) activeAudioPlayer = null;
  });

  playBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    if (audio.paused) {
      window.dispatchEvent(new CustomEvent("nexa-audio-pause", { detail: { audio } }));
      if (activeAudioPlayer && activeAudioPlayer !== audio) {
        try { activeAudioPlayer.pause(); } catch(err) {}
      }
      activeAudioPlayer = audio;
      audio.playbackRate = playbackRate;
      // Show a buffering hint so the first tap on a remote clip doesn't feel
      // frozen while it streams in.
      playBtn.classList.add("vn-loading");
      audio.play().then(() => {
        playBtn.classList.remove("vn-loading");
        playIcon.style.display = "none";
        pauseIcon.style.display = "block";
      }).catch(err => {
        playBtn.classList.remove("vn-loading");
        console.log("Audio play error:", err);
      });
    } else {
      audio.pause();
      playIcon.style.display = "block";
      pauseIcon.style.display = "none";
      if (activeAudioPlayer === audio) activeAudioPlayer = null;
    }
  });

  // Clear the buffering hint the moment playback actually starts or stalls out.
  audio.addEventListener("playing", () => playBtn.classList.remove("vn-loading"));
  audio.addEventListener("waiting", () => { if (!audio.paused) playBtn.classList.add("vn-loading"); });

  waveWrap.addEventListener("click", (e) => {
    e.stopPropagation();
    const rect = waveWrap.getBoundingClientRect();
    const clickX = e.clientX - rect.left;
    const pct = Math.max(0, Math.min(1, clickX / rect.width));
    if (audio.duration) {
      audio.currentTime = pct * audio.duration;
    }
  });

  speedBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    if (playbackRate === 1.0) playbackRate = 1.5;
    else if (playbackRate === 1.5) playbackRate = 2.0;
    else playbackRate = 1.0;
    speedBtn.textContent = playbackRate + "x";
    audio.playbackRate = playbackRate;
  });

  return container;
}

// Coalesce message-list renders: the two-direction listeners (A & B) can both
// fire in the same tick for one logical update, and a burst of edits/reads can
// fire several snapshots back-to-back. Collapsing to one render per animation
// frame keeps typing/receiving smooth with no double-work.
let _msgRenderScheduled = false;
function scheduleMessageRender() {
  if (_msgRenderScheduled) return;
  _msgRenderScheduled = true;
  requestAnimationFrame(() => {
    _msgRenderScheduled = false;
    renderMessageList();
  });
}

// WhatsApp-style floating "scroll to latest" button. Shown when the user has
// scrolled away from the bottom; its badge counts messages that arrive while
// scrolled up. (.messages has scroll-behavior:smooth, so a scrollTop jump
// animates on its own.)
let scrollDownNewCount = 0;

function isChatNearBottom() {
  const box = document.getElementById("messages");
  if (!box) return true;
  return box.scrollHeight - box.scrollTop - box.clientHeight < 200;
}

/* Force the messages view to the very bottom, instantly (bypassing the
   container's scroll-behavior:smooth so we never animate-lag behind), and
   re-assert after layout / async media / composer growth settle. This is what
   guarantees the latest message is fully in view — never "a little bit below"
   — on open, on send, and on receive. */
function scrollMessagesToBottom(immediate = true) {
  const box = document.getElementById("messages");
  if (!box) return;
  const doScroll = () => {
    const prevBehavior = box.style.scrollBehavior;
    box.style.scrollBehavior = "auto"; // override smooth for a hard jump
    box.scrollTop = box.scrollHeight;
    box.style.scrollBehavior = prevBehavior;
  };
  doScroll();
  if (!immediate) return;
  requestAnimationFrame(doScroll);
  setTimeout(doScroll, 60);
  setTimeout(doScroll, 200);
  setTimeout(doScroll, 450);
}

function scrollChatToBottom() {
  scrollMessagesToBottom();
  scrollDownNewCount = 0;
  updateScrollDownBtn();
}

function updateScrollDownBtn() {
  const btn = document.getElementById("scrollDownBtn");
  if (!btn) return;
  const nearBottom = isChatNearBottom();
  if (nearBottom) scrollDownNewCount = 0;
  btn.classList.toggle("show", !nearBottom);
  const badge = document.getElementById("scrollDownBadge");
  if (badge) {
    const showBadge = !nearBottom && scrollDownNewCount > 0;
    badge.style.display = showBadge ? "flex" : "none";
    if (showBadge) badge.textContent = scrollDownNewCount > 99 ? "99+" : String(scrollDownNewCount);
  }
}

function renderMessageList() {
  // A direct (imperative) render supersedes any pending coalesced render —
  // cancel it so a stale rAF doesn't overwrite a newer immediate render.
  _msgRenderScheduled = false;
  if (!selectedUser) return;
  const box = document.getElementById("messages");
  if (!box) return;

  const prevLast = allMessages.length ? allMessages[allMessages.length - 1] : null;
  const prevLastTime = prevLast ? (prevLast.createdAt || 0) : 0;
  // Dedupe by id — a seam-healed message can later slide back into the live
  // window, appearing in both arrays.
  const seenIds = new Set();
  allMessages = [..._olderMsgsA, ..._msgsA, ..._olderMsgsB, ..._msgsB]
    .filter(m => { if (seenIds.has(m.id)) return false; seenIds.add(m.id); return true; })
    .sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));

  // Check and apply disappearing messages filter
  const key = getChatKey(currentUser.uid, selectedUser.uid);
  const setting = disappearingSettings[key] || 'off';
  const msLimit = parseDisappearingMs(setting);

  if (msLimit > 0) {
    const now = Date.now();
    const validMsgs = [];
    const expiredDocIds = [];

    allMessages.forEach(msg => {
      if (msg.createdAt && (now - msg.createdAt) > msLimit) {
        expiredDocIds.push(msg.id);
      } else {
        validMsgs.push(msg);
      }
    });

    allMessages = validMsgs;

    if (expiredDocIds.length > 0) {
      expiredDocIds.forEach(docId => {
        db.collection("chats").doc(docId).delete().catch(err => console.warn("Purging expired msg error:", err));
      });
    }
  }

  // Preserve scroll position unless the user was already near the bottom.
  const wasNearBottom = box.scrollHeight - box.scrollTop - box.clientHeight < 120;
  const currentMsgIds = new Set(allMessages.map(m => m.id));

  // Remove deleted messages
  box.querySelectorAll(".msg").forEach(el => {
    if (el.dataset.msgId && !currentMsgIds.has(el.dataset.msgId)) {
      el.remove();
    }
  });

  // Handle empty chat container
  const emptyChatEl = box.querySelector(".empty-chat");
  if (!allMessages.length) {
    if (isSelfChat()) {
      box.innerHTML = '<div class="empty-chat"><div class="empty-chat-icon">📝</div><div style="font-size: 16px; font-weight: 600; margin-bottom: 6px;">Message yourself</div><div style="font-size: 13px; color: var(--text-3);">Jot down notes, links, or reminders — only you can see this.</div></div>';
    } else {
      box.innerHTML = '<div class="empty-chat"><div class="empty-chat-icon">💬</div><div style="font-size: 16px; font-weight: 600; margin-bottom: 6px;">Nexa Messenger</div><div style="font-size: 13px; color: var(--text-3);">No messages yet. Send a message to start conversation!</div></div>';
    }
    updateScrollDownBtn();
    return;
  } else if (emptyChatEl) {
    emptyChatEl.remove();
  }

  let maxTime = latestMsgTime[selectedUser.uid] || 0;
  let lastDateKey = null;
  let prevSender = null;
  // Stable reorder: ref walks front-to-back; each desired node is moved to the
  // ref position only when out of place (no-op moves are skipped → no shake).
  let ref = box.firstElementChild;
  const place = (node) => {
    if (node !== ref) box.insertBefore(node, ref);
    ref = node.nextElementSibling;
  };
  const seenDateKeys = new Set();

  allMessages.forEach((msg, idx) => {
    if (msg.createdAt > maxTime) maxTime = msg.createdAt;
    const fromMe = msg.from === currentUser.uid;

    const dateKey = new Date(msg.createdAt).toDateString();
    const senderKey = fromMe ? 'me' : 'them';
    const isGrouped = dateKey === lastDateKey && prevSender === senderKey;

    // Insert a date separator when the day changes.
    if (dateKey !== lastDateKey) {
      seenDateKeys.add(dateKey);
      let sep = box.querySelector(`.date-sep[data-date-key="${CSS.escape(dateKey)}"]`);
      if (!sep) {
        sep = document.createElement("div");
        sep.className = "date-sep";
        sep.dataset.dateKey = dateKey;
        const span = document.createElement("span");
        span.textContent = fmtDateSep(msg.createdAt);
        sep.appendChild(span);
        sep.classList.add("msg-new");
        setTimeout(() => { if (sep) sep.classList.remove("msg-new"); }, 200);
      }
      place(sep);
    }

    lastDateKey = dateKey;
    prevSender = senderKey;

    let msgEl = box.querySelector(`.msg[data-msg-id="${msg.id}"]`);

    if (!msgEl) {
      msgEl = buildMessage(msg.id, msg, fromMe);
      msgEl.classList.add("msg-new");
      setTimeout(() => { if (msgEl) msgEl.classList.remove("msg-new"); }, 200);
    } else {
      // In-place tick update without DOM re-attachment or shaking
      const tick = fromMe && msg.read ? "✓✓" : fromMe ? "✓" : "";
      const editLabel = msg.edited ? ' <span style="font-size: 9.5px; opacity: 0.55; font-style: italic;">(edited)</span>' : '';
      const metaEl = msgEl.querySelector(".msg-meta");
      if (metaEl) {
        metaEl.innerHTML = `<span>${formatTime(msg.createdAt)}</span>${tick ? `<span>${tick}</span>` : ""} ${editLabel}`;
      }

      // In-place reactions update for instant visual feedback
      updateMsgReactionsInPlace(msgEl, msg.id, msg.reactions);

      // In-place view once update
      if (msg.isViewOnce) {
        const badge = msgEl.querySelector(".view-once-badge");
        if (badge) {
          const isOpened = msg.viewOnceOpened;
          badge.className = `view-once-badge ${isOpened ? "opened" : ""}`;
          if (isOpened) {
            const mediaType = msg.image ? "Photo" : msg.video ? "Video" : msg.audio ? "Voice note" : "Media";
            badge.innerHTML = `<div class="view-once-circle">✓</div><span>Opened (${mediaType})</span>`;
            badge.onclick = null;
          }
        }
      }
    }

    place(msgEl);
    msgEl.classList.toggle("same-sender", !!isGrouped);
  });

  // Remove date separators whose day no longer has any messages.
  box.querySelectorAll(".date-sep").forEach(sep => {
    if (!seenDateKeys.has(sep.dataset.dateKey)) sep.remove();
  });

  if (allMessages.length > 0) {
    const lastM = allMessages[allMessages.length - 1];
    if (lastM.createdAt) {
      latestMsgTime[selectedUser.uid] = lastM.createdAt;
      latestMsgText[selectedUser.uid] = lastM.text || (lastM.image ? "📷 Photo" : lastM.video ? "🎥 Video" : lastM.audio ? "🎤 Voice" : "Message");
    }
    // Keep the instant-open cache for this chat fresh (newest windowed msgs).
    saveChatMsgCache(selectedUser.uid);
  }

  if (chatJustOpened) {
    // Force scroll to the very bottom on chat open — the helper overrides the
    // container's smooth-scroll and re-asserts across async layout / media /
    // both Firestore snapshots, so the latest message is truly in view.
    scrollMessagesToBottom();
    setTimeout(() => { chatJustOpened = false; }, 600);
  } else if (_loadingOlderRender) {
    // Older history was prepended — loadOlderMessages restores the scroll
    // position itself; never auto-scroll here.
  } else if (wasNearBottom) {
    scrollMessagesToBottom(false);
    box.style.scrollBehavior = "auto";
    box.scrollTop = box.scrollHeight;
    box.style.scrollBehavior = "";
  }

  // Badge the scroll-down button when a new incoming message lands while the
  // user is scrolled up reading older messages.
  const newLast = allMessages.length ? allMessages[allMessages.length - 1] : null;
  if (newLast && (newLast.createdAt || 0) > prevLastTime && !wasNearBottom && !chatJustOpened && newLast.from !== currentUser.uid) {
    scrollDownNewCount++;
  }
  updateScrollDownBtn();

  if (allMessages.length) {
    const last = allMessages[allMessages.length - 1];
    const prevEl = document.getElementById(`prev-${selectedUser.uid}`);
    if (prevEl) {
      const preview = last.text || (last.image ? "📷 Photo" : last.video ? "🎥 Video" : last.audio ? "🎤 Voice" : "📎 File");
      prevEl.textContent = (last.from === currentUser.uid ? "You: " : "") + preview;
    }
  }

  renderUsers();
}

let isViewOnceActive = false;

function toggleViewOnceMode() {
  isViewOnceActive = !isViewOnceActive;
  const btn = document.getElementById("viewOnceToggleBtn");
  if (btn) btn.classList.toggle("active", isViewOnceActive);
  showNotifToast(isViewOnceActive ? "1 View Once enabled for next media" : "View Once disabled", "info");
}

function openViewOnceModal(mediaUrl, mediaType, docId, caption) {
  const modal = document.getElementById("viewOnceModal");
  const container = document.getElementById("viewOnceContent");
  if (!modal || !container) return;

  container.innerHTML = "";
  if (mediaType === "image") {
    const img = document.createElement("img");
    img.src = mediaUrl;
    container.appendChild(img);
  } else if (mediaType === "video") {
    const vid = document.createElement("video");
    vid.src = mediaUrl;
    vid.controls = true;
    vid.controlsList = "nodownload nofullscreen noremoteplayback";
    vid.disablePictureInPicture = true;
    vid.autoplay = true;
    container.appendChild(vid);
  } else if (mediaType === "audio") {
    const aud = document.createElement("audio");
    aud.src = mediaUrl;
    aud.controls = true;
    aud.autoplay = true;
    container.appendChild(aud);
  }

  if (caption) {
    const cap = document.createElement("div");
    cap.className = "view-once-caption";
    cap.textContent = caption;
    container.appendChild(cap);
  }

  modal.classList.add("active");
  if (window.lucide) lucide.createIcons();

  if (docId) {
    db.collection("chats").doc(docId).update({
      viewOnceOpened: true,
      read: true
    }).catch(err => console.error("Error updating viewOnce state:", err));
  }
}

function closeViewOnceModal() {
  const modal = document.getElementById("viewOnceModal");
  const container = document.getElementById("viewOnceContent");
  if (modal) modal.classList.remove("active");
  if (container) container.innerHTML = "";
  renderMessageList();
}

/* View-once screenshot/save protection.
   Browsers can't block OS-level screenshots, but we can stop the obvious
   routes: right-click save, long-press save, image drag, the native download
   button, and we close the viewer if the user backgrounds the tab/app. */
document.addEventListener("contextmenu", e => {
  const modal = document.getElementById("viewOnceModal");
  if (modal && modal.classList.contains("active") && e.target.closest("#viewOnceContent")) {
    e.preventDefault();
    showNotifToast("🔒 View once media can't be saved", "info");
  }
});
document.addEventListener("dragstart", e => {
  const modal = document.getElementById("viewOnceModal");
  if (modal && modal.classList.contains("active")) { e.preventDefault(); }
});
document.addEventListener("visibilitychange", () => {
  if (document.hidden) {
    const modal = document.getElementById("viewOnceModal");
    if (modal && modal.classList.contains("active")) {
      // Blank the media before closing so it isn't left on screen.
      const container = document.getElementById("viewOnceContent");
      if (container) container.innerHTML = "";
      modal.classList.remove("active");
      renderMessageList();
    }
  }
});
// Block Ctrl+S / Cmd+S while the viewer is open.
document.addEventListener("keydown", e => {
  if ((e.ctrlKey || e.metaKey) && (e.key === "s" || e.key === "S" || e.key === "u" || e.key === "U")) {
    const modal = document.getElementById("viewOnceModal");
    if (modal && modal.classList.contains("active")) { e.preventDefault(); }
  }
});

function buildStoryReplyPreview(msg) {
  const isOwnStory = msg.storyReplyUid === currentUser?.uid;
  const authorName = isOwnStory ? "Your Story" : (escapeHtml(selectedUser?.displayName || "User") + "'s Story");

  // Fallback to active userStatuses cache if properties missing from msg document
  let storyObj = null;
  if (userStatuses && userStatuses[msg.storyReplyUid]) {
    storyObj = userStatuses[msg.storyReplyUid].find(s => s.id === msg.storyReplyId);
  }

  const url = safeMediaUrl(msg.storyUrl || storyObj?.url || "") || null;
  const text = msg.storyText || storyObj?.text || null;
  const type = msg.storyType || storyObj?.type || (url ? "image" : "text");
  const musicTitle = msg.storyMusicTitle || storyObj?.musicData?.title || null;

  let thumbnailHtml = "";
  if (url && type !== "music") {
    if (type === "video") {
      thumbnailHtml = `<div class="story-reply-thumb"><video src="${url}" style="width:40px; height:40px; object-fit:cover; border-radius:6px; pointer-events:none;"></video></div>`;
    } else {
      thumbnailHtml = `<div class="story-reply-thumb"><img src="${url}" style="width:40px; height:40px; object-fit:cover; border-radius:6px;" onerror="this.style.display='none'"></div>`;
    }
  } else if (type === "music" || musicTitle) {
    thumbnailHtml = `<div class="story-reply-thumb story-thumb-music">🎵</div>`;
  } else if (text) {
    thumbnailHtml = `<div class="story-reply-thumb story-thumb-text">📝</div>`;
  } else {
    thumbnailHtml = `<div class="story-reply-thumb story-thumb-default">📷</div>`;
  }

  let previewSubtext = "Story update";
  if (musicTitle) {
    previewSubtext = `🎵 ${escapeHtml(musicTitle)}`;
  } else if (text) {
    previewSubtext = escapeHtml(text);
  } else if (type === "video") {
    previewSubtext = "🎥 Video Story";
  } else if (type === "image" || url) {
    previewSubtext = "📷 Photo Story";
  }

  return `
    <div class="whatsapp-story-reply-card" onclick="openStoryViewer('${msg.storyReplyUid}', '${msg.storyReplyId}')">
      <div class="story-reply-info-col">
        <div class="story-reply-author"><i data-lucide="corner-up-left" style="width: 12px; height: 12px; vertical-align: -1px; display: inline-flex;"></i> ${authorName}</div>
        <div class="story-reply-subtext">${previewSubtext}</div>
      </div>
      ${thumbnailHtml}
    </div>
  `;
}

function buildReactionsHTML(id, reactions) {
  if (!reactions || !Object.keys(reactions).length) return "";
  let html = '<div class="msg-reactions">';
  for (const [emoji, users] of Object.entries(reactions)) {
    if (!Array.isArray(users) || !users.length) continue;
    const mine = currentUser && users.includes(currentUser.uid);
    
    const reactorNames = users.map(uId => {
      if (currentUser && uId === currentUser.uid) return "You";
      const found = (allUsersData || []).find(u => u.uid === uId);
      return found ? (found.displayName || found.name || "User") : "User";
    }).join(", ");

    html += `<div class="reaction${mine ? ' me-reacted' : ''}" 
                   title="${escapeHtml(reactorNames)}" 
                   onclick="event.stopPropagation(); toggleReact('${id}', '${emoji}')">
                <span class="reaction-emoji">${emoji}</span>
                <span class="reaction-count">${users.length}</span>
              </div>`;
  }
  html += '</div>';
  return html;
}

// Shared helper: patch a message's reaction strip IN PLACE so reacting feels
// instant (no full list re-render, no scroll jump). Used by group messages
// (toggleGroupMsgReact) and by the live 1:1 render reconciler. Mirrors the
// DOM shape produced by buildMessage/buildGroupMessage (.msg-reactions sits
// directly before .msg-meta inside .bubble).
function updateMsgReactionsInPlace(msgEl, id, reactions) {
  if (!msgEl) return;
  const newHTML = buildReactionsHTML(id, reactions);
  const reactEl = msgEl.querySelector(".msg-reactions");
  const getFirstEl = (html) => {
    const temp = document.createElement("div");
    temp.innerHTML = html;
    return temp.firstElementChild;
  };
  const swapWith = (oldEl, newEl) => {
    const parent = oldEl.parentNode;
    if (parent && parent.replaceChild) parent.replaceChild(newEl, oldEl);
    else oldEl.replaceWith(newEl);
  };
  if (reactEl) {
    if (newHTML) {
      swapWith(reactEl, getFirstEl(newHTML));
    } else if (reactEl.remove) {
      reactEl.remove();
    } else if (reactEl.parentNode) {
      reactEl.parentNode.removeChild(reactEl);
    }
  } else if (newHTML) {
    const bubble = msgEl.querySelector(".bubble");
    const metaEl = msgEl.querySelector(".msg-meta");
    if (bubble) {
      const newEl = getFirstEl(newHTML);
      if (metaEl) {
        bubble.insertBefore(newEl, metaEl);
      } else {
        bubble.appendChild(newEl);
      }
    }
  }
}

function buildMessage(id, msg, fromMe) {
  const el = document.createElement("div");
  el.className = `msg ${fromMe ? "me" : "them"}`;
  el.dataset.msgId = id;

  const row = document.createElement("div");
  row.className = "msg-row";

  const bubble = document.createElement("div");
  bubble.className = "bubble";

  let inner = "";

  // Handle story reply with WhatsApp-style preview card
  if (msg.storyReplyId && msg.storyReplyUid) {
    inner += buildStoryReplyPreview(msg);
  }

  // Handle regular reply
  if (msg.replyTo) {
    const replied = allMessages.find(m => m.id === msg.replyTo);
    // Prefer the loaded original; fall back to the denormalized fields stored
    // on the reply message itself so the preview shows on both sides.
    const rText = replied ? (replied.text || (replied.image ? "(Photo)" : replied.video ? "(Video)" : replied.audio ? "(Voice note)" : "(message)")) : (msg.replyToText || "(message)");
    const rFrom = replied ? replied.from : (msg.replyToFrom || null);
    const rAuthor = rFrom === currentUser.uid ? "You" : escapeHtml(selectedUser?.displayName || "User");
    inner += `<div class="reply-quote"><div class="reply-quote-author"><i data-lucide="corner-up-left" style="width: 11px; height: 11px; vertical-align: -1px; margin-right: 4px;"></i>${rAuthor}</div><div class="reply-quote-text">${linkify(typeof rText === "string" ? rText : "(message)")}</div></div>`;
  }

  // Forwarded label
  if (msg.forwarded) {
    const fFrom = msg.forwardedFrom ? ` from ${escapeHtml(msg.forwardedFrom)}` : '';
    inner += `<div class="forwarded-label"><i data-lucide="forward" style="width: 12px; height: 12px;"></i> Forwarded${fFrom}</div>`;
  }

  // In-chat group/channel invite card (WhatsApp-style Follow/Join
  if (msg.invite && msg.invite.kind) {
    const invHtml = buildInviteCardHTML(msg);
    if (invHtml) inner += invHtml;
  }

  // Handle View Once Media
  if (msg.isViewOnce && (msg.image || msg.video || msg.audio)) {
    const isOpened = msg.viewOnceOpened;
    const mediaType = msg.image ? "Photo" : msg.video ? "Video" : msg.audio ? "Voice note" : "Media";
    const typeKey = msg.image ? "image" : msg.video ? "video" : "audio";
    const mediaUrl = safeMediaUrl(msg.image || msg.video || msg.audio);

    const badge = document.createElement("div");
    badge.className = `view-once-badge ${isOpened ? "opened" : ""}`;

    if (isOpened) {
      badge.innerHTML = `
        <div class="view-once-circle">✓</div>
        <span>Opened (${mediaType})</span>
      `;
    } else {
      badge.innerHTML = `
        <div class="view-once-circle">1</div>
        <span>${mediaType} (Tap to view)</span>
      `;
      if (!fromMe || currentUser.uid === msg.to) {
        badge.onclick = () => openViewOnceModal(mediaUrl, typeKey, id, msg.caption);
      } else {
        badge.onclick = () => showNotifToast("Sent as View Once media", "info");
      }
    }

    bubble.appendChild(badge);
  } else {
    if (msg.type === 'poll' && msg.pollOptions) {
      inner += buildPollHTML(id, msg);
    } else if (msg.text) {
      inner += `<div class="msg-text">${linkify(msg.text)}</div>`;
    }

    if (msg.image) {
      const imgUrl = safeMediaUrl(msg.image);
      const cap = msg.caption ? `<div class="media-caption below">${escapeHtml(msg.caption)}</div>` : "";
      inner += imgUrl ? `<div class="media-container">
        <img src="${imgUrl}"
             onclick="viewImg('${imgUrl}')"
             loading="lazy"
             alt="shared image"
             draggable="false">
      </div>` + (cap || "") : "";
    }

    if (msg.video) {
      const vidUrl = safeMediaUrl(msg.video);
      const cap = msg.caption ? `<div class="media-caption below">${escapeHtml(msg.caption)}</div>` : "";
      inner += vidUrl ? `<div class="media-container">
        <video controls preload="metadata" controlslist="nodownload" disablepictureinpicture>
          <source src="${vidUrl}" type="video/mp4">
          Your browser does not support video playback.
        </video>
      </div>` + (cap || "") : "";
    }

    // Universal file/document attachment — any file type (PDF, doc, zip, audio,
    // etc.). Audio files render an inline player so they can be listened to in
    // the chat; everything else renders a tappable card to open/download.
    if (msg.fileUrl) {
      const fUrl = safeMediaUrl(msg.fileUrl);
      const fname = escapeHtml(msg.fileName || "Attachment");
      const fsize = msg.fileSize ? formatFileSize(msg.fileSize) : "";
      const ficon = fileIconFor(msg.fileName || "");
      const isAudioFile = /\.(mp3|wav|ogg|m4a|aac|flac|opus|webm)$/i.test(msg.fileName || "");
      if (fUrl && isAudioFile) {
        inner += `<div class="file-attach file-attach-audio">
          <div class="file-attach-icon">${ficon}</div>
          <div class="file-attach-info">
            <div class="file-attach-name">${fname}</div>
            ${fsize ? `<div class="file-attach-size">${fsize}</div>` : ""}
            <audio controls preload="metadata" src="${fUrl}" style="width:100%;margin-top:8px;"></audio>
          </div>
        </div>`;
      } else if (fUrl) {
        inner += `<a class="file-attach" href="${fUrl}" target="_blank" rel="noopener noreferrer" download="${fname}">
          <div class="file-attach-icon">${ficon}</div>
          <div class="file-attach-info">
            <div class="file-attach-name">${fname}</div>
            ${fsize ? `<div class="file-attach-size">${fsize}</div>` : ""}
          </div>
          <div class="file-attach-open">⤓</div>
        </a>`;
      }
    }
  }

  inner += buildReactionsHTML(id, msg.reactions);


  const tick = fromMe && msg.read ? "✓✓" : fromMe ? "✓" : "";
  const editLabel = msg.edited ? ' <span style="font-size: 9.5px; opacity: 0.55; font-style: italic;">(edited)</span>' : '';
  inner += `<div class="msg-meta"><span>${formatTime(msg.createdAt)}</span>${tick ? `<span>${tick}</span>` : ""} ${editLabel}</div>`;

  if (!msg.isViewOnce || !(msg.image || msg.video || msg.audio)) {
    bubble.innerHTML = inner;
    if (msg.audio && !msg.isViewOnce) {
      const vnNode = renderVoiceNotePlayer(id, msg.audio, msg.duration, fromMe);
      const metaEl = bubble.querySelector(".msg-meta");
      if (metaEl) {
        bubble.insertBefore(vnNode, metaEl);
      } else {
        bubble.appendChild(vnNode);
      }
    }
  } else {
    const metaDiv = document.createElement("div");
    metaDiv.className = "msg-meta";
    metaDiv.innerHTML = `<span>${formatTime(msg.createdAt)}</span>${tick ? `<span>${tick}</span>` : ""} ${editLabel}`;
    bubble.appendChild(metaDiv);
  }

  row.appendChild(bubble);

  // Touch swipe right to reply (WhatsApp style)
  let startX = 0, startY = 0, currentX = 0, isSwiping = false;

  row.addEventListener("touchstart", (e) => {
    if (e.touches.length !== 1) return;
    startX = e.touches[0].clientX;
    startY = e.touches[0].clientY;
    currentX = 0;
    isSwiping = false;
    row.style.transition = 'none';
  }, { passive: true });

  row.addEventListener("touchmove", (e) => {
    if (!startX) return;
    const touchX = e.touches[0].clientX;
    const touchY = e.touches[0].clientY;
    const diffX = touchX - startX;
    const diffY = Math.abs(touchY - startY);

    if (diffX > 8 && diffY < diffX) {
      isSwiping = true;
      currentX = Math.min(diffX * 0.55, 75);
      row.style.transform = `translateX(${currentX}px)`;
    }
  }, { passive: true });

  row.addEventListener("touchend", () => {
    if (isSwiping && currentX > 40) {
      if (navigator.vibrate) navigator.vibrate(25);
      replyMsg(id);
    }
    row.style.transition = 'transform 0.22s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
    row.style.transform = 'translateX(0)';
    startX = 0;
    currentX = 0;
    isSwiping = false;
  });

  el.appendChild(row);
  el.addEventListener("contextmenu", e => { e.preventDefault(); showCtxMenu(e, id, msg); });

  return el;
}

function replyMsg(msgId) {
  let msg = null;
  if (currentChatMode === 'group') {
    msg = currentGroupMessages.find(m => m.id === msgId);
  } else {
    msg = allMessages.find(m => m.id === msgId);
  }
  if (!msg) return;
  messageRepliedTo = msg;
  const isMine = (msg.from || msg.senderUid) === currentUser.uid;
  const author = isMine ? "You" : escapeHtml(msg.senderName || selectedUser?.displayName || "User");
  document.getElementById("replyStripWrap").innerHTML = `
    <div class="reply-strip">
      <i data-lucide="corner-up-left" class="reply-strip-ico" style="width: 15px; height: 15px; flex-shrink: 0; stroke-width: 2.4;"></i>
      <div class="reply-strip-info">
        <div class="reply-strip-author">Replying to ${author}</div>
        <div class="reply-strip-text">${escapeHtml(msg.text) || "(attachment)"}</div>
      </div>
      <button class="reply-strip-close" onclick="clearReply()" title="Cancel reply"><i data-lucide="x" style="width: 15px; height: 15px;"></i></button>
    </div>`;
  document.getElementById("text").focus();
  if (window.lucide) lucide.createIcons();
  closeCtxMenu();
}

function clearReply() {
  messageRepliedTo = null;
  document.getElementById("replyStripWrap").innerHTML = "";
}

function startEdit(msgId) {
  const msg = allMessages.find(m => m.id === msgId);
  if (!msg || !msg.text) { alert("Cannot edit this message"); return; }
  editingMessageId = msgId;
  document.getElementById("editText").value = msg.text;
  document.getElementById("editModal").classList.add("active");
  document.getElementById("editText").focus();
  closeCtxMenu();
}

function closeEditModal() {
  editingMessageId = null;
  document.getElementById("editText").value = "";
  document.getElementById("editModal").classList.remove("active");
}

function saveEditMessage() {
  if (!editingMessageId) return;
  const newText = document.getElementById("editText").value.trim();
  if (!newText) { alert("Cannot be empty"); return; }
  db.collection("chats").doc(editingMessageId).update({ text: newText, edited: true, editedAt: Date.now() })
    .then(() => {
      // The live listener covers the newest window; patch older pages locally.
      const m = findLocalMsg(editingMessageId);
      if (m) { m.text = newText; m.edited = true; renderMessageList(); }
      closeEditModal();
    })
    .catch(e => alert("Error: " + e.message));
}

function showCtxMenu(e, id, msg) {
  const menu = document.getElementById("ctxMenu");
  const fromMe = (msg.from || msg.senderUid) === currentUser.uid;
  const isGroup = currentChatMode === 'group';
  menu.innerHTML = `
    <div class="ctx-reaction-bar">
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '❤')` : `addQuickReact('${id}', '❤')`}">❤</button>
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '😂')` : `addQuickReact('${id}', '😂')`}">😂</button>
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '😮')` : `addQuickReact('${id}', '😮')`}">😮</button>
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '😢')` : `addQuickReact('${id}', '😢')`}">😢</button>
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '🙏')` : `addQuickReact('${id}', '🙏')`}">🙏</button>
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '👍')` : `addQuickReact('${id}', '👍')`}">👍</button>
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '🔥')` : `addQuickReact('${id}', '🔥')`}">🔥</button>
    </div>
    <div class="ctx-divider"></div>
    <div class="ctx-item" onclick="replyMsg('${id}')"><i data-lucide="corner-up-left" style="width: 15px; height: 15px;"></i> Reply</div>
    ${msg.text ? `<div class="ctx-item" onclick="copyMsg('${id}')"><i data-lucide="copy" style="width: 15px; height: 15px;"></i> Copy</div>` : ''}
    ${isGroup ? `<div class="ctx-item" onclick="pinGroupMsg('${id}')"><i data-lucide="pin" style="width: 15px; height: 15px;"></i> Pin</div>` : ''}
    ${fromMe && !isGroup && msg.text ? `<div class="ctx-item" onclick="startEdit('${id}')"><i data-lucide="pencil" style="width: 15px; height: 15px;"></i> Edit</div>` : ''}
    ${fromMe ? `<div class="ctx-item danger" onclick="${isGroup ? `deleteGroupMsg('${id}')` : `deleteMsg('${id}')`}"><i data-lucide="trash-2" style="width: 15px; height: 15px;"></i> Delete</div>` : ''}
  `;
  menu.classList.add("active");
  menu.style.top = Math.min(e.clientY, window.innerHeight - 240) + "px";
  menu.style.left = Math.min(e.clientX, window.innerWidth - 220) + "px";
  if (window.lucide) lucide.createIcons();
  setTimeout(() => document.addEventListener("click", closeCtxMenu, { once: true }), 0);
}

function closeCtxMenu() {
  document.getElementById("ctxMenu").classList.remove("active");
}

function copyMsg(id) {
  const msg = (currentChatMode === 'group')
    ? (currentGroupMessages.find(m => m.id === id))
    : (allMessages.find(m => m.id === id));
  if (msg && msg.text) {
    navigator.clipboard.writeText(msg.text);
    showNotifToast("✓ Copied", "success");
  }
  closeCtxMenu();
}

function deleteGroupMsg(id) {
  if (!selectedGroup) return;
  if (confirm("Delete this group message?")) {
    db.collection("groups").doc(selectedGroup.id).collection("messages").doc(id).delete()
      .catch(e => console.error("deleteGroupMsg error:", e));
  }
  closeCtxMenu();
}

async function toggleGroupMsgReact(msgId, emoji) {
  closeCtxMenu();
  if (!selectedGroup || !currentUser) return;
  const msg = currentGroupMessages.find(m => m.id === msgId);
  if (!msg) return;

  const r = { ...(msg.reactions || {}) };
  r[emoji] = r[emoji] ? [...r[emoji]] : [];
  const i = r[emoji].indexOf(currentUser.uid);

  if (i > -1) {
    r[emoji].splice(i, 1);
    if (!r[emoji].length) delete r[emoji];
  } else {
    r[emoji].push(currentUser.uid);
  }

  msg.reactions = r;
  // Instant in-place feedback — no full re-render, so the list doesn't jump
  // and the tap feels immediate. The Firestore snapshot reconciles later.
  const msgEl = document.querySelector(`.msg[data-msg-id="${msgId}"]`);
  updateMsgReactionsInPlace(msgEl, msgId, r);

  try {
    await db.collection("groups").doc(selectedGroup.id).collection("messages").doc(msgId).update({ reactions: r });
  } catch (err) {
    console.error("Error updating group reaction:", err);
  }
}

function deleteMsg(id) {
  if (confirm("Delete this message?")) {
    db.collection("chats").doc(id).delete().then(() => {
      // The live listener prunes window messages; older pages are pruned here.
      [_olderMsgsA, _olderMsgsB].forEach(arr => {
        const i = arr.findIndex(m => m.id === id);
        if (i > -1) arr.splice(i, 1);
      });
      renderMessageList();
    }).catch(e => console.error("deleteMsg error:", e));
  }
  closeCtxMenu();
}

function addQuickReact(msgId, emoji) {
  toggleReact(msgId, emoji);
  closeCtxMenu();
}

async function toggleReact(msgId, emoji) {
  if (!currentUser) return;
  if (currentChatMode === 'group') {
    await toggleGroupMsgReact(msgId, emoji);
    return;
  }
  const msg = allMessages.find(m => m.id === msgId);
  if (!msg) return;

  const r = { ...(msg.reactions || {}) };
  r[emoji] = r[emoji] ? [...r[emoji]] : [];
  const i = r[emoji].indexOf(currentUser.uid);
  const wasAdded = (i === -1);

  if (i > -1) {
    r[emoji].splice(i, 1);
    if (!r[emoji].length) delete r[emoji];
  } else {
    r[emoji].push(currentUser.uid);
  }

  // Optimistic UI update across all internal message arrays
  msg.reactions = r;
  const msgInA = _msgsA.find(m => m.id === msgId);
  if (msgInA) msgInA.reactions = r;
  const msgInB = _msgsB.find(m => m.id === msgId);
  if (msgInB) msgInB.reactions = r;

  renderMessageList();

  try {
    await db.collection("chats").doc(msgId).update({ reactions: r });
  } catch (err) {
    console.error("Error updating reaction:", err);
    showNotifToast("Error updating reaction: " + err.message, "error");
  }

  // Send push notification if reaction was added and message is from another user
  if (wasAdded && notificationSettings.reactions && msg.from !== currentUser.uid) {
    const myName = (document.getElementById('myName')?.textContent || 'Someone').trim();
    sendPushNotification(
      '🎉 New reaction!',
      `${myName} reacted ${emoji} to your message`,
      msg.from
    );
  }
}

window.toggleReact = toggleReact;
window.addQuickReact = addQuickReact;
window.renderMessages = renderMessageList;

// (Auto-save removed: media/files are only saved when the user explicitly
// chooses to — via the image viewer, video player, or the attachment card's
// download action.)


async function scrollToMsg(msgId) {
  let el = document.querySelector(`[data-msg-id="${msgId}"]`);
  // The target may live in unloaded history — page older chunks in until it
  // renders (or history is exhausted).
  let guard = 0;
  while (!el && selectedUser && msgPagingHasMore() && !msgPaging.loading && guard < 60) {
    guard++;
    await loadOlderMessages();
    el = document.querySelector(`[data-msg-id="${msgId}"]`);
  }
  if (!el) return;
  el.scrollIntoView({ behavior: "smooth", block: "center" });
  el.style.outline = "2px solid var(--cyan)";
  el.style.borderRadius = "14px";
  setTimeout(() => {
    el.style.outline = "";
    el.style.borderRadius = "";
  }, 1500);
}

/* =========================================================================
   FILE UPLOAD
   ========================================================================= */
async function uploadFile(file) {
  const form = new FormData();
  form.append("file", file);
  form.append("upload_preset", UPLOAD_PRESET);
  try {
    const res = await fetch(`https://api.cloudinary.com/v1_1/${CLOUD_NAME}/auto/upload`, {
      method: "POST",
      body: form
    });
    const data = await res.json();
    if (!res.ok) {
      throw new Error(data.error?.message || "Upload failed");
    }
    if (!data.secure_url) {
      throw new Error("No URL returned from server");
    }
    return data.secure_url;
  } catch (err) {
    console.error("Upload error:", err);
    throw err;
  }
}

function pickImage() {
  if (!selectedUser) { alert("Select a user first"); return; }
  document.getElementById("img").click();
}

// WhatsApp-style: the media button lets the user CHOOSE Photo or Video.
function pickMedia() {
  if (!selectedUser && !selectedGroup && !selectedChannel) { alert("Select a conversation first"); return; }
  // If the device supports a combined image+video picker, use one input that
  // accepts both and route by the picked type — simplest, most native feel.
  const inp = document.getElementById("mediaPickerInput") || (() => {
    const i = document.createElement("input");
    i.type = "file";
    i.id = "mediaPickerInput";
    i.hidden = true;
    i.accept = "image/*,video/*";
    document.body.appendChild(i);
    i.addEventListener("change", e => {
      const f = e.target.files[0];
      e.target.value = "";
      if (!f || (!selectedUser && !selectedGroup && !selectedChannel)) return;
      if (f.type.startsWith("image/")) openMediaComposer(f, "image");
      else if (f.type.startsWith("video/")) openMediaComposer(f, "video");
      else showNotifToast("Please pick a photo or video", "error");
    });
    return i;
  })();
  inp.click();
}

function pickFile() {
  if (!selectedUser && !selectedGroup && !selectedChannel) { alert("Select a conversation first"); return; }
  document.getElementById("fileInput").click();
}

document.getElementById("img").addEventListener("change", async e => {
  const file = e.target.files[0];
  e.target.value = "";
  if (!file || !selectedUser) return;
  if (!file.type.startsWith("image/")) {
    showNotifToast("Please pick an image", "error");
    return;
  }
  openMediaComposer(file, "image");
});

document.getElementById("fileInput").addEventListener("change", async e => {
  const file = e.target.files[0];
  e.target.value = "";
  if (!file || !selectedUser) return;

  // Always preview before sending — route every file through the composer so
  // the user sees what they're about to send (image, video, or any document).
  if (file.type.startsWith("image/")) {
    openMediaComposer(file, "image");
  } else if (file.type.startsWith("video/")) {
    openMediaComposer(file, "video");
  } else {
    openMediaComposer(file, "file");
  }
});

/* =========================================================================
   MEDIA COMPOSER (caption + view-once, WhatsApp-style)
   ========================================================================= */
let mcPendingFile = null;          // File awaiting send
let mcPendingType = "image";       // "image" | "video"
let mcViewOnce = false;            // composer-level view-once toggle
let mcObjectURL = null;            // object URL for the preview

function openMediaComposer(file, type) {
  mcPendingFile = file;
  mcPendingType = type;
  mcViewOnce = false;

  if (mcObjectURL) URL.revokeObjectURL(mcObjectURL);
  mcObjectURL = URL.createObjectURL(file);

  const preview = document.getElementById("mcPreview");
  preview.innerHTML = "";
  if (type === "image") {
    preview.innerHTML = `<img src="${mcObjectURL}" alt="preview">`;
  } else if (type === "video") {
    preview.innerHTML = `<video src="${mcObjectURL}" controls playsinline></video>`;
  } else {
    // Generic file: show an icon + name + size preview card. For audio and
    // video files, embed a playable element so the user can listen/watch
    // before sending. Everything else gets the icon card.
    const isAudio = file.type.startsWith("audio/");
    const isVideo = file.type.startsWith("video/");
    let playerHtml = "";
    if (isAudio) {
      playerHtml = `<audio src="${mcObjectURL}" controls style="width:100%;max-width:320px;"></audio>`;
    } else if (isVideo) {
      playerHtml = `<video src="${mcObjectURL}" controls playsinline style="max-width:100%;max-height:46vh;border-radius:12px;"></video>`;
    }
    preview.innerHTML = `
      <div class="mc-file-preview">
        <div class="mc-file-icon">${fileIconFor(file.name)}</div>
        <div class="mc-file-name">${escapeHtml(file.name)}</div>
        <div class="mc-file-size">${formatFileSize(file.size)}</div>
        ${playerHtml}
      </div>`;
  }

  document.getElementById("mcCaption").value = "";
  document.getElementById("mcTitle").textContent =
    type === "image" ? "Send photo"
    : type === "video" ? "Send video"
    : (mcPendingFile && mcPendingFile.type.startsWith("audio/")) ? "Send audio"
    : "Send file";

  // View-once only applies to image/video, not documents.
  const voBtn = document.getElementById("mcVOToggle");
  voBtn.classList.remove("active");
  voBtn.style.display = (type === "image" || type === "video") ? "" : "none";

  document.getElementById("mediaComposer").classList.add("active");
  if (window.lucide) lucide.createIcons();
}

function toggleMcViewOnce() {
  mcViewOnce = !mcViewOnce;
  document.getElementById("mcVOToggle").classList.toggle("active", mcViewOnce);
}

function cancelMediaComposer() {
  closeMediaComposer();
}

function closeMediaComposer() {
  document.getElementById("mediaComposer").classList.remove("active");
  if (mcObjectURL) { URL.revokeObjectURL(mcObjectURL); mcObjectURL = null; }
  mcPendingFile = null;
  mcViewOnce = false;
}

async function sendMediaComposer() {
  if (!mcPendingFile || (!selectedUser && !selectedGroup && !selectedChannel)) return;
  const caption = document.getElementById("mcCaption").value.trim();
  const btn = document.querySelector("#mediaComposer .mc-send-btn");
  btn.disabled = true;

  const isFile = mcPendingType !== "image" && mcPendingType !== "video";
  try {
    showNotifToast(
      mcPendingType === "image" ? "📷 Uploading image..."
      : mcPendingType === "video" ? "🎥 Uploading video..."
      : "📎 Uploading file...",
      "info"
    );
    const url = await uploadFile(mcPendingFile);
    if (url) {
      let extras;
      if (isFile) {
        // Generic file/document — store as an attachment card.
        extras = {
          fileUrl: url,
          fileName: mcPendingFile.name,
          fileSize: mcPendingFile.size
        };
        if (caption) extras.text = caption;
      } else {
        extras = { caption, isViewOnce: mcViewOnce, viewOnceOpened: false };
        if (mcPendingType === "image") extras.image = url;
        else extras.video = url;
      }
      if (currentChatMode === 'group') {
        await sendGroupMessageWithExtras(extras);
        showNotifToast("✓ Sent to group!", "success");
      } else if (currentChatMode === 'channel') {
        await sendChannelPostWithExtras(extras);
        showNotifToast("✓ Broadcasted!", "success");
      } else {
        await db.collection("chats").add(baseMsg(extras));
        clearReply();
        showNotifToast("✓ Sent!", "success");
        sendPushNotification(
          document.getElementById('myName').textContent || 'Nexa User',
          mcPendingType === "image" ? "You have received a new message (📷 Photo)"
          : mcPendingType === "video" ? "You have received a new message (🎥 Video)"
          : `You have received a new message (📎 ${mcPendingFile.name})`,
          selectedUser.uid
        );
      }
    }
    closeMediaComposer();
  } catch (err) {
    showNotifToast("Upload failed: " + err.message, "error");
    console.error(err);
  } finally {
    btn.disabled = false;
  }
}

/* =========================================================================
   SENDING MESSAGES
   ========================================================================= */
function handleKeyPress(e) {
  if (e.key !== "Enter") return;
  // On laptop/desktop (no coarse pointer / no touch), plain Enter sends and
  // Shift+Enter inserts a newline — like WhatsApp Web. On touch devices we
  // leave Enter as a newline (mobile keyboards rely on it) and the user sends
  // via the send button.
  const isTouch = (navigator.maxTouchPoints || 0) > 0 && window.matchMedia("(pointer: coarse)").matches;
  if (!e.shiftKey && !isTouch) {
    e.preventDefault();
    sendMessage();
  }
}

function updatePollButtonVisibility() {
  const pollBtn = document.getElementById("pollBtn");
  if (!pollBtn) return;
  const isCommunityChat = currentChatMode === 'channel' && !!selectedChannel;
  if (!isCommunityChat) {
    pollBtn.style.display = 'none';
    pollBtn.classList.add("hidden");
  } else {
    const hasText = (document.getElementById("text")?.value || "").trim().length > 0;
    pollBtn.style.display = 'inline-flex';
    pollBtn.classList.toggle("hidden", hasText);
  }
}

function toggleActionButtons() {
  const has = document.getElementById("text").value.trim().length > 0;
  document.getElementById("imgBtn").classList.toggle("hidden", has);
  document.getElementById("fileBtn").classList.toggle("hidden", has);
  document.getElementById("recordBtn").classList.toggle("hidden", has);
  updatePollButtonVisibility();
}

async function sendMessage() {
  if (currentChatMode === 'group') {
    await sendGroupMessage();
    return;
  }
  if (currentChatMode === 'channel') {
    await sendChannelPost();
    return;
  }
  if (!selectedUser) { alert("Select a user first"); return; }
  if (deletedChats[selectedUser.uid]) {
    delete deletedChats[selectedUser.uid];
    saveDeletedChats();
  }
  const text = document.getElementById("text").value.trim();
  if (!text) return;
  document.getElementById("text").value = "";
  const ta = document.getElementById("text");
  ta.style.height = "auto";
  toggleActionButtons();

  const extras = { text };
  if (messageRepliedTo) {
    extras.replyTo = messageRepliedTo.id;
    // Denormalize the quoted message so the reply preview renders on BOTH
    // sides (sender + receiver) even when the original isn't loaded in the
    // viewer's allMessages (e.g. older/lazily-loaded messages).
    extras.replyToText = messageRepliedTo.text || (messageRepliedTo.image ? "(Photo)" : messageRepliedTo.video ? "(Video)" : messageRepliedTo.audio ? "(Voice note)" : "(message)");
    extras.replyToFrom = messageRepliedTo.from || null;
  }
  const tempMsgObj = baseMsg(extras);
  tempMsgObj.id = "temp_" + Date.now() + "_" + Math.random().toString(36).substr(2, 5);

  const _hadChatBefore = latestMsgTime[selectedUser.uid] > 0;
  latestMsgTime[selectedUser.uid] = Date.now();
  latestMsgText[selectedUser.uid] = text;
  saveLatestMsgState();
  renderUsers();

  // Optimistic UI push for 0ms delay feedback
  _msgsA.push(tempMsgObj);
  renderMessageList();
  clearReply();

  // Premium touch: the send button does a quick "sent" pulse.
  pulseSendButton();

  // Scroll fully to the latest message so the just-sent bubble is in view.
  scrollMessagesToBottom();

  try {
    await db.collection("chats").add(baseMsg(extras));
    const idx = _msgsA.findIndex(m => m.id === tempMsgObj.id);
    if (idx > -1) _msgsA.splice(idx, 1);

    // Don't push-notify yourself in the self-chat.
    if (!isSelfChat()) {
      const senderName = document.getElementById('myName').textContent || 'Nexa User';
      sendPushNotification(
        senderName,
        `You have received a new message: ${text.substring(0, 80)}`,
        selectedUser.uid
      );
    } else {
      // Self-chat: mark our own message read immediately (single ticks → double).
      try {
        const lastDoc = await db.collection("chats")
          .where("from", "==", currentUser.uid)
          .where("to", "==", currentUser.uid)
          .orderBy("createdAt", "desc").limit(1).get();
        if (!lastDoc.empty) {
          await db.collection("chats").doc(lastDoc.docs[0].id).update({ read: true, readAt: Date.now() });
        }
      } catch (e) {}
    }
  } catch (err) {
    showNotifToast("Failed to send message: " + err.message, "error");
  }
}

function baseMsg(extras = {}) {
  const hasMedia = extras.image || extras.video || extras.audio;
  const msg = {
    from: currentUser.uid,
    to: selectedUser.uid,
    text: "",
    image: "",
    video: "",
    audio: "",
    caption: "",
    status: "sent",
    edited: false,
    read: false,
    createdAt: Date.now(),
    isViewOnce: (hasMedia && isViewOnceActive) ? true : false,
    viewOnceOpened: false,
    ...extras
  };

  if (hasMedia && isViewOnceActive && extras.isViewOnce === undefined) {
    isViewOnceActive = false;
    const btn = document.getElementById("viewOnceToggleBtn");
    if (btn) btn.classList.remove("active");
  }

  return msg;
}

/* =========================================================================
   VOICE RECORDING
   ========================================================================= */
let mediaRecorder = null;
let audioChunks = [];
let recordingBlob = null;
let recordingMime = "audio/webm";
let recDuration = 0;
let isRecording = false;
let recStart = 0;
let recTimer = null;

const recordBtn = document.getElementById("recordBtn");
const voicePanel = document.getElementById("voicePanel");
const voiceTimerEl = document.getElementById("voiceTimer");

recordBtn.addEventListener("mousedown", startRec);
recordBtn.addEventListener("touchstart", e => { e.preventDefault(); startRec(e); });

// Releasing the press just STOPS the recording (keeps the clip ready). The
// user then taps Send or Discard explicitly — send sends, discard discards.
document.addEventListener("mouseup", () => { if (isRecording) stopRec(); });
document.addEventListener("touchend", () => { if (isRecording) stopRec(); });

async function startRec(e) {
  if (isRecording || (!selectedUser && !selectedGroup && !selectedChannel)) {
    if (!selectedUser && !selectedGroup && !selectedChannel) alert("Select a conversation first");
    return;
  }
  if (currentChatMode === 'channel' && selectedChannel) {
    const isAdmin = (selectedChannel.admins || []).includes(currentUser.uid) || selectedChannel.ownerUid === currentUser.uid;
    if (!isAdmin) {
      showNotifToast("Only channel admins can record voice broadcasts", "error");
      return;
    }
  }
  try {
    const stream = await navigator.mediaDevices.getUserMedia({
      audio: { echoCancellation: true, noiseSuppression: true }
    });
    // Pick the best supported audio container so the blob is natively playable
    // (opus/webm on Chrome/Android, mp4 on iOS/Safari) — this removes the "can't
    // play it right away" problem from a mismatched container.
    const VN_MIME = ["audio/webm;codecs=opus", "audio/webm", "audio/ogg;codecs=opus", "audio/mp4"]
      .find(m => window.MediaRecorder && MediaRecorder.isTypeSupported(m)) || "";
    mediaRecorder = VN_MIME ? new MediaRecorder(stream, { mimeType: VN_MIME }) : new MediaRecorder(stream);
    audioChunks = [];
    mediaRecorder.addEventListener("dataavailable", e => { if (e.data && e.data.size) audioChunks.push(e.data); });
    mediaRecorder.addEventListener("stop", () => {
      // Use the recorder's actual MIME so the saved clip matches what was captured.
      const type = (mediaRecorder && mediaRecorder.mimeType) || VN_MIME || "audio/webm";
      recordingBlob = new Blob(audioChunks, { type });
      recordingMime = type;
      recDuration = Date.now() - recStart; // capture duration NOW (not at send)
    });
    // Start WITH a 250ms timeslice so dataavailable fires continuously — the
    // blob is already mostly formed when you hit stop, so sending is instant
    // instead of waiting for the whole clip to finalize.
    mediaRecorder.start(250);
    isRecording = true;
    recStart = Date.now();
    recordBtn.classList.add("active");
    voicePanel.classList.add("active");
    recTimer = setInterval(() => {
      const s = Math.floor((Date.now() - recStart) / 1000);
      voiceTimerEl.textContent = `${Math.floor(s / 60)}:${(s % 60).toString().padStart(2, "0")}`;
    }, 200);
  } catch (e) {
    alert("Microphone access denied");
  }
}

function stopRec() {
  if (!isRecording) return;
  if (mediaRecorder && mediaRecorder.state === "recording") {
    mediaRecorder.stop();
    if (mediaRecorder.stream) {
      mediaRecorder.stream.getTracks().forEach(t => t.stop());
    }
  }
  isRecording = false;
  recordBtn.classList.remove("active");
  clearInterval(recTimer);
}

async function sendVoice() {
  if (!selectedUser && !selectedGroup && !selectedChannel) return;
  if (!recordingBlob) { showNotifToast("No recording to send", "error"); return; }

  const btn = document.getElementById("vSendBtn");
  if (btn) { btn.disabled = true; btn.innerHTML = '<i data-lucide="send" style="width: 15px; height: 15px;"></i> Sending…'; if (window.lucide) lucide.createIcons(); }

  // Capture the blob/mime/duration BEFORE cancelVoice() clears them.
  const blob = recordingBlob;
  const mime = recordingMime;
  const dur = recDuration || 0;

  let tempMsg = null;
  if (currentChatMode === 'direct' && selectedUser) {
    const localUrl = URL.createObjectURL(blob);
    tempMsg = baseMsg({ audio: localUrl, duration: dur });
    tempMsg.id = "temp_vn_" + Date.now();
    tempMsg._localAudio = true;
    _msgsA.push(tempMsg);
    renderMessageList();
    scrollMessagesToBottom();
  } else {
    showNotifToast("Uploading voice note…", "info");
  }
  cancelVoice(); // hide the panel + reset state right away

  try {
    const ext = mime.includes("mp4") ? "m4a" : mime.includes("ogg") ? "ogg" : "webm";
    const f = new File([blob], `voice.${ext}`, { type: mime });
    const url = await uploadFile(f);
    if (url) {
      if (currentChatMode === 'group') {
        await sendGroupMessageWithExtras({ audio: url, duration: dur });
        showNotifToast("✓ Voice note sent to group!", "success");
      } else if (currentChatMode === 'channel') {
        await sendChannelPostWithExtras({ audio: url, duration: dur, text: '🎤 Voice Broadcast' });
        showNotifToast("✓ Voice broadcast posted!", "success");
      } else if (selectedUser) {
        await db.collection("chats").add(baseMsg({ audio: url, duration: dur }));
        // Replace the optimistic local-blob bubble with the stored one.
        if (tempMsg) {
          const idx = _msgsA.findIndex(m => m.id === tempMsg.id);
          if (idx > -1) { _msgsA.splice(idx, 1); renderMessageList(); }
        }
        const senderName = document.getElementById('myName').textContent || 'Nexa User';
        sendPushNotification(senderName, 'You have received a new message (🎤 Voice note)', selectedUser.uid);
      }
    }
  } catch (e) {
    if (tempMsg) {
      const idx = _msgsA.findIndex(m => m.id === tempMsg.id);
      if (idx > -1) { _msgsA.splice(idx, 1); renderMessageList(); }
    }
    showNotifToast("Failed to send voice note: " + e.message, "error");
  }
  if (btn) { btn.disabled = false; btn.innerHTML = '<i data-lucide="send" style="width: 15px; height: 15px;"></i> Send'; if (window.lucide) lucide.createIcons(); }
}

function cancelVoice() {
  // Stop any in-flight recording, then clear state. Discard just discards.
  if (mediaRecorder && mediaRecorder.state === "recording") {
    try { mediaRecorder.stop(); } catch (e) {}
    if (mediaRecorder.stream) mediaRecorder.stream.getTracks().forEach(t => t.stop());
  }
  isRecording = false;
  recordingBlob = null;
  audioChunks = [];
  recordBtn.classList.remove("active");
  clearInterval(recTimer);
  voicePanel.classList.remove("active");
  voiceTimerEl.textContent = "0:00";
}

function viewImg(url) {
  window.open(url, "_blank");
}

/* =========================================================================
   CHAT INFO PANEL
   ========================================================================= */
function openChatInfo() {
  if (!selectedUser) { alert("Select a user first"); return; }
  renderInfoContact();
  document.getElementById("infoPanel").classList.add("active");
  loadMediaGrid();
  renderThemeGrid();
}

// Render the contact profile header at the top of the Chat Info panel: avatar
// (opens full-pic viewer), display name, presence status, and the user's 24h
// profile note. Mirrors the contact-info header WhatsApp shows in a chat.
function renderInfoContact() {
  if (!selectedUser) return;
  const u = selectedUser;
  document.getElementById("infoContactAvatar").src = u.photo || "https://i.imgur.com/HeIi0wU.png";
  document.getElementById("infoContactName").textContent = u.displayName || u.name || "User";

  const pData = userPresenceCache[u.uid] || {};
  const online = isUserOnline(pData);
  const status = pData.status || (online ? "online" : "offline");
  const stEl = document.getElementById("infoContactStatus");
  if (online && status === "online") {
    stEl.innerHTML = '<span style="color:var(--online)">● online</span>';
  } else if (online && status === "away") {
    stEl.innerHTML = '<span style="color:#facc15">● away</span>';
  } else {
    stEl.innerHTML = '<span style="color:var(--text-3)">last seen ' + formatLastSeen(pData.lastSeen || 0) + "</span>";
  }

  const noteEl = document.getElementById("infoContactNote");
  if (!noteEl) return;
  noteEl.innerHTML = '<span style="color:var(--text-3);font-size:12px;">…</span>';
  fetchUserNote(u.uid).then(noteText => {
    if (!noteEl) return;
    noteEl.innerHTML = noteText ? ('<i data-lucide="file-text" style="width: 12px; height: 12px; vertical-align: -1px; color: var(--text-3);"></i> ' + linkify(noteText)) : "";
    if (window.lucide) lucide.createIcons();
  });
}

function closeChatInfo() {
  document.getElementById("infoPanel").classList.remove("active");
}

function loadMediaGrid() {
  const grid = document.getElementById("mediaGrid");
  grid.innerHTML = "";
  const media = allMessages.filter(m => m.image || m.video);
  if (!media.length) {
    grid.innerHTML = '<div style="grid-column: 1 / -1; text-align: center; color: var(--text-3); padding: 18px; font-size: 13px;">No media shared</div>';
    return;
  }
  media.forEach(m => {
    const el = document.createElement("div");
    el.className = "media-thumb";
    if (m.image) {
      el.innerHTML = `<img src="${safeMediaUrl(m.image)}" alt="media">`;
      el.addEventListener("click", () => viewImg(m.image));
    } else if (m.video) {
      el.innerHTML = `<video src="${safeMediaUrl(m.video)}" style="width: 100%; height: 100%; object-fit: cover;"></video>`;
      el.addEventListener("click", () => window.open(m.video, "_blank"));
    }
    grid.appendChild(el);
  });
}

/* =========================================================================
   THEMES
   ========================================================================= */
const THEMES = {
  light: { label: 'Nexa White', attr: 'light', tier: 'classic' },
  nexa: { label: 'Nexa Deep Blue', attr: 'nexa', tier: 'classic' }
};

let currentTheme = 'light';

/* =========================================================================
   FONT FAMILIES (font-design picker)
   ========================================================================= */
const FONT_FAMILIES = [
  { key: 'plus-jakarta', label: 'Font 1', sample: 'Ag', family: "'Plus Jakarta Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" },
  { key: 'inter', label: 'Font 2', sample: 'Ag', family: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" },
  { key: 'sora', label: 'Font 3', sample: 'Ag', family: "'Sora', 'Plus Jakarta Sans', sans-serif" },
  { key: 'dm-sans', label: 'Font 4', sample: 'Ag', family: "'DM Sans', 'Plus Jakarta Sans', sans-serif" },
  { key: 'poppins', label: 'Font 5', sample: 'Ag', family: "'Poppins', 'Plus Jakarta Sans', sans-serif" },
  { key: 'space-grotesk', label: 'Font 6', sample: 'Ag', family: "'Space Grotesk', 'Plus Jakarta Sans', sans-serif" }
];

let fontFamily = 'plus-jakarta';

function getFontStack(key) {
  const f = FONT_FAMILIES.find(x => x.key === key);
  return f ? f.family : FONT_FAMILIES[0].family;
}

function applyFontFamily(key) {
  fontFamily = key;
  const stack = getFontStack(key);
  document.documentElement.style.setProperty('--font-body', stack);
  if (document.body) document.body.style.fontFamily = stack;
}

function setFontFamily(key) {
  if (!FONT_FAMILIES.some(x => x.key === key)) return;
  applyFontFamily(key);
  savePrefs();
  renderFontPickerGrid();
}

function renderFontPickerGrid() {
  const grid = document.getElementById('fontPickerGrid');
  if (!grid) return;
  grid.innerHTML = '';
  FONT_FAMILIES.forEach(f => {
    const card = document.createElement('button');
    card.className = 'font-picker-card' + (fontFamily === f.key ? ' active' : '');
    card.innerHTML = '<div class="fp-card-name">' + f.label + '</div>' +
                      '<div class="fp-card-sample" style="font-family:' + f.family + '">' + f.sample + '</div>' +
                      (fontFamily === f.key ? '<div class="fp-card-check"><i data-lucide="check" style="width: 12px; height: 12px;"></i> Applied</div>' : '');
    card.onclick = () => setFontFamily(f.key);
    grid.appendChild(card);
  });
  if (window.lucide) lucide.createIcons();
}

function openFontPickerModal() {
  renderFontPickerGrid();
  const m = document.getElementById('fontPickerModal');
  if (m) m.classList.add('active');
}

function closeFontPickerModal() {
  const m = document.getElementById('fontPickerModal');
  if (m) m.classList.remove('active');
}

function handleFontPickerOverlayClick(ev) {
  if (ev.target === ev.currentTarget) closeFontPickerModal();
}

function setTheme(themeName) {
  const theme = THEMES[themeName];
  if (!theme) return;
  currentTheme = themeName;
  // 'light' is the default/base theme — it needs no data-theme attribute
  // override. Every other theme sets its attribute so its CSS applies.
  if (themeName === 'light') {
    document.documentElement.removeAttribute('data-theme');
  } else {
    document.documentElement.setAttribute('data-theme', theme.attr);
  }
  const tc = document.querySelector('meta[name="theme-color"]');
  if (tc) tc.setAttribute('content', themeName === 'nexa' ? '#060c1e' : '#ffffff');
  savePrefs();
}

function renderThemeGrid() {
  const grid = document.getElementById('themeGrid');
  if (!grid) return;
  grid.innerHTML = '';
  Object.entries(THEMES).forEach(([key, theme]) => {
    const btn = document.createElement('button');
    btn.className = `theme-btn ${currentTheme === key ? 'active' : ''}`;
    const icon = key === 'nexa' ? 'moon' : 'sun';
    btn.innerHTML = `<i data-lucide="${icon}" style="width: 14px; height: 14px;"></i> ${theme.label}`;
    btn.onclick = () => { setTheme(key); renderThemeGrid(); };
    grid.appendChild(btn);
  });
  if (window.lucide) lucide.createIcons();
}

/* =========================================================================
   CUSTOM CHAT WALLPAPER SYSTEM
   ========================================================================= */
const WALLPAPER_PRESETS = [
  { id: 'cosmic', title: 'Cosmic Aurora', url: 'https://images.unsplash.com/photo-1518709268805-4e9042af9f23?q=80&w=1200&auto=format&fit=crop', blur: 2, dimming: 45, tint: 'dark' },
  { id: 'cyberpunk', title: 'Neon Cyberpunk', url: 'https://images.unsplash.com/photo-1519501025264-65ba15a82390?q=80&w=1200&auto=format&fit=crop', blur: 3, dimming: 50, tint: 'dark' },
  { id: 'forest', title: 'Misty Forest', url: 'https://images.unsplash.com/photo-1511497584788-876761c1298b?q=80&w=1200&auto=format&fit=crop', blur: 2, dimming: 45, tint: 'dark' },
  { id: 'ocean', title: 'Midnight Ocean', url: 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?q=80&w=1200&auto=format&fit=crop', blur: 2, dimming: 40, tint: 'dark' },
  { id: 'sunset', title: 'Sunset Glow', url: 'https://images.unsplash.com/photo-1495616811223-4d98c6e9c869?q=80&w=1200&auto=format&fit=crop', blur: 3, dimming: 50, tint: 'dark' },
  { id: 'minimal', title: 'Minimal Abstract', url: 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=1200&auto=format&fit=crop', blur: 1, dimming: 40, tint: 'dark' },
  { id: 'pastel', title: 'Dreamy Cloud', url: 'https://images.unsplash.com/photo-1534447677768-be436bb09401?q=80&w=1200&auto=format&fit=crop', blur: 4, dimming: 45, tint: 'glass' },
  { id: 'obsidian', title: 'Obsidian Mesh', url: 'https://images.unsplash.com/photo-1550684848-fac1c5b4e853?q=80&w=1200&auto=format&fit=crop', blur: 1, dimming: 35, tint: 'dark' }
];

let globalWallpaper = null; // { url, blur, dimming, tintMode }
let perChatWallpapers = {}; // { [peerUid]: { url, blur, dimming, tintMode } }

let wallpaperModalState = {
  scope: 'current', // 'current' or 'global'
  selectedUrl: '',
  blur: 2,
  dimming: 50,
  tintMode: 'dark'
};

function applyCurrentWallpaper() {
  const wallpaperLayer = document.getElementById("chatWallpaperLayer");
  const wallpaperImg = document.getElementById("chatWallpaperImg");
  const wallpaperOverlay = document.getElementById("chatWallpaperOverlay");
  const chatArea = document.getElementById("chatArea");
  
  if (!chatArea || !wallpaperLayer) return;

  let config = null;
  let isPerChat = false;

  if (selectedUser && perChatWallpapers[selectedUser.uid]) {
    config = perChatWallpapers[selectedUser.uid];
    isPerChat = true;
  } else if (globalWallpaper) {
    config = globalWallpaper;
  }

  if (config && config.url) {
    chatArea.classList.add("has-custom-wallpaper");
    wallpaperLayer.classList.add("active");
    wallpaperImg.style.backgroundImage = `url("${safeMediaUrl(config.url)}")`;
    
    const blurPx = config.blur !== undefined ? config.blur : 2;
    wallpaperImg.style.filter = `blur(${blurPx}px)`;

    const dimmingPct = config.dimming !== undefined ? config.dimming : 50;
    const alpha = (dimmingPct / 100).toFixed(2);
    const tintMode = config.tintMode || 'dark';

    if (tintMode === 'light') {
      wallpaperOverlay.style.backgroundColor = `rgba(255, 255, 255, ${alpha})`;
      wallpaperOverlay.style.backdropFilter = `none`;
    } else if (tintMode === 'glass') {
      wallpaperOverlay.style.backgroundColor = `rgba(15, 23, 42, ${Math.min(alpha, 0.45)})`;
      wallpaperOverlay.style.backdropFilter = `blur(12px)`;
    } else {
      wallpaperOverlay.style.backgroundColor = `rgba(8, 12, 20, ${alpha})`;
      wallpaperOverlay.style.backdropFilter = `none`;
    }
  } else {
    chatArea.classList.remove("has-custom-wallpaper");
    wallpaperLayer.classList.remove("active");
    wallpaperImg.style.backgroundImage = "none";
  }

  updateWallpaperUIIndicators(isPerChat, config);
}

function updateWallpaperUIIndicators(isPerChat, config) {
  // Update Settings Tab Card
  const settingsThumb = document.getElementById("settingsWpThumb");
  const settingsTitle = document.getElementById("settingsWpTitle");
  const settingsSub = document.getElementById("settingsWpSub");
  if (settingsThumb) {
    if (globalWallpaper && globalWallpaper.url) {
      settingsThumb.style.backgroundImage = `url("${safeMediaUrl(globalWallpaper.url)}")`;
      settingsTitle.textContent = "Global Wallpaper Active";
      settingsSub.textContent = `Blur: ${globalWallpaper.blur || 0}px • Darken: ${globalWallpaper.dimming || 50}%`;
    } else {
      settingsThumb.style.backgroundImage = "none";
      settingsTitle.textContent = "Default Theme Background";
      settingsSub.textContent = "Applies to all chats unless overridden";
    }
  }

  // Update Chat Info Panel Card
  const infoThumb = document.getElementById("infoWpThumb");
  const infoTitle = document.getElementById("infoWpTitle");
  const infoSub = document.getElementById("infoWpSub");
  const infoResetBtn = document.getElementById("infoWpResetBtn");
  
  if (infoThumb) {
    if (isPerChat && config && config.url) {
      infoThumb.style.backgroundImage = `url("${safeMediaUrl(config.url)}")`;
      infoTitle.textContent = "Custom Wallpaper (This Chat)";
      infoSub.textContent = `Specific to ${selectedUser ? (selectedUser.displayName || 'this chat') : 'this chat'}`;
      if (infoResetBtn) infoResetBtn.style.display = "inline-flex";
    } else if (globalWallpaper && globalWallpaper.url) {
      infoThumb.style.backgroundImage = `url("${safeMediaUrl(globalWallpaper.url)}")`;
      infoTitle.textContent = "Using Global Wallpaper";
      infoSub.textContent = "Inherited from global settings";
      if (infoResetBtn) infoResetBtn.style.display = "none";
    } else {
      infoThumb.style.backgroundImage = "none";
      infoTitle.textContent = "Default Theme Background";
      infoSub.textContent = "No custom wallpaper applied";
      if (infoResetBtn) infoResetBtn.style.display = "none";
    }
  }
}

function openWallpaperModal(scope = 'current') {
  if (scope === 'current' && !selectedUser) {
    if (typeof showNotifToast === 'function') showNotifToast("Please select a chat first to customize its wallpaper", "info");
    else alert("Please select a chat first to customize its wallpaper");
    return;
  }
  wallpaperModalState.scope = scope;
  
  let existing = null;
  if (scope === 'current' && selectedUser) {
    existing = perChatWallpapers[selectedUser.uid] || globalWallpaper;
  } else {
    existing = globalWallpaper;
  }

  if (existing) {
    wallpaperModalState.selectedUrl = existing.url || '';
    wallpaperModalState.blur = existing.blur !== undefined ? existing.blur : 2;
    wallpaperModalState.dimming = existing.dimming !== undefined ? existing.dimming : 50;
    wallpaperModalState.tintMode = existing.tintMode || 'dark';
  } else {
    wallpaperModalState.selectedUrl = '';
    wallpaperModalState.blur = 2;
    wallpaperModalState.dimming = 50;
    wallpaperModalState.tintMode = 'dark';
  }

  renderWallpaperModalUI();
  const modal = document.getElementById("wallpaperModal");
  if (modal) modal.classList.add("active");
}

function closeWallpaperModal() {
  const modal = document.getElementById("wallpaperModal");
  if (modal) modal.classList.remove("active");
}

function switchWallpaperModalScope(scope) {
  if (scope === 'current' && !selectedUser) {
    if (typeof showNotifToast === 'function') showNotifToast("Select a chat first to apply wallpaper specifically to it", "info");
    else alert("Select a chat first to apply wallpaper specifically to it");
    return;
  }
  wallpaperModalState.scope = scope;
  
  let existing = (scope === 'current' && selectedUser) ? (perChatWallpapers[selectedUser.uid] || globalWallpaper) : globalWallpaper;
  if (existing) {
    wallpaperModalState.selectedUrl = existing.url || '';
    wallpaperModalState.blur = existing.blur !== undefined ? existing.blur : 2;
    wallpaperModalState.dimming = existing.dimming !== undefined ? existing.dimming : 50;
    wallpaperModalState.tintMode = existing.tintMode || 'dark';
  }

  renderWallpaperModalUI();
}

function renderWallpaperModalUI() {
  const tabCurrent = document.getElementById("wpScopeTabCurrent");
  const tabGlobal = document.getElementById("wpScopeTabGlobal");
  if (tabCurrent && tabGlobal) {
    tabCurrent.classList.toggle("active", wallpaperModalState.scope === 'current');
    tabGlobal.classList.toggle("active", wallpaperModalState.scope === 'global');
    tabCurrent.textContent = selectedUser ? `🎯 Chat with ${selectedUser.displayName || 'User'}` : "🎯 This Chat Only";
  }

  const modalTitle = document.getElementById("wpModalTitle");
  if (modalTitle) {
    modalTitle.textContent = wallpaperModalState.scope === 'current' 
      ? `🖼 Custom Background for ${selectedUser ? selectedUser.displayName : 'Chat'}` 
      : "🖼 Global Background for All Chats";
  }

  const grid = document.getElementById("wpPresetsGrid");
  if (grid) {
    grid.innerHTML = "";
    WALLPAPER_PRESETS.forEach(p => {
      const card = document.createElement("div");
      card.className = `wp-preset-card ${wallpaperModalState.selectedUrl === p.url ? 'active' : ''}`;
      card.innerHTML = `
        <div class="wp-preset-thumb" style="background-image: url('${p.url}')"></div>
        <div class="wp-preset-title">${p.title}</div>
      `;
      card.onclick = () => selectPresetWallpaper(p);
      grid.appendChild(card);
    });
  }

  const dimSlider = document.getElementById("wpDimmingSlider");
  const blurSlider = document.getElementById("wpBlurSlider");
  const urlInput = document.getElementById("wallpaperUrlInput");
  
  if (dimSlider) dimSlider.value = wallpaperModalState.dimming;
  if (blurSlider) blurSlider.value = wallpaperModalState.blur;
  if (urlInput) urlInput.value = wallpaperModalState.selectedUrl.startsWith("data:") ? "" : wallpaperModalState.selectedUrl;

  const dimDisplay = document.getElementById("wpDimmingValDisplay");
  const blurDisplay = document.getElementById("wpBlurValDisplay");
  if (dimDisplay) dimDisplay.textContent = `${wallpaperModalState.dimming}%`;
  if (blurDisplay) blurDisplay.textContent = `${wallpaperModalState.blur}px`;

  document.querySelectorAll(".wp-tint-pill").forEach(p => p.classList.remove("active"));
  const activeTintPill = document.getElementById(`wpTint${wallpaperModalState.tintMode.charAt(0).toUpperCase() + wallpaperModalState.tintMode.slice(1)}`);
  if (activeTintPill) activeTintPill.classList.add("active");

  updateLivePreview();
}

function selectPresetWallpaper(preset) {
  wallpaperModalState.selectedUrl = preset.url;
  wallpaperModalState.blur = preset.blur;
  wallpaperModalState.dimming = preset.dimming;
  wallpaperModalState.tintMode = preset.tint;
  renderWallpaperModalUI();
}

function updateWallpaperPreviewFromSliders() {
  const dimSlider = document.getElementById("wpDimmingSlider");
  const blurSlider = document.getElementById("wpBlurSlider");
  if (dimSlider) {
    wallpaperModalState.dimming = parseInt(dimSlider.value, 10);
    const dimDisplay = document.getElementById("wpDimmingValDisplay");
    if (dimDisplay) dimDisplay.textContent = `${wallpaperModalState.dimming}%`;
  }
  if (blurSlider) {
    wallpaperModalState.blur = parseInt(blurSlider.value, 10);
    const blurDisplay = document.getElementById("wpBlurValDisplay");
    if (blurDisplay) blurDisplay.textContent = `${wallpaperModalState.blur}px`;
  }
  updateLivePreview();
}

function setWallpaperTintMode(mode) {
  wallpaperModalState.tintMode = mode;
  document.querySelectorAll(".wp-tint-pill").forEach(p => p.classList.remove("active"));
  const pill = document.getElementById(`wpTint${mode.charAt(0).toUpperCase() + mode.slice(1)}`);
  if (pill) pill.classList.add("active");
  updateLivePreview();
}

function handleWallpaperUrlInput(val) {
  wallpaperModalState.selectedUrl = val.trim();
  updateLivePreview();
}

function triggerWallpaperFileUpload() {
  const fileInput = document.getElementById("wallpaperFileInput");
  if (fileInput) fileInput.click();
}

function handleWallpaperFileUpload(e) {
  const file = e.target.files[0];
  if (!file) return;
  if (file.size > 5 * 1024 * 1024) {
    if (typeof showNotifToast === 'function') showNotifToast("Image size must be less than 5MB", "error");
    else alert("Image size must be less than 5MB");
    return;
  }
  const reader = new FileReader();
  reader.onload = function(evt) {
    wallpaperModalState.selectedUrl = evt.target.result;
    renderWallpaperModalUI();
    if (typeof showNotifToast === 'function') showNotifToast("Custom image loaded", "success");
  };
  reader.readAsDataURL(file);
}

function updateLivePreview() {
  const previewImg = document.getElementById("wpLivePreviewImg");
  const previewOverlay = document.getElementById("wpLivePreviewOverlay");
  if (!previewImg || !previewOverlay) return;

  if (wallpaperModalState.selectedUrl) {
    previewImg.style.backgroundImage = `url("${wallpaperModalState.selectedUrl}")`;
    previewImg.style.filter = `blur(${wallpaperModalState.blur}px)`;

    const alpha = (wallpaperModalState.dimming / 100).toFixed(2);
    if (wallpaperModalState.tintMode === 'light') {
      previewOverlay.style.backgroundColor = `rgba(255, 255, 255, ${alpha})`;
      previewOverlay.style.backdropFilter = `none`;
    } else if (wallpaperModalState.tintMode === 'glass') {
      previewOverlay.style.backgroundColor = `rgba(15, 23, 42, ${Math.min(alpha, 0.45)})`;
      previewOverlay.style.backdropFilter = `blur(12px)`;
    } else {
      previewOverlay.style.backgroundColor = `rgba(8, 12, 20, ${alpha})`;
      previewOverlay.style.backdropFilter = `none`;
    }
  } else {
    previewImg.style.backgroundImage = "none";
    previewOverlay.style.backgroundColor = "rgba(8, 12, 20, 0.85)";
  }
}

function saveWallpaperFromModal() {
  if (!wallpaperModalState.selectedUrl) {
    if (typeof showNotifToast === 'function') showNotifToast("Please select or upload an image first", "info");
    else alert("Please select or upload an image first");
    return;
  }

  const config = {
    url: wallpaperModalState.selectedUrl,
    blur: wallpaperModalState.blur,
    dimming: wallpaperModalState.dimming,
    tintMode: wallpaperModalState.tintMode
  };

  if (wallpaperModalState.scope === 'current' && selectedUser) {
    perChatWallpapers[selectedUser.uid] = config;
    if (typeof showNotifToast === 'function') showNotifToast(`Wallpaper updated for chat with ${selectedUser.displayName}`, "success");
  } else {
    globalWallpaper = config;
    if (typeof showNotifToast === 'function') showNotifToast("Global chat wallpaper saved for all chats", "success");
  }

  savePrefs();
  applyCurrentWallpaper();
  closeWallpaperModal();
}

function removeActiveWallpaper() {
  if (wallpaperModalState.scope === 'current' && selectedUser) {
    delete perChatWallpapers[selectedUser.uid];
    if (typeof showNotifToast === 'function') showNotifToast(`Custom wallpaper removed for ${selectedUser.displayName}`, "info");
  } else {
    globalWallpaper = null;
    if (typeof showNotifToast === 'function') showNotifToast("Global chat wallpaper removed", "info");
  }
  savePrefs();
  applyCurrentWallpaper();
  closeWallpaperModal();
}

function resetCurrentChatWallpaper() {
  if (!selectedUser) return;
  delete perChatWallpapers[selectedUser.uid];
  savePrefs();
  applyCurrentWallpaper();
  if (typeof showNotifToast === 'function') showNotifToast("Reset chat background to default", "info");
}

/* =========================================================================
   CHAT ACTIONS: mute / favorite / clear / search
   ========================================================================= */
function muteChat() {
  if (!selectedUser) return;
  mutedChats[selectedUser.uid] = !mutedChats[selectedUser.uid];
  document.getElementById("muteText").textContent = mutedChats[selectedUser.uid] ? "Unmute Notifications" : "Mute Notifications";
  const mi = document.getElementById("muteIcon");
  if (mi) mi.innerHTML = `<i data-lucide="${mutedChats[selectedUser.uid] ? 'bell' : 'bell-off'}" style="width: 16px; height: 16px;"></i>`;
  if (window.lucide) lucide.createIcons();
  savePrefs();
}

function favoriteChat() {
  if (!selectedUser) return;
  favoritedChats[selectedUser.uid] = !favoritedChats[selectedUser.uid];
  const fi = document.getElementById("favIcon");
  if (fi) fi.innerHTML = '<i data-lucide="star" style="width: 16px; height: 16px;"></i>';
  if (window.lucide) lucide.createIcons();
  savePrefs();
  renderUsers();
}

async function clearChat() {
  if (!selectedUser || !confirm("Clear all messages?")) return;
  const me = currentUser.uid;
  const other = selectedUser.uid;
  try {
    // Delete the WHOLE conversation, not just the loaded window. Rules only
    // allow deleting your own messages, so filter to those.
    const queries = [
      db.collection("chats").where("from", "==", me).where("to", "==", other).get()
    ];
    if (!isSelfChat()) {
      queries.push(db.collection("chats").where("from", "==", other).where("to", "==", me).get());
    }
    const snaps = await Promise.all(queries);
    const refs = [];
    const seen = new Set();
    snaps.forEach(snap => snap.forEach(d => {
      if (!seen.has(d.id) && d.data().from === me) {
        seen.add(d.id);
        refs.push(d.ref);
      }
    }));
    // Firestore batches cap at 500 writes — chunk to be safe.
    for (let i = 0; i < refs.length; i += 400) {
      const batch = db.batch();
      refs.slice(i, i + 400).forEach(ref => batch.delete(ref));
      await batch.commit();
    }
    // Drop locally held history too — the live listener only prunes the
    // newest window, not the paged-in older messages.
    _olderMsgsA = [];
    _olderMsgsB = [];
    renderMessageList();
  } catch (e) {
    console.error("clearChat error:", e);
  }
}

function openSearchModal() {
  document.getElementById("searchModal").classList.add("active");
  document.getElementById("searchInput").focus();
}

function closeSearchModal() {
  document.getElementById("searchModal").classList.remove("active");
  document.getElementById("searchResults").innerHTML = "";
  document.getElementById("searchInput").value = "";
}

// Full-history search pool, cached per chat for 60s so every keystroke
// doesn't re-download the whole conversation.
let _searchPoolCache = { chatUid: null, fetchedAt: 0, pool: [] };
let _searchDebounce = null;

function searchMessages() {
  clearTimeout(_searchDebounce);
  _searchDebounce = setTimeout(runMessageSearch, 250);
}

async function runMessageSearch() {
  const q = document.getElementById("searchInput").value.trim().toLowerCase();
  const box = document.getElementById("searchResults");
  box.innerHTML = "";
  if (!q || !selectedUser) return;

  let pool = allMessages;
  try {
    const chatUid = selectedUser.uid;
    if (_searchPoolCache.chatUid !== chatUid || Date.now() - _searchPoolCache.fetchedAt > 60000) {
      const me = currentUser.uid;
      const queries = [
        db.collection("chats").where("from", "==", me).where("to", "==", chatUid).get()
      ];
      if (!isSelfChat()) {
        queries.push(db.collection("chats").where("from", "==", chatUid).where("to", "==", me).get());
      }
      const snaps = await Promise.all(queries);
      const map = new Map();
      snaps.forEach(snap => snap.forEach(d => map.set(d.id, { id: d.id, ...d.data() })));
      _searchPoolCache = { chatUid, fetchedAt: Date.now(), pool: [...map.values()] };
    }
    pool = _searchPoolCache.pool;
  } catch (e) {
    console.warn("Search full-history fetch failed, using loaded messages:", e);
  }

  const found = pool
    .filter(m => m.text && m.text.toLowerCase().includes(q))
    .sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))
    .slice(0, 50);
  if (!found.length) {
    box.innerHTML = '<div style="text-align: center; color: var(--text-3); padding: 24px; font-size: 13px;">No results</div>';
    return;
  }
  found.forEach(m => {
    const el = document.createElement("div");
    el.className = "search-result";
    const author = m.from === currentUser.uid ? "You" : escapeHtml(selectedUser?.displayName || "User");
    el.innerHTML = `<div class="res-author">${author}</div><div class="res-text">${escapeHtml(m.text)}</div>`;
    el.addEventListener("click", () => { scrollToMsg(m.id); closeSearchModal(); });
    box.appendChild(el);
  });
}

/* =========================================================================
   INCOMING MESSAGE NOTIFICATIONS (global — this is fine as a single
   listener set up once at login; it does not touch the open-chat DOM)
   ========================================================================= */
function listenIncoming() {
  db.collection("chats").where("to", "==", currentUser.uid).where("status", "==", "sent").onSnapshot(snap => {
    snap.docChanges().forEach(change => {
      if (change.type !== "added") return;
      const msg = change.doc.data();
      const uid = msg.from;
      if (blockedUsers[uid]) return;
      // Skip self-chat messages — they're handled by the self-chat listener,
      // not as incoming notifications (don't toast yourself).
      if (uid === currentUser.uid) {
        db.collection("chats").doc(change.doc.id).update({ status: "delivered" });
        return;
      }
      if (deletedChats[uid]) {
        delete deletedChats[uid];
        saveDeletedChats();
      }
      const preview = msg.text || (msg.image ? "📷 Photo" : msg.video ? "🎥 Video" : msg.audio ? "🎤 Voice" : "New message");
      if (!latestMsgTime[uid] || msg.createdAt > latestMsgTime[uid]) {
        latestMsgTime[uid] = msg.createdAt;
        latestMsgText[uid] = preview;
        saveLatestMsgState();
      }
      if (!selectedUser || selectedUser.uid !== uid) {
        if (!mutedChats[uid] && notificationSettings.messages) {
          const senderUser = allUsersData.find(u => u.uid === uid);
          const name = senderUser ? (senderUser.displayName || "User") : "Nexa User";
          showNotifToast(`${name}: You have received a new message`, 'info');
          // In-app notification history (deduped with FCM via the stable
          // message id below — recordNotificationHistory keeps the last 50).
          recordNotificationHistory({
            icon: notifHistoryIconFor('message'),
            title: name,
            body: preview.slice(0, 120),
            at: msg.createdAt || Date.now()
          });
        }
      }

      db.collection("chats").doc(change.doc.id).update({ status: "delivered" });
      renderUsers();
    });
  });
}

/* =========================================================================
   VOICE & VIDEO CALLING ENGINE (WebRTC + PeerJS + Firestore Signaling)
   ========================================================================= */
let nexaPeer = null;
let activeCall = null;
let localStream = null;
let remoteStream = null;
let callType = null;            // 'voice' | 'video'
let callDirection = null;       // 'outgoing' | 'incoming'
let callState = 'idle';         // 'idle' | 'ringing' | 'active' | 'ended'
let callStartTime = null;
let callTimerInterval = null;
let callSignalUnsub = null;
let incomingCallData = null;
let isMicMuted = false;
let isCameraOff = false;
let isSpeakerOn = true;
let currentFacingMode = 'user';
const CALL_TIMEOUT_MS = 45000;  // 45 seconds ring timeout
const MAX_RETRY_ATTEMPTS = 3;  // max PeerJS reconnect attempts
let callTimeoutTimer = null;
let vibrationInterval = null;

const NEXA_ICE_SERVERS = [
  { urls: 'stun:stun.l.google.com:19302' },
  { urls: 'stun:stun1.l.google.com:19302' },
  { urls: 'stun:stun2.l.google.com:19302' },
  { urls: 'stun:stun3.l.google.com:19302' },
  { urls: 'stun:stun4.l.google.com:19302' },
  { urls: 'stun:global.stun.twilio.com:3478' },
  // TURN relays — STUN-only ICE CANNOT traverse symmetric NAT / CGNAT (mobile
  // carriers, hotel WiFi, most home routers behind an ISP NAT). Without TURN,
  // calls "connect" at the signaling level but no media flows → one-way/no
  // audio. OpenRelay public test creds — rate-limited; replace with your own
  // Metered/Twilio TURN for production.
  { urls: 'turn:openrelay.metered.ca:80', username: 'openrelayproject', credential: 'openrelayproject' },
  { urls: 'turn:openrelay.metered.ca:443', username: 'openrelayproject', credential: 'openrelayproject' },
  { urls: 'turn:openrelay.metered.ca:443?transport=tcp', username: 'openrelayproject', credential: 'openrelayproject' }
];

// ── Initialize PeerJS (called once on login) ──
function initPeerJS() {
  if (!currentUser) return;

  if (typeof Peer === 'undefined') {
    console.warn("⏳ Waiting for PeerJS script to load...");
    if (!document.getElementById("peerjs-dynamic-script")) {
      const script = document.createElement("script");
      script.id = "peerjs-dynamic-script";
      script.src = "https://unpkg.com/peerjs@1.5.4/dist/peerjs.min.js";
      script.onload = () => initPeerJS();
      script.onerror = () => {
        const fallbackScript = document.createElement("script");
        fallbackScript.src = "https://cdn.jsdelivr.net/npm/peerjs@1.5.4/dist/peerjs.min.js";
        fallbackScript.onload = () => initPeerJS();
        document.head.appendChild(fallbackScript);
      };
      document.head.appendChild(script);
    } else {
      setTimeout(initPeerJS, 500);
    }
    return;
  }

  if (nexaPeer && !nexaPeer.destroyed && nexaPeer.open && !nexaPeer.disconnected) {
    return;
  }

  if (nexaPeer && !nexaPeer.destroyed && nexaPeer.disconnected) {
    console.log('📞 PeerJS disconnected, attempting reconnect...');
    try { nexaPeer.reconnect(); return; } catch (e) {}
  }

  const sessionPeerId = 'nexa_user_' + currentUser.uid;

  if (nexaPeer && !nexaPeer.destroyed) {
    try { nexaPeer.destroy(); } catch (e) {}
  }

  nexaPeer = new Peer(sessionPeerId, {
    debug: 0,
    config: {
      iceServers: NEXA_ICE_SERVERS
    }
  });

  nexaPeer.on('open', (id) => {
    console.log('📞 PeerJS connected with ID:', id);
    db.collection("users").doc(currentUser.uid).set({ peerId: id }, { merge: true });
  });

  nexaPeer.on('call', (incomingMediaCall) => {
    console.log('📞 Incoming PeerJS media call received from:', incomingMediaCall.peer);
    activeCall = incomingMediaCall;
    if (callState === 'active' && localStream) {
      // Attach stream handlers BEFORE answer() — PeerJS can fire `stream`
      // synchronously during answer(), and attaching after means the remote
      // audio/video fires into zero listeners → "I hear them but can't see them".
      setupCallStreamHandlers(incomingMediaCall);
      incomingMediaCall.answer(localStream);
    }
  });

  nexaPeer.on('error', (err) => {
    console.warn('⚠ PeerJS error:', err.type, err.message);
    if (err.type === 'unavailable-id') {
      const fallbackId = 'nexa_user_' + currentUser.uid + '_' + Date.now();
      nexaPeer = new Peer(fallbackId, { debug: 0, config: { iceServers: NEXA_ICE_SERVERS } });
      nexaPeer.on('open', (id) => {
        db.collection("users").doc(currentUser.uid).set({ peerId: id }, { merge: true });
      });
      nexaPeer.on('call', (incomingMediaCall) => {
        activeCall = incomingMediaCall;
        if (callState === 'active' && localStream) {
          setupCallStreamHandlers(incomingMediaCall);
          incomingMediaCall.answer(localStream);
        }
      });
    }
  });

  nexaPeer.on('disconnected', () => {
    console.log('📞 PeerJS disconnected, reconnecting...');
    if (nexaPeer && !nexaPeer.destroyed) {
      try { nexaPeer.reconnect(); } catch (e) {}
    }
  });

  listenForIncomingCalls();
}

async function ensurePeerConnected() {
  if (!currentUser) return false;
  if (nexaPeer && !nexaPeer.destroyed && nexaPeer.open && !nexaPeer.disconnected) return true;

  initPeerJS();

  let attempts = 0;
  while ((!nexaPeer || nexaPeer.destroyed || !nexaPeer.open || nexaPeer.disconnected) && attempts < MAX_RETRY_ATTEMPTS) {
    if (nexaPeer && nexaPeer.disconnected && !nexaPeer.destroyed) {
      try { nexaPeer.reconnect(); } catch (e) {}
    }
    await new Promise(r => setTimeout(r, 100));
    attempts++;
  }

  return Boolean(nexaPeer && !nexaPeer.destroyed && nexaPeer.open && !nexaPeer.disconnected);
}

// ── Firestore Signaling: Listen for incoming calls ──
function listenForIncomingCalls() {
  if (!currentUser) return;
  if (callSignalUnsub) callSignalUnsub();

  callSignalUnsub = db.collection("calls")
    .where("to", "==", currentUser.uid)
    .where("status", "==", "ringing")
    .onSnapshot(snap => {
      snap.docChanges().forEach(change => {
        if (change.type === "added" || change.type === "modified") {
          const data = change.doc.data();
          if (data.status === "ringing" && callState === "idle") {
            incomingCallData = { id: change.doc.id, ...data };
            showIncomingCallUI(incomingCallData);
          }
        }
        if (change.type === "modified") {
          const data = change.doc.data();
          if (data.status === "ended" || data.status === "declined" || data.status === "missed") {
            if (callState === "ringing" && callDirection === "incoming") {
              cleanupCall();
            }
          }
        }
      });
    });
}

// ── Start a Call (Outgoing) ──
async function startCall(type) {
   if (!selectedUser) { showNotifToast("Select a user first", "error"); return; }
   // Prevent self calls
   if (selectedUser.uid === currentUser.uid) {
     showNotifToast("You cannot call yourself", "warning");
     return;
   }
   if (callState !== 'idle') { showNotifToast("Already in a call", "warning"); return; }
   if (callTimeoutTimer) clearTimeout(callTimeoutTimer);

  const peerReady = await ensurePeerConnected();
  if (!peerReady) {
    showNotifToast("Connecting call network... Please try again", "warning");
    return;
  }

  callType = type;  // 'voice' or 'video'
  callDirection = 'outgoing';
  callState = 'ringing';

  const peerName = selectedUser.displayName || "User";
  const peerPhoto = selectedUser.photo || "https://i.imgur.com/HeIi0wU.png";

  // Set up call UI
  document.getElementById("callPeerAvatar").src = peerPhoto;
  document.getElementById("callPeerName").textContent = peerName;
  document.getElementById("callStatusLabel").textContent = type === 'video' ? "Nexa Video Call" : "Nexa Voice Call";
  document.getElementById("callTimerDisplay").style.display = "none";
  document.getElementById("callProfileCard").style.display = "flex";
  document.getElementById("activeCallControls").style.display = "flex";
  document.getElementById("incomingCallControls").style.display = "none";
  document.getElementById("remoteVideo").classList.remove("active");
  document.getElementById("localVideoContainer").classList.remove("active");

  // Show call overlay
  document.getElementById("callOverlay").classList.add("active");
  document.getElementById("callConnectingSpinner").classList.remove("hidden");
  if (window.lucide) lucide.createIcons();

  // Play outgoing ringtone (samsung)
  playRingtone('outgoing');

  // Get local media stream
  try {
    const constraints = {
      audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true },
      video: type === 'video' ? { facingMode: currentFacingMode, width: { ideal: 1280 }, height: { ideal: 720 } } : false
    };
    localStream = await navigator.mediaDevices.getUserMedia(constraints);

    // Show local video PiP if video call
    if (type === 'video') {
      const localVid = document.getElementById("localVideo");
      localVid.srcObject = localStream;
      document.getElementById("localVideoContainer").classList.add("active");
    }
  } catch (err) {
    console.error("❌ Media access error:", err);
    showNotifToast("Cannot access " + (type === 'video' ? "camera" : "microphone") + ": " + err.message, "error");
    cleanupCall();
    return;
  }

  // Write call signal to Firestore
  const callDocId = `call_${currentUser.uid}_${selectedUser.uid}_${Date.now()}`;
  try {
    await db.collection("calls").doc(callDocId).set({
      from: currentUser.uid,
      to: selectedUser.uid,
      type: type,
      status: "ringing",
      callerName: document.getElementById("myName")?.textContent || "Nexa User",
      callerPhoto: document.getElementById("myPic")?.src || "https://i.imgur.com/HeIi0wU.png",
      callerPeerId: nexaPeer.id,
      createdAt: Date.now()
    });

    incomingCallData = { id: callDocId };

    // Send FCM push for incoming call
    sendPushNotification(
      `📞 Incoming ${type === 'video' ? 'Video' : 'Voice'} Call`,
      `${document.getElementById("myName")?.textContent || "Someone"} is calling you`,
      selectedUser.uid,
      { isCall: "true", callType: type }
    );

    // Listen for call status changes (answer / decline)
    listenCallStatus(callDocId);

    // Set timeout for unanswered call
    callTimeoutTimer = setTimeout(() => {
      if (callState === 'ringing') {
        db.collection("calls").doc(callDocId).update({ status: "missed" }).catch(() => {});
        logCallToChat('missed', callType, 0);
        showNotifToast("Call not answered", "info");
        cleanupCall();
      }
    }, CALL_TIMEOUT_MS);

  } catch (err) {
    console.error("❌ Error creating call signal:", err);
    showNotifToast("Failed to start call", "error");
    cleanupCall();
  }
}

// ── Listen for call status changes ──
function listenCallStatus(callDocId) {
  const unsub = db.collection("calls").doc(callDocId).onSnapshot(doc => {
    if (!doc.exists) return;
    const data = doc.data();

    // If still ringing, keep listening
    if (data.status === "ringing") {
      return;
    }

    // Call answered – handle caller side only (callee already handled in answerCall)
    if (data.status === "answered") {
      // Caller side – initiate PeerJS call to the callee
      if (callDirection === "outgoing" && callState === "ringing") {
        stopRingtone();
        clearTimeout(callTimeoutTimer);
        callState = "active";
        document.getElementById("callStatusLabel").textContent = "Connected";
        document.getElementById("callConnectingSpinner").classList.add("hidden");
        const recipientPeerId = data.receiverPeerId;
        if (recipientPeerId && localStream) {
          activeCall = nexaPeer.call(recipientPeerId, localStream);
          setupCallStreamHandlers(activeCall);
        }
        startCallTimer();
      }
      // If callState is already 'active' (callee side), just ignore - call is working
      return;
    }

    // Outgoing call declined
    if (data.status === "declined") {
      if (callState === "ringing" && callDirection === "outgoing") {
        stopRingtone();
        clearTimeout(callTimeoutTimer);
        showNotifToast("Call declined", "info");
        logCallToChat('declined', callType, 0);
        cleanupCall();
      }
      unsub();
      return;
    }

    // Call missed (no answer within timeout)
    if (data.status === "missed") {
      if (callState !== "active") {
        cleanupCall();
      }
      unsub();
      return;
    }

    // Call ended after active conversation
    if (data.status === "ended") {
      const duration = callStartTime ? Math.floor((Date.now() - callStartTime) / 1000) : 0;
      if (callState === "active") {
        logCallToChat('completed', callType, duration);
      }
      cleanupCall();
      unsub();
      return;
    }

    // Call failed (e.g., PeerJS error)
    if (data.status === "failed") {
      if (callState === "ringing") {
        showNotifToast("Call failed. Please try again.", "error");
      }
      cleanupCall();
      unsub();
      return;
    }

    // Unknown status – just keep listening, do NOT cleanupCall
  });
}

// ── Show Incoming Call UI ──
function showIncomingCallUI(data) {
  callDirection = 'incoming';
  callState = 'ringing';
  callType = data.type || 'voice';
  incomingCallData = data;

  const peerName = data.callerName || "Unknown";
  const peerPhoto = data.callerPhoto || "https://i.imgur.com/HeIi0wU.png";

  document.getElementById("callPeerAvatar").src = peerPhoto;
  document.getElementById("callPeerName").textContent = peerName;
  document.getElementById("callStatusLabel").textContent = data.type === 'video' ? "Nexa Video Call" : "Nexa Voice Call";
  document.getElementById("callTimerDisplay").style.display = "none";
  document.getElementById("callProfileCard").style.display = "flex";
  document.getElementById("activeCallControls").style.display = "none";
  document.getElementById("incomingCallControls").style.display = "flex";
  document.getElementById("remoteVideo").classList.remove("active");
  document.getElementById("localVideoContainer").classList.remove("active");

  document.getElementById("callOverlay").classList.add("active");
  if (window.lucide) lucide.createIcons();

  // Play incoming ringtone (iphone)
  playRingtone('incoming');

  // Vibrate device
  if (navigator.vibrate) {
    vibrationInterval = setInterval(() => {
      navigator.vibrate([400, 200, 400, 200, 400]);
    }, 2000);
    navigator.vibrate([400, 200, 400, 200, 400]);
  }

  // Auto-decline after timeout
  callTimeoutTimer = setTimeout(() => {
    if (callState === 'ringing' && callDirection === 'incoming') {
      rejectCall();
    }
  }, CALL_TIMEOUT_MS);
}

// ── Answer Incoming Call ──
async function answerCall() {
  if (!incomingCallData || callState !== 'ringing') return;

  stopRingtone();
  clearTimeout(callTimeoutTimer);
  if (vibrationInterval) { clearInterval(vibrationInterval); vibrationInterval = null; }
  if (navigator.vibrate) navigator.vibrate(0);

  // Keep callState as 'ringing' until Firestore update and PeerJS media exchange complete
  // Switch to active controls
  document.getElementById("activeCallControls").style.display = "flex";
  document.getElementById("incomingCallControls").style.display = "none";
  document.getElementById("callStatusLabel").textContent = "Connecting…";

  // Get local media
  try {
    const constraints = {
      audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true },
      video: callType === 'video' ? { facingMode: currentFacingMode, width: { ideal: 1280 }, height: { ideal: 720 } } : false
    };
    localStream = await navigator.mediaDevices.getUserMedia(constraints);

    if (callType === 'video') {
      const localVid = document.getElementById("localVideo");
      localVid.srcObject = localStream;
      document.getElementById("localVideoContainer").classList.add("active");
    }
  } catch (err) {
    console.error("❌ Media access error on answer:", err);
    showNotifToast("Cannot access " + (callType === 'video' ? "camera" : "microphone"), "error");
    rejectCall();
    return;
  }

  // Now set state to active AFTER media is acquired
  callState = 'active';

  // Answer the PeerJS call if it already came in
  if (activeCall) {
    // Attach handlers BEFORE answer() — PeerJS can fire `stream`
    // synchronously during answer(). Attaching after = no video/audio.
    setupCallStreamHandlers(activeCall);
    activeCall.answer(localStream);
  } else {
    // Wait for PeerJS call to arrive
    nexaPeer.on('call', (mediaCall) => {
      activeCall = mediaCall;
      setupCallStreamHandlers(mediaCall);
      mediaCall.answer(localStream);
    });
  }

  // Update Firestore signal to "answered" AFTER answering PeerJS call
  // This way the caller gets receiverPeerId and initiates nexaPeer.call()
  try {
    await db.collection("calls").doc(incomingCallData.id).update({
      status: "answered",
      receiverPeerId: nexaPeer.id,
      answeredAt: Date.now()
    });
  } catch (err) {
    console.error("Error updating call status:", err);
  }

  // Listen for further status changes (ended, etc.)
  listenCallStatus(incomingCallData.id);
  startCallTimer();
  document.getElementById("callStatusLabel").textContent = "Connected";
  document.getElementById("callConnectingSpinner").classList.add("hidden");
}

// ── Reject Incoming Call ──
async function rejectCall() {
  stopRingtone();
  clearTimeout(callTimeoutTimer);
  if (vibrationInterval) { clearInterval(vibrationInterval); vibrationInterval = null; }
  if (navigator.vibrate) navigator.vibrate(0);

  if (incomingCallData && incomingCallData.id) {
    try {
      await db.collection("calls").doc(incomingCallData.id).update({
        status: "declined",
        declinedAt: Date.now()
      });
    } catch (err) {
      console.error("Error declining call:", err);
    }
  }

  cleanupCall();
}

// ── End Active Call ──
async function endCall() {
  stopRingtone();
  clearTimeout(callTimeoutTimer);
  if (vibrationInterval) { clearInterval(vibrationInterval); vibrationInterval = null; }
  if (navigator.vibrate) navigator.vibrate(0);

  const duration = callStartTime ? Math.floor((Date.now() - callStartTime) / 1000) : 0;

  if (incomingCallData && incomingCallData.id) {
    try {
      await db.collection("calls").doc(incomingCallData.id).update({
        status: "ended",
        endedAt: Date.now(),
        duration: duration
      });
    } catch (err) {
      console.error("Error ending call:", err);
    }
  }

  if (callState === 'active' && duration > 0) {
    logCallToChat('completed', callType, duration);
  } else if (callState === 'ringing' && callDirection === 'outgoing') {
    logCallToChat('cancelled', callType, 0);
  }

  cleanupCall(true); // keep UI briefly to show duration
}

// ── Set up PeerJS media stream handlers ──
function setupCallStreamHandlers(mediaCall) {
  // Attach the PeerJS 'stream' handler immediately. PeerJS can fire this
  // synchronously during answer(), so this MUST be registered before answer().
  mediaCall.on('stream', (stream) => {
    console.log('📞 Remote stream received:', stream.getTracks().map(t => t.kind).join(','));
    remoteStream = stream;

    // Always pipe remote audio through the dedicated <audio> element
    // The <video id="remoteVideo"> is display:none during voice calls
    // and browsers won't play audio from a hidden video element
    const remoteAudio = document.getElementById("remoteCallAudio");
    if (remoteAudio) {
      remoteAudio.srcObject = stream;
      remoteAudio.muted = !isSpeakerOn;
      remoteAudio.volume = 1.0;
      remoteAudio.play().catch(err => console.warn('📞 Remote audio play warning:', err));
    }

    const remoteVid = document.getElementById("remoteVideo");
    if (callType === 'video') {
      // For video calls: show remote video and pipe stream to video element too
      if (remoteVid) {
        remoteVid.srcObject = stream;
        remoteVid.muted = true; // audio comes from remoteCallAudio, avoid echo
        remoteVid.classList.add("active");
        remoteVid.play().catch(err => console.warn('📞 Remote video play warning:', err));
      }
      document.getElementById("callProfileCard").style.display = "none";
    } else {
      // For voice calls: hide video, show profile card
      if (remoteVid) remoteVid.classList.remove("active");
      document.getElementById("callProfileCard").style.display = "flex";
    }

    document.getElementById("callConnectingSpinner").classList.add("hidden");
    document.getElementById("callStatusLabel").textContent = "In call";
  });

  // Fallback: PeerJS's 'stream' may not fire for renegotiated/late tracks.
  // ontrack catches those and re-applies the stream to both elements.
  const wireOnTrack = () => {
    if (!mediaCall.peerConnection) return false;
    mediaCall.peerConnection.ontrack = (event) => {
      console.log('📞 Dynamic remote track added:', event.track.kind);
      // Use the event stream if available, else the remoteStream we already have.
      const stream = (event.streams && event.streams[0]) ? event.streams[0] : remoteStream;
      if (stream) {
        remoteStream = stream;
        const remoteAudio = document.getElementById("remoteCallAudio");
        if (remoteAudio && event.track.kind === 'audio') {
          remoteAudio.srcObject = stream;
          remoteAudio.play().catch(() => {});
        }
        const remoteVid = document.getElementById("remoteVideo");
        if (remoteVid && callType === 'video' && event.track.kind === 'video') {
          remoteVid.srcObject = stream;
          remoteVid.classList.add("active");
          remoteVid.play().catch(() => {});
          document.getElementById("callProfileCard").style.display = "none";
        }
      }
    };
    mediaCall.peerConnection.oniceconnectionstatechange = () => {
      const st = mediaCall.peerConnection.iceConnectionState;
      console.log('📞 ICE Connection State:', st);
      if (st === 'failed') {
        console.warn('⚠ ICE connection failed, attempting restart...');
        try { mediaCall.peerConnection.restartIce(); } catch (e) {}
      }
    };
    return true;
  };
  // peerConnection may not exist yet — retry until it's available.
  if (!wireOnTrack()) {
    let attempts = 0;
    const poll = setInterval(() => {
      if (wireOnTrack() || ++attempts > 10) clearInterval(poll);
    }, 200);
  }

  mediaCall.on('close', () => {
    console.log('📞 Call stream closed');
    if (callState === 'active') {
      const duration = callStartTime ? Math.floor((Date.now() - callStartTime) / 1000) : 0;
      logCallToChat('completed', callType, duration);
      cleanupCall();
    }
  });

  mediaCall.on('error', (err) => {
    console.error('📞 Call error:', err);
    showNotifToast("Call connection error", "error");
    document.getElementById("callConnectingSpinner").classList.add("hidden");
    cleanupCall();
  });
}

// ── Call Timer ──
function startCallTimer() {
  callStartTime = Date.now();
  document.getElementById("callTimerDisplay").style.display = "block";

  callTimerInterval = setInterval(() => {
    const elapsed = Math.floor((Date.now() - callStartTime) / 1000);
    const mins = Math.floor(elapsed / 60).toString().padStart(2, '0');
    const secs = (elapsed % 60).toString().padStart(2, '0');
    const timeStr = `${mins}:${secs}`;

    document.getElementById("callTimerDisplay").textContent = timeStr;
    document.getElementById("floatingCallTimer").textContent = timeStr;
  }, 1000);
}

// ── Ringtone Control ──
function playRingtone(type) {
  stopRingtone();
  try {
    if (type === 'incoming') {
      const ring = document.getElementById("ringtoneIncoming");
      ring.currentTime = 0;
      ring.volume = 1;
      ring.play().catch(e => console.warn("Incoming ringtone play error:", e));
    } else {
      const ring = document.getElementById("ringtoneOutgoing");
      ring.currentTime = 0;
      ring.volume = 0.7;
      ring.play().catch(e => console.warn("Outgoing ringtone play error:", e));
    }
  } catch (e) {
    console.warn("Ringtone error:", e);
  }
}

function stopRingtone() {
  try {
    const incoming = document.getElementById("ringtoneIncoming");
    if (incoming) { incoming.pause(); incoming.currentTime = 0; }
    const outgoing = document.getElementById("ringtoneOutgoing");
    if (outgoing) { outgoing.pause(); outgoing.currentTime = 0; }
  } catch (e) {}
}

// ── Media Controls ──
function toggleMuteMic() {
  if (!localStream) return;
  isMicMuted = !isMicMuted;
  localStream.getAudioTracks().forEach(t => { t.enabled = !isMicMuted; });
  const btn = document.getElementById("btnMuteMic");
  btn.classList.toggle("muted", isMicMuted);
  showNotifToast(isMicMuted ? "🔇 Mic muted" : "🔊 Mic unmuted", "info");
}

function toggleVideoCamera() {
  if (!localStream) return;
  const videoTracks = localStream.getVideoTracks();
  if (videoTracks.length === 0) {
    // No video track — try adding one
    addVideoTrack();
    return;
  }
  isCameraOff = !isCameraOff;
  videoTracks.forEach(t => { t.enabled = !isCameraOff; });
  const btn = document.getElementById("btnToggleVideo");
  btn.classList.toggle("off", isCameraOff);

  if (isCameraOff) {
    document.getElementById("localVideoContainer").classList.remove("active");
  } else {
    document.getElementById("localVideoContainer").classList.add("active");
  }
  showNotifToast(isCameraOff ? "📷 Camera off" : "📷 Camera on", "info");
}

async function addVideoTrack() {
  try {
    const videoStream = await navigator.mediaDevices.getUserMedia({
      video: { facingMode: currentFacingMode, width: { ideal: 1280 }, height: { ideal: 720 } }
    });
    const videoTrack = videoStream.getVideoTracks()[0];
    localStream.addTrack(videoTrack);

    document.getElementById("localVideo").srcObject = localStream;
    document.getElementById("localVideoContainer").classList.add("active");

    // Replace track on active call
    if (activeCall && activeCall.peerConnection) {
      const sender = activeCall.peerConnection.getSenders().find(s => s.track && s.track.kind === 'video');
      if (sender) {
        sender.replaceTrack(videoTrack);
      } else {
        activeCall.peerConnection.addTrack(videoTrack, localStream);
      }
    }

    isCameraOff = false;
    document.getElementById("btnToggleVideo").classList.remove("off");
    callType = 'video';
    showNotifToast("📷 Camera on", "info");
  } catch (err) {
    showNotifToast("Cannot access camera: " + err.message, "error");
  }
}

async function flipCamera() {
  if (!localStream) return;
  currentFacingMode = currentFacingMode === 'user' ? 'environment' : 'user';

  try {
    const newStream = await navigator.mediaDevices.getUserMedia({
      video: { facingMode: currentFacingMode, width: { ideal: 1280 }, height: { ideal: 720 } }
    });
    const newTrack = newStream.getVideoTracks()[0];

    // Replace in local stream
    const oldTrack = localStream.getVideoTracks()[0];
    if (oldTrack) {
      localStream.removeTrack(oldTrack);
      oldTrack.stop();
    }
    localStream.addTrack(newTrack);
    document.getElementById("localVideo").srcObject = localStream;

    // Replace in PeerConnection
    if (activeCall && activeCall.peerConnection) {
      const sender = activeCall.peerConnection.getSenders().find(s => s.track && s.track.kind === 'video');
      if (sender) sender.replaceTrack(newTrack);
    }

    showNotifToast("📷 Camera flipped", "info");
  } catch (err) {
    showNotifToast("Cannot flip camera: " + err.message, "error");
  }
}

function toggleCallSpeaker() {
  isSpeakerOn = !isSpeakerOn;
  const remoteVid = document.getElementById("remoteVideo");
  if (remoteVid) {
    remoteVid.muted = !isSpeakerOn;
    remoteVid.volume = isSpeakerOn ? 1.0 : 0.0;
  }
  // Voice calls play audio through the dedicated <audio> element, so mute
  // that too — otherwise speaker toggle has no effect on voice-only calls.
  const remoteAudio = document.getElementById("remoteCallAudio");
  if (remoteAudio) {
    remoteAudio.muted = !isSpeakerOn;
    remoteAudio.volume = isSpeakerOn ? 1.0 : 0.0;
  }

  const btn = document.getElementById("callSpeakerBtn");
  if (btn) btn.classList.toggle("active", isSpeakerOn);
  showNotifToast(isSpeakerOn ? "🔊 Speaker on" : "🔇 Speaker off", "info");
}

// ── Minimize / Maximize Call Overlay ──
function minimizeCallOverlay() {
  if (callState !== 'active') return;
  document.getElementById("callOverlay").classList.remove("active");

  const bar = document.getElementById("floatingCallBar");
  const peerName = document.getElementById("callPeerName").textContent;
  const peerPhoto = document.getElementById("callPeerAvatar").src;
  document.getElementById("floatingCallName").textContent = peerName;
  document.getElementById("floatingCallAvatar").src = peerPhoto;
  bar.classList.add("active");

  if (window.lucide) lucide.createIcons();
}

function maximizeCallOverlay() {
  document.getElementById("floatingCallBar").classList.remove("active");
  document.getElementById("callOverlay").classList.add("active");
  if (window.lucide) lucide.createIcons();
}

// ── Cleanup Call ──
function cleanupCall() {
  stopRingtone();
  clearTimeout(callTimeoutTimer);
  clearInterval(callTimerInterval);
  if (vibrationInterval) { clearInterval(vibrationInterval); vibrationInterval = null; }
  if (navigator.vibrate) navigator.vibrate(0);

  // Close PeerJS call
  if (activeCall) {
    try { activeCall.close(); } catch (e) {}
    activeCall = null;
  }

  // Stop all local media tracks
  if (localStream) {
    localStream.getTracks().forEach(t => t.stop());
    localStream = null;
  }

  remoteStream = null;

  // Reset UI
  document.getElementById("callOverlay").classList.remove("active");
  document.getElementById("floatingCallBar").classList.remove("active");
  document.getElementById("remoteVideo").classList.remove("active");
  document.getElementById("localVideoContainer").classList.remove("active");
  document.getElementById("remoteVideo").srcObject = null;
  document.getElementById("localVideo").srcObject = null;

  callState = 'idle';
  callDirection = null;
  callStartTime = null;
  incomingCallData = null;
  isMicMuted = false;
  isCameraOff = false;
}

// ── Log Call to Chat ──
function logCallToChat(result, type, durationSec) {
  if (!currentUser) return;

  const otherUid = callDirection === 'outgoing'
    ? (selectedUser ? selectedUser.uid : null)
    : (incomingCallData ? incomingCallData.from : null);

  if (!otherUid) return;

  let emoji = type === 'video' ? '📹' : '📞';
  let text = '';
  if (result === 'completed') {
    const mins = Math.floor(durationSec / 60);
    const secs = durationSec % 60;
    text = `${emoji} ${type === 'video' ? 'Video' : 'Voice'} call • ${mins}m ${secs.toString().padStart(2, '0')}s`;
  } else if (result === 'missed') {
    emoji = '🚫';
    text = `${emoji} Missed ${type === 'video' ? 'video' : 'voice'} call`;
  } else if (result === 'declined') {
    text = `${emoji} ${type === 'video' ? 'Video' : 'Voice'} call declined`;
  } else if (result === 'cancelled') {
    text = `${emoji} ${type === 'video' ? 'Video' : 'Voice'} call cancelled`;
  }

  if (text) {
    db.collection("chats").add({
      from: currentUser.uid,
      to: otherUid,
      text: text,
      callLog: true,
      callType: type,
      callResult: result,
      callDuration: durationSec,
      createdAt: Date.now(),
      read: false,
      status: "sent"
    }).catch(err => console.warn("Error logging call:", err));
  }
}

// ── Public Entry Points (called from chat header buttons) ──
async function openVoiceCall() {
  if (!selectedUser) { showNotifToast("Select a user first", "error"); return; }
  if (isSelfChat()) { showNotifToast("You can't call yourself", "info"); return; }
  startCall('voice');
}

function openCallModal(type) {
  if (!selectedUser) { showNotifToast("Select a user first", "error"); return; }
  if (isSelfChat()) { showNotifToast("You can't call yourself", "info"); return; }
  startCall(type || 'video');
}

function openUserProfile() {
  if (!selectedUser) { alert("Select a user first"); return; }
  const u = selectedUser;
  const photo = u.photo || "https://i.imgur.com/HeIi0wU.png";

  document.getElementById('userProfileAvatar').src = photo;
  document.getElementById('userProfileName').textContent = u.displayName || u.name || 'User';
  const profHandle = document.getElementById('userProfileHandle');
  if (profHandle) {
    const uh = u.username || u.usernameField || '';
    profHandle.textContent = uh ? '@' + uh : '';
  }

  // Presence / status from the shared cache (timestamp-based).
  const pData = userPresenceCache[u.uid] || {};
  const online = isUserOnline(pData);
  const status = pData.status || (online ? 'online' : 'offline');
  const statusEl = document.getElementById('userProfileStatus');
  if (online && status === 'online') {
    statusEl.innerHTML = '<span style="color:var(--online)">● online</span>';
  } else if (online && status === 'away') {
    statusEl.innerHTML = '<span style="color:#facc15">● away</span>';
  } else {
    statusEl.innerHTML = '<span style="color:var(--text-3)">last seen ' + formatLastSeen(pData.lastSeen || 0) + '</span>';
  }

  // Bio / username / phone if present in the user doc.
  const bio = u.bio || u.about || '';
  const username = u.username || u.usernameField || '';
  const bioEl = document.getElementById('userProfileBio');
  if (bio) {
    bioEl.innerHTML = linkify(bio);
  } else if (username) {
    bioEl.innerHTML = '<span style="color:var(--text-3)">@</span>' + escapeHtml(username);
  } else {
    bioEl.textContent = 'Hey! I am using Nexa.';
  }

  // Fetch the user's 24h profile note (if any).
  const noteEl = document.getElementById('userProfileNote');
  if (noteEl) noteEl.innerHTML = '<div style="color:var(--text-3);font-size:12px;">Loading note…</div>';
  fetchUserNote(u.uid).then(noteText => {
    if (!noteEl) return;
    if (noteText) {
      noteEl.innerHTML = '<i data-lucide="file-text" style="width: 13px; height: 13px; vertical-align: -2px; color: var(--text-3);"></i> ' + linkify(noteText);
      if (window.lucide) lucide.createIcons();
    } else {
      noteEl.innerHTML = '';
    }
  });

  document.getElementById('userProfileModal').classList.add('active');
}

function closeUserProfile() {
  document.getElementById('userProfileModal').classList.remove('active');
}

function closeUserProfile(event) {
  if (event && event.target !== document.getElementById('userProfileModal')) return;
  document.getElementById('userProfileModal').classList.remove('active');
}

/* =========================================================================
   STORIES / STATUS
   ========================================================================= */
let userStatuses = {};
let storyViewerData = null;
let storyViewerIndex = 0;
let storyTimer = null;
let storyProgressStart = null;
let storyAnimFrame = null;
let viewingStoryUid = null;
let storyCurrentDuration = 0;

// Stories expire after 24h. We scope the listener to createdAt >= (now-24h)
// so Firestore only returns recent stories instead of the ENTIRE status
// collection on every change — a single-field range query (createdAt is
// single-field auto-indexed), so no composite index is needed. This is the
// single biggest read saver for the stories feature: a growing `status`
// collection used to be re-read in full on every post/view anywhere.
const STATUS_TTL_MS = 24 * 60 * 60 * 1000;
let statusListenerUnsub = null;
let statusGcRunning = false;
function startStatusListener() {
  if (statusListenerUnsub) return;
  const cutoff = Date.now() - STATUS_TTL_MS;
  statusListenerUnsub = db.collection("status")
    .where("createdAt", ">=", cutoff)
    .onSnapshot(snap => {
      // docChanges() keeps processing O(changes) on updates; the initial
      // snapshot is already bounded to the last 24h by the where clause.
      userStatuses = {};
      const now = Date.now();
      snap.forEach(doc => {
        const s = doc.data();
        // Client-side TTL guard: the server query cutoff is fixed at listener
        // creation, so on very long sessions a story could age past 24h while
        // still matching the original query. Drop it here if expired.
        if (s.createdAt && (now - s.createdAt) > STATUS_TTL_MS) return;
        const uid = s.uid;
        if (!uid) return;
        if (!userStatuses[uid]) userStatuses[uid] = [];
        userStatuses[uid].push({ id: doc.id, ...s });
      });
      renderStoriesBar();
      renderUsers();
      saveCachedStories();
    }, err => console.error("Status listener error:", err));
  // One-time garbage collection of stories older than 24h. The scoped listener
  // never returns them, so they'd otherwise linger in storage forever. We read
  // ONLY the expired slice (createdAt < cutoff), bounded, and batch-delete —
  // any signed-in client may delete an expired story per firestore.rules.
  purgeExpiredStories();
}

// Delete stories past their 24h TTL. Bounded to the expired slice so it never
// reads the whole collection; runs once per session on listener start.
function purgeExpiredStories() {
  if (statusGcRunning || !db) return;
  statusGcRunning = true;
  const cutoff = Date.now() - STATUS_TTL_MS;
  db.collection("status").where("createdAt", "<", cutoff).get().then(snap => {
    if (snap.empty) return;
    let batch = db.batch();
    let count = 0;
    snap.forEach(d => { batch.delete(d.ref); count++; if (count % 400 === 0) { batch.commit(); batch = db.batch(); } });
    if (count % 400 !== 0) batch.commit();
    console.log("Purged", count, "expired stories (>" + Math.round(STATUS_TTL_MS/3600000) + "h old).");
  }).catch(err => console.warn("Story GC error:", err)).finally(() => { statusGcRunning = false; });
}

// Cache the stories bar so a returning user sees story rings instantly on
// open (before the status listener connects). Only a light projection is kept
// (id/uid/type/createdAt/text/url) — heavy blobs like musicData are dropped.
// Stories older than 24h are discarded on restore, matching the live TTL.
function saveCachedStories() {
  if (!currentUser) return;
  try {
    const light = {};
    for (const uid in userStatuses) {
      light[uid] = (userStatuses[uid] || []).map(s => ({
        id: s.id, uid: s.uid, type: s.type, createdAt: s.createdAt,
        text: s.text || null, url: s.url || null
      }));
    }
    localStorage.setItem("nexa_cached_stories_" + currentUser.uid, JSON.stringify(light));
  } catch (e) {}
}

function loadCachedStories() {
  const uid = (currentUser && currentUser.uid) || localStorage.getItem("nexa_last_uid");
  if (!uid) return;
  try {
    const saved = localStorage.getItem("nexa_cached_stories_" + uid);
    if (!saved) return;
    const parsed = JSON.parse(saved);
    const now = Date.now();
    userStatuses = {};
    for (const k in parsed) {
      const arr = (parsed[k] || []).filter(s => s.createdAt && (now - s.createdAt) < STATUS_TTL_MS);
      if (arr.length) userStatuses[k] = arr;
    }
  } catch (e) {}
}

function renderStoriesBar() {
  renderStoriesTab();
  updateStoryTabDot();
  const bar = document.getElementById("storiesBar");
  if (!bar) return;
  bar.innerHTML = "";
  const addEl = document.createElement("div");
  addEl.className = "story-item add-story";
  const myStatuses = userStatuses[currentUser?.uid] || [];
  const myRingClass = myStatuses.length ? "story-ring unseen" : "story-ring add-own";
  const myPicSrc = document.getElementById("myPic")?.src || "https://i.imgur.com/HeIi0wU.png";

  addEl.onclick = () => {
    if (myStatuses.length > 0) {
      openStoryViewer(currentUser.uid);
    } else {
      openStatusModal();
    }
  };

  addEl.innerHTML = `
    <div class="${myRingClass}">
      <div class="story-ring-inner">${myStatuses.length ? `<img src="${myPicSrc}" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">` : "+"}</div>
    </div>
    <div class="story-name">My Story</div>
  `;
  bar.appendChild(addEl);

  const seenMap = getSeenStories();
  allUsersData.forEach(user => {
    const statuses = userStatuses[user.uid];
    if (!statuses || !statuses.length) return;
    const allSeen = statuses.every(s => seenMap[s.id]);
    const el = document.createElement("div");
    el.className = "story-item";
    el.onclick = () => openStoryViewer(user.uid);
    const photo = user.photo || "https://i.imgur.com/HeIi0wU.png";
    const name = escapeHtml(user.displayName || "User");
    el.innerHTML = `
      <div class="story-ring ${allSeen ? 'seen' : 'unseen'}">
        <div class="story-ring-inner"><img src="${photo}" onerror="this.src='https://i.imgur.com/HeIi0wU.png'"></div>
      </div>
      <div class="story-name">${name}</div>
    `;
    bar.appendChild(el);
  });
}

function getSeenStories() {
  try {
    return JSON.parse(localStorage.getItem("seenStories") || "{}");
  } catch (e) {
    return {};
  }
}

function markStorySeen(id) {
  const seen = getSeenStories();
  seen[id] = true;
  localStorage.setItem("seenStories", JSON.stringify(seen));
  updateStoryTabDot();
}

// WhatsApp-style "new story" indicator: light up the Stories tab (mobile
// bottom nav + desktop sidebar) with a green dot whenever at least one
// contact has a story the user hasn't viewed yet. Your OWN stories don't
// trigger it. Cleared automatically once every story is seen.
function updateStoryTabDot() {
  const seenMap = getSeenStories();
  const myUid = currentUser ? currentUser.uid : localStorage.getItem("nexa_last_uid");
  let hasUnseen = false;
  for (const uid in userStatuses) {
    if (uid === myUid) continue; // your own story isn't a "new story" alert
    const statuses = userStatuses[uid] || [];
    if (statuses.some(s => !seenMap[s.id])) { hasUnseen = true; break; }
  }
  ["storyTabDotMobile", "storyTabDotDesktop"].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.style.display = hasUnseen ? "block" : "none";
  });
}

function openStoryViewer(uid, storyId = null) {
  const statuses = userStatuses[uid];
  if (!statuses || !statuses.length) {
    showNotifToast("This story is no longer available", "error");
    return;
  }

  viewingStoryUid = uid;
  storyViewerData = statuses.sort((a, b) => a.createdAt - b.createdAt);

  storyViewerIndex = 0;
  if (storyId) {
    const idx = storyViewerData.findIndex(s => s.id === storyId);
    if (idx !== -1) {
      storyViewerIndex = idx;
    } else {
      showNotifToast("That specific story is no longer available, showing latest instead", "info");
    }
  }

  const isOwnStory = (uid === currentUser.uid);

  let user = allUsersData.find(u => u.uid === uid);
  if (uid === currentUser.uid) {
    user = {
      displayName: document.getElementById("myName").textContent || "My Story",
      photo: document.getElementById("myPic").src || "https://i.imgur.com/HeIi0wU.png"
    };
  }

  document.getElementById("storyUserAv").src = user?.photo || "https://i.imgur.com/HeIi0wU.png";
  document.getElementById("storyUserName").textContent = user?.displayName || "User";

  const deleteBtn = document.getElementById("storyDeleteBtn");
  if (isOwnStory) {
    deleteBtn.style.display = "block";
  } else {
    deleteBtn.style.display = "none";
  }

  document.getElementById("storyViewerOverlay").classList.add("active");
  renderStorySlide(storyViewerIndex);
}

function renderStorySlide(idx) {
  if (!storyViewerData || idx >= storyViewerData.length) {
    closeStoryViewer();
    return;
  }

  const story = storyViewerData[idx];
  markStorySeen(story.id);

  // Record view in Firestore if viewing someone else's story
  if (currentUser && story.uid !== currentUser.uid) {
    const views = story.views || [];
    if (!views.includes(currentUser.uid)) {
      db.collection("status").doc(story.id).update({
        views: firebase.firestore.FieldValue.arrayUnion(currentUser.uid)
      }).catch(e => console.log("View update error:", e));
      if (!story.views) story.views = [];
      story.views.push(currentUser.uid);
    }
  }

  // Check reshared status
  const resharedBadge = document.getElementById("resharedBadge");
  if (story.resharedFrom && story.resharedFrom.name) {
    document.getElementById("resharedName").textContent = story.resharedFrom.name;
    resharedBadge.style.display = "inline-flex";
  } else {
    resharedBadge.style.display = "none";
  }

  // Own story vs Other story UI
  const isOwnStory = (story.uid === currentUser.uid);
  const statsBar = document.getElementById("storyStatsBar");
  const actionsBar = document.getElementById("storyViewerActions");

  const viewsList = story.views || [];
  const likesList = story.likes || [];
  const resharesList = story.reshares || [];

  if (isOwnStory) {
    if (statsBar) {
      statsBar.style.display = "flex";
      document.getElementById("storyViewsText").innerHTML = `<i data-lucide="eye" style="width: 13px; height: 13px;"></i> ${viewsList.length} view${viewsList.length === 1 ? '' : 's'}`;
      document.getElementById("storyLikesText").innerHTML = `<i data-lucide="heart" style="width: 13px; height: 13px;"></i> ${likesList.length} like${likesList.length === 1 ? '' : 's'}`;
      document.getElementById("storyResharesText").innerHTML = `<i data-lucide="repeat" style="width: 13px; height: 13px;"></i> ${resharesList.length} reshare${resharesList.length === 1 ? '' : 's'}`;
    }
    if (actionsBar) actionsBar.style.display = "none";
  } else {
    if (statsBar) statsBar.style.display = "none";
    if (actionsBar) actionsBar.style.display = "flex";

    const likeBtn = document.getElementById("storyLikeBtn");
    const hasLiked = likesList.includes(currentUser.uid);
    if (likeBtn) {
      likeBtn.classList.toggle("active-like", hasLiked);
      likeBtn.innerHTML = `<i data-lucide="heart" style="width: 18px; height: 18px;"></i>`;
    }
  }
  if (window.lucide) lucide.createIcons();

  const progressRow = document.getElementById("storyProgressRow");
  progressRow.innerHTML = "";
  storyViewerData.forEach((s, i) => {
    const bar = document.createElement("div");
    bar.className = "story-prog-bar";
    const fill = document.createElement("div");
    fill.className = "story-prog-fill";
    fill.id = `spf-${i}`;
    if (i < idx) fill.style.width = "100%";
    bar.appendChild(fill);
    progressRow.appendChild(bar);
  });

  document.getElementById("storyUserTime").textContent = formatTime(story.createdAt);

  const bg = document.getElementById("storyViewerBg");
  bg.innerHTML = "";
  if (story.url && story.type !== "music") {
    const bgEl = story.type === "video" ? document.createElement("video") : document.createElement("img");
    bgEl.src = story.url;
    if (story.type === "video") {
      bgEl.autoplay = true;
      bgEl.muted = true;
      bgEl.loop = true;
    }
    bg.appendChild(bgEl);
  } else {
    bg.style.background = "linear-gradient(135deg, var(--primary), var(--primary-dk))";
  }

  const content = document.getElementById("storyViewerContent");
  content.innerHTML = "";

  // Story media loading indicator — show while the image/video buffers so the
  // viewer never looks frozen, hide the moment it's ready to display.
  const loadingEl = document.getElementById("storyLoading");
  const showLoading = () => { if (loadingEl) loadingEl.classList.add("show"); };
  const hideLoading = () => { if (loadingEl) loadingEl.classList.remove("show"); };

  if (storyAudioElement) {
    storyAudioElement.pause();
    storyAudioElement = null;
  }

  const musicBadge = document.getElementById("musicBadge");

  function startStoryAnimation(duration) {
    storyCurrentDuration = duration;
    storyPauseElapsed = 0;
    isStoryPaused = false;
    const indicator = document.getElementById("storyPauseIndicator");
    if (indicator) indicator.classList.remove("show");

    clearTimeout(storyTimer);
    cancelAnimationFrame(storyAnimFrame);
    const fillEl = document.getElementById(`spf-${idx}`);
    if (!fillEl) return;
    storyProgressStart = Date.now();

    function animFill() {
      if (isStoryPaused) return;
      const elapsed = Date.now() - storyProgressStart;
      const pct = Math.min((elapsed / storyCurrentDuration) * 100, 100);
      fillEl.style.width = pct + "%";
      if (pct < 100) {
        storyAnimFrame = requestAnimationFrame(animFill);
      } else {
        storyPauseElapsed = 0;
        storyViewerIndex++;
        renderStorySlide(storyViewerIndex);
      }
    }

    storyAnimFrame = requestAnimationFrame(animFill);
  }

  if (story.type === "image" && story.url) {
    const img = document.createElement("img");
    showLoading();
    img.onload = hideLoading;
    img.onerror = hideLoading;
    img.src = story.url;
    img.alt = "story";
    img.style.cssText = "width: 100%; height: 100%; object-fit: contain;";
    content.appendChild(img);

    if (story.text) {
      const txt = document.createElement("div");
      txt.style.cssText = "position: absolute; bottom: 130px; left: 0; right: 0; text-align: center; font-size: 18px; font-weight: 700; color: white; text-shadow: 0 2px 8px rgba(0,0,0,0.8); padding: 0 20px; word-break: break-word; line-height: 1.4; z-index: 5;";
      txt.innerHTML = linkify(story.text);
      content.appendChild(txt);
    }

    let duration = 5000;
    if (story.musicData && story.musicData.previewUrl) {
      duration = story.musicData.duration * 1000 || 5000;
      playStoryMusic(story.musicData);
      musicBadge.textContent = `🎵 ${story.musicData.title}`;
      musicBadge.style.display = "block";
    } else {
      musicBadge.style.display = "none";
    }
    startStoryAnimation(duration);
  } else if (story.type === "video" && story.url) {
    const vid = document.createElement("video");
    showLoading();
    vid.addEventListener("canplay", hideLoading);
    vid.addEventListener("error", hideLoading);
    vid.src = story.url;
    vid.autoplay = true;
    vid.playsInline = true;
    vid.style.cssText = "width: 100%; height: 100%; object-fit: contain;";
    content.appendChild(vid);

    let metadataLoaded = false;
    let videoDuration = 0;

    vid.addEventListener("loadedmetadata", () => {
      metadataLoaded = true;
      videoDuration = Math.ceil(vid.duration * 1000);

      if (story.musicData && story.musicData.previewUrl) {
        vid.muted = true;
        const musicDuration = story.musicData.duration * 1000 || 5000;
        playStoryMusic(story.musicData);
        musicBadge.textContent = `🎵 ${story.musicData.title}`;
        musicBadge.style.display = "block";
        startStoryAnimation(musicDuration);
      } else {
        musicBadge.style.display = "none";
        startStoryAnimation(videoDuration > 0 ? videoDuration : 5000);
      }
    });

    setTimeout(() => {
      if (!metadataLoaded) {
        if (story.musicData && story.musicData.previewUrl) {
          vid.muted = true;
          playStoryMusic(story.musicData);
          musicBadge.textContent = `🎵 ${story.musicData.title}`;
          musicBadge.style.display = "block";
          startStoryAnimation(story.musicData.duration * 1000 || 5000);
        } else {
          musicBadge.style.display = "none";
          startStoryAnimation(5000);
        }
      }
    }, 3000);

    if (story.text) {
      const txt = document.createElement("div");
      txt.style.cssText = "position: absolute; bottom: 130px; left: 0; right: 0; text-align: center; font-size: 18px; font-weight: 700; color: white; text-shadow: 0 2px 8px rgba(0,0,0,0.8); padding: 0 20px; word-break: break-word; line-height: 1.4; z-index: 5;";
      txt.innerHTML = linkify(story.text);
      content.appendChild(txt);
    }
  } else if (story.type === "text" || story.text) {
    hideLoading();
    const div = document.createElement("div");
    div.className = "story-text-slide";
    div.innerHTML = linkify(story.text || "");
    content.appendChild(div);

    let duration = 6000;
    if (story.musicData && story.musicData.previewUrl) {
      duration = story.musicData.duration * 1000 || 6000;
      playStoryMusic(story.musicData);
      musicBadge.textContent = `🎵 ${story.musicData.title}`;
      musicBadge.style.display = "block";
    } else {
      musicBadge.style.display = "none";
    }
    startStoryAnimation(duration);
  } else if (story.type === "music" && story.musicData) {
    hideLoading();
    const overlay = document.createElement("div");
    overlay.style.cssText = "position: absolute; inset: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 20px; padding: 40px; text-align: center; z-index: 5;";

    const label = document.createElement("div");
    label.style.cssText = "font-size: 14px; color: rgba(255,255,255,0.8); font-weight: 500; letter-spacing: 0.5px; text-transform: uppercase;";
    label.textContent = "🎵 Now Playing";
    overlay.appendChild(label);

    const title = document.createElement("div");
    title.style.cssText = "font-size: 24px; font-weight: 700; color: white; line-height: 1.4; word-break: break-word;";
    title.textContent = story.musicData.title;
    overlay.appendChild(title);

    const artist = document.createElement("div");
    artist.style.cssText = "font-size: 16px; color: rgba(255,255,255,0.7);";
    artist.textContent = story.musicData.artist;
    overlay.appendChild(artist);

    content.appendChild(overlay);

    const duration = story.musicData.duration * 1000 || 30000;
    playStoryMusic(story.musicData);
    musicBadge.textContent = `🎵 ${story.musicData.title}`;
    musicBadge.style.display = "block";
    startStoryAnimation(duration);
  } else if (story.type === "prompt") {
    // "Add Yours" prompt story — a standalone text prompt that invites responses.
    const overlay = document.createElement("div");
    overlay.className = "prompt-story-overlay";
    overlay.style.cssText = "position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:36px;text-align:center;z-index:5;background:linear-gradient(135deg, var(--primary), var(--primary-dk));";

    const tag = document.createElement("div");
    tag.style.cssText = "font-size:13px;color:rgba(255,255,255,0.75);font-weight:600;letter-spacing:0.5px;text-transform:uppercase;";
    tag.textContent = "➕ Add Yours";
    overlay.appendChild(tag);

    const promptEl = document.createElement("div");
    promptEl.style.cssText = "font-size:26px;font-weight:800;color:#fff;line-height:1.35;word-break:break-word;text-shadow:0 2px 12px rgba(0,0,0,0.5);";
    promptEl.innerHTML = linkify(story.promptText || story.text || "");
    overlay.appendChild(promptEl);

    const pid = story.promptId;
    let responseCount = 0;
    if (pid) {
      Object.values(userStatuses).forEach(arr => {
        (arr || []).forEach(s => { if (s.promptId === pid && s.type !== "prompt") responseCount++; });
      });
    }

    const ctaWrap = document.createElement("div");
    ctaWrap.style.cssText = "display:flex;flex-direction:column;gap:10px;align-items:center;margin-top:8px;";

    const addBtn = document.createElement("button");
    addBtn.textContent = "➕ Add Yours";
    addBtn.className = "addyours-cta-btn";
    addBtn.onclick = (ev) => { ev.stopPropagation(); openAddYoursComposer(story.promptId || null, story.promptText || story.text || ""); };
    ctaWrap.appendChild(addBtn);

    if (responseCount > 0) {
      const viewAll = document.createElement("button");
      viewAll.textContent = `👁 View all ${responseCount} response${responseCount > 1 ? 's' : ''}`;
      viewAll.className = "addyours-viewall-btn";
      viewAll.onclick = (ev) => { ev.stopPropagation(); openPromptResponsesModal(story.promptId || pid); };
      ctaWrap.appendChild(viewAll);
    }
    overlay.appendChild(ctaWrap);
    content.appendChild(overlay);

    musicBadge.style.display = "none";
    startStoryAnimation(8000);
  } else if (story.type === "question") {
    // "Ask a Question" sticker story — viewers tap to type an answer that is
    // sent to the asker as a chat reply (WhatsApp-style).
    const overlay = document.createElement("div");
    overlay.className = "prompt-story-overlay";
    overlay.style.cssText = "position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:36px;text-align:center;z-index:5;background:linear-gradient(135deg,#7c3aed,#5b21b6);";

    const tag = document.createElement("div");
    tag.style.cssText = "font-size:13px;color:rgba(255,255,255,0.8);font-weight:600;letter-spacing:0.5px;text-transform:uppercase;";
    tag.textContent = "❓ Ask a Question";
    overlay.appendChild(tag);

    const qEl = document.createElement("div");
    qEl.style.cssText = "font-size:26px;font-weight:800;color:#fff;line-height:1.35;word-break:break-word;text-shadow:0 2px 12px rgba(0,0,0,0.5);";
    qEl.innerHTML = linkify(story.questionText || story.text || "");
    overlay.appendChild(qEl);

    const ctaWrap = document.createElement("div");
    ctaWrap.style.cssText = "display:flex;flex-direction:column;gap:10px;align-items:center;margin-top:8px;";

    const answerBtn = document.createElement("button");
    answerBtn.textContent = "💬 Type your answer";
    answerBtn.className = "addyours-cta-btn";
    answerBtn.onclick = (ev) => { ev.stopPropagation(); answerStoryQuestion(story); };
    ctaWrap.appendChild(answerBtn);
    overlay.appendChild(ctaWrap);
    content.appendChild(overlay);

    musicBadge.style.display = "none";
    startStoryAnimation(8000);
  } else {
    musicBadge.style.display = "none";
    startStoryAnimation(5000);
  }

  // Interactive sticker overlay (WhatsApp-style): a small card riding on the
  // image/video/text. Tapping it brings up the SAME prompt for the viewer —
  // Add Yours → composer seeded with the same prompt (post your own photo);
  // Question → type an answer → chat reply to the asker.
  if (story.sticker && story.sticker.text) {
    const sk = story.sticker;
    const isQ = sk.type === "question";
    const skEl = document.createElement("div");
    skEl.className = "story-sticker-overlay";
    skEl.innerHTML = `
      <div class="ws-sticker-card ${isQ ? "ws-sticker-card--question" : "ws-sticker-card--addyours"}">
        <div class="ws-sticker-accent"></div>
        <div class="ws-sticker-content">
          <div class="ws-sticker-label">${isQ ? "❓ Ask a Question" : "➕ Add Yours"}</div>
          <div class="ws-sticker-text">${escapeHtml(sk.text)}</div>
        </div>
      </div>`;
    skEl.onclick = (ev) => {
      ev.stopPropagation();
      if (sk.type === "question") {
        answerStoryQuestion(story);
      } else {
        openAddYoursComposer(sk.id, sk.text);
      }
    };
    content.appendChild(skEl);
  } else if (story.promptId && story.type !== "prompt") {
    // Legacy: an "Add Yours" response story (has promptId, isn't a prompt).
    // Tapping reopens the composer seeded with the same prompt so the viewer
    // can post their own; "View all responses" browses the chain.
    const sticker = document.createElement("div");
    sticker.className = "addyours-sticker";
    sticker.innerHTML = `<div class="addyours-sticker-icon">➕</div>
      <div class="addyours-sticker-body">
        <div class="addyours-sticker-label">Add Yours</div>
        <div class="addyours-sticker-prompt">${escapeHtml(story.promptText || "")}</div>
        <div class="addyours-sticker-viewall" onclick="event.stopPropagation(); openPromptResponsesModal('${escapeHtml(story.promptId)}');">View all responses</div>
      </div>`;
    sticker.onclick = (ev) => { ev.stopPropagation(); openAddYoursComposer(story.promptId, story.promptText || story.text || ""); };
    content.appendChild(sticker);
  }
}

// Send a typed answer to an "Ask a Question" story as a chat reply to the asker.
async function answerStoryQuestion(story) {
  if (!currentUser || !story) return;
  if (story.uid === currentUser.uid) {
    showNotifToast("This is your own question", "info");
    return;
  }
  // Pause the running story while the viewer types an answer.
  if (typeof pauseStoryPlayback === "function") pauseStoryPlayback();
  // The question text lives on story.sticker.text (WhatsApp-style image sticker)
  // or, for legacy standalone question stories, on story.questionText/story.text.
  const sticker = story.sticker || null;
  const question = (sticker && sticker.type === "question" && sticker.text)
    || story.questionText || story.text || "";
  const qId = (sticker && sticker.id) || story.questionId || null;
  const answer = window.prompt("Answer: " + question, "");
  if (answer === null || !answer.trim()) return;
  const reply = answer.trim();
  try {
    await db.collection("chats").add({
      from: currentUser.uid,
      to: story.uid,
      text: "❓ " + question + "\n💬 " + reply,
      storyReplyId: story.id || null,
      storyReplyUid: story.uid,
      storyQuestionId: qId,
      storyQuestionText: question,
      storyQuestionAnswer: reply,
      storyUrl: story.url || null,
      storyText: story.text || null,
      storyType: story.type || "question",
      createdAt: Date.now(),
      read: false,
      status: "sent"
    });
    showNotifToast("✓ Answer sent", "success");
    const owner = (typeof allUsersData !== "undefined" ? allUsersData.find(u => u.uid === story.uid) : null);
    const myName = (document.getElementById("myName")?.textContent) || "Nexa User";
    if (typeof sendPushNotification === "function") {
      sendPushNotification("Answer from " + myName, "❓ " + question + " → " + reply, owner ? owner.oneSignalPlayerId : null);
    }
  } catch (err) {
    showNotifToast("Failed to send answer: " + err.message, "error");
  }
}

let isStoryPaused = false;
let storyPauseElapsed = 0;

function handleStoryContentClick(e) {
  if (e.target.closest('#storyViewerActions') || e.target.closest('#storyViewerHeader') || e.target.closest('#storyStatsBar') || e.target.closest('.story-sticker-overlay') || e.target.closest('.addyours-sticker') || e.target.closest('.prompt-story-overlay') || e.target.closest('.addyours-cta-btn') || e.target.closest('.addyours-viewall-btn')) return;
  toggleStoryPause();
}

function toggleStoryPause() {
  if (isStoryPaused) {
    resumeStoryPlayback();
  } else {
    pauseStoryPlayback();
  }
}

function pauseStoryPlayback() {
  isStoryPaused = true;
  cancelAnimationFrame(storyAnimFrame);
  clearTimeout(storyTimer);
  if (storyProgressStart) {
    storyPauseElapsed += Date.now() - storyProgressStart;
  }
  if (storyAudioElement && !storyAudioElement.paused) storyAudioElement.pause();
  const vid = document.querySelector('#storyViewerContent video');
  if (vid && !vid.paused) vid.pause();
  // No on-screen PAUSED indicator — it would block the sticker. The story
  // simply pauses on tap and resumes on the next tap.
}

function resumeStoryPlayback() {
  isStoryPaused = false;
  const indicator = document.getElementById('storyPauseIndicator');
  if (indicator) indicator.classList.remove('show');

  if (storyAudioElement && storyAudioElement.paused) storyAudioElement.play().catch(() => {});
  const vid = document.querySelector('#storyViewerContent video');
  if (vid && vid.paused) vid.play().catch(() => {});

  if (storyCurrentDuration > 0) {
    storyProgressStart = Date.now();
    const idx = storyViewerIndex;
    const fillEl = document.getElementById(`spf-${idx}`);

    function animFill() {
      if (isStoryPaused) return;
      const currentElapsed = storyPauseElapsed + (Date.now() - storyProgressStart);
      const pct = Math.min((currentElapsed / storyCurrentDuration) * 100, 100);
      if (fillEl) fillEl.style.width = pct + "%";

      if (pct < 100) {
        storyAnimFrame = requestAnimationFrame(animFill);
      } else {
        storyPauseElapsed = 0;
        storyViewerIndex++;
        renderStorySlide(storyViewerIndex);
      }
    }
    storyAnimFrame = requestAnimationFrame(animFill);
  }
}

function playStoryMusic(musicData) {
  if (!musicData || !musicData.previewUrl) return;

  if (storyAudioElement) {
    storyAudioElement.pause();
  }

  storyAudioElement = new Audio();
  storyAudioElement.src = musicData.previewUrl;
  storyAudioElement.volume = 1;
  storyAudioElement.play().catch(err => console.log("Music play error:", err));
}

function prevStorySlide() {
  if (storyViewerIndex > 0) {
    storyViewerIndex--;
    renderStorySlide(storyViewerIndex);
  }
}

function nextStorySlide() {
  storyViewerIndex++;
  renderStorySlide(storyViewerIndex);
}

function stopAllStoryMedia() {
  clearTimeout(storyTimer);
  if (window.storySlideTimer) {
    clearInterval(window.storySlideTimer);
    window.storySlideTimer = null;
  }
  cancelAnimationFrame(storyAnimFrame);

  if (storyAudioElement) {
    try {
      storyAudioElement.pause();
      storyAudioElement.removeAttribute("src");
      storyAudioElement.load();
    } catch(e) {}
    storyAudioElement = null;
  }

  const overlay = document.getElementById("storyViewerOverlay");
  if (overlay) {
    overlay.querySelectorAll("video, audio").forEach(media => {
      try {
        media.pause();
        media.removeAttribute("src");
        media.load();
      } catch(e) {}
    });
  }

  const content = document.getElementById("storyViewerContent");
  if (content) content.innerHTML = "";
}

function closeStoryViewer() {
  stopAllStoryMedia();
  const overlay = document.getElementById("storyViewerOverlay");
  if (overlay) overlay.classList.remove("active");
  const replyInput = document.getElementById("storyReplyInput");
  if (replyInput) replyInput.value = "";
  const musicBadge = document.getElementById("musicBadge");
  if (musicBadge) musicBadge.style.display = "none";
  const loadingEl = document.getElementById("storyLoading");
  if (loadingEl) loadingEl.classList.remove("show");
  storyViewerData = null;
  viewingStoryUid = null;
  isStoryPaused = false;
  storyPauseElapsed = 0;
}

// Auto-stop story when switching tab or leaving page
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "hidden") {
    closeStoryViewer();
    if (activeAudioPlayer) {
      try { activeAudioPlayer.pause(); } catch(e) {}
    }
  }
});

window.addEventListener("pagehide", closeStoryViewer);

window.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
    closeStoryViewer();
    closeViewOnceModal();
    closeProfilePicModal();
  }
});

function deleteStory() {
  if (!storyViewerData || storyViewerIndex >= storyViewerData.length) return;
  const story = storyViewerData[storyViewerIndex];
  if (confirm("Delete this story?")) {
    db.collection("status").doc(story.id).delete().then(() => {
      storyViewerData.splice(storyViewerIndex, 1);
      if (storyViewerIndex >= storyViewerData.length) storyViewerIndex--;
      if (storyViewerData.length > 0) {
        renderStorySlide(storyViewerIndex);
      } else {
        closeStoryViewer();
      }
      showNotifToast("✓ Story deleted", "success");
    });
  }
}

async function sendStoryReply() {
  if (!storyViewerData || !viewingStoryUid || viewingStoryUid === currentUser.uid) return;
  const text = document.getElementById("storyReplyInput").value.trim();
  if (!text) return;

  const currentStory = storyViewerData[storyViewerIndex] || {};

  try {
    await db.collection("chats").add({
      from: currentUser.uid,
      to: viewingStoryUid,
      text: text,
      storyReplyId: currentStory.id || null,
      storyReplyUid: viewingStoryUid,
      storyUrl: currentStory.url || null,
      storyText: currentStory.text || null,
      storyType: currentStory.type || "image",
      storyMusicTitle: currentStory.musicData ? currentStory.musicData.title : null,
      createdAt: Date.now(),
      read: false,
      status: "sent"
    });

    showNotifToast("✓ Reply sent", "success");
    const storyOwner = allUsersData.find(u => u.uid === viewingStoryUid);
    sendPushNotification(
      `Story reply from ${document.getElementById('myName').textContent || 'Nexa User'}`,
      text,
      storyOwner ? storyOwner.oneSignalPlayerId : null
    );
    document.getElementById("storyReplyInput").value = "";
  } catch (err) {
    showNotifToast("Failed to send reply", "error");
  }
}

async function reshareStory() {
  if (!storyViewerData || storyViewerIndex >= storyViewerData.length) return;
  const story = storyViewerData[storyViewerIndex];
  if (!currentUser) return;

  if (story.uid === currentUser.uid) {
    showNotifToast("You cannot reshare your own story", "info");
    return;
  }

  const storyOwner = allUsersData.find(u => u.uid === story.uid);
  const ownerName = storyOwner ? (storyOwner.displayName || "User") : (document.getElementById("storyUserName")?.textContent || "User");

  try {
    const resharedData = {
      uid: currentUser.uid,
      type: story.type || "image",
      url: story.url || null,
      text: story.text || null,
      musicData: story.musicData || null,
      // Carry the interactive sticker + prompt/question chain forward so a
      // reshared "Add Yours" / "Ask a Question" story keeps the same prompt
      // viewers can tap into (WhatsApp-style reshare of an Add Yours story).
      sticker: story.sticker || null,
      promptId: story.promptId || null,
      promptText: story.promptText || null,
      questionId: story.questionId || null,
      questionText: story.questionText || null,
      resharedFrom: {
        uid: story.uid,
        name: ownerName
      },
      createdAt: Date.now(),
      expireAt: firebase.firestore.Timestamp.fromMillis(Date.now() + 24 * 60 * 60 * 1000),
      views: [],
      likes: [],
      reshares: []
    };

    await db.collection("status").add(resharedData);
    await db.collection("status").doc(story.id).update({
      reshares: firebase.firestore.FieldValue.arrayUnion(currentUser.uid)
    });

    if (!story.reshares) story.reshares = [];
    story.reshares.push(currentUser.uid);

    showNotifToast("🔁 Reshared story to your updates!", "success");
    renderStorySlide(storyViewerIndex);
  } catch (err) {
    showNotifToast("Failed to reshare story: " + err.message, "error");
  }
}

async function reactToStory() {
  if (!storyViewerData || storyViewerIndex >= storyViewerData.length) return;
  const story = storyViewerData[storyViewerIndex];
  if (!currentUser) return;

  const likes = story.likes || [];
  const hasLiked = likes.includes(currentUser.uid);

  try {
    if (hasLiked) {
      await db.collection("status").doc(story.id).update({
        likes: firebase.firestore.FieldValue.arrayRemove(currentUser.uid)
      });
      story.likes = likes.filter(id => id !== currentUser.uid);
      showNotifToast("Unliked story", "info");
    } else {
      await db.collection("status").doc(story.id).update({
        likes: firebase.firestore.FieldValue.arrayUnion(currentUser.uid)
      });
      if (!story.likes) story.likes = [];
      story.likes.push(currentUser.uid);
      showNotifToast("❤ Liked story!", "success");
    }
    renderStorySlide(storyViewerIndex);
  } catch (err) {
    showNotifToast("Error reacting to story: " + err.message, "error");
  }
}

/* =========================================================================
   "ADD YOURS" — WhatsApp-style prompt sticker on stories.
   A prompt story (type:"prompt") carries a promptId + promptText. Viewers tap
   "Add Yours" to post their own response (image/video/text) linked to the same
   promptId, forming a reshare chain. "View all responses" browses the chain.
   ========================================================================= */
let addYoursFile = null;
let addYoursTab = "image";
let activeAddYoursPrompt = { promptId: null, promptText: "" };

// ── Story stickers (WhatsApp-style) ───────────────────────────────────
// A sticker is ATTACHED to a normal photo/video/text story (never posted on
// its own). pendingStorySticker = { type:"addYours"|"question", text, id }.
// On share, it is saved as statusData.sticker. Viewers who tap the sticker
// overlay get the same prompt (Add Yours → post own photo w/ same promptId;
// Question → type answer → chat reply to asker).
let pendingStorySticker = null;
// When the user taps a sticker (➕/❓) before picking a photo, we remember
// their choice here, open the image picker, and re-run attachStorySticker()
// once a photo is chosen so the sticker attaches smoothly (WhatsApp flow).
let pendingStickerChoice = null;

function updateStoryStickerPreview() {
  refreshStickerToolButton();
  renderStoryStickerPreview();
  renderStickerOnImage();
}

// Enable/disable the sticker toolbar button based on whether there's content
// (a photo/video file, or typed text) to attach the sticker to.
// NOTE: we never set the `disabled` HTML attribute — a disabled button
// swallows clicks silently (so tapping ➕ "Add Yours" did nothing at all,
// which is the bug the user hit). Instead we only toggle a visual class and
// let toggleStickerTray/attachStorySticker handle the no-media case by
// opening the image picker first (WhatsApp-style: pick a photo, then sticker).
function refreshStickerToolButton() {
  const btn = document.getElementById("stickerToolBtn");
  if (!btn) return;
  const hasMedia = (statusTab === "image" || statusTab === "video") && statusFile;
  const hasText = statusTab === "text" && (document.getElementById("statusTextInput")?.value.trim() || "");
  const hasMusic = statusTab === "music" && selectedMusicData;
  btn.classList.toggle("disabled", !(hasMedia || hasText || hasMusic));
}

function toggleStickerTray(ev) {
  ev.stopPropagation();
  const btn = document.getElementById("stickerToolBtn");
  const hasContent = btn && !btn.classList.contains("disabled");
  if (!hasContent) {
    // No media yet → start the WhatsApp flow: open the image picker so the
    // user can choose a photo first, THEN add the sticker. We don't toast a
    // generic "pick a photo first" because that reads as "the button's broken".
    showNotifToast("Pick a photo first — stickers attach to your photo", "info");
    pendingStickerChoice = null; // cleared; user must tap a sticker again after media
    switchStatusTab("image");
    return;
  }
  const tray = document.getElementById("stickerTray");
  if (!tray) return;
  tray.classList.toggle("open");
}

function closeStickerTray() {
  const tray = document.getElementById("stickerTray");
  if (tray) tray.classList.remove("open");
}

// Render the sticker ON the preview image (WhatsApp-style sticker card).
function renderStickerOnImage() {
  const host = document.getElementById("stickerOnImage");
  if (!host) return;
  if (!pendingStorySticker) { host.innerHTML = ""; host.classList.remove("has-sticker"); return; }
  const s = pendingStorySticker;
  const isQ = s.type === "question";
  host.classList.add("has-sticker");
  host.innerHTML = `
    <div class="ws-sticker-card ${isQ ? "ws-sticker-card--question" : "ws-sticker-card--addyours"}">
      <div class="ws-sticker-accent"></div>
      <div class="ws-sticker-content">
        <div class="ws-sticker-label">${isQ ? "❓ Ask a Question" : "➕ Add Yours"}</div>
        <div class="ws-sticker-text">${escapeHtml(s.text)}</div>
      </div>
      <button class="ws-sticker-remove" onclick="event.stopPropagation(); removeStorySticker()" title="Remove sticker"><i data-lucide="x" style="width: 13px; height: 13px;"></i></button>
    </div>`;
}

function renderStoryStickerPreview() {
  const el = document.getElementById("storyStickerPreview");
  if (!el) return;
  if (!pendingStorySticker) { el.innerHTML = ""; return; }
  const s = pendingStorySticker;
  const icon = s.type === "question" ? "❓" : "➕";
  const label = s.type === "question" ? "Ask a Question" : "Add Yours";
  el.innerHTML = `
    <div class="story-sticker-chip">
      <span class="story-sticker-chip-icon">${icon}</span>
      <span class="story-sticker-chip-label">${label}</span>
      <span class="story-sticker-chip-text">${escapeHtml(s.text)}</span>
      <button class="story-sticker-chip-remove" onclick="removeStorySticker()" title="Remove"><i data-lucide="x" style="width: 11px; height: 11px;"></i></button>
    </div>`;
}

function attachStorySticker(type) {
  if (!currentUser) { showNotifToast("Please login first", "error"); return; }
  // Stickers ride on photo/video/text/music stories — require content first.
  const hasMedia = (statusTab === "image" || statusTab === "video") && statusFile;
  const hasText = statusTab === "text" && (document.getElementById("statusTextInput").value.trim() || "");
  const hasMusic = statusTab === "music" && selectedMusicData;
  if (!hasMedia && !hasText && !hasMusic) {
    // WhatsApp flow: no media yet → remember which sticker the user wants,
    // open the image picker, and once a photo is chosen we re-invoke this to
    // prompt for the sticker text and attach it. This is the fix for "the
    // Add Yours plus button does nothing" — it now opens the picker.
    pendingStickerChoice = type;
    showNotifToast("Pick a photo first, then add your sticker", "info");
    switchStatusTab("image");
    closeStickerTray();
    return;
  }
  const text = window.prompt(type === "question" ? "Ask a question:" : "Add Yours prompt:", "");
  if (text === null) { pendingStickerChoice = null; return; }
  const t = text.trim();
  if (!t) { showNotifToast("Please type something for the sticker", "error"); return; }
  const id = (type === "question" ? "q_" : "prompt_") + Date.now() + "_" + Math.random().toString(36).substr(2, 8);
  pendingStorySticker = { type, text: t, id };
  pendingStickerChoice = null;
  closeStickerTray();
  renderStickerOnImage();
  renderStoryStickerPreview();
}

function removeStorySticker() {
  pendingStorySticker = null;
  renderStickerOnImage();
  renderStoryStickerPreview();
}

function openAddYoursComposer(promptId, promptText) {
  activeAddYoursPrompt = { promptId: promptId || ("prompt_" + Date.now()), promptText: promptText || "" };
  addYoursFile = null;
  addYoursTab = "image";
  const modal = document.getElementById("addYoursModalOverlay");
  if (!modal) return;
  // Pause the running story so it doesn't auto-advance behind the composer.
  if (typeof pauseStoryPlayback === "function") pauseStoryPlayback();
  document.getElementById("addYoursPromptDisplay").textContent = promptText || "Add Yours";
  document.getElementById("addYoursCaption").value = "";
  document.getElementById("addYoursPreview").innerHTML = "<span>Tap to add a photo</span>";
  document.getElementById("addYoursImgInput").value = "";
  document.getElementById("addYoursVidInput").value = "";
  document.getElementById("addYoursTypeTextBtn").classList.remove("active");
  document.getElementById("addYoursTypeImgBtn").classList.add("active");
  document.getElementById("addYoursTypeVidBtn").classList.remove("active");
  document.getElementById("addYoursCaption").style.display = "block";
  document.getElementById("addYoursCaption").placeholder = "Add a caption (optional)…";
  modal.classList.add("active");
}

function closeAddYoursModal() {
  document.getElementById("addYoursModalOverlay").classList.remove("active");
  addYoursFile = null;
}

function handleAddYoursOverlayClick(e) {
  if (e.target === document.getElementById("addYoursModalOverlay")) closeAddYoursModal();
}

function setAddYoursType(tab) {
  addYoursTab = tab;
  ["Img", "Vid", "Text"].forEach(t => {
    document.getElementById("addYoursType" + t + "Btn").classList.remove("active");
  });
  document.getElementById("addYoursType" + (tab === "image" ? "Img" : tab === "video" ? "Vid" : "Text") + "Btn").classList.add("active");
  const caption = document.getElementById("addYoursCaption");
  const preview = document.getElementById("addYoursPreview");
  addYoursFile = null;
  if (tab === "image") {
    caption.style.display = "block";
    caption.placeholder = "Add a caption (optional)…";
    preview.innerHTML = "<span>Tap to add a photo</span>";
    document.getElementById("addYoursImgInput").click();
  } else if (tab === "video") {
    caption.style.display = "block";
    caption.placeholder = "Add a caption (optional)…";
    preview.innerHTML = "<span>Tap to add a video</span>";
    document.getElementById("addYoursVidInput").click();
  } else {
    caption.style.display = "block";
    caption.placeholder = "Type your response…";
    preview.innerHTML = "<span>Text response</span>";
    caption.focus();
  }
}

function previewAddYoursImage(e) {
  const file = e.target.files[0];
  if (!file) return;
  addYoursFile = file;
  addYoursTab = "image";
  const reader = new FileReader();
  reader.onload = (ev) => {
    document.getElementById("addYoursPreview").innerHTML = `<img src="${ev.target.result}" style="width:100%;height:100%;object-fit:cover;">`;
  };
  reader.readAsDataURL(file);
}

function previewAddYoursVideo(e) {
  const file = e.target.files[0];
  if (!file) return;
  addYoursFile = file;
  addYoursTab = "video";
  const reader = new FileReader();
  reader.onload = (ev) => {
    document.getElementById("addYoursPreview").innerHTML = `<video src="${ev.target.result}" style="width:100%;height:100%;object-fit:cover;" controls></video>`;
  };
  reader.readAsDataURL(file);
}

async function shareAddYoursResponse() {
  if (!currentUser) { showNotifToast("Please login first", "error"); return; }
  const caption = document.getElementById("addYoursCaption").value.trim();
  if (addYoursTab === "text" && !caption) {
    showNotifToast("Please type your response", "error");
    return;
  }
  if ((addYoursTab === "image" || addYoursTab === "video") && !addYoursFile) {
    showNotifToast("Please select a file", "error");
    return;
  }
  const btn = document.getElementById("addYoursShareBtn");
  btn.disabled = true;
  btn.textContent = "Sharing...";
  try {
    let statusData = {
      uid: currentUser.uid,
      type: addYoursTab,
      createdAt: Date.now(),
      expireAt: firebase.firestore.Timestamp.fromMillis(Date.now() + 24 * 60 * 60 * 1000),
      views: [],
      likes: [],
      reshares: [],
      promptId: activeAddYoursPrompt.promptId,
      promptText: activeAddYoursPrompt.promptText
    };
    if (caption) statusData.text = caption;

    if (addYoursTab === "image" && addYoursFile) {
      statusData.url = await uploadFile(addYoursFile);
    } else if (addYoursTab === "video" && addYoursFile) {
      statusData.url = await uploadFile(addYoursFile);
    }
    await db.collection("status").add(statusData);
    closeAddYoursModal();
    showNotifToast("✓ Added your response!", "success");
    // Refresh the current story slide to update the response count.
    if (typeof renderStorySlide === "function" && storyViewerData) renderStorySlide(storyViewerIndex);
  } catch (err) {
    showNotifToast("Failed to share: " + err.message, "error");
  } finally {
    btn.disabled = false;
    btn.textContent = "Share";
  }
}

function openPromptResponsesModal(promptId) {
  const modal = document.getElementById("promptResponsesOverlay");
  if (!modal) return;
  // Pause the running story while browsing responses.
  if (typeof pauseStoryPlayback === "function") pauseStoryPlayback();
  const list = document.getElementById("promptResponsesList");
  const titleEl = document.getElementById("promptResponsesTitle");
  list.innerHTML = '<div style="color:var(--text-3);font-size:13px;padding:20px;text-align:center;">Loading responses…</div>';

  // Gather all stories (prompt + responses) matching this promptId.
  const all = [];
  Object.entries(userStatuses).forEach(([uid, arr]) => {
    (arr || []).forEach(s => {
      if (s.promptId === promptId) all.push(s);
    });
  });
  all.sort((a, b) => a.createdAt - b.createdAt);

  const promptStory = all.find(s => s.type === "prompt");
  const responses = all.filter(s => s.type !== "prompt");
  if (promptStory) {
    titleEl.textContent = promptStory.promptText || promptStory.text || "Add Yours";
  } else if (responses.length) {
    titleEl.textContent = responses[0].promptText || "Add Yours";
  } else {
    titleEl.textContent = "Add Yours";
  }

  if (responses.length === 0) {
    list.innerHTML = '<div style="color:var(--text-3);font-size:13px;padding:30px 20px;text-align:center;">No responses yet. Be the first to add yours!</div>';
  } else {
    list.innerHTML = "";
    responses.forEach(s => {
      const user = allUsersData.find(u => u.uid === s.uid);
      const photo = user?.photo || "https://i.imgur.com/HeIi0wU.png";
      const name = user?.displayName || "User";
      const card = document.createElement("div");
      card.className = "prompt-response-card";
      let mediaHtml = "";
      if (s.type === "image" && s.url) {
        mediaHtml = `<img src="${safeMediaUrl(s.url)}" class="prompt-response-media" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">`;
      } else if (s.type === "video" && s.url) {
        mediaHtml = `<video src="${safeMediaUrl(s.url)}" class="prompt-response-media" controls></video>`;
      } else {
        mediaHtml = `<div class="prompt-response-text">${linkify(s.text || "")}</div>`;
      }
      card.innerHTML = `
        <div class="prompt-response-head">
          <img src="${photo}" class="prompt-response-av" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
          <div>
            <div class="prompt-response-name">${escapeHtml(name)}</div>
            <div class="prompt-response-time">${formatTime(s.createdAt)}</div>
          </div>
        </div>
        ${mediaHtml}
        ${s.text && s.type !== "text" ? `<div class="prompt-response-caption">${linkify(s.text)}</div>` : ""}
        <button class="prompt-response-viewbtn" onclick="closePromptResponsesModal(); openStoryViewer('${s.uid}', '${s.id}')">View story</button>
      `;
      list.appendChild(card);
    });
  }
  modal.classList.add("active");
}

function closePromptResponsesModal() {
  document.getElementById("promptResponsesOverlay").classList.remove("active");
}

function handlePromptResponsesOverlayClick(e) {
  if (e.target === document.getElementById("promptResponsesOverlay")) closePromptResponsesModal();
}

/* =========================================================================
   STORY ANALYTICS MODAL (Viewers, Likes, Reshares)
   ========================================================================= */
let currentAnalyticsTab = 'views';

function openStoryAnalyticsModal() {
  if (!storyViewerData || storyViewerIndex >= storyViewerData.length) return;
  const story = storyViewerData[storyViewerIndex];
  if (!currentUser || story.uid !== currentUser.uid) return;

  cancelAnimationFrame(storyAnimFrame);
  clearTimeout(storyTimer);

  const views = story.views || [];
  const likes = story.likes || [];
  const reshares = story.reshares || [];

  document.getElementById("cntViews").textContent = views.length;
  document.getElementById("cntLikes").textContent = likes.length;
  document.getElementById("cntReshares").textContent = reshares.length;

  document.getElementById("storyAnalyticsModal").classList.add("active");
  switchAnalyticsTab('views');
}

function closeStoryAnalyticsModal() {
  document.getElementById("storyAnalyticsModal").classList.remove("active");
  if (storyViewerData) {
    renderStorySlide(storyViewerIndex);
  }
}

function handleAnalyticsOverlayClick(e) {
  if (e.target === document.getElementById("storyAnalyticsModal")) {
    closeStoryAnalyticsModal();
  }
}

function switchAnalyticsTab(tab) {
  currentAnalyticsTab = tab;
  document.querySelectorAll(".analytics-tab").forEach(b => b.classList.remove("active"));
  const btn = document.getElementById(`aTab${tab.charAt(0).toUpperCase() + tab.slice(1)}`);
  if (btn) btn.classList.add("active");

  const list = document.getElementById("analyticsList");
  if (!list || !storyViewerData) return;
  list.innerHTML = "";

  const story = storyViewerData[storyViewerIndex];
  const uids = story[tab] || [];

  if (!uids.length) {
    list.innerHTML = `<div class="no-analytics">No ${tab} yet</div>`;
    return;
  }

  uids.forEach(uid => {
    let u = allUsersData.find(usr => usr.uid === uid);
    if (uid === currentUser.uid) {
      u = {
        displayName: (document.getElementById("myName")?.textContent || "You") + " (You)",
        photo: document.getElementById("myPic")?.src || "https://i.imgur.com/HeIi0wU.png"
      };
    }

    const name = escapeHtml(u ? (u.displayName || "User") : "User");
    const photo = u?.photo || "https://i.imgur.com/HeIi0wU.png";

    const item = document.createElement("div");
    item.className = "analytics-user-item";
    item.innerHTML = `
      <img src="${photo}" class="analytics-user-av" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
      <div class="analytics-user-name">${name}</div>
    `;
    list.appendChild(item);
  });
}

function openStatusModal() {
  statusTab = 'image';
  statusFile = null;
  document.getElementById("statusImageInput").value = "";
  document.getElementById("statusVideoInput").value = "";
  document.getElementById("statusTextInput").value = "";
  document.getElementById("statusPreview").innerHTML = "<span>Preview</span>";
  document.getElementById("musicResults").innerHTML = "";
  document.getElementById("musicSelectedBox").innerHTML = "";
  selectedMusicData = null;
  musicDuration = 30;
  document.getElementById("musicSearchInput").value = "";

  pendingStorySticker = null;
  renderStoryStickerPreview();
  renderStickerOnImage();
  closeStickerTray();

  switchStatusTab('image');
  document.getElementById("statusModalOverlay").classList.add("active");
}

function closeStatusModal() {
  document.getElementById("statusModalOverlay").classList.remove("active");
  statusFile = null;
  selectedMusicData = null;
  pendingStorySticker = null;
  renderStoryStickerPreview();
  renderStickerOnImage();
  closeStickerTray();
}

function handleStatusOverlayClick(e) {
  if (e.target === document.getElementById("statusModalOverlay")) {
    closeStatusModal();
  } else if (!e.target.closest("#stickerTray") && !e.target.closest("#stickerToolBtn")) {
    closeStickerTray();
  }
}

function switchStatusTab(tab) {
  statusTab = tab;
  closeStickerTray();
  document.querySelectorAll(".status-tab").forEach(el => el.classList.remove("active"));
  const tabBtn = document.getElementById("tab" + tab.charAt(0).toUpperCase() + tab.slice(1));
  if (tabBtn) tabBtn.classList.add("active");

  const textInput = document.getElementById("statusTextInput");

  if (tab === "image") {
    textInput.style.display = "block";
    textInput.placeholder = "Add caption (optional)…";
    document.getElementById("statusImageInput").click();
  } else if (tab === "video") {
    textInput.style.display = "block";
    textInput.placeholder = "Add caption (optional)…";
    document.getElementById("statusVideoInput").click();
  } else if (tab === "text") {
    textInput.style.display = "block";
    textInput.placeholder = "Add text (required)…";
    textInput.focus();
  } else if (tab === "music") {
    textInput.style.display = "block";
    textInput.placeholder = "Add caption (optional)…";
    document.getElementById("musicSearchInput").focus();
  }
  refreshStickerToolButton();
  renderStickerOnImage();
}

function previewStatusImage(e) {
  const file = e.target.files[0];
  if (!file) { pendingStickerChoice = null; return; }
  statusFile = file;
  statusTab = "image";
  const reader = new FileReader();
  reader.onload = (ev) => {
    document.getElementById("statusPreview").innerHTML = `<img src="${ev.target.result}" style="width: 100%; height: 100%; object-fit: cover;">`;
    refreshStickerToolButton();
    renderStickerOnImage();
    // If the user tapped a sticker before picking a photo (WhatsApp flow),
    // now that we have media, prompt for the sticker text and attach it.
    if (pendingStickerChoice) { const c = pendingStickerChoice; pendingStickerChoice = null; attachStorySticker(c); }
  };
  reader.readAsDataURL(file);
}

function previewStatusVideo(e) {
  const file = e.target.files[0];
  if (!file) { pendingStickerChoice = null; return; }
  statusFile = file;
  statusTab = "video";
  const reader = new FileReader();
  reader.onload = (ev) => {
    document.getElementById("statusPreview").innerHTML = `<video src="${ev.target.result}" style="width: 100%; height: 100%; object-fit: cover;" controls></video>`;
    refreshStickerToolButton();
    renderStickerOnImage();
    if (pendingStickerChoice) { const c = pendingStickerChoice; pendingStickerChoice = null; attachStorySticker(c); }
  };
  reader.readAsDataURL(file);
}

async function searchTrendingMusic() {
  const query = document.getElementById("musicSearchInput").value.trim();
  if (!query) return;

  const btn = document.getElementById("musicSearchBtn");
  const resultsDiv = document.getElementById("musicResults");

  btn.disabled = true;
  btn.textContent = "Searching...";
  resultsDiv.innerHTML = "";
  resultsDiv.style.display = "flex";

  try {
    const response = await fetch(
      `https://itunes.apple.com/search?term=${encodeURIComponent(query)}&media=music&limit=8`
    );
    const data = await response.json();

    if (!data.results || data.results.length === 0) {
      resultsDiv.innerHTML = '<div style="text-align: center; color: var(--text-3); padding: 20px; width: 100%; font-size: 12px;">No songs found. Try another search.</div>';
      btn.disabled = false;
      btn.textContent = "Search";
      return;
    }

    resultsDiv.innerHTML = "";
    data.results.slice(0, 8).forEach(track => {
      const el = document.createElement("div");
      el.className = "music-result";
      const img = track.artworkUrl100 || track.artworkUrl60 || "";
      const title = (track.trackName || "Unknown").substring(0, 40);
      const artist = (track.artistName || "Unknown").substring(0, 30);
      const previewUrl = track.previewUrl || "";

      el.innerHTML = `
        <img src="${safeMediaUrl(img)}" class="music-result-img" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
        <div class="music-result-info">
          <div class="music-result-title">${escapeHtml(title)}</div>
          <div class="music-result-artist">${escapeHtml(artist)}</div>
        </div>
      `;

      el.addEventListener("click", () => {
        selectMusic({
          title: title,
          artist: artist,
          albumArt: img,
          previewUrl: previewUrl,
          duration: 30
        });
      });

      resultsDiv.appendChild(el);
    });

  } catch (err) {
    resultsDiv.innerHTML = '<div style="text-align: center; color: var(--danger); padding: 20px; width: 100%; font-size: 12px;">Search failed. Try again.</div>';
  }

  btn.disabled = false;
  btn.textContent = "Search";
}

function selectMusic(musicData) {
  selectedMusicData = musicData;
  musicDuration = musicData.duration || 30;

  const resultsDiv = document.getElementById("musicResults");
  resultsDiv.innerHTML = "";
  resultsDiv.style.display = "none";

  const selectedBox = document.getElementById("musicSelectedBox");
  selectedBox.innerHTML = `
    <div class="music-selected">
      <div class="music-selected-info">
        <img src="${safeMediaUrl(musicData.albumArt)}" class="music-selected-img" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
        <div class="music-selected-meta">
          <div class="music-selected-title">${escapeHtml(musicData.title)}</div>
          <div class="music-selected-artist">${escapeHtml(musicData.artist)}</div>
        </div>
      </div>
      <div class="music-duration-slider">
        <div class="duration-label">Duration: <strong id="musicDurationLabel">${musicDuration}s</strong></div>
        <div class="slider-control">
          <input type="range" min="5" max="60" value="${musicDuration}" oninput="updateMusicDuration(this.value)">
          <div class="duration-display">${musicDuration}s</div>
        </div>
        <div class="duration-hint">🎵 Music will play in background</div>
      </div>
      <button class="music-remove-btn" onclick="removeMusicSelection()" title="Remove music"><i data-lucide="x" style="width: 14px; height: 14px;"></i></button>
    </div>
  `;
  selectedBox.style.display = "block";
}

function updateMusicDuration(value) {
  musicDuration = parseInt(value);
  document.getElementById("musicDurationLabel").textContent = musicDuration + "s";
  document.querySelector(".duration-display").textContent = musicDuration + "s";
  if (selectedMusicData) {
    selectedMusicData.duration = musicDuration;
  }
}

function removeMusicSelection() {
  selectedMusicData = null;
  musicDuration = 30;
  document.getElementById("musicSelectedBox").innerHTML = "";
  document.getElementById("musicSelectedBox").style.display = "none";
  document.getElementById("musicSearchInput").value = "";
  document.getElementById("musicResults").innerHTML = "";
  document.getElementById("musicResults").style.display = "none";
  document.getElementById("musicSearchInput").focus();
}

async function shareStatus() {
  if (!currentUser) {
    showNotifToast("Please login first", "error");
    return;
  }

  const shareBtn = document.getElementById("shareStoryBtn");
  shareBtn.disabled = true;
  shareBtn.textContent = "Sharing...";

  // Close the composer right away so it feels instant — the story posts in the
  // background. (Uploading media still takes a moment on slow networks, but the
  // user isn't stuck watching the button.)
  const fileToUpload = statusFile;
  const textVal = document.getElementById("statusTextInput").value.trim();

  try {
    let statusData = {
      uid: currentUser.uid,
      type: statusTab,
      createdAt: Date.now(),
      expireAt: firebase.firestore.Timestamp.fromMillis(Date.now() + 24 * 60 * 60 * 1000),
      views: [],
      likes: [],
      reshares: []
    };

    if (statusTab === "image" && statusFile) {
      showNotifToast("📤 Uploading photo...", "info");
      const url = await uploadFile(fileToUpload);
      statusData.url = url;
      if (textVal) statusData.text = textVal;
      if (selectedMusicData) {
        statusData.musicData = {
          title: selectedMusicData.title,
          artist: selectedMusicData.artist,
          albumArt: selectedMusicData.albumArt,
          previewUrl: selectedMusicData.previewUrl,
          duration: selectedMusicData.duration || 30
        };
      }
    } else if (statusTab === "video" && statusFile) {
      showNotifToast("📤 Uploading video...", "info");
      const url = await uploadFile(fileToUpload);
      statusData.url = url;
      if (textVal) statusData.text = textVal;
      if (selectedMusicData) {
        statusData.musicData = {
          title: selectedMusicData.title,
          artist: selectedMusicData.artist,
          albumArt: selectedMusicData.albumArt,
          previewUrl: selectedMusicData.previewUrl,
          duration: selectedMusicData.duration || 30
        };
      }
    } else if (statusTab === "text") {
      if (!textVal) {
        showNotifToast("Please enter some text", "error");
        shareBtn.disabled = false;
        shareBtn.textContent = "Share";
        return;
      }
      statusData.text = textVal;
      if (selectedMusicData) {
        statusData.musicData = {
          title: selectedMusicData.title,
          artist: selectedMusicData.artist,
          albumArt: selectedMusicData.albumArt,
          previewUrl: selectedMusicData.previewUrl,
          duration: selectedMusicData.duration || 30
        };
      }
    } else if (statusTab === "music") {
      if (!selectedMusicData) {
        showNotifToast("Please select a song", "error");
        shareBtn.disabled = false;
        shareBtn.textContent = "Share";
        return;
      }
      statusData.type = "music";
      statusData.musicData = {
        title: selectedMusicData.title,
        artist: selectedMusicData.artist,
        albumArt: selectedMusicData.albumArt,
        previewUrl: selectedMusicData.previewUrl,
        duration: selectedMusicData.duration || 30
      };
      if (textVal) statusData.text = textVal;
    }

    // Attach an interactive sticker (WhatsApp-style) onto the photo/video/text
    // story. The sticker rides on the image; viewers who tap it get the same
    // prompt. addYours sticker doubles as a promptId so response chains work.
    if (pendingStorySticker && pendingStorySticker.text) {
      const s = pendingStorySticker;
      statusData.sticker = { type: s.type, text: s.text, id: s.id };
      if (s.type === "addYours") {
        statusData.promptId = s.id;
        statusData.promptText = s.text;
      } else if (s.type === "question") {
        statusData.questionId = s.id;
        statusData.questionText = s.text;
      }
    }

    await db.collection("status").add(statusData);
    closeStatusModal();
    showNotifToast("✓ Story shared!", "success");

  } catch (err) {
    showNotifToast("Failed to share: " + err.message, "error");
  }

  shareBtn.disabled = false;
  shareBtn.textContent = "Share";
}

function openAI() {
  openAskifyComingSoon();
}

function closeAI() {
  const panel = document.getElementById("aiPanel");
  if (panel) {
    panel.classList.remove("active");
  }
}

function openCommunityComingSoon() {
  const modal = document.getElementById("communityComingModal");
  if (modal) {
    modal.classList.add("active");
    return;
  }
  showNotifToast("Community is coming soon!", "info");
}

function closeCommunityComing() {
  const modal = document.getElementById("communityComingModal");
  if (modal) {
    modal.classList.remove("active");
  }
}

function handleCommunityComingClick(e) {
  if (e.target === document.getElementById("communityComingModal")) {
    closeCommunityComing();
  }
}

function openAskifyComingSoon() {
  const modal = document.getElementById("askifyComingModal");
  if (modal) {
    modal.classList.add("active");
    return;
  }
  showNotifToast("Askify AI is coming soon!", "info");
}

function closeAskifyComing() {
  const modal = document.getElementById("askifyComingModal");
  if (modal) {
    modal.classList.remove("active");
  }
}

function handleAskifyComingClick(e) {
  if (e.target === document.getElementById("askifyComingModal")) {
    closeAskifyComing();
  }
}


/* =========================================================================
   FORMATTING HELPERS
   ========================================================================= */
function fmtDateSep(ts) {
  const d = new Date(ts);
  const today = new Date();
  const yesterday = new Date(today);
  yesterday.setDate(yesterday.getDate() - 1);

  if (d.toDateString() === today.toDateString()) return "Today";
  if (d.toDateString() === yesterday.toDateString()) return "Yesterday";

  const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
  return `${months[d.getMonth()]} ${d.getDate()}, ${d.getFullYear()}`;
}

function formatTime(ts) {
  const d = new Date(ts);
  const now = new Date();
  const today = d.toDateString() === now.toDateString();

  const h = d.getHours().toString().padStart(2, "0");
  const m = d.getMinutes().toString().padStart(2, "0");

  if (today) return `${h}:${m}`;

  const yesterday = new Date(now);
  yesterday.setDate(yesterday.getDate() - 1);
  if (d.toDateString() === yesterday.toDateString()) return "Yesterday";

  const days = Math.floor((now - d) / (24 * 60 * 60 * 1000));

  if (days < 7) return `${days}d ago`;

  const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
  return `${months[d.getMonth()]} ${d.getDate()}`;
}

function savePrefs() {
  const prefs = {
    theme: currentTheme,
    fontFamily: fontFamily,
    muted: mutedChats,
    favorited: favoritedChats,
    blocked: blockedUsers,
    globalWallpaper: globalWallpaper,
    perChatWallpapers: perChatWallpapers
  };
  localStorage.setItem("nexaPrefs", JSON.stringify(prefs));
}

function loadPrefs() {
  try {
    const prefs = JSON.parse(localStorage.getItem("nexaPrefs") || "{}");
    currentTheme = prefs.theme || 'light';
    // Fallback: if a saved theme was removed from the picker, reset to the
    // default so the user isn't left with no theme applied.
    if (!THEMES[currentTheme]) currentTheme = 'light';
    fontFamily = prefs.fontFamily || 'plus-jakarta';
    if (!FONT_FAMILIES.some(x => x.key === fontFamily)) fontFamily = 'plus-jakarta';
    applyFontFamily(fontFamily);
    mutedChats = prefs.muted || {};
    favoritedChats = prefs.favorited || {};
    blockedUsers = prefs.blocked || {};
    globalWallpaper = prefs.globalWallpaper || null;
    perChatWallpapers = prefs.perChatWallpapers || {};
    setTheme(currentTheme);
    applyCurrentWallpaper();
  } catch (e) {}
}

document.addEventListener("DOMContentLoaded", () => {
  if (window.lucide) lucide.createIcons();
});

/* =========================================================================
   INSTANT OPEN (WhatsApp-style): restore the last-known UI from localStorage
   BEFORE Firebase auth resolves, so a returning user sees their chat list,
   last-message previews, unread badges and stories instantly — exactly as
   they left them. This runs on DOMContentLoaded (after every function above
   is defined, but still before the async onAuthStateChanged callback fires),
   so the users list paints immediately instead of showing "Loading users…".
   Live Firestore listeners then blend in any updates a moment later. It is a
   pure cache rehydrate — every piece is re-validated and overwritten by the
   live listeners once they connect.
   ========================================================================= */
function restoreInstantState() {
  try {
    // Only paint the cached UI for a device that still believes it's signed in.
    // If the user signed out, the flag is cleared and we skip the restore so no
    // stale chats flash before the auth-gate bounces to login.html.
    if (localStorage.getItem("nexa_signed_in") !== "1") return;
    const uid = localStorage.getItem("nexa_last_uid");
    if (!uid) return; // never signed in here — nothing cached to restore

    // Own profile header (avatar + name) so the shell looks complete.
    try {
      const prof = JSON.parse(localStorage.getItem("nexa_my_profile_" + uid) || "null");
      if (prof) {
        if (prof.name) { const n = document.getElementById("myName"); if (n) n.textContent = prof.name; }
        const pic = safeMediaUrl(prof.photo) || "";
        if (pic) {
          const mp = document.getElementById("myPic"); if (mp) mp.src = pic;
          const st = document.getElementById("storyTabMyPic"); if (st) st.src = pic;
        }
      }
    } catch (e) {}

    // Chat ordering + last-message previews (drives list sort + subtitles).
    loadLatestMsgState();

    // Muted / favorited / blocked maps + theme (pure localStorage read).
    loadPrefs();

    // Locally-deleted (cleared) chats so the list matches what they left.
    try {
      const dc = localStorage.getItem("nexa_deleted_chats_" + uid);
      if (dc) deletedChats = JSON.parse(dc);
    } catch (e) {}

    // The user list itself (names, avatars) — renders instantly.
    try {
      const saved = localStorage.getItem("nexa_cached_users_" + uid);
      if (saved) {
        allUsersData = JSON.parse(saved);
        allUsersData.forEach(u => {
          u.photo = safeMediaUrl(u.photo) || "https://i.imgur.com/HeIi0wU.png";
        });
        window.allUsersData = allUsersData;
        _refreshMyProfile();
      }
    } catch (e) {}

    // Unread badges from the last session.
    try {
      const un = JSON.parse(localStorage.getItem("nexa_unread_" + uid) || "null");
      if (un && typeof un === "object") unreadMessages = un;
    } catch (e) {}

    // Stories bar from the last session (dropped if aged past 24h).
    loadCachedStories();

    // Paint the whole thing now — no waiting for the network.
    renderUsers();
    renderActiveNowBar();
    renderStoriesBar();
    updateTotalUnreadBadge();
    console.log("⚡ Instant state restored from cache");
  } catch (e) {
    console.warn("Instant restore failed (non-fatal):", e);
  }
}
document.addEventListener("DOMContentLoaded", restoreInstantState);

console.log("✨ All initialization complete! Nexa Messenger is ready.");
function openProfilePic(src, returnTo) {
  const modal = document.getElementById('profilePicModal');
  const img = document.getElementById('modalProfileImg');
  if (!modal || !img) return;
  img.src = src || "https://i.imgur.com/HeIi0wU.png";
  // Track where to return to ('profile' = the contact profile modal).
  modal.dataset.returnTo = returnTo || "";
  modal.classList.add('active');
}

function closeProfilePicModal(event) {
  // Close via backdrop click OR explicit call (event may be undefined).
  if (event && event.target && event.target.id !== 'profilePicModal') return;
  const modal = document.getElementById('profilePicModal');
  if (modal) modal.classList.remove('active');
}

// Back button on the full-profile-pic viewer: returns to the contact profile
// modal (or the Chat Info panel) when opened from there, otherwise just closes.
function profilePicGoBack() {
  const modal = document.getElementById('profilePicModal');
  if (!modal) return;
  const returnTo = modal.dataset.returnTo || "";
  modal.classList.remove('active');
  if (returnTo === 'profile') {
    const profileModal = document.getElementById('userProfileModal');
    if (profileModal) profileModal.classList.add('active');
  } else if (returnTo === 'chatinfo') {
    const infoPanel = document.getElementById('infoPanel');
    if (infoPanel) infoPanel.classList.add('active');
  }
}

// ═══════════════════════════════════════════════════════════════════════════
// NEXA COMMUNITY MODULE: GROUPS & CHANNELS
// ═══════════════════════════════════════════════════════════════════════════

let currentChatMode = 'direct'; // 'direct' | 'group' | 'channel'
let selectedGroup = null;
let selectedChannel = null;
let myGroups = [];
let currentGroupMessages = [];
let myChannels = [];
let discoverChannels = [];
let unsubMyGroups = null;
let unsubGroupMessages = null;
let unsubGroupDoc = null;
let unsubChannelPosts = null;
let unsubChannelComments = null;
let unsubChannelDoc = null;
let activeCommunitySubTab = 'groups';
let activeChannelSubPill = 'following';
let pendingGroupAvatar = null;
let pendingChannelAvatar = null;
let selectedGroupMemberUids = new Set();
let activeCommentPostId = null;
let currentDiscoverCategory = 'All';

function formatMessageText(text) {
  return linkify(text);
}

// ── Community tab activity glow ──────────────────────────────────────────────
// Shows a green pulsing dot on the Community tab (desktop + mobile) when any
// group/channel the user follows has unseen activity (new message/post. The dot
// clears when the user opens the Community tab or opens the group/channel itself).
function communitySeenKey() {
  return `nexa_comm_last_viewed_${currentUser?.uid || 'anon'}`;
}

function getCommunitySeen() {
  try { return JSON.parse(localStorage.getItem(communitySeenKey()) || '{}'); } catch (e) { return {}; }
}

function markCommunitySeen(id) {
  const seen = getCommunitySeen();
  seen[id] = Date.now();
  try { localStorage.setItem(communitySeenKey(), JSON.stringify(seen)); } catch (e) {}
}

function hasCommunityActivity() {
  if (!currentUser) return false;
  const seenMap = getCommunitySeen();
  const now = Date.now();
  const recentCutoff = now - 10 * 24 * 3600 * 1000; // ignore ancient timestamps

  for (const g of myGroups) {
    const last = g.lastMessageTime || 0;
    if (last && last > (seenMap['g_' + g.id] || 0) && last > recentCutoff) return true;
  }
  for (const c of myChannels) {
    const last = c.lastPostTime || 0;
    if (last && last > (seenMap['c_' + c.id] || 0) && last > recentCutoff) return true;
  }
  return false;
}

function updateCommunityTabDot() {
  const show = hasCommunityActivity();
  ['commTabDotDesktop', 'commTabDotMobile'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.style.display = show ? 'block' : 'none';
  });
}

function clearCommunitySeen() {
  const now = Date.now();
  for (const g of myGroups) markCommunitySeen('g_' + g.id);
  for (const c of myChannels) markCommunitySeen('c_' + c.id);
  updateCommunityTabDot();
}

// ── Entry point when Community tab is clicked ──────────────────────────────
function renderCommunityTab() {
  if (!currentUser) return;
  clearCommunitySeen();
  listenMyGroups();
  listenMyChannels();
  loadDiscoverChannels();
  if (window.lucide) lucide.createIcons();
}

// ── Sub-tab switcher: Groups vs Channels ───────────────────────────────────
function switchCommunitySubTab(tab) {
  activeCommunitySubTab = tab;
  const btnGroups = document.getElementById('commSubTabGroups');
  const btnChannels = document.getElementById('commSubTabChannels');
  const paneGroups = document.getElementById('commGroupsContainer');
  const paneChannels = document.getElementById('commChannelsContainer');

  if (btnGroups) btnGroups.classList.toggle('active', tab === 'groups');
  if (btnChannels) btnChannels.classList.toggle('active', tab === 'channels');

  if (paneGroups) paneGroups.style.display = tab === 'groups' ? 'flex' : 'none';
  if (paneChannels) paneChannels.style.display = tab === 'channels' ? 'flex' : 'none';

  if (tab === 'groups') {
    renderMyGroups();
  } else {
    renderChannelsFollowing();
    renderChannelsDiscover();
  }
  if (window.lucide) lucide.createIcons();
}

// ── Channels sub-pill: Following vs Discover ───────────────────────────────
function switchChannelSubPill(pill) {
  activeChannelSubPill = pill;
  const pillFollowing = document.getElementById('commPillFollowing');
  const pillDiscover = document.getElementById('commPillDiscover');
  const viewFollowing = document.getElementById('commChannelsFollowingList');
  const viewDiscover = document.getElementById('commChannelsDiscoverView');

  if (pillFollowing) pillFollowing.classList.toggle('active', pill === 'following');
  if (pillDiscover) pillDiscover.classList.toggle('active', pill === 'discover');

  if (viewFollowing) viewFollowing.style.display = pill === 'following' ? 'flex' : 'none';
  if (viewDiscover) viewDiscover.style.display = pill === 'discover' ? 'flex' : 'none';

  if (pill === 'discover') {
    renderChannelsDiscover();
  }
  if (window.lucide) lucide.createIcons();
}

// ── Community Create Menu Dropdown ─────────────────────────────────────────
function toggleCommunityCreateMenu(e) {
  if (e) e.stopPropagation();
  const menu = document.getElementById('commCreateMenu');
  if (!menu) return;
  const isOpen = menu.style.display === 'flex';
  menu.style.display = isOpen ? 'none' : 'flex';
  if (!isOpen) {
    setTimeout(() => document.addEventListener('click', closeCommunityCreateMenu, { once: true }), 50);
  }
}

function closeCommunityCreateMenu() {
  const menu = document.getElementById('commCreateMenu');
  if (menu) menu.style.display = 'none';
}

function handleCommModalOverlayClick(event, modalId) {
  if (event.target === document.getElementById(modalId)) {
    document.getElementById(modalId).classList.remove('active');
  }
}

// ═══════════════════════════════════════════════════════════════════════════
// GROUPS IMPLEMENTATION (WHATSAPP + TELEGRAM STYLE)
// ═══════════════════════════════════════════════════════════════════════════

function openCreateGroupModal() {
  closeCommunityCreateMenu();
  const modal = document.getElementById('createGroupModal');
  if (!modal) return;
  document.getElementById('groupSubjectInput').value = '';
  document.getElementById('groupDescInput').value = '';
  document.getElementById('groupOnlyAdminsCanPost').checked = false;
  document.getElementById('groupAvatarPreview').innerHTML = '<i data-lucide="camera" style="width:24px;height:24px;"></i>';
  pendingGroupAvatar = null;
  selectedGroupMemberUids = new Set();
  updateSelectedGroupCount();

  // Reset and populate contacts picker
  const searchInput = document.getElementById('createGroupMemberSearch');
  if (searchInput) searchInput.value = '';
  renderCreateGroupContacts('');

  modal.classList.add('active');
  if (window.lucide) lucide.createIcons();
}

function renderCreateGroupContacts(filterText = '') {
  const picker = document.getElementById('groupContactsPicker');
  if (!picker) return;
  const q = (filterText || '').toLowerCase().trim();
  let contacts = (allUsersData || []).filter(u => u.uid !== currentUser?.uid);
  if (q) {
    contacts = contacts.filter(u => (u.displayName || u.name || '').toLowerCase().includes(q));
  }
  if (!contacts.length) {
    picker.innerHTML = `<div style="padding:16px;text-align:center;color:var(--text-3);font-size:12px;">${q ? `No contacts match "${escapeHtml(q)}"` : 'No contacts found yet.'}</div>`;
    return;
  }
  picker.innerHTML = contacts.map(u => {
    const photo = u.photo || 'https://i.imgur.com/HeIi0wU.png';
    const name = escapeHtml(u.displayName || u.name || 'Nexa User');
    const checked = selectedGroupMemberUids.has(u.uid);
    return `
      <div class="comm-contact-row" onclick="toggleGroupContactSelect('${u.uid}', this)">
        <input type="checkbox" id="chk_grp_${u.uid}" ${checked ? 'checked' : ''} style="pointer-events:none;">
        <img src="${escapeHtml(photo)}" class="comm-contact-av" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
        <span class="comm-contact-name">${name}</span>
      </div>
    `;
  }).join('');
}

function filterCreateGroupContacts(query) {
  renderCreateGroupContacts(query);
}

function closeCreateGroupModal() {
  const modal = document.getElementById('createGroupModal');
  if (modal) modal.classList.remove('active');
}

function handleGroupAvatarSelected(input) {
  const file = input.files[0];
  if (!file) return;
  if (!file.type.startsWith('image/')) {
    showNotifToast('Please choose an image file', 'error');
    return;
  }
  pendingGroupAvatar = file;
  const preview = document.getElementById('groupAvatarPreview');
  const reader = new FileReader();
  reader.onload = e => {
    preview.innerHTML = `<img src="${e.target.result}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
  };
  reader.readAsDataURL(file);
}

function toggleGroupContactSelect(uid, rowEl) {
  const chk = rowEl.querySelector('input[type="checkbox"]');
  if (selectedGroupMemberUids.has(uid)) {
    selectedGroupMemberUids.delete(uid);
    if (chk) chk.checked = false;
  } else {
    selectedGroupMemberUids.add(uid);
    if (chk) chk.checked = true;
  }
  updateSelectedGroupCount();
}

function updateSelectedGroupCount() {
  const cnt = document.getElementById('selectedGroupMembersCount');
  if (cnt) cnt.textContent = selectedGroupMemberUids.size;
}

async function submitCreateGroup() {
  const name = document.getElementById('groupSubjectInput').value.trim();
  if (!name) {
    showNotifToast('Please enter a group subject', 'error');
    return;
  }
  const desc = document.getElementById('groupDescInput').value.trim();
  const onlyAdminsCanPost = document.getElementById('groupOnlyAdminsCanPost').checked;
  const btn = document.getElementById('btnSubmitCreateGroup');
  btn.disabled = true;
  btn.innerHTML = '<span>Creating…</span>';

  try {
    let avatarUrl = '';
    if (pendingGroupAvatar) {
      showNotifToast('Uploading group icon…', 'info');
      avatarUrl = await uploadFile(pendingGroupAvatar);
    }

    const memberUids = [currentUser.uid, ...Array.from(selectedGroupMemberUids)];
    const groupDoc = {
      name,
      description: desc,
      avatarUrl: avatarUrl || 'https://i.imgur.com/HeIi0wU.png',
      createdBy: currentUser.uid,
      ownerUid: currentUser.uid,
      admins: [currentUser.uid],
      memberUids,
      membersCount: memberUids.length,
      createdAt: Date.now(),
      updatedAt: Date.now(),
      lastMessage: 'Group created',
      lastMessageTime: Date.now(),
      lastSenderName: document.getElementById('myName').textContent || currentUser.displayName || 'Creator',
      settings: {
        onlyAdminsCanPost,
        onlyAdminsCanEditInfo: true
      }
    };

    const docRef = await db.collection('groups').add(groupDoc);

    // Write initial membership documents
    const batch = db.batch();
    const membersColl = docRef.collection('members');

    // Owner doc
    batch.set(membersColl.doc(currentUser.uid), {
      uid: currentUser.uid,
      role: 'owner',
      joinedAt: Date.now(),
      addedBy: currentUser.uid
    });

    // Member docs
    selectedGroupMemberUids.forEach(uid => {
      batch.set(membersColl.doc(uid), {
        uid,
        role: 'member',
        joinedAt: Date.now(),
        addedBy: currentUser.uid
      });
    });

    // Initial system message
    const msgRef = docRef.collection('messages').doc();
    batch.set(msgRef, {
      senderUid: 'system',
      senderName: 'Nexa',
      text: `${currentUser.displayName || 'You'} created group "${name}"`,
      createdAt: Date.now(),
      isSystem: true
    });

    await batch.commit();

    closeCreateGroupModal();
    showNotifToast('✓ Group created!', 'success');

    const createdGroup = { id: docRef.id, ...groupDoc };
    selectGroupChat(createdGroup);
  } catch (err) {
    showNotifToast('Failed to create group: ' + err.message, 'error');
    console.error(err);
  } finally {
    btn.disabled = false;
    btn.innerHTML = '<span>Create Group</span>';
  }
}

// ── Listen to user's joined groups ─────────────────────────────────────────
function listenMyGroups() {
  if (unsubMyGroups) unsubMyGroups();
  unsubMyGroups = db.collection('groups')
    .where('memberUids', 'array-contains', currentUser.uid)
    .onSnapshot(snap => {
      myGroups = [];
      snap.forEach(doc => {
        myGroups.push({ id: doc.id, ...doc.data() });
      });
      myGroups.sort((a, b) => (b.lastMessageTime || 0) - (a.lastMessageTime || 0));
      renderMyGroups();
      updateCommunityTabDot();
    }, err => {
      console.error('Groups listener error:', err);
    });
}

function renderMyGroups() {
  const container = document.getElementById('commGroupsList');
  if (!container) return;

  const query = (document.getElementById('commGroupSearchInput')?.value || '').toLowerCase().trim();
  const filtered = myGroups.filter(g => (g.name || '').toLowerCase().includes(query));

  if (!filtered.length) {
    container.innerHTML = `
      <div class="comm-empty-state">
        <div class="comm-empty-icon">👥</div>
        <div class="comm-empty-title">${myGroups.length ? 'No Matching Groups' : 'No Groups Yet'}</div>
        <div class="comm-empty-desc">Groups are private and invite-only. Create your own group or ask an admin to add you.</div>
        <button class="comm-action-btn-sm" onclick="openCreateGroupModal()">
          <i data-lucide="plus" style="width: 14px; height: 14px;"></i> Create Group
        </button>
      </div>
    `;
    if (window.lucide) lucide.createIcons();
    return;
  }

  container.innerHTML = filtered.map(g => {
    const activeClass = (selectedGroup && selectedGroup.id === g.id) ? ' active' : '';
    const avatar = g.avatarUrl || 'https://i.imgur.com/HeIi0wU.png';
    const timeStr = g.lastMessageTime ? new Date(g.lastMessageTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '';
    const sender = g.lastSenderName ? `${escapeHtml(g.lastSenderName)}: ` : '';
    const preview = escapeHtml(g.lastMessage || 'No messages yet');

    return `
      <div class="comm-group-item${activeClass}" onclick="handleSelectGroupItem('${g.id}')">
        <img src="${escapeHtml(avatar)}" class="comm-group-avatar" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
        <div class="comm-group-info">
          <div class="comm-group-name-row">
            <div class="comm-group-name">${escapeHtml(g.name || 'Group')}</div>
            <div class="comm-group-time">${timeStr}</div>
          </div>
          <div class="comm-group-preview-row">
            <div class="comm-group-preview">${sender}${preview}</div>
            <div class="comm-group-members-badge">${g.membersCount || 1} members</div>
          </div>
        </div>
      </div>
    `;
  }).join('');

  if (window.lucide) lucide.createIcons();
}

function filterMyGroups() {
  renderMyGroups();
}

function handleSelectGroupItem(groupId) {
  const grp = myGroups.find(g => g.id === groupId);
  if (grp) selectGroupChat(grp);
}

// ── Select and Open Group Chat Room ────────────────────────────────────────
function selectGroupChat(group) {
  markCommunitySeen('g_' + group.id);
  updateCommunityTabDot();
  if (unsubMessagesA) { unsubMessagesA(); unsubMessagesA = null; }
  if (unsubMessagesB) { unsubMessagesB(); unsubMessagesB = null; }
  if (unsubTyping) { unsubTyping(); unsubTyping = null; }
  if (unsubChatPresence) { unsubChatPresence(); unsubChatPresence = null; }
  if (unsubGroupMessages) { unsubGroupMessages(); unsubGroupMessages = null; }
  if (unsubChannelPosts) { unsubChannelPosts(); unsubChannelPosts = null; }

  currentChatMode = 'group';
  selectedGroup = group;
  selectedUser = null;
  selectedChannel = null;

  // Header configuration: strictly hide call buttons
  const voiceBtn = document.getElementById('headerVoiceCallBtn');
  const videoBtn = document.getElementById('headerVideoCallBtn');
  const infoBtn = document.getElementById('headerInfoBtn');

  if (voiceBtn) voiceBtn.style.display = 'none';
  if (videoBtn) videoBtn.style.display = 'none';
  if (infoBtn) infoBtn.style.display = 'flex';
  updatePollButtonVisibility();

  document.getElementById('chatName').textContent = group.name || 'Group';
  setChatHandle("");
  document.getElementById('chatPic').src = group.avatarUrl || 'https://i.imgur.com/HeIi0wU.png';
  document.getElementById('chatStatusDot').style.display = 'none';
  document.getElementById('status').textContent = `${group.membersCount || 1} members`;

  // Permission check for announcement mode
  const isAdmin = (group.admins || []).includes(currentUser.uid) || group.ownerUid === currentUser.uid;
  const isBroadcastOnly = group.settings?.onlyAdminsCanPost && !isAdmin;

  const inputArea = document.querySelector('.input-area');
  let noticeEl = document.getElementById('commBroadcastNotice');
  if (isBroadcastOnly) {
    if (inputArea) inputArea.style.display = 'none';
    if (!noticeEl) {
      noticeEl = document.createElement('div');
      noticeEl.id = 'commBroadcastNotice';
      noticeEl.className = 'comm-broadcast-notice';
      inputArea.parentNode.insertBefore(noticeEl, inputArea);
    }
    noticeEl.textContent = 'Only admins can send messages to this group';
    noticeEl.style.display = 'block';
  } else {
    if (inputArea) inputArea.style.display = 'flex';
    if (noticeEl) noticeEl.style.display = 'none';
  }

  // Active state in sidebar
  document.querySelectorAll('.comm-group-item').forEach(e => e.classList.remove('active'));

  // Mobile drawer
  if (window.innerWidth <= 768) {
    const app = document.getElementById('app');
    if (app) app.classList.add('chat-open-mobile');
    document.body.classList.add('chat-open-mobile');
    const bNav = document.getElementById('mobileBottomNav');
    if (bNav) bNav.classList.add('hidden');
  }

  // Live listener for group document updates (pinned message, name, avatar, membersCount, settings)
  if (unsubGroupDoc) { unsubGroupDoc(); unsubGroupDoc = null; }
  unsubGroupDoc = db.collection('groups').doc(group.id).onSnapshot(docSnap => {
    if (!docSnap.exists) return;
    selectedGroup = { id: docSnap.id, ...docSnap.data() };
    document.getElementById('chatName').textContent = selectedGroup.name || 'Group';
    setChatHandle("");
    document.getElementById('chatPic').src = selectedGroup.avatarUrl || 'https://i.imgur.com/HeIi0wU.png';
    document.getElementById('status').textContent = `${selectedGroup.membersCount || 1} members`;

    // Sync pinned announcement banner
    syncCommPinnedBanner();

    // Re-check announcement mode
    const nowAdmin = (selectedGroup.admins || []).includes(currentUser.uid) || selectedGroup.ownerUid === currentUser.uid;
    const nowBroadcastOnly = selectedGroup.settings?.onlyAdminsCanPost && !nowAdmin;
    if (nowBroadcastOnly) {
      if (inputArea) inputArea.style.display = 'none';
      if (noticeEl) noticeEl.style.display = 'block';
    } else {
      if (inputArea) inputArea.style.display = 'flex';
      if (noticeEl) noticeEl.style.display = 'none';
    }
  }, err => console.warn('Group doc sync error:', err));

  // Preload group members so @mentions work immediately (the old flow only
  // populated currentGroupMembersList when Group Info was opened, so typing @
  // right after opening a group showed an empty mention list).
  loadGroupMembersForMentions(group.id);

  loadGroupMessages(group.id);
  if (window.lucide) lucide.createIcons();
}

async function loadGroupMembersForMentions(groupId) {
  try {
    const snap = await db.collection('groups').doc(groupId).collection('members').get();
    const members = [];
    snap.forEach(d => members.push({ id: d.id, ...d.data() }));
    if (selectedGroup && selectedGroup.id === groupId) {
      currentGroupMembersList = members;
    }
  } catch (err) {
    console.warn('Failed to preload group members for mentions:', err);
  }
}

function loadGroupMessages(groupId) {
  if (unsubGroupMessages) unsubGroupMessages();
  const box = document.getElementById('messages');
  if (box) box.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-3);">Loading group messages…</div>';

  unsubGroupMessages = db.collection('groups').doc(groupId).collection('messages')
    .orderBy('createdAt', 'desc')
    .limit(60)
    .onSnapshot(snap => {
      const msgs = [];
      snap.forEach(doc => {
        msgs.push({ id: doc.id, ...doc.data() });
      });
      msgs.sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));
      renderGroupMessageList(msgs);
    }, err => {
      console.error('Group messages error:', err);
    });
}

function renderGroupMessageList(messages) {
  currentGroupMessages = messages || [];
  const box = document.getElementById('messages');
  if (!box) return;

  if (!messages.length) {
    box.innerHTML = `
      <div class="empty-chat">
        <div class="empty-chat-icon">👥</div>
        <div style="font-size: 16px; font-weight: 600; margin-bottom: 6px;">${escapeHtml(selectedGroup?.name || 'Group')}</div>
        <div style="font-size: 13px; color: var(--text-3);">This is the start of your group. Send a message to say hello!</div>
      </div>
    `;
    return;
  }

  box.innerHTML = '';
  let lastDateKey = null;

  messages.forEach(msg => {
    if (msg.isSystem) {
      const sysEl = document.createElement('div');
      sysEl.className = 'date-sep';
      sysEl.innerHTML = `<span>${escapeHtml(msg.text)}</span>`;
      box.appendChild(sysEl);
      return;
    }

    const dateKey = msg.createdAt ? new Date(msg.createdAt).toDateString() : null;
    if (dateKey && dateKey !== lastDateKey) {
      lastDateKey = dateKey;
      const sep = document.createElement("div");
      sep.className = "date-sep";
      sep.innerHTML = `<span>${fmtDateSep(msg.createdAt)}</span>`;
      box.appendChild(sep);
    }

    const fromMe = msg.senderUid === currentUser.uid;
    const msgEl = buildGroupMessage(msg.id, msg, fromMe);
    box.appendChild(msgEl);
  });

  scrollMessagesToBottom();
}

function buildGroupMessage(id, msg, fromMe) {
  const el = document.createElement("div");
  el.className = `msg ${fromMe ? "me" : "them"}`;
  el.dataset.msgId = id;

  const row = document.createElement("div");
  row.className = "msg-row";

  const bubble = document.createElement("div");
  bubble.className = "bubble";

  let inner = "";

  // 1. WhatsApp Group Sender Name for incoming messages
  if (!fromMe) {
    const senderName = escapeHtml(msg.senderName || "Member");
    inner += `<div class="group-msg-sender" style="font-size: 12px; font-weight: 700; color: #53bdeb; margin-bottom: 4px; line-height: 1.2;">${senderName}</div>`;
  }

  // 2. Reply Quote Card
  if (msg.replyTo) {
    const rAuthor = escapeHtml(msg.replyToName || (msg.replyToFrom === currentUser.uid ? "You" : "Member"));
    const rText = linkify(typeof msg.replyToText === "string" ? msg.replyToText : "(message)");
    inner += `<div class="reply-quote"><div class="reply-quote-author"><i data-lucide="corner-up-left" style="width: 11px; height: 11px; vertical-align: -1px; margin-right: 4px;"></i>${rAuthor}</div><div class="reply-quote-text">${rText}</div></div>`;
  }

  // 2b. Forwarded label
  if (msg.forwarded) {
    const fFrom = msg.forwardedFrom ? ` from ${escapeHtml(msg.forwardedFrom)}` : '';
    inner += `<div class="forwarded-label"><i data-lucide="forward" style="width: 12px; height: 12px;"></i> Forwarded${fFrom}</div>`;
  }

  // 2b2. In-chat group/channel invite card (renders even inside group streams)
  if (msg.invite && msg.invite.kind) {
    const invHtml = buildInviteCardHTML(msg);
    if (invHtml) inner += invHtml;
  }

  // 2c. Poll card
  if (msg.type === 'poll' && msg.pollOptions) {
    inner += buildPollHTML(id, msg);
  }

  // 3. Text (skip raw text for polls since poll card shows the question)
  if (msg.text && msg.type !== 'poll') {
    const textHtml = typeof renderMentionText === 'function' ? renderMentionText(linkify(msg.text)) : linkify(msg.text);
    inner += `<div class="msg-text">${textHtml}</div>`;
  }

  // 4. Image
  if (msg.image) {
    const imgUrl = safeMediaUrl(msg.image);
    const cap = msg.caption ? `<div class="media-caption below">${escapeHtml(msg.caption)}</div>` : "";
    if (imgUrl) {
      inner += `<div class="media-container"><img src="${imgUrl}" onclick="viewImg('${imgUrl}')" loading="lazy" alt="shared image" draggable="false"></div>${cap}`;
    }
  }

  // 5. Video
  if (msg.video) {
    const vidUrl = safeMediaUrl(msg.video);
    const cap = msg.caption ? `<div class="media-caption below">${escapeHtml(msg.caption)}</div>` : "";
    if (vidUrl) {
      inner += `<div class="media-container"><video controls preload="metadata" controlslist="nodownload"><source src="${vidUrl}" type="video/mp4"></video></div>${cap}`;
    }
  }

  // 6. Generic File / Document Attachment
  if (msg.fileUrl) {
    const fUrl = safeMediaUrl(msg.fileUrl);
    const fname = escapeHtml(msg.fileName || "Attachment");
    const fsize = msg.fileSize ? formatFileSize(msg.fileSize) : "";
    const ficon = fileIconFor(msg.fileName || "");
    const isAudioFile = /\.(mp3|wav|ogg|m4a|aac|flac|opus|webm)$/i.test(msg.fileName || "");
    if (fUrl && isAudioFile) {
      inner += `<div class="file-attach file-attach-audio">
        <div class="file-attach-icon">${ficon}</div>
        <div class="file-attach-info">
          <div class="file-attach-name">${fname}</div>
          ${fsize ? `<div class="file-attach-size">${fsize}</div>` : ""}
          <audio controls preload="metadata" src="${fUrl}" style="width:100%;margin-top:8px;"></audio>
        </div>
      </div>`;
    } else if (fUrl) {
      inner += `<a class="file-attach" href="${fUrl}" target="_blank" rel="noopener noreferrer" download="${fname}">
        <div class="file-attach-icon">${ficon}</div>
        <div class="file-attach-info">
          <div class="file-attach-name">${fname}</div>
          ${fsize ? `<div class="file-attach-size">${fsize}</div>` : ""}
        </div>
        <div class="file-attach-open">⤓</div>
      </a>`;
    }
  }

  // 7. Reactions
  inner += buildReactionsHTML(id, msg.reactions);

  // 8. Timestamp & Sent Tick
  const tick = fromMe ? "✓" : "";
  inner += `<div class="msg-meta"><span>${formatTime(msg.createdAt)}</span>${tick ? `<span>${tick}</span>` : ""}</div>`;

  bubble.innerHTML = inner;

  // 9. Voice Note Player
  if (msg.audio) {
    const vnNode = renderVoiceNotePlayer(id, msg.audio, msg.duration || 0, fromMe);
    const metaEl = bubble.querySelector(".msg-meta");
    if (metaEl) {
      bubble.insertBefore(vnNode, metaEl);
    } else {
      bubble.appendChild(vnNode);
    }
  }

  row.appendChild(bubble);

  // 10. Swipe right to reply (WhatsApp style)
  let startX = 0, startY = 0, currentX = 0, isSwiping = false;

  row.addEventListener("touchstart", (e) => {
    if (e.touches.length !== 1) return;
    startX = e.touches[0].clientX;
    startY = e.touches[0].clientY;
    currentX = 0;
    isSwiping = false;
    row.style.transition = 'none';
  }, { passive: true });

  row.addEventListener("touchmove", (e) => {
    if (!startX) return;
    const touchX = e.touches[0].clientX;
    const touchY = e.touches[0].clientY;
    const diffX = touchX - startX;
    const diffY = Math.abs(touchY - startY);

    if (diffX > 8 && diffY < diffX) {
      isSwiping = true;
      currentX = Math.min(diffX * 0.55, 75);
      row.style.transform = `translateX(${currentX}px)`;
    }
  }, { passive: true });

  row.addEventListener("touchend", () => {
    if (isSwiping && currentX > 40) {
      if (navigator.vibrate) navigator.vibrate(25);
      replyMsg(id);
    }
    row.style.transition = 'transform 0.22s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
    row.style.transform = 'translateX(0)';
    startX = 0;
    currentX = 0;
    isSwiping = false;
  });

  // 11. Context menu
  el.appendChild(row);
  el.addEventListener("contextmenu", e => { e.preventDefault(); showCtxMenu(e, id, msg); });

  return el;
}

async function sendGroupMessage() {
  if (!selectedGroup) return;
  const ta = document.getElementById('text');
  const text = ta.value.trim();
  if (!text) return;
  ta.value = '';
  ta.style.height = 'auto';
  toggleActionButtons();

  const extras = { text };
  if (messageRepliedTo) {
    extras.replyTo = messageRepliedTo.id;
    extras.replyToText = messageRepliedTo.text || (messageRepliedTo.image ? "(Photo)" : messageRepliedTo.video ? "(Video)" : messageRepliedTo.audio ? "(Voice note)" : "(message)");
    extras.replyToFrom = messageRepliedTo.from || messageRepliedTo.senderUid || null;
    extras.replyToName = messageRepliedTo.senderName || (extras.replyToFrom === currentUser.uid ? 'You' : 'Member');
    clearReply();
  }

  await sendGroupMessageWithExtras(extras);
}

async function sendGroupMessageWithExtras(extras) {
  if (!selectedGroup) return;
  try {
    const senderName = document.getElementById('myName').textContent || currentUser.displayName || 'Member';
    const payload = {
      senderUid: currentUser.uid,
      senderName,
      senderAvatar: currentUser.photoURL || 'https://i.imgur.com/HeIi0wU.png',
      createdAt: Date.now(),
      ...extras
    };

    await db.collection('groups').doc(selectedGroup.id).collection('messages').add(payload);

    // Update group recency snippet
    const snippet = extras.text || (extras.image ? '📷 Photo' : extras.video ? '🎥 Video' : extras.audio ? '🎤 Voice note' : 'Attachment');
    await db.collection('groups').doc(selectedGroup.id).update({
      lastMessage: snippet,
      lastMessageTime: Date.now(),
      lastSenderName: senderName,
      updatedAt: Date.now()
    });
  } catch (err) {
    showNotifToast('Failed to send: ' + err.message, 'error');
    console.error(err);
  }
}

let currentGroupMembersList = [];

// ── Group Info Modal & Member Management ───────────────────────────────────
async function openGroupInfo() {
  if (!selectedGroup) return;
  const modal = document.getElementById('groupInfoModal');
  if (!modal) return;

  document.getElementById('groupInfoAvatar').src = selectedGroup.avatarUrl || 'https://i.imgur.com/HeIi0wU.png';
  document.getElementById('groupInfoName').textContent = selectedGroup.name || 'Group';
  document.getElementById('groupInfoMeta').textContent = `${selectedGroup.membersCount || 1} members`;
  document.getElementById('groupInfoDesc').textContent = selectedGroup.description || 'No description provided';

  const isOwner = selectedGroup.ownerUid === currentUser.uid;
  const isAdmin = isOwner || (selectedGroup.admins || []).includes(currentUser.uid);

  // Edit avatar & details visibility for admins
  const avatarBadge = document.getElementById('groupAvatarEditBadge');
  if (avatarBadge) avatarBadge.style.display = isAdmin ? 'flex' : 'none';

  const editNameBtn = document.getElementById('groupEditNameBtn');
  if (editNameBtn) editNameBtn.style.display = isAdmin ? 'inline-flex' : 'none';

  const editDescBtn = document.getElementById('groupEditDescBtn');
  if (editDescBtn) editDescBtn.style.display = isAdmin ? 'inline-flex' : 'none';

  const searchInput = document.getElementById('groupMemberSearchInput');
  if (searchInput) searchInput.value = '';

  // Admin controls visibility
  const adminSection = document.getElementById('groupAdminControlsSection');
  const addMemberBtn = document.getElementById('groupAddMemberBtn');
  if (adminSection) adminSection.style.display = isAdmin ? 'block' : 'none';
  if (addMemberBtn) addMemberBtn.style.display = isAdmin ? 'block' : 'none';

  const onlyAdminsCheck = document.getElementById('groupInfoOnlyAdminsCanPost');
  if (onlyAdminsCheck) onlyAdminsCheck.checked = !!selectedGroup.settings?.onlyAdminsCanPost;

  // Load members
  const membersList = document.getElementById('groupInfoMembersList');
  if (membersList) {
    membersList.innerHTML = '<div style="color:var(--text-3);padding:10px;text-align:center;">Loading members…</div>';
    try {
      const snap = await db.collection('groups').doc(selectedGroup.id).collection('members').get();
      currentGroupMembersList = [];
      snap.forEach(d => currentGroupMembersList.push({ id: d.id, ...d.data() }));

      document.getElementById('groupInfoMemberCount').textContent = currentGroupMembersList.length;
      renderGroupInfoMembers('');
    } catch (err) {
      console.error(err);
    }
  }

  // Owner delete button
  const deleteBtn = document.getElementById('groupDeleteBtn');
  if (deleteBtn) deleteBtn.style.display = isOwner ? 'inline-flex' : 'none';

  // Load WhatsApp-style Shared Media
  loadGroupSharedMedia();

  modal.classList.add('active');
  if (window.lucide) lucide.createIcons();
}

function renderGroupInfoMembers(filterText = '') {
  const membersList = document.getElementById('groupInfoMembersList');
  if (!membersList || !selectedGroup) return;

  const isAdmin = selectedGroup.ownerUid === currentUser.uid || (selectedGroup.admins || []).includes(currentUser.uid);
  const q = (filterText || '').toLowerCase().trim();

  let filtered = currentGroupMembersList;
  if (q) {
    filtered = filtered.filter(m => {
      const uData = (allUsersData || []).find(u => u.uid === m.uid) || {};
      const name = (uData.displayName || uData.name || '').toLowerCase();
      return name.includes(q) || (m.role || '').toLowerCase().includes(q);
    });
  }

  if (!filtered.length) {
    membersList.innerHTML = `<div style="color:var(--text-3);padding:14px;text-align:center;font-size:12px;">${q ? `No members match "${escapeHtml(q)}"` : 'No members found'}</div>`;
    return;
  }

  membersList.innerHTML = filtered.map(m => {
    const uData = (allUsersData || []).find(u => u.uid === m.uid) || {};
    const name = m.uid === currentUser.uid ? 'You' : escapeHtml(uData.displayName || 'Member');
    const photo = uData.photo || 'https://i.imgur.com/HeIi0wU.png';
    const role = m.role || 'member';
    const canManage = isAdmin && m.uid !== currentUser.uid && role !== 'owner';

    let actionMenu = '';
    if (canManage) {
      actionMenu = `
        <button class="comm-member-menu-btn" onclick="openMemberActionsMenu('${m.uid}', '${role}')" title="Manage member">
          <i data-lucide="more-vertical" style="width: 14px; height: 14px;"></i>
        </button>
      `;
    }

    return `
      <div class="comm-member-row">
        <img src="${escapeHtml(photo)}" class="comm-member-av" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
        <div class="comm-member-name">${name}</div>
        <div class="comm-role-chip ${role}">${role}</div>
        ${actionMenu}
      </div>
    `;
  }).join('');

  if (window.lucide) lucide.createIcons();
}

function filterGroupInfoMembers(query) {
  renderGroupInfoMembers(query);
}

function triggerChangeGroupAvatar() {
  if (!selectedGroup) return;
  const isAdmin = selectedGroup.ownerUid === currentUser.uid || (selectedGroup.admins || []).includes(currentUser.uid);
  if (!isAdmin) {
    showNotifToast('Only group admins can change the group icon', 'info');
    return;
  }
  const input = document.getElementById('groupChangeAvatarInput');
  if (input) input.click();
}

async function handleUpdateGroupAvatar(input) {
  const file = input.files[0];
  if (!file || !selectedGroup) return;
  if (!file.type.startsWith('image/')) {
    showNotifToast('Please choose an image file', 'error');
    return;
  }
  try {
    showNotifToast('Uploading group icon…', 'info');
    const url = await uploadFile(file);
    if (!url) throw new Error('Upload failed');
    await db.collection('groups').doc(selectedGroup.id).update({ avatarUrl: url, updatedAt: Date.now() });
    selectedGroup.avatarUrl = url;
    document.getElementById('groupInfoAvatar').src = url;
    document.getElementById('chatPic').src = url;
    renderMyGroups();
    showNotifToast('✓ Group icon updated!', 'success');
  } catch (e) {
    showNotifToast('Failed to update icon: ' + e.message, 'error');
  }
}

async function editGroupName() {
  if (!selectedGroup) return;
  const isAdmin = selectedGroup.ownerUid === currentUser.uid || (selectedGroup.admins || []).includes(currentUser.uid);
  if (!isAdmin) {
    showNotifToast('Only group admins can edit the name', 'info');
    return;
  }
  const newName = prompt('Enter new group subject:', selectedGroup.name || '');
  if (!newName || !newName.trim() || newName.trim() === selectedGroup.name) return;
  try {
    const val = newName.trim().slice(0, 50);
    await db.collection('groups').doc(selectedGroup.id).update({ name: val, updatedAt: Date.now() });
    selectedGroup.name = val;
    document.getElementById('groupInfoName').textContent = val;
    document.getElementById('chatName').textContent = val;
    setChatHandle("");
    renderMyGroups();
    showNotifToast('✓ Group name updated', 'success');
  } catch (e) {
    showNotifToast('Error updating name: ' + e.message, 'error');
  }
}

async function editGroupDesc() {
  if (!selectedGroup) return;
  const isAdmin = selectedGroup.ownerUid === currentUser.uid || (selectedGroup.admins || []).includes(currentUser.uid);
  if (!isAdmin) {
    showNotifToast('Only group admins can edit the description', 'info');
    return;
  }
  const newDesc = prompt('Enter group description:', selectedGroup.description || '');
  if (newDesc === null) return;
  try {
    const val = newDesc.trim().slice(0, 300);
    await db.collection('groups').doc(selectedGroup.id).update({ description: val, updatedAt: Date.now() });
    selectedGroup.description = val;
    document.getElementById('groupInfoDesc').textContent = val || 'No description provided';
    showNotifToast('✓ Description updated', 'success');
  } catch (e) {
    showNotifToast('Error updating description: ' + e.message, 'error');
  }
}

async function shareGroupInviteLink() {
  if (!selectedGroup) return;
  const inviteText = `Join my Nexa group "${selectedGroup.name}":`;
  const shareData = {
    title: selectedGroup.name,
    text: inviteText,
    url: window.location.href.split('#')[0]
  };
  if (navigator.share) {
    try {
      await navigator.share(shareData);
      showNotifToast('✓ Shared group invite', 'success');
      return;
    } catch (e) {
      if (e.name === 'AbortError') return;
    }
  }
  const copyStr = `${shareData.text}\n${shareData.url}`;
  if (navigator.clipboard) {
    await navigator.clipboard.writeText(copyStr);
    showNotifToast('✓ Group invite link copied to clipboard!', 'success');
  } else {
    showNotifToast('Link: ' + copyStr, 'info');
  }
}

function closeGroupInfo() {
  const modal = document.getElementById('groupInfoModal');
  if (modal) modal.classList.remove('active');
}

async function deleteCurrentGroup() {
  if (!selectedGroup) return;
  const isOwner = selectedGroup.ownerUid === currentUser.uid;
  if (!isOwner) {
    showNotifToast('Only the group creator can delete this group', 'error');
    return;
  }

  const groupName = selectedGroup.name || 'Group';
  if (!confirm(`Permanently delete "${groupName}" and remove all messages? This cannot be undone.`)) return;

  try {
    const groupId = selectedGroup.id;
    await db.collection('groups').doc(groupId).delete();
    closeGroupInfo();
    resetCommunityChatMode();
    showNotifToast(`✓ Group "${groupName}" deleted`, 'success');
  } catch (err) {
    showNotifToast('Error deleting group: ' + err.message, 'error');
  }
}

async function toggleGroupOnlyAdminsCanPost(checked) {
  if (!selectedGroup) return;
  try {
    await db.collection('groups').doc(selectedGroup.id).update({
      'settings.onlyAdminsCanPost': checked
    });
    selectedGroup.settings = selectedGroup.settings || {};
    selectedGroup.settings.onlyAdminsCanPost = checked;
    showNotifToast(`✓ Permissions updated`, 'success');
  } catch (err) {
    showNotifToast('Failed to update permission: ' + err.message, 'error');
  }
}

async function leaveCurrentGroup() {
  if (!selectedGroup) return;
  if (!confirm(`Are you sure you want to leave "${selectedGroup.name}"?`)) return;

  try {
    const groupId = selectedGroup.id;
    const batch = db.batch();
    const groupRef = db.collection('groups').doc(groupId);
    const memberRef = groupRef.collection('members').doc(currentUser.uid);

    batch.delete(memberRef);
    batch.update(groupRef, {
      memberUids: firebase.firestore.FieldValue.arrayRemove(currentUser.uid),
      membersCount: firebase.firestore.FieldValue.increment(-1)
    });

    await batch.commit();
    closeGroupInfo();
    resetCommunityChatMode();
    showNotifToast('You left the group', 'info');
  } catch (err) {
    showNotifToast('Error leaving group: ' + err.message, 'error');
  }
}

function openAddMemberModal() {
  if (!selectedGroup) return;
  const modal = document.getElementById('addMemberModal');
  if (!modal) return;

  const searchInput = document.getElementById('addMemberSearchInput');
  if (searchInput) searchInput.value = '';

  selectedGroupMemberUids = new Set();
  renderAddMemberContacts('');

  modal.classList.add('active');
}

function renderAddMemberContacts(filterText = '') {
  if (!selectedGroup) return;
  const picker = document.getElementById('addMemberContactsPicker');
  if (!picker) return;

  const currentUids = new Set(selectedGroup.memberUids || []);
  let availableContacts = (allUsersData || []).filter(u => !currentUids.has(u.uid) && u.uid !== currentUser?.uid);
  const q = (filterText || '').toLowerCase().trim();
  if (q) {
    availableContacts = availableContacts.filter(u => (u.displayName || u.name || '').toLowerCase().includes(q));
  }

  if (!availableContacts.length) {
    picker.innerHTML = `<div style="padding:20px;text-align:center;color:var(--text-3);font-size:12px;">${q ? `No contacts match "${escapeHtml(q)}"` : 'All your contacts are already in this group!'}</div>`;
    return;
  }

  picker.innerHTML = availableContacts.map(u => {
    const checked = selectedGroupMemberUids.has(u.uid);
    return `
      <div class="comm-contact-row" onclick="toggleAddMemberSelect('${u.uid}', this)">
        <input type="checkbox" id="add_chk_${u.uid}" ${checked ? 'checked' : ''} style="pointer-events:none;">
        <img src="${escapeHtml(u.photo || 'https://i.imgur.com/HeIi0wU.png')}" class="comm-contact-av" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
        <span class="comm-contact-name">${escapeHtml(u.displayName || 'User')}</span>
      </div>
    `;
  }).join('');
}

function filterAddMemberContacts(query) {
  renderAddMemberContacts(query);
}

function closeAddMemberModal() {
  const modal = document.getElementById('addMemberModal');
  if (modal) modal.classList.remove('active');
}

function toggleAddMemberSelect(uid, rowEl) {
  const chk = rowEl.querySelector('input[type="checkbox"]');
  if (selectedGroupMemberUids.has(uid)) {
    selectedGroupMemberUids.delete(uid);
    if (chk) chk.checked = false;
  } else {
    selectedGroupMemberUids.add(uid);
    if (chk) chk.checked = true;
  }
}

async function submitAddMembersToGroup() {
  if (!selectedGroup || !selectedGroupMemberUids.size) {
    closeAddMemberModal();
    return;
  }
  try {
    const groupId = selectedGroup.id;
    const batch = db.batch();
    const groupRef = db.collection('groups').doc(groupId);
    const membersColl = groupRef.collection('members');

    selectedGroupMemberUids.forEach(uid => {
      batch.set(membersColl.doc(uid), {
        uid,
        role: 'member',
        joinedAt: Date.now(),
        addedBy: currentUser.uid
      });
      batch.update(groupRef, {
        memberUids: firebase.firestore.FieldValue.arrayUnion(uid),
        membersCount: firebase.firestore.FieldValue.increment(1)
      });
    });

    await batch.commit();
    closeAddMemberModal();
    showNotifToast(`✓ Added ${selectedGroupMemberUids.size} member(s)`, 'success');
    openGroupInfo();
  } catch (err) {
    showNotifToast('Failed to add members: ' + err.message, 'error');
  }
}

async function openMemberActionsMenu(targetUid, currentRole) {
  if (!selectedGroup) return;
  const targetUser = (allUsersData || []).find(u => u.uid === targetUid) || {};
  const name = targetUser.displayName || 'Member';
  const isTargetAdmin = currentRole === 'admin';

  const action = prompt(
    `Manage ${name}:\n1. ${isTargetAdmin ? 'Dismiss as Admin' : 'Make Group Admin'}\n2. Remove from Group\n\nEnter 1 or 2 (or Cancel):`
  );

  if (action === '1') {
    try {
      const newRole = isTargetAdmin ? 'member' : 'admin';
      const batch = db.batch();
      const groupRef = db.collection('groups').doc(selectedGroup.id);
      const memberRef = groupRef.collection('members').doc(targetUid);

      batch.update(memberRef, { role: newRole });
      if (newRole === 'admin') {
        batch.update(groupRef, { admins: firebase.firestore.FieldValue.arrayUnion(targetUid) });
      } else {
        batch.update(groupRef, { admins: firebase.firestore.FieldValue.arrayRemove(targetUid) });
      }
      await batch.commit();
      showNotifToast(`✓ Updated role for ${name}`, 'success');
      openGroupInfo();
    } catch (e) {
      showNotifToast('Failed to update role: ' + e.message, 'error');
    }
  } else if (action === '2') {
    if (!confirm(`Remove ${name} from this group?`)) return;
    try {
      const batch = db.batch();
      const groupRef = db.collection('groups').doc(selectedGroup.id);
      const memberRef = groupRef.collection('members').doc(targetUid);

      batch.delete(memberRef);
      batch.update(groupRef, {
        memberUids: firebase.firestore.FieldValue.arrayRemove(targetUid),
        admins: firebase.firestore.FieldValue.arrayRemove(targetUid),
        membersCount: firebase.firestore.FieldValue.increment(-1)
      });
      await batch.commit();
      showNotifToast(`✓ Removed ${name}`, 'success');
      openGroupInfo();
    } catch (e) {
      showNotifToast('Failed to remove member: ' + e.message, 'error');
    }
  }
}

async function openAddChannelAdminModal() {
  if (!selectedChannel) return;
  const subs = (selectedChannel.subscriberUids || []).filter(u => u !== currentUser.uid && !(selectedChannel.admins || []).includes(u));
  if (!subs.length) {
    showNotifToast('No eligible subscribers found to promote to admin', 'info');
    return;
  }
  const promptList = subs.map((u, i) => {
    const user = (allUsersData || []).find(usr => usr.uid === u) || {};
    return `${i + 1}. ${user.displayName || 'Subscriber'} (${user.email || u})`;
  }).join('\n');

  const pickedIndex = prompt(`Select subscriber to make Admin:\n${promptList}\n\nEnter number:`);
  if (!pickedIndex) return;
  const idx = parseInt(pickedIndex) - 1;
  if (idx >= 0 && idx < subs.length) {
    const targetUid = subs[idx];
    try {
      await db.collection('channels').doc(selectedChannel.id).update({
        admins: firebase.firestore.FieldValue.arrayUnion(targetUid)
      });
      selectedChannel.admins = [...(selectedChannel.admins || []), targetUid];
      showNotifToast('✓ Admin added!', 'success');
      openChannelInfo();
    } catch (e) {
      showNotifToast('Failed to add admin: ' + e.message, 'error');
    }
  }
}

// ═══════════════════════════════════════════════════════════════════════════
// CHANNELS IMPLEMENTATION (TELEGRAM + WHATSAPP CHANNELS STYLE)
// ═══════════════════════════════════════════════════════════════════════════

function openCreateChannelModal() {
  closeCommunityCreateMenu();
  const modal = document.getElementById('createChannelModal');
  if (!modal) return;
  document.getElementById('channelNameInput').value = '';
  document.getElementById('channelHandleInput').value = '';
  document.getElementById('channelDescInput').value = '';
  document.getElementById('channelCategorySelect').value = 'General';
  document.getElementById('channelAvatarPreview').innerHTML = '<i data-lucide="camera" style="width:24px;height:24px;"></i>';
  pendingChannelAvatar = null;
  modal.classList.add('active');
  if (window.lucide) lucide.createIcons();
}

function closeCreateChannelModal() {
  const modal = document.getElementById('createChannelModal');
  if (modal) modal.classList.remove('active');
}

function handleChannelAvatarSelected(input) {
  const file = input.files[0];
  if (!file) return;
  if (!file.type.startsWith('image/')) {
    showNotifToast('Please choose an image file', 'error');
    return;
  }
  pendingChannelAvatar = file;
  const preview = document.getElementById('channelAvatarPreview');
  const reader = new FileReader();
  reader.onload = e => {
    preview.innerHTML = `<img src="${e.target.result}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
  };
  reader.readAsDataURL(file);
}

function sanitizeChannelHandle(input) {
  input.value = input.value.toLowerCase().replace(/[^a-z0-9_]/g, '');
}

async function submitCreateChannel() {
  const name = document.getElementById('channelNameInput').value.trim();
  const rawHandle = document.getElementById('channelHandleInput').value.trim();
  const desc = document.getElementById('channelDescInput').value.trim();
  const category = document.getElementById('channelCategorySelect').value;

  if (!name) { showNotifToast('Please enter channel name', 'error'); return; }
  if (!rawHandle) { showNotifToast('Please enter a unique channel handle', 'error'); return; }

  const handle = '@' + rawHandle.replace(/^@/, '');
  const btn = document.getElementById('btnSubmitCreateChannel');
  btn.disabled = true;
  btn.innerHTML = '<span>Creating…</span>';

  try {
    // Check if handle taken
    const existing = await db.collection('channels').where('handle', '==', handle).get();
    if (!existing.empty) {
      showNotifToast('Handle is already taken. Please pick another.', 'error');
      btn.disabled = false;
      btn.innerHTML = '<span>Create Channel</span>';
      return;
    }

    let avatarUrl = '';
    if (pendingChannelAvatar) {
      showNotifToast('Uploading channel icon…', 'info');
      avatarUrl = await uploadFile(pendingChannelAvatar);
    }

    const chanDoc = {
      name,
      handle,
      description: desc,
      category,
      avatarUrl: avatarUrl || 'https://i.imgur.com/HeIi0wU.png',
      ownerUid: currentUser.uid,
      admins: [currentUser.uid],
      subscribersCount: 1,
      subscriberUids: [currentUser.uid],
      isPublic: true,
      createdAt: Date.now(),
      lastPostTime: Date.now()
    };

    const docRef = await db.collection('channels').add(chanDoc);
    await docRef.collection('subscribers').doc(currentUser.uid).set({
      uid: currentUser.uid,
      joinedAt: Date.now(),
      role: 'owner'
    });

    closeCreateChannelModal();
    showNotifToast('✓ Channel created!', 'success');

    const createdChan = { id: docRef.id, ...chanDoc };
    selectChannelFeed(createdChan);
    // Switch to Channels tab so the new channel appears in the following list
    switchCommunitySubTab('channels');
  } catch (err) {
    showNotifToast('Failed to create channel: ' + err.message, 'error');
    console.error(err);
  } finally {
    btn.disabled = false;
    btn.innerHTML = '<span>Create Channel</span>';
  }
}

// ── Listen to user's subscribed channels ───────────────────────────────────
function listenMyChannels() {
  db.collection('channels')
    .where('subscriberUids', 'array-contains', currentUser.uid)
    .onSnapshot(snap => {
      myChannels = [];
      snap.forEach(doc => {
        myChannels.push({ id: doc.id, ...doc.data() });
      });
      myChannels.sort((a, b) => (b.lastPostTime || 0) - (a.lastPostTime || 0));

      const countBadge = document.getElementById('commFollowingCount');
      if (countBadge) countBadge.textContent = myChannels.length;

      if (activeCommunitySubTab === 'channels') {
        renderChannelsFollowing();
      }
      updateCommunityTabDot();
    }, err => console.error('Channels listener error:', err));
}

function renderChannelsFollowing() {
  const container = document.getElementById('commChannelsFollowingList');
  if (!container) return;

  if (!myChannels.length) {
    container.innerHTML = `<div class="comm-ws-empty-hint">Channels you create or follow will appear here.</div>`;
    return;
  }

  container.innerHTML = myChannels.map(c => {
    const avatar = c.avatarUrl || 'https://i.imgur.com/HeIi0wU.png';
    const activeClass = (selectedChannel && selectedChannel.id === c.id) ? ' active' : '';
    const subs = formatCount(c.subscribersCount || 1);

    return `
      <div class="comm-group-item${activeClass}" onclick="handleSelectChannelItem('${c.id}')">
        <img src="${escapeHtml(avatar)}" class="comm-group-avatar" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
        <div class="comm-group-info">
          <div class="comm-group-name-row">
            <div class="comm-group-name">${escapeHtml(c.name || 'Channel')}</div>
            <div class="comm-group-time">${escapeHtml(c.category || 'General')}</div>
          </div>
          <div class="comm-group-preview-row">
            <div class="comm-group-preview" style="color:var(--primary-lt);font-weight:600;">${escapeHtml(c.handle || '')}</div>
            <div class="comm-group-members-badge">${subs} subscribers</div>
          </div>
        </div>
      </div>
    `;
  }).join('');

  if (window.lucide) lucide.createIcons();
}

function handleSelectChannelItem(channelId) {
  const chan = myChannels.find(c => c.id === channelId) || discoverChannels.find(c => c.id === channelId);
  if (chan) selectChannelFeed(chan);
}

// ── Public Discover Channels Directory ─────────────────────────────────────
let unsubDiscoverChannels = null;
function listenDiscoverChannels() {
  if (unsubDiscoverChannels || !db) return;
  unsubDiscoverChannels = db.collection('channels')
    .where('isPublic', '==', true)
    .limit(50)
    .onSnapshot(snap => {
      discoverChannels = [];
      snap.forEach(d => discoverChannels.push({ id: d.id, ...d.data() }));
      discoverChannels.sort((a, b) => (b.subscribersCount || 0) - (a.subscribersCount || 0));
      if (activeCommunitySubTab === 'channels') {
        renderChannelsDiscover();
      }
    }, err => {
      console.warn('Discover channels listener error:', err);
    });
}

function loadDiscoverChannels() {
  listenDiscoverChannels();
}

function filterDiscoverChannels() {
  renderChannelsDiscover();
}

function renderChannelsDiscover() {
  const carousel = document.getElementById('commDiscoverCarousel');
  const listContainer = document.getElementById('commDiscoverList');
  if (!listContainer) return;

  const query = (document.getElementById('commDiscoverSearchInput')?.value || '').toLowerCase().trim();
  let filtered = discoverChannels;

  if (query) {
    filtered = filtered.filter(c => 
      (c.name || '').toLowerCase().includes(query) || 
      (c.handle || '').toLowerCase().includes(query) ||
      (c.description || '').toLowerCase().includes(query)
    );
  }

  // 1. WhatsApp-Style Horizontal Recommended Channels Carousel
  if (carousel) {
    const recommended = discoverChannels.slice(0, 10);
    if (!recommended.length) {
      carousel.innerHTML = '<div style="padding:10px 16px;color:var(--text-3);font-size:12px;">No channels found</div>';
    } else {
      carousel.innerHTML = recommended.map(c => {
        const isSubscribed = (c.subscriberUids || []).includes(currentUser.uid);
        const subs = formatCount(c.subscribersCount || 1);
        const avatar = c.avatarUrl || 'https://i.imgur.com/HeIi0wU.png';

        return `
          <div class="ws-channel-card" onclick="handleSelectChannelItem('${c.id}')">
            <div class="ws-channel-card-avatar-wrap">
              <img src="${escapeHtml(avatar)}" class="ws-channel-card-avatar" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
              <div class="ws-channel-verified-badge">✓</div>
            </div>
            <div class="ws-channel-card-title">${escapeHtml(c.name)}</div>
            <div class="ws-channel-card-followers">${subs} followers</div>
            <button class="ws-channel-card-btn ${isSubscribed ? 'following' : 'follow'}" onclick="event.stopPropagation(); toggleSubscribeChannel('${c.id}')">
              ${isSubscribed ? 'Following' : 'Follow'}
            </button>
          </div>
        `;
      }).join('');
    }
  }

  // 2. Full Explore More List (WhatsApp Style)
  if (!filtered.length) {
    listContainer.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-3);font-size:13px;">No channels found matching your search.</div>';
    return;
  }

  listContainer.innerHTML = filtered.map(c => {
    const isSubscribed = (c.subscriberUids || []).includes(currentUser.uid);
    const subs = formatCount(c.subscribersCount || 1);
    const avatar = c.avatarUrl || 'https://i.imgur.com/HeIi0wU.png';

    return `
      <div class="comm-discover-card" onclick="handleSelectChannelItem('${c.id}')">
        <img src="${escapeHtml(avatar)}" class="comm-discover-avatar" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
        <div class="comm-discover-info">
          <div class="comm-discover-title-row">
            <span class="comm-discover-name">${escapeHtml(c.name)}</span>
            <span class="comm-discover-handle">${escapeHtml(c.handle || '')}</span>
          </div>
          <div class="comm-discover-subline">
            <span>👥 ${subs} followers</span>
          </div>
        </div>
        <button class="comm-sub-btn ${isSubscribed ? 'following' : 'follow'}" onclick="event.stopPropagation(); toggleSubscribeChannel('${c.id}')">
          ${isSubscribed ? 'Following' : '+ Follow'}
        </button>
      </div>
    `;
  }).join('');

  if (window.lucide) lucide.createIcons();
}

async function _resolveChannelData(channelId) {
  // Prefer live subscription docs; fall back to discover cache
  const cached = myChannels.find(c => c.id === channelId) || discoverChannels.find(c => c.id === channelId);
  if (cached && cached.subscriberUids && cached.name) return cached;
  try {
    const snap = await db.collection('channels').doc(channelId).get();
    if (snap.exists) return { id: snap.id, ...snap.data() };
  } catch (e) {}
  return cached || null;
}

async function resolveChannelForInvite(channelId) {
  const chan = await _resolveChannelData(channelId);
  if (!chan) { showNotifToast('Channel not found', 'error'); return; }
  selectChannelFeed(chan);
  switchTab('community');
  switchCommunitySubTab('channels');
}

async function resolveGroupForInvite(groupId, groupName) {
  const _joinerUid = currentUser?.uid;
  let group = myGroups.find(g => g.id === groupId);
  if (!group) { try {
    const snap = await db.collection('groups').doc(groupId).get();
    if (snap.exists) group = { id: snap.id, ...snap.data() };
  } catch (e) {} }
  if (!group && groupName) group = { id: groupId, name: groupName };
  if (!group) { showNotifToast('Group not found', 'error'); return; }
  const isMember = (group.memberUids || []).includes(_joinerUid) || (group.createdBy === _joinerUid);
// Already a member? Jump straight in
  if (isMember) {
    switchTab('community');
    switchCommunitySubTab('groups');
    selectGroupChat(group);
    return;
  }
// Not a member — the invite link / in-chat Follow/Join card must add them first.
  const myName = document.getElementById('myName')?.textContent || currentUser.displayName || 'Member';
  try {
    await db.collection('groups').doc(group.id).collection('members').doc(_joinerUid).set({
      uid: currentUser.uid,
      displayName: myName,
      photo: currentUser.photoURL || 'https://i.imgur.com/HeIi0wU.png',
      role: 'member',
      joinedAt: Date.now()
    });
    await db.collection('groups').doc(group.id).update({
      memberUids: firebase.firestore.FieldValue.arrayUnion(currentUser.uid),
      membersCount: firebase.firestore.FieldValue.increment(1)
    });
    group.memberUids = [...(group.memberUids || []), currentUser.uid];
    group.membersCount = (group.membersCount || 0) + 1;
    showNotifToast(`✓ Joined "${group.name}"!`, 'success');
    switchTab('community');
    switchCommunitySubTab('groups');
    selectGroupChat(group);
  } catch (err) {
    showNotifToast('Failed to join group: ' + err.message, 'error');
  }
}

// Build a clickable in-chat invite card for group/channel invites
function buildInviteCardHTML(msg) {
  const inv = msg.invite || {};
  if (!inv || !inv.kind) return '';
  // Base64-encode the name so apostrophes/quotes in a group name can't break
  // out of the inline onclick JS string.
  const nameB64 = typeof btoa === 'function' ? btoa(unescape(encodeURIComponent(inv.name || ''))).replace(/=+$/, '') : encodeURIComponent(inv.name || '');
  if (inv.kind === 'group') {
    return `<div class="nexa-invite-card" data-kind="group">
      <div class="nexa-invite-icon">👥</div>
      <div class="nexa-invite-body">
        <div class="nexa-invite-title">${escapeHtml(inv.name || 'Group Invite')}</div>
        <div class="nexa-invite-sub">${escapeHtml(inv.description || 'Join the group conversation')}</div>
      </div>
      <button class="nexa-invite-cta" onclick="resolveGroupInviteWithName('${inv.id}', '${nameB64}')">Join</button>
    </div>`;
  }
  if (inv.kind === 'channel') {
    return `<div class="nexa-invite-card" data-kind="channel">
      <div class="nexa-invite-icon">📢</div>
      <div class="nexa-invite-body">
        <div class="nexa-invite-title">${escapeHtml(inv.name || 'Channel Invite')}</div>
        <div class="nexa-invite-sub">${escapeHtml(inv.description || 'Follow the channel to get updates')}</div>
      </div>
      <button class="nexa-invite-cta" onclick="resolveChannelFromInvite('${inv.id}')">Follow</button>
    </div>`;
  }
  return '';
}

// Safely decode the base64 group name (see buildInviteCardHTML) and resolve.
function resolveGroupInviteWithName(groupId, nameB64) {
  let name = '';
  try {
    name = nameB64 && typeof atob === 'function'
      ? decodeURIComponent(escape(atob(nameB64)))
      : decodeURIComponent(nameB64 || '');
  } catch (e) { name = ''; }
  resolveGroupFromInvite(groupId, name);
}

// Entry points for invite card buttons (exposed to inline onclick)
async function resolveChannelFromInvite(channelId) {
  const chan = await _resolveChannelData(channelId);
  if (!chan) { showNotifToast('Channel not found or no longer exists', 'error'); return; }
  const isSubscribed = (chan.subscriberUids || []).includes(currentUser.uid);
  if (!isSubscribed) {
    try {
      await db.collection('channels').doc(channelId).collection('subscribers').doc(currentUser.uid).set({
        uid: currentUser.uid,
        joinedAt: Date.now()
      });
      await db.collection('channels').doc(channelId).update({
        subscriberUids: firebase.firestore.FieldValue.arrayUnion(currentUser.uid),
        subscribersCount: firebase.firestore.FieldValue.increment(1)
      });
      chan.subscriberUids = [...(chan.subscriberUids || []), currentUser.uid];
      chan.subscribersCount = (chan.subscribersCount || 0) + 1;
      showNotifToast(`✓ Following "${chan.name}"!`, 'success');
    } catch (err) {
      showNotifToast('Failed to follow: ' + err.message, 'error');
      return;
    }
  }
  // Kick any stale myChannels/discover entries so the Following list is current
  // without waiting for the slow onSnapshot round-trip.
  if (!myChannels.some(c => c.id === channelId)) myChannels.push({ ...chan });
  discoverChannels = discoverChannels.map(c => c.id === channelId ? { ...c, subscriberUids: chan.subscriberUids, subscribersCount: chan.subscribersCount } : c);
  renderChannelsDiscover();
  renderChannelsFollowing();
  selectChannelFeed(chan);
  switchTab('community');
  switchCommunitySubTab('channels');
}

async function resolveGroupFromInvite(groupId, groupName) {
  await resolveGroupForInvite(groupId, groupName);
}

// Send a group invite as a chat message to a 1:1 contact
async function sendGroupInviteToChat(contactUid) {
  if (!selectedGroup || !contactUid) return;
  const me = document.getElementById('myName')?.textContent || currentUser.displayName || 'Nexa User';
  const inv = {
    kind: 'group',
    id: selectedGroup.id,
    name: selectedGroup.name,
    description: selectedGroup.description || `Join "${selectedGroup.name}" on Nexa`
  };
  try {
    await db.collection('chats').add({
      from: currentUser.uid,
      to: contactUid,
      text: `🔗 ${me} invited you to join the group "${inv.name}"`,
      invite: inv,
      createdAt: Date.now(),
      read: false
    });
    showNotifToast(`✓ Group invite sent`, 'success');
  } catch (err) {
    showNotifToast('Invite failed: ' + err.message, 'error');
  }
}

// Send a channel invite as a chat message to a 1:1 contact
async function sendChannelInviteToChat(contactUid) {
  if (!selectedChannel || !contactUid) return;
  const me = document.getElementById('myName')?.textContent || currentUser.displayName || 'Nexa User';
  const inv = {
    kind: 'channel',
    id: selectedChannel.id,
    name: selectedChannel.name,
    description: selectedChannel.description || `Follow "${selectedChannel.name}" on Nexa`
  };
  try {
    await db.collection('chats').add({
      from: currentUser.uid,
      to: contactUid,
      text: `📢 ${me} invited you to follow the channel "${inv.name}"`,
      invite: inv,
      createdAt: Date.now(),
      read: false
    });
    showNotifToast(`✓ Channel invite sent`, 'success');
  } catch (err) {
    showNotifToast('Invite failed: ' + err.message, 'error');
  }
}

async function toggleSubscribeChannel(channelId) {
  let chan = discoverChannels.find(c => c.id === channelId) || myChannels.find(c => c.id === channelId);
  // Read the freshest server state so a stale cached subscriberUids array
  // (e.g. the Discover snapshot loaded before another device followed) can't
  // cause a "Follow" press to actually unsubscribe (or vice versa).
  try {
    const fresh = await db.collection('channels').doc(channelId).get();
    if (fresh.exists) chan = { id: fresh.id, ...fresh.data() };
  } catch (e) {}
  if (!chan) {
    showNotifToast('Channel not found or no longer exists', 'error');
    return;
  }

  const isSubscribed = (chan.subscriberUids || []).includes(currentUser.uid);
  try {
    const chanRef = db.collection('channels').doc(channelId);
    const subRef = chanRef.collection('subscribers').doc(currentUser.uid);

    if (isSubscribed) {
      await subRef.delete();
      await chanRef.update({
        subscriberUids: firebase.firestore.FieldValue.arrayRemove(currentUser.uid),
        subscribersCount: firebase.firestore.FieldValue.increment(-1)
      });
      chan.subscriberUids = (chan.subscriberUids || []).filter(u => u !== currentUser.uid);
      chan.subscribersCount = Math.max(0, (chan.subscribersCount || 1) - 1);
      showNotifToast('Unsubscribed from ' + chan.name, 'info');
    } else {
      await subRef.set({
        uid: currentUser.uid,
        joinedAt: Date.now()
      });
      await chanRef.update({
        subscriberUids: firebase.firestore.FieldValue.arrayUnion(currentUser.uid),
        subscribersCount: firebase.firestore.FieldValue.increment(1)
      });
      chan.subscriberUids = [...(chan.subscriberUids || []), currentUser.uid];
      chan.subscribersCount = (chan.subscribersCount || 0) + 1;
      showNotifToast('✓ Subscribed to ' + chan.name, 'success');
    }

    // Reflect the new state immediately in the local caches (the Discover
    // onSnapshot holds a stale array, so without this a Follow tap "does
    // nothing" — the button and Following list only update on the next reload).
    discoverChannels = discoverChannels.map(c => c.id === channelId ? { ...c, subscriberUids: chan.subscriberUids, subscribersCount: chan.subscribersCount } : c);
    const existingIdx = myChannels.findIndex(c => c.id === channelId);
    if (isSubscribed) {
      if (existingIdx > -1) myChannels.splice(existingIdx, 1);
    } else {
      if (existingIdx > -1) {
        myChannels[existingIdx] = { ...myChannels[existingIdx], ...chan };
      } else {
        myChannels.push({ ...chan });
      }
    }
    renderChannelsDiscover();
    renderChannelsFollowing();
    if (selectedChannel && selectedChannel.id === channelId) {
      selectedChannel = { ...selectedChannel, subscriberUids: chan.subscriberUids, subscribersCount: chan.subscribersCount };
      const statusEl = document.getElementById('status');
      if (statusEl) statusEl.textContent = `${formatCount(chan.subscribersCount || 0)} subscribers • ${chan.handle || ''}`;
    }
  } catch (err) {
    showNotifToast('Subscription error: ' + err.message, 'error');
  }
}

// ── Select and Open Channel Broadcast Feed ─────────────────────────────────
function flushAllPendingPostReactions() {
  const pending = [..._postReactionTimers.keys()];
  for (const postId of pending) {
    const timer = _postReactionTimers.get(postId);
    if (timer) clearTimeout(timer);
    _postReactionTimers.delete(postId);
    flushPostReaction(postId);
  }
}

function selectChannelFeed(channel) {
  // Persist any debounced reaction writes before leaving this channel feed.
  flushAllPendingPostReactions();
  markCommunitySeen('c_' + channel.id);
  updateCommunityTabDot();
  if (unsubMessagesA) { unsubMessagesA(); unsubMessagesA = null; }
  if (unsubMessagesB) { unsubMessagesB(); unsubMessagesB = null; }
  if (unsubTyping) { unsubTyping(); unsubTyping = null; }
  if (unsubChatPresence) { unsubChatPresence(); unsubChatPresence = null; }
  if (unsubGroupMessages) { unsubGroupMessages(); unsubGroupMessages = null; }
  if (unsubChannelPosts) { unsubChannelPosts(); unsubChannelPosts = null; }

  currentChatMode = 'channel';
  selectedChannel = channel;
  selectedGroup = null;
  selectedUser = null;

  // Header configuration: strictly hide call buttons
  const voiceBtn = document.getElementById('headerVoiceCallBtn');
  const videoBtn = document.getElementById('headerVideoCallBtn');
  const infoBtn = document.getElementById('headerInfoBtn');

  if (voiceBtn) voiceBtn.style.display = 'none';
  if (videoBtn) videoBtn.style.display = 'none';
  if (infoBtn) infoBtn.style.display = 'flex';
  updatePollButtonVisibility();

  document.getElementById('chatName').textContent = channel.name || 'Channel';
  setChatHandle("");
  document.getElementById('chatPic').src = channel.avatarUrl || 'https://i.imgur.com/HeIi0wU.png';
  document.getElementById('chatStatusDot').style.display = 'none';
  document.getElementById('status').textContent = `${formatCount(channel.subscribersCount || 1)} subscribers • ${channel.handle || ''}`;

  // Broadcast-only input bar logic
  const isAdmin = (channel.admins || []).includes(currentUser.uid) || channel.ownerUid === currentUser.uid;
  const inputArea = document.querySelector('.input-area');
  let noticeEl = document.getElementById('commBroadcastNotice');

  if (isAdmin) {
    if (inputArea) inputArea.style.display = 'flex';
    if (noticeEl) noticeEl.style.display = 'none';
    const ta = document.getElementById('text');
    if (ta) ta.placeholder = 'Broadcast an announcement to subscribers…';
  } else {
    if (inputArea) inputArea.style.display = 'none';
    if (!noticeEl) {
      noticeEl = document.createElement('div');
      noticeEl.id = 'commBroadcastNotice';
      noticeEl.className = 'comm-broadcast-notice';
      inputArea.parentNode.insertBefore(noticeEl, inputArea);
    }
    noticeEl.innerHTML = '📢 <span>This channel is broadcast-only. You can comment on individual posts.</span>';
    noticeEl.style.display = 'block';
  }

  // Mobile drawer
  if (window.innerWidth <= 768) {
    const app = document.getElementById('app');
    if (app) app.classList.add('chat-open-mobile');
    document.body.classList.add('chat-open-mobile');
    const bNav = document.getElementById('mobileBottomNav');
    if (bNav) bNav.classList.add('hidden');
  }

  // Live listener for channel document updates (settings, subscriber count, avatar, admins)
  if (unsubChannelDoc) { unsubChannelDoc(); unsubChannelDoc = null; }
  unsubChannelDoc = db.collection('channels').doc(channel.id).onSnapshot(docSnap => {
    if (!docSnap.exists) return;
    selectedChannel = { id: docSnap.id, ...docSnap.data() };
    document.getElementById('chatName').textContent = selectedChannel.name || 'Channel';
    document.getElementById('chatPic').src = selectedChannel.avatarUrl || 'https://i.imgur.com/HeIi0wU.png';
    document.getElementById('status').textContent = `${formatCount(selectedChannel.subscribersCount || 1)} subscribers • ${selectedChannel.handle || ''}`;

    // Sync pinned announcement banner for channel
    syncCommPinnedBanner();

    // If the comments setting flips, re-render posts so the user comment button/appears-disappears live
    const prevCommentsAllowed = window._nexaPrevChannelCommentsAllowed;
    const nowCommentsAllowed = (selectedChannel.settings && selectedChannel.settings.allowComments) !== false;
    if (prevCommentsAllowed !== undefined && prevCommentsAllowed !== nowCommentsAllowed) {
      const cachedPosts = window._nexaChannelPostsCache;
      if (Array.isArray(cachedPosts) && cachedPosts.length) renderChannelPostsList(cachedPosts);
    }
    window._nexaPrevChannelCommentsAllowed = nowCommentsAllowed;

    // Update input area if admin status changed
    const nowAdmin = (selectedChannel.admins || []).includes(currentUser.uid) || selectedChannel.ownerUid === currentUser.uid;
    if (nowAdmin) {
      if (inputArea) inputArea.style.display = 'flex';
      if (noticeEl) noticeEl.style.display = 'none';
    } else {
      if (inputArea) inputArea.style.display = 'none';
      if (noticeEl) noticeEl.style.display = 'block';
    }
  }, err => console.warn('Channel doc sync error:', err));

  loadChannelPosts(channel.id);
  if (window.lucide) lucide.createIcons();
}

function loadChannelPosts(channelId) {
  if (unsubChannelPosts) unsubChannelPosts();
  const box = document.getElementById('messages');
  if (box) box.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-3);">Loading broadcast posts…</div>';

  unsubChannelPosts = db.collection('channels').doc(channelId).collection('posts')
    .orderBy('createdAt', 'desc')
    .limit(40)
    .onSnapshot(snap => {
      const posts = [];
      snap.forEach(doc => {
        posts.push({ id: doc.id, ...doc.data() });
      });
      posts.sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));
      const prev = window._nexaChannelPostsCache || [];
      // Reactions-only delta? Patch just the pill strips in place instead of
      // re-rendering the whole list (no scroll jump, and recordPostView isn't
      // re-fired for every post). Full render only when content/comments/views
      // actually change.
      let reactionsOnly = prev.length === posts.length;
      if (reactionsOnly) {
        for (let i = 0; i < posts.length; i++) {
          const a = prev.find(p => p.id === posts[i].id);
          if (!a) { reactionsOnly = false; break; }
          const keys = new Set([...Object.keys(a), ...Object.keys(posts[i])]);
          for (const k of keys) {
            if (k === 'reactions') continue;
            const va = a[k];
            const vb = posts[i][k];
            const same = va === vb || (typeof va === 'object' && va !== null && JSON.stringify(va) === JSON.stringify(vb));
            if (!same) { reactionsOnly = false; break; }
          }
          if (!reactionsOnly) break;
        }
      }
      window._nexaChannelPostsCache = posts;
      if (reactionsOnly) {
        for (const p of posts) {
          const prevP = prev.find(x => x.id === p.id);
          if (prevP && JSON.stringify(prevP.reactions || {}) !== JSON.stringify(p.reactions || {})) {
            patchChannelPostReactions(p.id, p.reactions || {});
          }
        }
        return;
      }
      renderChannelPostsList(posts);
    }, err => {
      console.error('Channel posts error:', err);
    });
}

function renderChannelPostsList(posts) {
  const box = document.getElementById('messages');
  if (!box) return;

  if (!posts.length) {
    box.innerHTML = `
      <div class="empty-chat">
        <div class="empty-chat-icon">📢</div>
        <div style="font-size: 16px; font-weight: 600; margin-bottom: 6px;">${escapeHtml(selectedChannel?.name || 'Channel')}</div>
        <div style="font-size: 13px; color: var(--text-3);">No posts have been broadcast yet. Stay tuned for announcements!</div>
      </div>
    `;
    return;
  }

  box.innerHTML = '';
  posts.forEach(post => {
    // Record view impression accurately per unique user
    recordPostView(post);

    const wrap = document.createElement('div');
    wrap.className = 'channel-post-wrap';

    let mediaHtml = '';
    if (post.type === 'poll' && post.pollOptions) {
      mediaHtml = buildPollHTML(post.id, post);
    } else if (post.image) {
      mediaHtml = `<div class="channel-post-media" onclick="openProfilePic('${post.image}')"><img src="${escapeHtml(post.image)}" loading="lazy"></div>`;
    } else if (post.video) {
      mediaHtml = `<div class="channel-post-media"><video src="${escapeHtml(post.video)}" controls></video></div>`;
    } else if (post.audio) {
      mediaHtml = `<div class="channel-post-audio" style="margin: 8px 0; padding: 10px 14px; background: rgba(255,255,255,0.06); border-radius: 12px; display: flex; align-items: center; gap: 10px;">
        <span style="font-size: 20px;">🎤</span>
        <audio controls src="${safeMediaUrl(post.audio)}" style="width: 100%; height: 36px;"></audio>
      </div>`;
    }

    const isChannelAdmin = selectedChannel && (selectedChannel.ownerUid === currentUser.uid || (selectedChannel.admins || []).includes(currentUser.uid));
    const commentsAllowedForAll = (selectedChannel.settings && selectedChannel.settings.allowComments) !== false;
    const commentsOnThisPost = (post.commentsEnabled !== false) && commentsAllowedForAll;

    // Viewer count (👁️) is admin-only — regular subscribers shouldn't see
    // how many people viewed a broadcast post.
    const uniqueViews = (post.viewsUids && Array.isArray(post.viewsUids) && post.viewsUids.length) ? post.viewsUids.length : (post.viewsCount || 1);
    const views = formatCount(uniqueViews);
    const commentsCount = post.commentsCount || 0;
    const timeStr = post.createdAt ? new Date(post.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '';

    const replyQuote = post.postReplyToComment;
    let replyQuoteHtml = '';
    if (replyQuote) {
      replyQuoteHtml = `
        <div class="channel-post-reply-quote">
          <div style="flex: 1; min-width: 0;">
            <div class="reply-quote-name"><i data-lucide="corner-down-right" style="width: 11px; height: 11px; vertical-align: middle; margin-right: 4px;"></i>${escapeHtml(replyQuote.authorName || 'User')}</div>
            <div class="reply-quote-text">${linkify(replyQuote.commentText || '')}</div>
          </div>
        </div>
      `;
    }

    wrap.innerHTML = `
      <div class="channel-post-card" id="post_${post.id}">
        <div class="channel-post-header">
          <div class="channel-post-author-box">
            <span class="channel-post-author-name">${escapeHtml(post.authorName || selectedChannel.name)}</span>
          </div>
          <div style="display: flex; align-items: center; gap: 8px;">
            <span class="channel-post-time">${timeStr}</span>
            ${isChannelAdmin ? `
              <button class="comm-icon-btn-xs" onclick="pinChannelPost('${post.id}')" title="Pin / Unpin post" style="opacity: 0.65;">
                <i data-lucide="pin" style="width: 12px; height: 12px;"></i>
              </button>
              <button class="comm-icon-btn-xs" onclick="openPostCommentsToggle('${post.id}')" title="Comments on/off for this post" style="opacity: 0.65;"><i data-lucide="message-circle" style="width: 12px; height: 12px;"></i></button>
              <button class="comm-icon-btn-xs" onclick="deleteChannelPost('${post.id}')" title="Delete this post" style="opacity: 0.65;"><i data-lucide="trash-2" style="width: 12px; height: 12px;"></i></button>
            ` : ''}
          </div>
        </div>
        ${replyQuoteHtml}
        ${post.forwarded ? `<div class="forwarded-label" style="margin-bottom: 6px;"><i data-lucide="forward" style="width: 12px; height: 12px;"></i> Forwarded${post.forwardedFrom ? ` from ${escapeHtml(post.forwardedFrom)}` : ''}</div>` : ''}
        ${mediaHtml}
        <div class="channel-post-text">${formatMessageText(post.text || '')}</div>
        <div class="channel-post-footer">
          ${isChannelAdmin ? `
          <div class="channel-post-views">
            <i data-lucide="eye" style="width:13px;height:13px;"></i>
            <span>${views}</span>
          </div>
          ` : '<div></div>'}
          <div class="channel-post-actions">
            <div class="channel-reaction-pills" id="reactPills_${post.id}">${buildChannelReactionPillsHTML(post)}</div>
            <div style="position: relative; display: inline-flex;">
              <button class="channel-reaction-pill" onclick="toggleChannelQuickReactMenu('${post.id}')" title="React with emoji">
                <i data-lucide="smile-plus" style="width:13px;height:13px;"></i>
              </button>
              <div class="channel-quick-react-popup" id="quickReact_${post.id}">
                <button class="qr-emoji-btn" onclick="toggleChannelPostReaction('${post.id}', '❤️')">❤️</button>
                <button class="qr-emoji-btn" onclick="toggleChannelPostReaction('${post.id}', '👍')">👍</button>
                <button class="qr-emoji-btn" onclick="toggleChannelPostReaction('${post.id}', '🔥')">🔥</button>
                <button class="qr-emoji-btn" onclick="toggleChannelPostReaction('${post.id}', '👏')">👏</button>
                <button class="qr-emoji-btn" onclick="toggleChannelPostReaction('${post.id}', '😂')">😂</button>
                <button class="qr-emoji-btn" onclick="toggleChannelPostReaction('${post.id}', '😮')">😮</button>
              </div>
            </div>
            ${isChannelAdmin ? `
              <button class="channel-comments-btn" onclick="openChannelComments('${post.id}')" title="Discussion Comments">
                <i data-lucide="message-circle" style="width:13px;height:13px;"></i>
                <span>${commentsCount} Comments</span>
              </button>
            ` : commentsOnThisPost ? `
              <button class="channel-reaction-pill user-comment-btn" onclick="toggleUserChannelComment('${post.id}')" title="Comment on this post">
                <i data-lucide="message-circle" style="width:13px;height:13px;"></i>
                <span>Comment</span>
              </button>
            ` : ''
            }
            <button class="channel-reaction-pill" onclick="shareChannelPost('${post.id}')" title="Forward / Share Post">
              <i data-lucide="share-2" style="width:12px;height:12px;"></i>
              <span>Share</span>
            </button>
          </div>
        </div>
        ${!isChannelAdmin && commentsOnThisPost ? `
        <div class="channel-user-comment-box" id="userCommentBox_${post.id}" style="display:none;">
          <textarea id="userCommentInput_${post.id}" class="user-comment-input" rows="1" maxlength="1000" placeholder="Write a comment…" onkeydown="handleUserCommentKey(event, '${post.id}')"></textarea>
          <button class="comments-send-btn" onclick="submitUserChannelComment('${post.id}')" title="Send"><i data-lucide="send" style="width: 16px; height: 16px;"></i></button>
        </div>
        ` : ''
        }
      </div>
    `;
    box.appendChild(wrap);
  });

  scrollMessagesToBottom();
  if (window.lucide) lucide.createIcons();
}

async function deleteChannelPost(postId) {
  if (!selectedChannel || !postId) return;
  const isAdmin = (selectedChannel.admins || []).includes(currentUser.uid) || selectedChannel.ownerUid === currentUser.uid;
  if (!isAdmin) {
    showNotifToast('Only channel admins can delete posts', 'error');
    return;
  }
  if (!confirm('Delete this broadcast post? This cannot be undone.')) return;
  try {
    await db.collection('channels').doc(selectedChannel.id).collection('posts').doc(postId).delete();
    const cachedPosts = window._nexaChannelPostsCache || [];
    window._nexaChannelPostsCache = cachedPosts.filter(p => p.id !== postId);
    showNotifToast('✓ Post deleted', 'success');
    const remaining = cachedPosts.filter(p => p.id !== postId);
    if (remaining.length) renderChannelPostsList(remaining);
  } catch (err) {
    showNotifToast('Failed to delete post: ' + err.message, 'error');
  }
}

async function recordPostView(post) {
  if (!selectedChannel || !currentUser || !post || !post.id) return;
  const postId = post.id;
  const uids = post.viewsUids || [];
  if (uids.includes(currentUser.uid)) return;

  const key = `nexa_view_${selectedChannel.id}_${postId}_${currentUser.uid}`;
  if (sessionStorage.getItem(key)) return;
  sessionStorage.setItem(key, '1');

  try {
    await db.collection('channels').doc(selectedChannel.id).collection('posts').doc(postId).update({
      viewsUids: firebase.firestore.FieldValue.arrayUnion(currentUser.uid),
      viewsCount: firebase.firestore.FieldValue.increment(1)
    });
  } catch (e) {
    console.warn('recordPostView error:', e);
  }
}

async function shareChannelPost(postId) {
  if (!selectedChannel) return;
  const card = document.getElementById(`post_${postId}`);
  const text = card ? (card.querySelector('.channel-post-text')?.textContent || '').trim() : '';
  const shareData = {
    title: `${selectedChannel.name} broadcast on Nexa`,
    text: `"${text}" — via ${selectedChannel.name} (${selectedChannel.handle || ''})`,
    url: window.location.href
  };

  if (navigator.share) {
    try {
      await navigator.share(shareData);
      showNotifToast('✓ Shared broadcast post', 'success');
      return;
    } catch (e) {
      if (e.name === 'AbortError') return;
    }
  }

  // Fallback to clipboard
  const copyStr = `${shareData.text}\n${shareData.url}`;
  if (navigator.clipboard) {
    await navigator.clipboard.writeText(copyStr);
    showNotifToast('✓ Post text copied to clipboard to share!', 'success');
  } else {
    showNotifToast('Sharing not supported on this browser', 'info');
  }
}

async function sendChannelPost() {
  if (!selectedChannel) return;
  const ta = document.getElementById('text');
  const text = ta.value.trim();
  if (!text) return;
  ta.value = '';
  ta.style.height = 'auto';
  toggleActionButtons();

  await sendChannelPostWithExtras({ text });
}

async function sendChannelPostWithExtras(extras) {
  if (!selectedChannel) return;
  try {
    const authorName = document.getElementById('myName').textContent || currentUser.displayName || selectedChannel.name;
    const postPayload = {
      authorUid: currentUser.uid,
      authorName,
      createdAt: Date.now(),
      viewsUids: [currentUser.uid],
      viewsCount: 1,
      commentsCount: 0,
      reactions: {},
      ...extras
    };

    const docRef = await db.collection('channels').doc(selectedChannel.id).collection('posts').add(postPayload);
    sessionStorage.setItem(`nexa_view_${selectedChannel.id}_${docRef.id}_${currentUser.uid}`, '1');

    await db.collection('channels').doc(selectedChannel.id).update({
      lastPostTime: Date.now()
    });
  } catch (err) {
    showNotifToast('Failed to broadcast: ' + err.message, 'error');
  }
}

function toggleChannelQuickReactMenu(postId) {
  const popup = document.getElementById(`quickReact_${postId}`);
  if (!popup) return;
  const isShown = popup.classList.contains('active');
  document.querySelectorAll('.channel-quick-react-popup.active').forEach(p => p.classList.remove('active'));
  if (!isShown) {
    popup.classList.add('active');
    const dismissHandler = (e) => {
      if (!popup.contains(e.target)) {
        popup.classList.remove('active');
        document.removeEventListener('click', dismissHandler);
      }
    };
    setTimeout(() => document.addEventListener('click', dismissHandler), 10);
  }
}

// Build just the channel post reaction pill strip (counts + "me" highlight).
// Kept standalone so a reaction can patch ONLY the pills in place instead of
// re-rendering the whole post list (which also re-fires recordPostView tour).
function buildChannelReactionPillsHTML(post) {
  if (!post) return "";
  const rMap = post.reactions || {};
  const myUid = currentUser ? currentUser.uid : '';
  return Object.keys(rMap).map(em => {
    const val = rMap[em];
    const count = Array.isArray(val) ? val.length : (typeof val === 'number' ? val : 0);
    const hasReacted = Array.isArray(val) && myUid && val.includes(myUid);
    if (count <= 0) return '';
    return `<button class="channel-reaction-pill ${hasReacted ? 'me-reacted' : ''}" onclick="toggleChannelPostReaction('${post.id}', '${em}')"><span>${em}</span> <span>${count}</span></button>`;
  }).join('');
}

// Swap the pill strip for one post in place (instant feedback, no scroll jump).
function patchChannelPostReactions(postId, reactions) {
  const host = document.getElementById(`reactPills_${postId}`);
  if (!host) return;
  host.innerHTML = buildChannelReactionPillsHTML({ id: postId, reactions: reactions || {} });
}

// Per-post debounced reaction persistence. The DOM gets patched instantly on
// tap; the Firestore write is coalesced (one transaction per post after a short
// idle window) so rapidly tapping several emoji on one post doesn't trigger a
// read-modify-write transaction per tap.
const _pendingPostReactRevs = new Map(); // postId -> "1"/"2"... per-call revision
const _postReactionTimers = new Map();   // postId -> timeout id
function postReactionRevision(postId) {
  const cur = (_pendingPostReactRevs.get(postId) || 0) + 1;
  _pendingPostReactRevs.set(postId, cur);
  return cur;
}
function schedulePostReactionFlush(postId) {
  if (_postReactionTimers.has(postId)) clearTimeout(_postReactionTimers.get(postId));
  const timer = setTimeout(() => {
    _postReactionTimers.delete(postId);
    flushPostReaction(postId);
  }, 300);
  _postReactionTimers.set(postId, timer);
}
async function flushPostReaction(postId) {
  if (!selectedChannel || !currentUser) return;
  try {
    const myUid = currentUser.uid;
    const postRef = db.collection('channels').doc(selectedChannel.id).collection('posts').doc(postId);
    await db.runTransaction(async (transaction) => {
      const postDoc = await transaction.get(postRef);
      if (!postDoc.exists) return;
      const serverReactions = (postDoc.data() || {}).reactions || {};
      // Our local intent for each emoji (aggregated during the debounce window).
      const cached = (window._nexaChannelPostsCache || []).find(p => p.id === postId);
      const localReactions = (cached && cached.reactions) || {};

      // Merge on the transaction's fresh server read so a concurrent reaction
      // by another user survives: base each emoji list on the server's, then
      // add/remove ONLY our uid to match what we tapped locally.
      const allEmojis = new Set([...Object.keys(serverReactions), ...Object.keys(localReactions)]);
      const final = {};
      for (const em of allEmojis) {
        // Legacy numeric-count format ("❤️": 3) — treat as empty list; the
        // current-app writes always use uid arrays, and the array migration
        // has been in place across releases.
        const rawBase = serverReactions[em];
        let baseList = Array.isArray(rawBase) ? [...rawBase] : [];
        const wantMine = Array.isArray(localReactions[em]) && localReactions[em].includes(myUid);
        const hasMine = baseList.includes(myUid);
        if (wantMine && !hasMine) baseList.push(myUid);
        else if (!wantMine && hasMine) baseList.splice(baseList.indexOf(myUid), 1);
        if (baseList.length) final[em] = baseList;
      }
      transaction.update(postRef, { reactions: final });
    });
  } catch (e) {
    console.error('Flush reaction error:', e);
  }
}

function toggleChannelPostReaction(postId, emoji) {
  if (!selectedChannel || !currentUser) return;
  const popup = document.getElementById(`quickReact_${postId}`);
  if (popup) popup.classList.remove('active');

  // Optimistic local patch so the tap reflects instantly on this device;
  // the debounced flush (and subsequent snapshot) reconcile on the server.
  const cachedIdx = (window._nexaChannelPostsCache || []).findIndex(p => p.id === postId);
  if (cachedIdx > -1) {
    const post = window._nexaChannelPostsCache[cachedIdx];
    const cur = { ...(post.reactions || {}) };
    const emojiList = cur[emoji] && Array.isArray(cur[emoji]) ? [...cur[emoji]] : [];
    const idx = emojiList.indexOf(currentUser.uid);
    if (idx > -1) {
      emojiList.splice(idx, 1);
    } else {
      emojiList.push(currentUser.uid);
    }
    if (emojiList.length) {
      cur[emoji] = emojiList;
    } else {
      delete cur[emoji];
    }
    post.reactions = cur;
    patchChannelPostReactions(postId, cur);
  }

  postReactionRevision(postId);
  schedulePostReactionFlush(postId);
}

// ── Channel Post Discussion Comments Drawer ────────────────────────────────

// Lightweight comment composer for regular (non-admin) subscribers. Users
// can drop a comment on a post but the full discussion drawer is admin-only.
let activeUserCommentPostId = null;

function toggleUserChannelComment(postId) {
  if (!selectedChannel || !currentUser) return;
  const postMeta = (window._nexaChannelPostsCache || []).find(p => p.id === postId);
  const commentsOnThisPost = ((selectedChannel.settings && selectedChannel.settings.allowComments) !== false) && (postMeta ? postMeta.commentsEnabled !== false : true);
  if (!commentsOnThisPost) {
    showNotifToast('Comments are disabled on this post', 'error');
    return;
  }
  const box = document.getElementById(`userCommentBox_${postId}`);
  if (!box) return;

  // Keep only one inline composer open at a time
  if (activeUserCommentPostId && activeUserCommentPostId !== postId) {

    const prev = document.getElementById(`userCommentBox_${activeUserCommentPostId}`);
    if (prev) prev.style.display = 'none';
  }

  if (box.style.display === 'none') {
    box.style.display = 'flex';
    activeUserCommentPostId = postId;
    const ta = document.getElementById(`userCommentInput_${postId}`);
    if (ta) { ta.focus(); ta.placeholder = 'Write a comment…'; }
  } else {
    box.style.display = 'none';
    if (activeUserCommentPostId === postId) activeUserCommentPostId = null;
  }
}

function handleUserCommentKey(e, postId) {
  if (e.key === 'Enter' && !e.shiftKey) {
    e.preventDefault();
    submitUserChannelComment(postId);
  }
}

async function submitUserChannelComment(postId) {
  if (!selectedChannel || !currentUser) return;
  if (activeUserCommentPostId) {
    const prev = document.getElementById(`userCommentBox_${activeUserCommentPostId}`);
    if (prev) prev.style.display = 'none';
  }

  const ta = document.getElementById(`userCommentInput_${postId}`);
  const text = ta ? ta.value.trim() : '';
  if (!text) return;
  ta.value = '';

  try {
    const postRef = db.collection('channels').doc(selectedChannel.id).collection('posts').doc(postId);
    const postSnap = await postRef.get();
    const postOwner = postSnap.exists ? postSnap.data().authorUid : null;

    await postRef.collection('comments').add({
      authorUid: currentUser.uid,
      authorName: document.getElementById('myName').textContent || currentUser.displayName || 'Subscriber',
      authorAvatar: currentUser.photoURL || 'https://i.imgur.com/HeIi0wU.png',
      text,
      createdAt: Date.now()
    });

    await postRef.update({
      commentsCount: firebase.firestore.FieldValue.increment(1)
    });

    showNotifToast('✓ Comment posted', 'success');
  } catch (err) {
    showNotifToast('Failed to post comment: ' + err.message, 'error');
  }
}

let pendingChannelCommentReply = null; // { commentId, authorName, commentText, postId }

function openChannelComments(postId) {
  if (!selectedChannel) return;
  const isOwner = selectedChannel.ownerUid === currentUser.uid;
  const isAdmin = isOwner || (selectedChannel.admins || []).includes(currentUser.uid);
  if (!isAdmin) {
    showNotifToast('Discussion comments are accessible to channel admins only', 'info');
    return;
  }

  activeCommentPostId = postId;
  cancelChannelCommentReply();
  const drawer = document.getElementById('channelCommentsDrawer');
  if (!drawer) return;

  const postEl = document.getElementById(`post_${postId}`);
  const postText = postEl ? postEl.querySelector('.channel-post-text')?.textContent || 'Discussion' : 'Discussion';
  document.getElementById('commentsOriginalPostSnippet').textContent = postText;

  const commentsAllowed = selectedChannel.settings?.allowComments !== false;
  const postMeta = (window._nexaChannelPostsCache || []).find(p => p.id === activeCommentPostId);
  const commentsOnThisPost = commentsAllowed && (postMeta ? postMeta.commentsEnabled !== false : true);

  // Toggle input bar or notice depending on channel setting
  const inputBar = drawer.querySelector('.comments-input-bar');
  let disabledNotice = document.getElementById('channelCommentsDisabledNotice');
  if (!commentsOnThisPost && !isAdmin) {
    if (inputBar) inputBar.style.display = 'none';
    if (!disabledNotice) {
      disabledNotice = document.createElement('div');
      disabledNotice.id = 'channelCommentsDisabledNotice';
      disabledNotice.className = 'comm-broadcast-notice';
      disabledNotice.style.cssText = 'padding: 14px; text-align: center; color: var(--text-3); font-size: 12.5px; border-top: 1px solid var(--border-dk);';
      drawer.appendChild(disabledNotice);
    }
    disabledNotice.innerHTML = '🔒 <span>Comments are disabled by the channel admin.</span>';
    disabledNotice.style.display = 'block';
  } else {
    if (inputBar) inputBar.style.display = 'flex';
    if (disabledNotice) disabledNotice.style.display = 'none';
  }

  loadChannelComments(postId);
  drawer.classList.add('open');
  if (window.lucide) lucide.createIcons();
}

function closeChannelComments() {
  if (unsubChannelComments) { unsubChannelComments(); unsubChannelComments = null; }
  activeCommentPostId = null;
  cancelChannelCommentReply();
  const drawer = document.getElementById('channelCommentsDrawer');
  if (drawer) drawer.classList.remove('open');
}

function prepareChannelCommentReply(commentId, authorName, commentText) {
  pendingChannelCommentReply = {
    commentId,
    authorName,
    commentText,
    postId: activeCommentPostId
  };
  const bar = document.getElementById('channelCommentReplyBar');
  if (bar) {
    const titleEl = document.getElementById('channelCommentReplyTitle');
    const snippetEl = document.getElementById('channelCommentReplySnippet');
    if (titleEl) titleEl.textContent = `Reply to ${authorName} in channel`;
    if (snippetEl) snippetEl.textContent = commentText;
    bar.style.display = 'flex';
  }
  const input = document.getElementById('channelCommentInput');
  if (input) {
    input.focus();
    input.placeholder = `Reply to ${authorName} and broadcast to channel…`;
  }
  if (window.lucide) lucide.createIcons();
}

function cancelChannelCommentReply() {
  pendingChannelCommentReply = null;
  const bar = document.getElementById('channelCommentReplyBar');
  if (bar) bar.style.display = 'none';
  const input = document.getElementById('channelCommentInput');
  if (input) input.placeholder = 'Write a comment…';
}

function loadChannelComments(postId) {
  if (unsubChannelComments) unsubChannelComments();
  const list = document.getElementById('channelCommentsList');
  if (list) list.innerHTML = '<div style="color:var(--text-3);text-align:center;padding:20px;">Loading comments…</div>';

  unsubChannelComments = db.collection('channels').doc(selectedChannel.id).collection('posts').doc(postId).collection('comments')
    .orderBy('createdAt', 'asc')
    .onSnapshot(snap => {
      const comments = [];
      snap.forEach(d => comments.push({ id: d.id, ...d.data() }));
      renderChannelComments(comments);
    }, err => console.error(err));
}

function renderChannelComments(comments) {
  const list = document.getElementById('channelCommentsList');
  if (!list) return;

  if (!comments.length) {
    list.innerHTML = '<div class="comments-empty">No comments yet. Start the conversation!</div>';
    return;
  }

  const isOwner = selectedChannel.ownerUid === currentUser.uid;
  const isAdmin = isOwner || (selectedChannel.admins || []).includes(currentUser.uid);

  list.innerHTML = comments.map(c => {
    const avatar = c.authorAvatar || 'https://i.imgur.com/HeIi0wU.png';
    const timeStr = c.createdAt ? new Date(c.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '';
    const canDelete = c.authorUid === currentUser.uid || isAdmin;

    let deleteBtn = '';
    if (canDelete) {
      deleteBtn = `
        <button class="comm-icon-btn-xs" style="opacity: 0.6;" onclick="deleteChannelComment('${c.id}')" title="Delete comment">
          <i data-lucide="trash" style="width: 12px; height: 12px; color: var(--danger);"></i>
        </button>
      `;
    }

    let replyBtn = '';
    if (isAdmin) {
      const safeAuthor = (c.authorName || 'User').replace(/'/g, "\\'");
      const rawText = (c.text || '').replace(/\r?\n/g, ' ').replace(/'/g, "\\'").replace(/"/g, '&quot;');
      replyBtn = `
        <button class="comm-icon-btn-xs" style="color: var(--primary);" onclick="prepareChannelCommentReply('${c.id}', '${safeAuthor}', '${rawText}')" title="Reply to Channel (WhatsApp style)">
          <i data-lucide="corner-up-left" style="width: 13px; height: 13px;"></i>
          <span style="font-size: 11px; margin-left: 3px; font-weight: 600;">Reply to Channel</span>
        </button>
      `;
    }

    return `
      <div class="comment-card" id="comment_${c.id}">
        <img src="${escapeHtml(avatar)}" class="comment-avatar" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
        <div class="comment-body">
          <div class="comment-author-row">
            <span class="comment-author-name">${escapeHtml(c.authorName || 'User')}</span>
            <span class="comment-time">${timeStr}</span>
            <div style="margin-left: auto; display: flex; align-items: center; gap: 6px;">
              ${replyBtn}
              ${deleteBtn}
            </div>
          </div>
          <div class="comment-text">${formatMessageText(c.text || '')}</div>
        </div>
      </div>
    `;
  }).join('');

  list.scrollTop = list.scrollHeight;
  if (window.lucide) lucide.createIcons();
}

async function deleteChannelComment(commentId) {
  if (!selectedChannel || !activeCommentPostId) return;
  if (!confirm('Delete this comment?')) return;
  try {
    const postRef = db.collection('channels').doc(selectedChannel.id).collection('posts').doc(activeCommentPostId);
    await postRef.collection('comments').doc(commentId).delete();
    await postRef.update({
      commentsCount: firebase.firestore.FieldValue.increment(-1)
    });
    showNotifToast('✓ Comment deleted', 'info');
  } catch (e) {
    showNotifToast('Failed to delete comment: ' + e.message, 'error');
  }
}

function handleCommentInputKey(e) {
  if (e.key === 'Enter' && !e.shiftKey) {
    e.preventDefault();
    submitChannelComment();
  }
}

async function submitChannelComment() {
  if (!selectedChannel || !activeCommentPostId) return;
  const isOwner = selectedChannel.ownerUid === currentUser.uid;
  const isAdmin = isOwner || (selectedChannel.admins || []).includes(currentUser.uid);
  const commentsAllowed = selectedChannel.settings?.allowComments !== false;
  const postMeta = (window._nexaChannelPostsCache || []).find(p => p.id === activeCommentPostId);
  const commentsOnThisPost = commentsAllowed && (postMeta ? postMeta.commentsEnabled !== false : true);

  if (!commentsOnThisPost && !isAdmin) {
    showNotifToast('Comments are disabled on this post', 'error');
    return;
  }

  const input = document.getElementById('channelCommentInput');
  const text = input.value.trim();
  if (!text) return;
  input.value = '';

  const replyContext = pendingChannelCommentReply;
  cancelChannelCommentReply();

  try {
    const postRef = db.collection('channels').doc(selectedChannel.id).collection('posts').doc(activeCommentPostId);
    await postRef.collection('comments').add({
      authorUid: currentUser.uid,
      authorName: document.getElementById('myName').textContent || currentUser.displayName || 'Subscriber',
      authorAvatar: currentUser.photoURL || 'https://i.imgur.com/HeIi0wU.png',
      text,
      createdAt: Date.now()
    });

    await postRef.update({
      commentsCount: firebase.firestore.FieldValue.increment(1)
    });

    // If admin replied to a follower's comment, broadcast to the main channel feed (WhatsApp style)
    if (replyContext && isAdmin) {
      await sendChannelPostWithExtras({
        text,
        postReplyToComment: {
          commentId: replyContext.commentId,
          authorName: replyContext.authorName,
          commentText: replyContext.commentText,
          parentPostId: replyContext.postId
        }
      });
      showNotifToast('✓ Replied to channel broadcast!', 'success');
    }
  } catch (err) {
    showNotifToast('Failed to comment: ' + err.message, 'error');
  }
}

// ── Channel Info & Management Modal ────────────────────────────────────────
function openChannelInfo() {
  if (!selectedChannel) return;
  const modal = document.getElementById('channelInfoModal');
  if (!modal) return;

  document.getElementById('channelInfoAvatar').src = selectedChannel.avatarUrl || 'https://i.imgur.com/HeIi0wU.png';
  document.getElementById('channelInfoName').textContent = selectedChannel.name || 'Channel';
  document.getElementById('channelInfoHandle').textContent = selectedChannel.handle || '';
  document.getElementById('channelInfoMeta').textContent = `${formatCount(selectedChannel.subscribersCount || 1)} subscribers • ${selectedChannel.category || 'General'}`;
  document.getElementById('channelInfoDesc').textContent = selectedChannel.description || 'No description';

  const isOwner = selectedChannel.ownerUid === currentUser.uid;
  const isAdmin = isOwner || (selectedChannel.admins || []).includes(currentUser.uid);

  // Edit avatar and metadata visibility for admins/owners
  const avatarBadge = document.getElementById('channelAvatarEditBadge');
  if (avatarBadge) avatarBadge.style.display = isAdmin ? 'flex' : 'none';

  const editNameBtn = document.getElementById('channelEditNameBtn');
  if (editNameBtn) editNameBtn.style.display = isAdmin ? 'inline-flex' : 'none';

  const editDescBtn = document.getElementById('channelEditDescBtn');
  if (editDescBtn) editDescBtn.style.display = isAdmin ? 'inline-flex' : 'none';

  const adminSec = document.getElementById('channelAdminSection');
  if (adminSec) adminSec.style.display = isOwner ? 'block' : 'none';

  // Channel settings (e.g. comments toggle)
  const settingsSec = document.getElementById('channelSettingsSection');
  if (settingsSec) settingsSec.style.display = isAdmin ? 'block' : 'none';

  const commentsToggle = document.getElementById('channelCommentsToggle');
  if (commentsToggle) {
    const commentsAllowed = selectedChannel.settings?.allowComments !== false;
    commentsToggle.checked = commentsAllowed;
  }

  const perPostCommentsSection = document.getElementById('channelPerPostCommentsSection');
  const perPostCommentsToggle = document.getElementById('channelPerPostCommentsToggle');
  const perPostCommentsStatus = document.getElementById('channelPerPostCommentsStatus');
  if (perPostCommentsSection && perPostCommentsToggle && activeCommentPostId) {
    const cachedPost = (window._nexaChannelPostsCache || []).find(p => p.id === activeCommentPostId);
    const postCommentsEnabled = cachedPost ? (cachedPost.commentsEnabled !== false) : true;
    perPostCommentsSection.style.display = 'block';
    perPostCommentsToggle.checked = postCommentsEnabled;
    if (perPostCommentsStatus) perPostCommentsStatus.textContent = postCommentsEnabled ? 'Subscribers can comment on this post' : 'Comments are off for this post';
  } else if (perPostCommentsSection) {
    perPostCommentsSection.style.display = 'none';
  }
  const isSubscribed = (selectedChannel.subscriberUids || []).includes(currentUser.uid);
  const subBtn = document.getElementById('channelSubToggleBtn');
  if (subBtn) {
    subBtn.innerHTML = isSubscribed ? '<i data-lucide="bell-off" style="width:16px;height:16px;"></i> Unsubscribe' : '<i data-lucide="bell" style="width:16px;height:16px;"></i> Subscribe';
    subBtn.className = isSubscribed ? 'comm-btn-danger' : 'comm-action-btn-sm';
  }

  // Owner delete button
  const deleteBtn = document.getElementById('channelDeleteBtn');
  if (deleteBtn) deleteBtn.style.display = isOwner ? 'inline-flex' : 'none';

  // Render channel admins list
  const adminsList = document.getElementById('channelAdminsList');
  if (adminsList) {
    const adminUids = selectedChannel.admins || [selectedChannel.ownerUid];
    adminsList.innerHTML = adminUids.map(uid => {
      const uData = (allUsersData || []).find(u => u.uid === uid) || {};
      const name = uid === currentUser.uid ? 'You' : escapeHtml(uData.displayName || 'Admin');
      const photo = uData.photo || 'https://i.imgur.com/HeIi0wU.png';
      const isCreator = uid === selectedChannel.ownerUid;
      const roleText = isCreator ? 'Owner' : 'Admin';

      let dismissBtn = '';
      if (isOwner && !isCreator && uid !== currentUser.uid) {
        dismissBtn = `
          <button class="comm-icon-btn-xs" onclick="dismissChannelAdmin('${uid}')" title="Dismiss as admin" style="margin-left:auto;">
            <i data-lucide="user-x" style="width:13px;height:13px;color:var(--danger);"></i>
          </button>
        `;
      }

      return `
        <div class="comm-member-row">
          <img src="${escapeHtml(photo)}" class="comm-member-av" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
          <div class="comm-member-name">${name}</div>
          <div class="comm-role-chip ${isCreator ? 'owner' : 'admin'}">${roleText}</div>
          ${dismissBtn}
        </div>
      `;
    }).join('');
  }

  // Load WhatsApp-style Shared Media
  loadChannelSharedMedia();

  modal.classList.add('active');
  if (window.lucide) lucide.createIcons();
}

async function dismissChannelAdmin(targetUid) {
  if (!selectedChannel) return;
  const isOwner = selectedChannel.ownerUid === currentUser.uid;
  if (!isOwner) return;
  if (!confirm('Dismiss this user as channel admin?')) return;

  try {
    await db.collection('channels').doc(selectedChannel.id).update({
      admins: firebase.firestore.FieldValue.arrayRemove(targetUid)
    });
    selectedChannel.admins = (selectedChannel.admins || []).filter(u => u !== targetUid);
    showNotifToast('✓ Admin dismissed', 'info');
    openChannelInfo();
  } catch (e) {
    showNotifToast('Failed to dismiss admin: ' + e.message, 'error');
  }
}

async function toggleChannelCommentsSetting(allow) {
  if (!selectedChannel) return;
  const isOwner = selectedChannel.ownerUid === currentUser.uid;
  const isAdmin = isOwner || (selectedChannel.admins || []).includes(currentUser.uid);
  if (!isAdmin) {
    showNotifToast('Only channel admins can change this setting', 'error');
    return;
  }
  try {
    await db.collection('channels').doc(selectedChannel.id).update({
      'settings.allowComments': allow,
      updatedAt: Date.now()
    });
    selectedChannel.settings = selectedChannel.settings || {};
    selectedChannel.settings.allowComments = allow;
    showNotifToast(`✓ Comments ${allow ? 'enabled' : 'disabled'} for this channel`, 'success');
  } catch (err) {
    showNotifToast('Failed to update setting: ' + err.message, 'error');
  }
}

async function toggleChannelPostCommentsSetting(allow) {
  if (!selectedChannel || !activeCommentPostId) return;
  const isOwner = selectedChannel.ownerUid === currentUser.uid;
  const isAdmin = isOwner || (selectedChannel.admins || []).includes(currentUser.uid);
  if (!isAdmin) {
    showNotifToast('Only channel admins can change this setting', 'error');
    return;
  }
  try {
    await db.collection('channels').doc(selectedChannel.id).collection('posts').doc(activeCommentPostId).update({
      commentsEnabled: allow,
      updatedAt: Date.now()
    });
    const cache = window._nexaChannelPostsCache || [];
    window._nexaChannelPostsCache = cache.map(p => p.id === activeCommentPostId ? { ...p, commentsEnabled: allow, updatedAt: Date.now() } : p);
    if (document.getElementById('messages')) renderChannelPostsList(window._nexaChannelPostsCache || []);
    showNotifToast(`✓ Comments ${allow ? 'enabled' : 'disabled'} for this post`, 'success');
  } catch (err) {
    showNotifToast('Failed to update setting: ' + err.message, 'error');
  }
}
let postCommentsTogglePostId = null;
function openPostCommentsToggle(postId) {
  postCommentsTogglePostId = postId;
  activeCommentPostId = postId;
  const cached = (window._nexaChannelPostsCache || []).find(p => p.id === postId);
  const cb = document.getElementById('postCommentsToggleCheck');
  const modal = document.getElementById('postCommentsToggleModal');
  if (cb) cb.checked = cached ? cached.commentsEnabled !== false : true;
  if (modal) modal.classList.add('active');
  if (window.lucide) lucide.createIcons();
}
function closePostCommentsToggle() {
  const modal = document.getElementById('postCommentsToggleModal');
  if (modal) modal.classList.remove('active');
}
async function savePostCommentsToggle() {
  const cb = document.getElementById('postCommentsToggleCheck');
  // Use the post this modal was opened for, not the shared activeCommentPostId
  // (which the admin comments drawer can clobber while the modal is open).
  const postId = postCommentsTogglePostId || activeCommentPostId;
  if (!cb || !postId) return;
  // Preserve the captured post id for the settings write.
  activeCommentPostId = postId;
  closePostCommentsToggle();
  await toggleChannelPostCommentsSetting(cb.checked);
}
function triggerChangeChannelAvatar() {
  if (!selectedChannel) return;
  const isOwner = selectedChannel.ownerUid === currentUser.uid;
  const isAdmin = isOwner || (selectedChannel.admins || []).includes(currentUser.uid);
  if (!isAdmin) {
    showNotifToast('Only channel admins can change the channel icon', 'info');
    return;
  }
  const input = document.getElementById('channelChangeAvatarInput');
  if (input) input.click();
}

async function handleUpdateChannelAvatar(input) {
  const file = input.files[0];
  if (!file || !selectedChannel) return;
  if (!file.type.startsWith('image/')) {
    showNotifToast('Please choose an image file', 'error');
    return;
  }
  try {
    showNotifToast('Uploading channel icon…', 'info');
    const url = await uploadFile(file);
    if (!url) throw new Error('Upload failed');
    await db.collection('channels').doc(selectedChannel.id).update({ avatarUrl: url, updatedAt: Date.now() });
    selectedChannel.avatarUrl = url;
    document.getElementById('channelInfoAvatar').src = url;
    document.getElementById('chatPic').src = url;
    showNotifToast('✓ Channel icon updated!', 'success');
  } catch (e) {
    showNotifToast('Failed to update icon: ' + e.message, 'error');
  }
}

async function editChannelName() {
  if (!selectedChannel) return;
  const isOwner = selectedChannel.ownerUid === currentUser.uid;
  const isAdmin = isOwner || (selectedChannel.admins || []).includes(currentUser.uid);
  if (!isAdmin) {
    showNotifToast('Only channel admins can edit the name', 'info');
    return;
  }
  const newName = prompt('Enter new channel name:', selectedChannel.name || '');
  if (!newName || !newName.trim() || newName.trim() === selectedChannel.name) return;
  try {
    const val = newName.trim().slice(0, 50);
    await db.collection('channels').doc(selectedChannel.id).update({ name: val, updatedAt: Date.now() });
    selectedChannel.name = val;
    document.getElementById('channelInfoName').textContent = val;
    document.getElementById('chatName').textContent = val;
    setChatHandle("");
    showNotifToast('✓ Channel name updated', 'success');
  } catch (e) {
    showNotifToast('Error updating name: ' + e.message, 'error');
  }
}

async function editChannelDesc() {
  if (!selectedChannel) return;
  const isOwner = selectedChannel.ownerUid === currentUser.uid;
  const isAdmin = isOwner || (selectedChannel.admins || []).includes(currentUser.uid);
  if (!isAdmin) {
    showNotifToast('Only channel admins can edit description', 'info');
    return;
  }
  const newDesc = prompt('Enter channel description:', selectedChannel.description || '');
  if (newDesc === null) return;
  try {
    const val = newDesc.trim().slice(0, 300);
    await db.collection('channels').doc(selectedChannel.id).update({ description: val, updatedAt: Date.now() });
    selectedChannel.description = val;
    document.getElementById('channelInfoDesc').textContent = val || 'No description';
    showNotifToast('✓ Description updated', 'success');
  } catch (e) {
    showNotifToast('Error updating description: ' + e.message, 'error');
  }
}

function closeChannelInfo() {
  const modal = document.getElementById('channelInfoModal');
  if (modal) modal.classList.remove('active');
}

async function deleteCurrentChannel() {
  if (!selectedChannel) return;
  const isOwner = selectedChannel.ownerUid === currentUser.uid;
  if (!isOwner) {
    showNotifToast('Only the channel creator can delete this channel', 'error');
    return;
  }

  const channelName = selectedChannel.name || 'Channel';
  if (!confirm(`Permanently delete channel "${channelName}"? All posts and subscriber records will be removed. This cannot be undone.`)) return;

  try {
    const channelId = selectedChannel.id;
    await db.collection('channels').doc(channelId).delete();
    closeChannelInfo();
    resetCommunityChatMode();
    showNotifToast(`✓ Channel "${channelName}" deleted`, 'success');
  } catch (err) {
    showNotifToast('Error deleting channel: ' + err.message, 'error');
  }
}

async function toggleChannelSubscriptionFromInfo() {
  if (!selectedChannel) return;
  await toggleSubscribeChannel(selectedChannel.id);
  openChannelInfo();
}

async function shareCurrentChannel() {
  if (!selectedChannel) return;
  const shareData = {
    title: `Follow ${selectedChannel.name} on Nexa`,
    text: `Check out ${selectedChannel.name} (${selectedChannel.handle || ''}) on Nexa Messenger: ${selectedChannel.description || ''}`,
    url: window.location.href
  };

  if (navigator.share) {
    try {
      await navigator.share(shareData);
      showNotifToast('✓ Shared channel invite', 'success');
      return;
    } catch (e) {
      if (e.name === 'AbortError') return;
    }
  }

  const copyStr = `${shareData.text}\n${shareData.url}`;
  if (navigator.clipboard) {
    await navigator.clipboard.writeText(copyStr);
    showNotifToast('✓ Channel link copied to clipboard!', 'success');
  } else {
    showNotifToast('Sharing not supported on this browser', 'info');
  }
}

// ── Universal Community Info Router ────────────────────────────────────────
function openActiveCommunityInfo() {
  if (currentChatMode === 'group') {
    openGroupInfo();
  } else if (currentChatMode === 'channel') {
    openChannelInfo();
  }
}

function handleChatPeerClick() {
  if (currentChatMode === 'group') {
    openGroupInfo();
  } else if (currentChatMode === 'channel') {
    openChannelInfo();
  } else {
    openUserProfile();
  }
}

// ── Reset from Community mode to 1:1 Direct Chat ───────────────────────────
function resetCommunityChatMode() {
  currentChatMode = 'direct';
  selectedGroup = null;
  selectedChannel = null;
  currentGroupMessages = [];

  if (unsubGroupMessages) { unsubGroupMessages(); unsubGroupMessages = null; }
  if (unsubGroupDoc) { unsubGroupDoc(); unsubGroupDoc = null; }
  if (unsubChannelPosts) { unsubChannelPosts(); unsubChannelPosts = null; }
  if (unsubChannelComments) { unsubChannelComments(); unsubChannelComments = null; }
  if (unsubChannelDoc) { unsubChannelDoc(); unsubChannelDoc = null; }

  // Hide pinned banner
  const pinnedBanner = document.getElementById('commPinnedBanner');
  if (pinnedBanner) pinnedBanner.style.display = 'none';

  // Clear messages area
  const box = document.getElementById("messages");
  if (box) {
    box.innerHTML = `
      <div class="empty-chat">
        <div class="empty-chat-icon">💬</div>
        <div style="font-size: 16px; font-weight: 600; margin-bottom: 6px;">Select a conversation</div>
        <div style="font-size: 13px; color: var(--text-3);">Choose a chat or group from the left to start messaging.</div>
      </div>
    `;
  }
  document.getElementById("chatName").textContent = "Nexa Messenger";
  setChatHandle("");
  document.getElementById("chatPic").src = "https://i.imgur.com/HeIi0wU.png";
  document.getElementById("status").textContent = "";

  // Close mobile drawer if open
  if (window.innerWidth <= 768) {
    const app = document.getElementById('app');
    if (app) app.classList.remove('chat-open-mobile');
    document.body.classList.remove('chat-open-mobile');
    const bNav = document.getElementById('mobileBottomNav');
    if (bNav) bNav.classList.remove('hidden');
  }

  // Restore buttons depending on active direct chat
  const voiceBtn = document.getElementById('headerVoiceCallBtn');
  const videoBtn = document.getElementById('headerVideoCallBtn');
  const infoBtn = document.getElementById('headerInfoBtn');

  const showCalls = currentChatMode === 'direct' && !!selectedUser;
  if (voiceBtn) voiceBtn.style.display = showCalls ? 'flex' : 'none';
  if (videoBtn) videoBtn.style.display = showCalls ? 'flex' : 'none';
  if (infoBtn) infoBtn.style.display = (selectedUser || selectedGroup || selectedChannel) ? 'flex' : 'none';
  updatePollButtonVisibility();

  // Restore input area
  const inputArea = document.querySelector('.input-area');
  const noticeEl = document.getElementById('commBroadcastNotice');
  if (inputArea) inputArea.style.display = 'flex';
  if (noticeEl) noticeEl.style.display = 'none';
  const ta = document.getElementById('text');
  if (ta) ta.placeholder = 'Type a message…';
}

// ── WhatsApp-Style Pinned Messages & Shared Media ───────────────────────────
function syncCommPinnedBanner() {
  const banner = document.getElementById('commPinnedBanner');
  if (!banner) return;

  const currentObj = currentChatMode === 'group' ? selectedGroup : selectedChannel;
  if (!currentObj || !currentObj.pinnedMessage) {
    banner.style.display = 'none';
    return;
  }

  const pin = currentObj.pinnedMessage;
  const isOwnerOrAdmin = currentChatMode === 'group'
    ? (selectedGroup.ownerUid === currentUser.uid || (selectedGroup.admins || []).includes(currentUser.uid))
    : (selectedChannel.ownerUid === currentUser.uid || (selectedChannel.admins || []).includes(currentUser.uid));

  document.getElementById('commPinnedTitle').textContent = `Pinned by ${pin.authorName || 'Admin'}`;
  document.getElementById('commPinnedSnippet').textContent = pin.text || (pin.hasImage ? '📷 Photo' : pin.hasVideo ? '📹 Video' : 'Attachment');

  const unpinBtn = document.getElementById('commPinnedUnpinBtn');
  if (unpinBtn) unpinBtn.style.display = isOwnerOrAdmin ? 'flex' : 'none';

  banner.style.display = 'flex';
  if (window.lucide) lucide.createIcons();
}

function jumpToPinnedMessage() {
  const currentObj = currentChatMode === 'group' ? selectedGroup : selectedChannel;
  if (!currentObj || !currentObj.pinnedMessage) return;
  const targetId = currentObj.pinnedMessage.id;
  
  const el = currentChatMode === 'group'
    ? document.querySelector(`[data-msg-id="${targetId}"]`)
    : document.getElementById(`post_${targetId}`);

  if (el) {
    el.scrollIntoView({ behavior: 'smooth', block: 'center' });
    el.classList.add('msg-highlight');
    setTimeout(() => el.classList.remove('msg-highlight'), 1800);
  } else {
    showNotifToast('Pinned message is earlier in history', 'info');
  }
}

async function unpinCurrentCommMessage() {
  if (!selectedGroup && !selectedChannel) return;
  const isGroup = currentChatMode === 'group';
  const coll = isGroup ? 'groups' : 'channels';
  const id = isGroup ? selectedGroup.id : selectedChannel.id;

  try {
    await db.collection(coll).doc(id).update({
      pinnedMessage: firebase.firestore.FieldValue.delete()
    });
    if (isGroup && selectedGroup) delete selectedGroup.pinnedMessage;
    if (!isGroup && selectedChannel) delete selectedChannel.pinnedMessage;
    syncCommPinnedBanner();
    showNotifToast('✓ Message unpinned', 'info');
  } catch (e) {
    showNotifToast('Failed to unpin message: ' + e.message, 'error');
  }
}

async function pinGroupMsg(msgId) {
  closeCtxMenu();
  if (!selectedGroup) return;
  const isAdmin = selectedGroup.ownerUid === currentUser.uid || (selectedGroup.admins || []).includes(currentUser.uid);
  if (!isAdmin) {
    showNotifToast('Only group admins can pin messages', 'error');
    return;
  }

  const msg = currentGroupMessages.find(m => m.id === msgId);
  if (!msg) return;

  const payload = {
    id: msgId,
    authorName: msg.senderName || 'Member',
    text: msg.text || '',
    hasImage: !!msg.image,
    hasVideo: !!msg.video,
    pinnedAt: Date.now(),
    pinnedBy: currentUser.uid
  };

  try {
    await db.collection('groups').doc(selectedGroup.id).update({
      pinnedMessage: payload
    });
    selectedGroup.pinnedMessage = payload;
    syncCommPinnedBanner();
    showNotifToast('📌 Message pinned to group!', 'success');
  } catch (e) {
    showNotifToast('Failed to pin message: ' + e.message, 'error');
  }
}

async function pinChannelPost(postId) {
  if (!selectedChannel) return;
  const isAdmin = selectedChannel.ownerUid === currentUser.uid || (selectedChannel.admins || []).includes(currentUser.uid);
  if (!isAdmin) {
    showNotifToast('Only channel admins can pin posts', 'error');
    return;
  }

  const isCurrentlyPinned = selectedChannel.pinnedMessage?.id === postId;
  if (isCurrentlyPinned) {
    await unpinCurrentCommMessage();
    return;
  }

  const card = document.getElementById(`post_${postId}`);
  const postText = card ? (card.querySelector('.channel-post-text')?.textContent || '').trim() : '';

  const payload = {
    id: postId,
    authorName: selectedChannel.name || 'Channel',
    text: postText,
    pinnedAt: Date.now(),
    pinnedBy: currentUser.uid
  };

  try {
    await db.collection('channels').doc(selectedChannel.id).update({
      pinnedMessage: payload
    });
    selectedChannel.pinnedMessage = payload;
    syncCommPinnedBanner();
    showNotifToast('📌 Post pinned to channel!', 'success');
  } catch (e) {
    showNotifToast('Failed to pin post: ' + e.message, 'error');
  }
}

async function loadGroupSharedMedia() {
  if (!selectedGroup) return;
  const grid = document.getElementById('groupMediaGrid');
  const countEl = document.getElementById('groupMediaCount');
  if (!grid) return;

  grid.innerHTML = '<div class="comm-media-empty">Loading media…</div>';
  try {
    const snap = await db.collection('groups').doc(selectedGroup.id).collection('messages')
      .orderBy('createdAt', 'desc')
      .limit(60)
      .get();

    const mediaList = [];
    snap.forEach(doc => {
      const d = doc.data();
      if (d.image || d.video) {
        mediaList.push({ id: doc.id, ...d });
      }
    });

    if (countEl) countEl.textContent = mediaList.length;
    if (!mediaList.length) {
      grid.innerHTML = '<div class="comm-media-empty">No photos or videos shared yet</div>';
      return;
    }

    grid.innerHTML = mediaList.slice(0, 8).map(m => {
      if (m.image) {
        return `
          <div class="comm-media-thumb" onclick="openProfilePic('${escapeHtml(m.image)}')">
            <img src="${escapeHtml(m.image)}" loading="lazy">
          </div>
        `;
      } else if (m.video) {
        return `
          <div class="comm-media-thumb" onclick="openProfilePic('${escapeHtml(m.video)}')">
            <video src="${escapeHtml(m.video)}" preload="metadata"></video>
          </div>
        `;
      }
      return '';
    }).join('');
  } catch (e) {
    grid.innerHTML = '<div class="comm-media-empty">Failed to load media</div>';
  }
}

async function loadChannelSharedMedia() {
  if (!selectedChannel) return;
  const grid = document.getElementById('channelMediaGrid');
  const countEl = document.getElementById('channelMediaCount');
  if (!grid) return;

  grid.innerHTML = '<div class="comm-media-empty">Loading media…</div>';
  try {
    const snap = await db.collection('channels').doc(selectedChannel.id).collection('posts')
      .orderBy('createdAt', 'desc')
      .limit(40)
      .get();

    const mediaList = [];
    snap.forEach(doc => {
      const d = doc.data();
      if (d.image || d.video) {
        mediaList.push({ id: doc.id, ...d });
      }
    });

    if (countEl) countEl.textContent = mediaList.length;
    if (!mediaList.length) {
      grid.innerHTML = '<div class="comm-media-empty">No media broadcast yet</div>';
      return;
    }

    grid.innerHTML = mediaList.slice(0, 8).map(m => {
      if (m.image) {
        return `
          <div class="comm-media-thumb" onclick="openProfilePic('${escapeHtml(m.image)}')">
            <img src="${escapeHtml(m.image)}" loading="lazy">
          </div>
        `;
      } else if (m.video) {
        return `
          <div class="comm-media-thumb" onclick="openProfilePic('${escapeHtml(m.video)}')">
            <video src="${escapeHtml(m.video)}" preload="metadata"></video>
          </div>
        `;
      }
      return '';
    }).join('');
  } catch (e) {
    grid.innerHTML = '<div class="comm-media-empty">Failed to load media</div>';
  }
}

function formatCount(num) {
  if (!num) return '0';
  if (num >= 1000000) return (num / 1000000).toFixed(1) + 'M';
  if (num >= 1000) return (num / 1000).toFixed(1) + 'k';
  return String(num);
}

// ═══════════════════════════════════════════════════════════════════════
// FEATURE 1: INTERACTIVE POLLS (Groups)
// ═══════════════════════════════════════════════════════════════════════

function openPollModal() {
  if (!selectedGroup && !selectedUser) {
    showNotifToast('Select a chat or group to create a poll', 'info');
    return;
  }
  const modal = document.getElementById('pollCreateModal');
  if (!modal) return;
  document.getElementById('pollQuestionInput').value = '';
  const container = document.getElementById('pollOptionsContainer');
  container.innerHTML = `
    <input type="text" class="nexa-poll-option-input" placeholder="Option 1" maxlength="100">
    <input type="text" class="nexa-poll-option-input" placeholder="Option 2" maxlength="100">
  `;
  document.getElementById('pollMultiVoteCheck').checked = false;
  modal.classList.add('active');
  if (window.lucide) lucide.createIcons();
}

function closePollModal() {
  const modal = document.getElementById('pollCreateModal');
  if (modal) modal.classList.remove('active');
}

function addPollOption() {
  const container = document.getElementById('pollOptionsContainer');
  const count = container.querySelectorAll('.nexa-poll-option-input').length;
  if (count >= 10) {
    showNotifToast('Maximum 10 options allowed', 'info');
    return;
  }
  const inp = document.createElement('input');
  inp.type = 'text';
  inp.className = 'nexa-poll-option-input';
  inp.placeholder = `Option ${count + 1}`;
  inp.maxLength = 100;
  container.appendChild(inp);
  inp.focus();
}

async function submitPoll() {
  if (!selectedChannel && !selectedGroup && !selectedUser) return;
  const question = (document.getElementById('pollQuestionInput').value || '').trim();
  if (!question) {
    showNotifToast('Please enter a question', 'error');
    return;
  }
  const optInputs = document.querySelectorAll('#pollOptionsContainer .nexa-poll-option-input');
  const options = [];
  optInputs.forEach(inp => {
    const v = (inp.value || '').trim();
    if (v) options.push(v);
  });
  if (options.length < 2) {
    showNotifToast('At least 2 options are required', 'error');
    return;
  }
  const multiVote = document.getElementById('pollMultiVoteCheck').checked;

  closePollModal();

  // Build poll options with empty vote arrays
  const pollOptions = {};
  options.forEach((opt, i) => {
    pollOptions[`opt_${i}`] = { text: opt, voters: [] };
  });

  const pollPayload = {
    type: 'poll',
    pollQuestion: question,
    pollOptions: pollOptions,
    pollMultiVote: multiVote,
    text: `📊 Poll: ${question}`
  };

  if (currentChatMode === 'channel' && selectedChannel) {
    await sendChannelPostWithExtras(pollPayload);
    showNotifToast('✓ Poll broadcast!', 'success');
  } else if (currentChatMode === 'group' && selectedGroup) {
    await sendGroupMessageWithExtras(pollPayload);
    showNotifToast('✓ Poll created in group!', 'success');
  } else if (selectedUser) {
    await db.collection("chats").add(baseMsg(pollPayload));
    showNotifToast('✓ Poll sent!', 'success');
    sendPushNotification(document.getElementById('myName').textContent || 'Nexa User', `📊 Poll: ${question}`, selectedUser.uid);
  }
}

async function votePollOption(msgId, optKey) {
  if (!currentUser) return;
  let msgRef;
  if (currentChatMode === 'channel' && selectedChannel) {
    msgRef = db.collection('channels').doc(selectedChannel.id).collection('posts').doc(msgId);
  } else if (currentChatMode === 'group' && selectedGroup) {
    msgRef = db.collection('groups').doc(selectedGroup.id).collection('messages').doc(msgId);
  } else {
    msgRef = db.collection('chats').doc(msgId);
  }
  try {
    await db.runTransaction(async tx => {
      const doc = await tx.get(msgRef);
      if (!doc.exists) return;
      const data = doc.data();
      if (data.type !== 'poll') return;
      const opts = data.pollOptions || {};
      const multiVote = !!data.pollMultiVote;
      const uid = currentUser.uid;

      // Check if already voted on this option -> toggle off
      if (opts[optKey] && Array.isArray(opts[optKey].voters) && opts[optKey].voters.includes(uid)) {
        opts[optKey].voters = opts[optKey].voters.filter(u => u !== uid);
      } else {
        // If single vote, remove from all other options first
        if (!multiVote) {
          for (const k of Object.keys(opts)) {
            if (opts[k] && Array.isArray(opts[k].voters)) {
              opts[k].voters = opts[k].voters.filter(u => u !== uid);
            }
          }
        }
        if (!opts[optKey]) opts[optKey] = { text: '', voters: [] };
        if (!Array.isArray(opts[optKey].voters)) opts[optKey].voters = [];
        opts[optKey].voters.push(uid);
      }

      tx.update(msgRef, { pollOptions: opts });
    });
  } catch (err) {
    console.error('votePollOption error:', err);
    showNotifToast('Vote failed: ' + err.message, 'error');
  }
}

function buildPollHTML(id, msg) {
  const opts = msg.pollOptions || {};
  const question = linkify(msg.pollQuestion || '');
  const multiVote = !!msg.pollMultiVote;
  const uid = currentUser ? currentUser.uid : '';

  let totalVotes = 0;
  for (const k of Object.keys(opts)) {
    totalVotes += (Array.isArray(opts[k]?.voters) ? opts[k].voters.length : 0);
  }

  let optionsHtml = '';
  const sortedKeys = Object.keys(opts).sort();
  for (const k of sortedKeys) {
    const opt = opts[k];
    if (!opt || !opt.text) continue;
    const voters = Array.isArray(opt.voters) ? opt.voters : [];
    const voted = uid && voters.includes(uid);
    const count = voters.length;
    const pct = totalVotes > 0 ? Math.round((count / totalVotes) * 100) : 0;

    optionsHtml += `
      <div class="poll-option-row ${voted ? 'voted' : ''}" onclick="votePollOption('${id}', '${k}')">
        <div class="poll-option-bar" style="width: ${pct}%;"></div>
        <div class="poll-option-check"></div>
        <div class="poll-option-text">${linkify(opt.text)}</div>
        <div class="poll-option-pct">${pct}%</div>
      </div>
    `;
  }

  return `
    <div class="poll-card">
      <div class="poll-type-badge">📊 ${multiVote ? 'Select multiple' : 'Select one'}</div>
      <div class="poll-question">${question}</div>
      ${optionsHtml}
      <div class="poll-total-votes">${totalVotes} vote${totalVotes !== 1 ? 's' : ''}</div>
    </div>
  `;
}


// ═══════════════════════════════════════════════════════════════════════
// FEATURE 2: FORWARD MESSAGES
// ═══════════════════════════════════════════════════════════════════════

let forwardingMessage = null;
let forwardSelectedTargets = [];
let inviteModalMode = null; // 'invite' when the forward modal is used asthe "Invite to Chat" picker
let inviteModalSourceKind = null; // 'group' | 'channel'

function openForwardModal(msgId) {
  closeCtxMenu();
  // Find message (including older pages loaded via scroll-up paging)
  let msg = null;
  if (currentChatMode === 'group') {
    msg = currentGroupMessages.find(m => m.id === msgId);
  } else {
    msg = allMessages.find(m => m.id === msgId)
      || _olderMsgsA.find(m => m.id === msgId)
      || _olderMsgsB.find(m => m.id === msgId)
      || (window._nexaChannelPostsCache || []).find(m => m.id === msgId);
  }
  if (!msg) { showNotifToast('Message not found', 'error'); return; }

  forwardingMessage = { ...msg, id: msgId };
  forwardSelectedTargets = [];

  const modal = document.getElementById('forwardModal');
  if (!modal) return;

  const searchInput = document.getElementById('forwardSearchInput');
  if (searchInput) searchInput.value = '';

  inviteModalMode = null;
  const title = document.querySelector('#forwardModal .comm-modal-title');
  const search = document.getElementById('forwardSearchInput');
  const sendBtn = document.getElementById('forwardSendBtn');
  if (title) title.innerHTML = '<i data-lucide="forward" style="width: 15px; height: 15px; vertical-align: -2px;"></i> Forward To';
  if (search) search.placeholder = 'Search contacts or groups…';
  if (sendBtn) sendBtn.textContent = 'Forward';
  renderForwardTargets('');
  modal.classList.add('active');
  if (window.lucide) lucide.createIcons();
}

// Open the forward modal as an "Invite to Chat" contact picker for groups/channels
function openGroupInvitePicker() {
  openInvitePicker('group');
}

function openChannelInvitePicker() {
  openInvitePicker('channel');
}

function openInvitePicker(kind) {
  if (kind === 'group' && !selectedGroup) { showNotifToast('No group selected', 'error'); return; }
  if (kind === 'channel' && !selectedChannel) { showNotifToast('No channel selected', 'error'); return; }
  closeCtxMenu();

  forwardingMessage = null;
  forwardSelectedTargets = [];
  inviteModalMode = 'invite';
  inviteModalSourceKind = kind;

  const modal = document.getElementById('forwardModal');
  if (!modal) return;

  const title = document.querySelector('#forwardModal .comm-modal-title');
  const search = document.getElementById('forwardSearchInput');
  const sendBtn = document.getElementById('forwardSendBtn');
  if (title) title.textContent = kind === 'group' ? '👥 Invite to Group' : '📢 Invite to Channel';
  if (search) search.placeholder = '🔍 Search contacts to invite…';
  if (sendBtn) sendBtn.textContent = 'Send Invite';

  renderForwardTargets('');
  modal.classList.add('active');
  if (window.lucide) lucide.createIcons();
}

function closeForwardModal() {
  const modal = document.getElementById('forwardModal');
  if (modal) modal.classList.remove('active');
  forwardingMessage = null;
  forwardSelectedTargets = [];
  inviteModalMode = null;
  inviteModalSourceKind = null;
}

function renderForwardTargets(filterText) {
  const list = document.getElementById('forwardTargetsList');
  if (!list) return;
  const q = (filterText || '').toLowerCase().trim();

  let targets = [];

  // Add contacts
  (allUsersData || []).forEach(u => {
    if (u.uid === currentUser.uid) return;
    const name = u.displayName || u.name || 'User';
    if (q && !name.toLowerCase().includes(q)) return;
    targets.push({
      id: u.uid,
      name: name,
      photo: u.photo || 'https://i.imgur.com/HeIi0wU.png',
      type: 'contact'
    });
  });

if (inviteModalMode === 'invite') {
  if (!targets.length) { list.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-3);font-size:12px;">No contacts found</div>'; return; }
  list.innerHTML = targets.map(t => {
    const sel = forwardSelectedTargets.includes(t.type + ':' + t.id);
    return `
      <div class="forward-target-row ${sel ? 'selected' : ''}" onclick="toggleForwardTarget('${t.id}', '${t.type}')">
        <img src="${escapeHtml(t.photo)}" class="forward-target-av" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
        <div class="forward-target-name">${escapeHtml(t.name)}</div>
        <div class="forward-target-type">Contact</div>
        <div class="forward-check"></div>
      </div>
    `;
  }).join('');
  return;
}
  // Add groups the user is in (live myGroups feeds window._userGroups)
  const fwdGroups = (window._userGroups && window._userGroups.length ? window._userGroups : myGroups);
  (fwdGroups || []).forEach(g => {
    const name = g.name || 'Group';
    if (q && !name.toLowerCase().includes(q)) return;
    targets.push({
      id: g.id,
      name: name,
      photo: g.avatarUrl || 'https://i.imgur.com/HeIi0wU.png',
      type: 'group'
    });
  });

  // Add channels the user follows
  (myChannels || []).forEach(c => {
    const name = c.name || 'Channel';
    if (q && !name.toLowerCase().includes(q)) return;
    targets.push({
      id: c.id,
      name: name,
      photo: c.avatarUrl || 'https://i.imgur.com/HeIi0wU.png',
      type: 'channel'
    });
  });

  if (!targets.length) {
    list.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-3);font-size:12px;">No contacts or groups found</div>';
    return;
  }

  list.innerHTML = targets.map(t => {
    const selected = forwardSelectedTargets.includes(t.type + ':' + t.id);
    return `
      <div class="forward-target-row ${selected ? 'selected' : ''}" onclick="toggleForwardTarget('${t.id}', '${t.type}')">
        <img src="${escapeHtml(t.photo)}" class="forward-target-av" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
        <div class="forward-target-name">${escapeHtml(t.name)}</div>
        <div class="forward-target-type">${t.type === 'group' ? '👥 Group' : t.type === 'channel' ? '📢 Channel' : '👤 Contact'}</div>
        <div class="forward-check"></div>
      </div>
    `;
  }).join('');
}

function filterForwardTargets(val) {
  renderForwardTargets(val);
}

function toggleForwardTarget(id, type) {
  const key = type + ':' + id;
  const idx = forwardSelectedTargets.indexOf(key);
  if (idx >= 0) {
    forwardSelectedTargets.splice(idx, 1);
  } else {
    forwardSelectedTargets.push(key);
  }

  const btn = document.getElementById('forwardSendBtn');
  if (btn) btn.disabled = forwardSelectedTargets.length === 0;

  // Re-render to reflect selection state
  const searchVal = document.getElementById('forwardSearchInput')?.value || '';
  renderForwardTargets(searchVal);
}

function cleanPayload(obj) {
  const out = {};
  for (const k of Object.keys(obj)) {
    if (obj[k] !== undefined) out[k] = obj[k];
  }
  return out;
}

async function executeForward() {
  if (inviteModalMode === 'invite') {
    const kind = inviteModalSourceKind;
    let count = 0;
    for (const key of forwardSelectedTargets) {
      const sepIdx = key.indexOf(':');
      const targetId = key.slice(sepIdx + 1);
      try {
        if (kind === 'group') await sendGroupInviteToChat(targetId);
        else if (kind === 'channel') await sendChannelInviteToChat(targetId);
        count +=  1;
      } catch (e) {}
    }
    closeForwardModal();
    showNotifToast(`✓ Invite sent to ${count} chat${count === 1 ? '' : 's'}`, count ? 'success' : 'error');
    return;
  }

  if (!forwardingMessage || !forwardSelectedTargets.length) return;
  const msg = forwardingMessage;
  const targets = forwardSelectedTargets.slice();

  closeForwardModal();

  let successCount = 0;
  let firstError = ''
  for (const key of targets) {
    const sepIdx = key.indexOf(':');
    const targetId = key.slice(sepIdx + 1);
    const targetType = key.slice(0, sepIdx);
    try {
      if (targetType === 'group') {
        // Forward to group
        const senderName = document.getElementById('myName')?.textContent || currentUser.displayName || 'Member';
        const payload = {
          senderUid: currentUser.uid,
          senderName: senderName,
          senderAvatar: currentUser.photoURL || 'https://i.imgur.com/HeIi0wU.png',
          createdAt: Date.now(),
          forwarded: true,
          forwardedFrom: msg.senderName || msg.from || 'Unknown'
        };
        if (msg.text) payload.text = msg.text;
        if (msg.image) payload.image = msg.image;
        if (msg.video) payload.video = msg.video;
        if (msg.audio) payload.audio = msg.audio;
        if (msg.fileUrl) { payload.fileUrl = msg.fileUrl; payload.fileName = msg.fileName; payload.fileSize = msg.fileSize; }
        if (msg.caption) payload.caption = msg.caption;
        if (msg.type === 'poll') {
          payload.type = 'poll';
          payload.pollQuestion = msg.pollQuestion;
          // Reset voters on forward
          const newOpts = {};
          for (const k of Object.keys(msg.pollOptions || {})) {
            newOpts[k] = { text: msg.pollOptions[k].text, voters: [] };
          }
          payload.pollOptions = newOpts;
          payload.pollMultiVote = msg.pollMultiVote;
        }

        await db.collection('groups').doc(targetId).collection('messages').add(cleanPayload(payload));
        const snippet = payload.text || (payload.image ? '📷 Photo' : payload.video ? '🎥 Video' : 'Attachment');
        await db.collection('groups').doc(targetId).update({
          lastMessage: '⤳ ' + snippet,
          lastMessageTime: Date.now(),
          lastSenderName: senderName,
          updatedAt: Date.now()
        });
      } else if (targetType === 'channel') {
        // Forward to channel (visible to subscribers in the feed)
        const senderName = document.getElementById('myName')?.textContent || currentUser.displayName || 'Member';
        const payload = {
          authorUid: currentUser.uid,
          authorName: senderName,
          senderAvatar: currentUser.photoURL || 'https://i.imgur.com/HeIi0wU.png',
          channelId: targetId,
          createdAt: Date.now(),
          forwarded: true,
          forwardedFrom: msg.senderName || msg.from || 'Unknown'
        };
        if (msg.text) payload.text = msg.text;
        if (msg.image) payload.image = msg.image;
        if (msg.video) payload.video = msg.video;
        if (msg.audio) payload.audio = msg.audio;
        if (msg.fileUrl) { payload.fileUrl = msg.fileUrl; payload.fileName = msg.fileName; payload.fileSize = msg.fileSize; }
        if (msg.caption) payload.caption = msg.caption;
        if (msg.type === 'poll') {
          payload.type = 'poll';
          payload.pollQuestion = msg.pollQuestion;
          // Reset voters on forward
          const newOpts = {};
          for (const k of Object.keys(msg.pollOptions || {})) {
            newOpts[k] = { text: msg.pollOptions[k].text, voters: [] };
          }
          payload.pollOptions = newOpts;
          payload.pollMultiVote = msg.pollMultiVote;
        }
        await db.collection('channels').doc(targetId).collection('posts').add(cleanPayload(payload));
        const snippet = payload.text || (payload.image ? '📷 Photo' : payload.video ? '🎥 Video' : 'Attachment');
        await db.collection('channels').doc(targetId).update({
          lastPost: '⤳ ' + snippet,
          lastPostTime: Date.now(),
          lastPosterName: senderName,
          updatedAt: Date.now()
        });
      } else {
        // Forward to 1:1 contact
        const payload = {
          from: currentUser.uid,
          to: targetId,
          createdAt: Date.now(),
          read: false,
          forwarded: true,
          forwardedFrom: msg.senderName || msg.from || 'Unknown'
        };
        if (msg.text) payload.text = msg.text;
        if (msg.image) payload.image = msg.image;
        if (msg.video) payload.video = msg.video;
        if (msg.audio) payload.audio = msg.audio;
        if (msg.fileUrl) { payload.fileUrl = msg.fileUrl; payload.fileName = msg.fileName; payload.fileSize = msg.fileSize; }
        if (msg.caption) payload.caption = msg.caption;

        await db.collection('chats').add(cleanPayload(payload));
      }
      successCount++;
    } catch (err) {
      console.error('Forward error:', err);
      if (!firstError) firstError = (err && err.message) || String(err);
    }
  }

  if (successCount > 0) {
    showNotifToast(`✓ Forwarded to ${successCount} chat${successCount > 1 ? 's' : ''}`, 'success');
  } else {
    showNotifToast('Forward failed' + (firstError ? ': ' + firstError : ''), 'error');
  }
}


// ═══════════════════════════════════════════════════════════════════════
// FEATURE 3: STARRED MESSAGES
// ═══════════════════════════════════════════════════════════════════════

function getStarredMsgsKey() {
  return `nexa_starred_${currentUser?.uid || 'anon'}`;
}

function getStarredMessages() {
  try {
    return JSON.parse(localStorage.getItem(getStarredMsgsKey()) || '[]');
  } catch { return []; }
}

function saveStarredMessages(arr) {
  localStorage.setItem(getStarredMsgsKey(), JSON.stringify(arr));
}

function isMessageStarred(msgId) {
  return getStarredMessages().some(s => s.id === msgId);
}

function toggleStarMessage(msgId) {
  closeCtxMenu();
  let msg = null;
  if (currentChatMode === 'group') {
    msg = currentGroupMessages.find(m => m.id === msgId);
  } else {
    msg = allMessages.find(m => m.id === msgId);
  }
  if (!msg) return;

  const starred = getStarredMessages();
  const idx = starred.findIndex(s => s.id === msgId);

  if (idx >= 0) {
    starred.splice(idx, 1);
    saveStarredMessages(starred);
    showNotifToast('Unstarred', 'info');
  } else {
    starred.push({
      id: msgId,
      text: msg.text || (msg.image ? '📷 Photo' : msg.video ? '🎥 Video' : msg.audio ? '🎤 Voice' : 'Attachment'),
      from: msg.from || msg.senderUid || '',
      fromName: msg.senderName || (msg.from === currentUser.uid ? 'You' : 'User'),
      createdAt: msg.createdAt || Date.now(),
      chatMode: currentChatMode,
      groupId: selectedGroup?.id || null,
      channelId: selectedChannel?.id || null,
      targetUid: selectedUser?.uid || null
    });
    // Cap at 200
    if (starred.length > 200) starred.splice(0, starred.length - 200);
    saveStarredMessages(starred);
    showNotifToast('⭐ Starred', 'success');
  }
}

function openStarredDrawer() {
  const drawer = document.getElementById('starredDrawer');
  if (!drawer) return;
  renderStarredMessages();
  drawer.classList.add('active');
  if (window.lucide) lucide.createIcons();
}

function closeStarredDrawer() {
  const drawer = document.getElementById('starredDrawer');
  if (drawer) drawer.classList.remove('active');
}

function renderStarredMessages() {
  const list = document.getElementById('starredMessagesList');
  if (!list) return;

  const starred = getStarredMessages();
  if (!starred.length) {
    list.innerHTML = '<div class="comments-empty">No starred messages yet. Long-press or right-click a message and tap ⭐ Star to save it here.</div>';
    return;
  }

  list.innerHTML = starred.slice().reverse().map(s => {
    const authorName = s.fromName || (s.from === currentUser?.uid ? 'You' : 'User');
    const time = s.createdAt ? new Date(s.createdAt).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '';
    const snippet = escapeHtml(typeof s.text === 'string' ? s.text.substring(0, 120) : 'Attachment');

    return `
      <div class="starred-msg-card" onclick="jumpToStarredMsg('${s.id}', '${s.chatMode || ''}', '${s.groupId || ''}', '${s.targetUid || ''}')">
        <div class="starred-msg-header">
          <span class="starred-msg-author">${escapeHtml(authorName)}</span>
          <span>
            <span class="starred-msg-time">${time}</span>
            <button class="starred-msg-unstar" onclick="event.stopPropagation(); unstarFromDrawer('${s.id}')" title="Unstar"><i data-lucide="x" style="width: 13px; height: 13px;"></i></button>
          </span>
        </div>
        <div class="starred-msg-text">${snippet}</div>
      </div>
    `;
  }).join('');
}

function unstarFromDrawer(msgId) {
  const starred = getStarredMessages();
  const idx = starred.findIndex(s => s.id === msgId);
  if (idx >= 0) {
    starred.splice(idx, 1);
    saveStarredMessages(starred);
    renderStarredMessages();
    showNotifToast('Unstarred', 'info');
  }
}

function jumpToStarredMsg(msgId, chatMode, groupId, targetUid) {
  closeStarredDrawer();
  // Try to scroll to it if in current chat
  const el = document.querySelector(`[data-msg-id="${msgId}"]`);
  if (el) {
    el.scrollIntoView({ behavior: 'smooth', block: 'center' });
    el.classList.add('pinned-target-flash');
    setTimeout(() => el.classList.remove('pinned-target-flash'), 2000);
  } else {
    showNotifToast('Message is in another chat', 'info');
  }
}


// ═══════════════════════════════════════════════════════════════════════
// FEATURE 4: @MENTIONS IN GROUPS
// ═══════════════════════════════════════════════════════════════════════

let mentionActive = false;
let mentionStartPos = -1;

function handleMentionInput(e) {
  if (currentChatMode !== 'group' || !selectedGroup) {
    hideMentionsPopup();
    return;
  }
  const ta = document.getElementById('text');
  if (!ta) return;

  const val = ta.value;
  const cursorPos = ta.selectionStart;

  // Find the @ nearest before cursor
  let atIdx = -1;
  for (let i = cursorPos - 1; i >= 0; i--) {
    if (val[i] === '@') { atIdx = i; break; }
    if (val[i] === ' ' || val[i] === '\n') break;
  }

  if (atIdx >= 0) {
    const query = val.substring(atIdx + 1, cursorPos).toLowerCase();
    mentionActive = true;
    mentionStartPos = atIdx;
    showMentionsPopup(query);
  } else {
    hideMentionsPopup();
  }
}

function showMentionsPopup(query) {
  const popup = document.getElementById('mentionsPopup');
  const list = document.getElementById('mentionsList');
  if (!popup || !list) return;

  const members = currentGroupMembersList || [];
  const filtered = members.filter(m => {
    if (m.uid === currentUser.uid) return false;
    const uData = (allUsersData || []).find(u => u.uid === m.uid) || {};
    const name = (uData.displayName || uData.name || '').toLowerCase();
    return !query || name.includes(query);
  });

  if (!filtered.length) {
    hideMentionsPopup();
    return;
  }

  list.innerHTML = filtered.slice(0, 8).map(m => {
    const uData = (allUsersData || []).find(u => u.uid === m.uid) || {};
    const name = uData.displayName || uData.name || 'Member';
    const photo = uData.photo || 'https://i.imgur.com/HeIi0wU.png';
    return `
      <div class="mention-row" onclick="insertMention('${escapeHtml(name)}', '${m.uid}')">
        <img src="${escapeHtml(photo)}" class="mention-row-av" onerror="this.src='https://i.imgur.com/HeIi0wU.png'">
        <div class="mention-row-name">${escapeHtml(name)}</div>
      </div>
    `;
  }).join('');

  popup.style.display = 'block';
}

function hideMentionsPopup() {
  mentionActive = false;
  mentionStartPos = -1;
  const popup = document.getElementById('mentionsPopup');
  if (popup) popup.style.display = 'none';
}

function insertMention(name, uid) {
  const ta = document.getElementById('text');
  if (!ta) return;

  const val = ta.value;
  const cursorPos = ta.selectionStart;
  const before = val.substring(0, mentionStartPos);
  const after = val.substring(cursorPos);
  const mentionText = `@${name} `;

  ta.value = before + mentionText + after;
  const newPos = before.length + mentionText.length;
  ta.setSelectionRange(newPos, newPos);
  ta.focus();

  hideMentionsPopup();
}

// Render @mention highlights in message text
function renderMentionText(text) {
  if (!text) return '';
  // Match @Name patterns and highlight them
  return text.replace(/@(\w[\w\s]{0,30}?)(?=\s|$|[,.])/g, '<span class="mention-highlight">@$1</span>');
}


// ═══════════════════════════════════════════════════════════════════════
// FEATURE 5: MUTE NOTIFICATIONS (Per Group & Channel)
// ═══════════════════════════════════════════════════════════════════════

function getMutedChatsKey() {
  return `nexa_muted_${currentUser?.uid || 'anon'}`;
}

function getMutedChats() {
  try {
    return JSON.parse(localStorage.getItem(getMutedChatsKey()) || '{}');
  } catch { return {}; }
}

function saveMutedChats(obj) {
  localStorage.setItem(getMutedChatsKey(), JSON.stringify(obj));
}

function isChatMuted(chatId) {
  return !!getMutedChats()[chatId];
}

function toggleMuteGroup() {
  if (!selectedGroup) return;
  const muted = getMutedChats();
  const id = selectedGroup.id;
  if (muted[id]) {
    delete muted[id];
    showNotifToast('🔔 Group unmuted', 'success');
  } else {
    muted[id] = true;
    showNotifToast('🔕 Group muted', 'success');
  }
  saveMutedChats(muted);
  updateGroupMuteUI();
}

function toggleMuteChannel() {
  if (!selectedChannel) return;
  const muted = getMutedChats();
  const id = selectedChannel.id;
  if (muted[id]) {
    delete muted[id];
    showNotifToast('🔔 Channel unmuted', 'success');
  } else {
    muted[id] = true;
    showNotifToast('🔕 Channel muted', 'success');
  }
  saveMutedChats(muted);
  updateChannelMuteUI();
}

function updateGroupMuteUI() {
  if (!selectedGroup) return;
  const isMuted = isChatMuted(selectedGroup.id);
  const label = document.getElementById('groupMuteLabel');
  if (label) label.textContent = isMuted ? 'Unmute' : 'Mute';
}

function updateChannelMuteUI() {
  if (!selectedChannel) return;
  const isMuted = isChatMuted(selectedChannel.id);
  const label = document.getElementById('channelMuteLabel');
  if (label) label.textContent = isMuted ? 'Unmute' : 'Mute';
}


// ═══════════════════════════════════════════════════════════════════════
// FEATURE 6: GROUP INVITE LINK (Enhanced — generate unique code)
// ═══════════════════════════════════════════════════════════════════════

function generateInviteCode() {
  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
  let code = '';
  for (let i = 0; i < 12; i++) code += chars.charAt(Math.floor(Math.random() * chars.length));
  return code;
}

async function getOrCreateGroupInviteLink() {
  if (!selectedGroup) return '';
  const groupRef = db.collection('groups').doc(selectedGroup.id);
  const doc = await groupRef.get();
  const data = doc.data();

  if (data.inviteCode) {
    return `${window.location.origin}${window.location.pathname}?joinGroup=${data.inviteCode}`;
  }

  // Generate and save
  const code = generateInviteCode();
  await groupRef.update({ inviteCode: code });
  return `${window.location.origin}${window.location.pathname}?joinGroup=${code}`;
}

async function resetGroupInviteLink() {
  if (!selectedGroup) return;
  const isAdmin = selectedGroup.ownerUid === currentUser.uid || (selectedGroup.admins || []).includes(currentUser.uid);
  if (!isAdmin) { showNotifToast('Only admins can reset the invite link', 'error'); return; }

  if (!confirm('Reset the invite link? The old link will stop working.')) return;

  const code = generateInviteCode();
  await db.collection('groups').doc(selectedGroup.id).update({ inviteCode: code });
  showNotifToast('✓ Invite link reset', 'success');
}


// ═══════════════════════════════════════════════════════════════════════
// INTEGRATION: Wire features into existing UI
// ═══════════════════════════════════════════════════════════════════════

// --- Enhanced showCtxMenu with Forward, Star, Poll items ---
const _origShowCtxMenu = showCtxMenu;
showCtxMenu = function(e, id, msg) {
  const menu = document.getElementById("ctxMenu");
  const fromMe = (msg.from || msg.senderUid) === currentUser.uid;
  const isGroup = currentChatMode === 'group';
  // Group admins/creator may delete ANY group message; senders may delete
  // their own. Outside groups only the sender can delete.
  const canDeleteGroupMsg = isGroup && selectedGroup
    ? (fromMe || selectedGroup.ownerUid === currentUser.uid || (selectedGroup.admins || []).includes(currentUser.uid))
    : fromMe;
  const starred = isMessageStarred(id);
  menu.innerHTML = `
    <div class="ctx-reaction-bar">
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '❤')` : `addQuickReact('${id}', '❤')`}">❤</button>
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '😂')` : `addQuickReact('${id}', '😂')`}">😂</button>
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '😮')` : `addQuickReact('${id}', '😮')`}">😮</button>
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '😢')` : `addQuickReact('${id}', '😢')`}">😢</button>
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '🙏')` : `addQuickReact('${id}', '🙏')`}">🙏</button>
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '👍')` : `addQuickReact('${id}', '👍')`}">👍</button>
      <button class="ctx-emoji-btn" onclick="${isGroup ? `toggleGroupMsgReact('${id}', '🔥')` : `addQuickReact('${id}', '🔥')`}">🔥</button>
    </div>
    <div class="ctx-divider"></div>
    <div class="ctx-item" onclick="replyMsg('${id}')"><i data-lucide="corner-up-left" style="width: 15px; height: 15px;"></i> Reply</div>
    <div class="ctx-item" onclick="openForwardModal('${id}')"><i data-lucide="forward" style="width: 15px; height: 15px;"></i> Forward</div>
    <div class="ctx-item" onclick="toggleStarMessage('${id}')"><i data-lucide="${starred ? 'star-off' : 'star'}" style="width: 15px; height: 15px;"></i> ${starred ? 'Unstar' : 'Star'}</div>
    ${msg.text ? `<div class="ctx-item" onclick="copyMsg('${id}')"><i data-lucide="copy" style="width: 15px; height: 15px;"></i> Copy</div>` : ''}
    ${isGroup ? `<div class="ctx-item" onclick="pinGroupMsg('${id}')"><i data-lucide="pin" style="width: 15px; height: 15px;"></i> Pin</div>` : ''}
    ${fromMe && !isGroup && msg.text ? `<div class="ctx-item" onclick="startEdit('${id}')"><i data-lucide="pencil" style="width: 15px; height: 15px;"></i> Edit</div>` : ''}
    ${canDeleteGroupMsg ? `<div class="ctx-item danger" onclick="${isGroup ? `deleteGroupMsg('${id}')` : `deleteMsg('${id}')`}"><i data-lucide="trash-2" style="width: 15px; height: 15px;"></i> Delete</div>` : ''}
  `;
  menu.classList.add("active");
  menu.style.top = Math.min(e.clientY, window.innerHeight - 280) + "px";
  menu.style.left = Math.min(e.clientX, window.innerWidth - 220) + "px";
  if (window.lucide) lucide.createIcons();
  setTimeout(() => document.addEventListener("click", closeCtxMenu, { once: true }), 0);
};

// --- Wire @mentions into textarea ---
(function wireMentions() {
  const ta = document.getElementById('text');
  if (!ta) return;
  ta.addEventListener('input', handleMentionInput);
  ta.addEventListener('keydown', function(e) {
    if (e.key === 'Escape' && mentionActive) {
      e.preventDefault();
      hideMentionsPopup();
    }
  });
})();

// --- Update mute UI when opening group/channel info ---
const _origOpenGroupInfo = openGroupInfo;
openGroupInfo = async function() {
  await _origOpenGroupInfo();
  updateGroupMuteUI();
};

const _origOpenChannelInfo = openChannelInfo;
openChannelInfo = function() {
  _origOpenChannelInfo();
  updateChannelMuteUI();
};

// --- Check and handle group invite URL param (?joinGroup=CODE) ---
async function checkChannelInviteUrlParam() {
  if (!currentUser) return;
  const params = new URLSearchParams(window.location.search);
  const channelId = params.get('joinChannel');
  if (!channelId) return;

  try {
    window.history.replaceState({}, document.title, window.location.pathname);
  } catch (e) {}

  const chan = await _resolveChannelData(channelId);
  if (!chan) {
    showNotifToast('Invalid or expired channel invite link', 'error');
    return;
  }

  const isSubscribed = (chan.subscriberUids || []).includes(currentUser.uid);
  if (isSubscribed) {
    showNotifToast(`Opening "${chan.name}"…`, 'info');
    selectChannelFeed(chan);
    switchTab('community');
    switchCommunitySubTab('channels');
    return;
  }

  const confirmFollow = confirm(`You have been invited to follow the channel "${chan.name}". Do you want to follow?`);
  if (!confirmFollow) return;

  await db.collection('channels').doc(channelId).collection('subscribers').doc(currentUser.uid).set({
    uid: currentUser.uid,
    joinedAt: Date.now()
  });
  await db.collection('channels').doc(channelId).update({
    subscriberUids: firebase.firestore.FieldValue.arrayUnion(currentUser.uid),
    subscribersCount: firebase.firestore.FieldValue.increment(1)
  });

  showNotifToast(`✓ Following "${chan.name}"!`, 'success');
  selectChannelFeed({ ...chan, subscriberUids: [...(chan.subscriberUids || []), currentUser.uid] });
  switchTab('community');
  switchCommunitySubTab('channels');
}

async function checkGroupInviteUrlParam() {
  if (!currentUser) return;
  const params = new URLSearchParams(window.location.search);
  const inviteCode = params.get('joinGroup');
  if (!inviteCode) return;

  // Clean URL parameter without page reload
  try {
    window.history.replaceState({}, document.title, window.location.pathname);
  } catch (e) {}

  try {
    const snap = await db.collection('groups').where('inviteCode', '==', inviteCode).limit(1).get();
    if (snap.empty) {
      showNotifToast('Invalid or expired group invite link', 'error');
      return;
    }

    const doc = snap.docs[0];
    const group = { id: doc.id, ...doc.data() };
    
    // Check if user is already a member
    const memSnap = await db.collection('groups').doc(group.id).collection('members').doc(currentUser.uid).get();
    if (memSnap.exists) {
      showNotifToast(`Opening "${group.name}"…`, 'info');
      switchTab('community');
      switchCommunitySubTab('groups');
      selectGroupChat(group);
      return;
    }

    const confirmJoin = confirm(`You have been invited to join the group "${group.name}". Do you want to join?`);
    if (!confirmJoin) return;

    const myName = document.getElementById('myName')?.textContent || currentUser.displayName || 'Member';
    await db.collection('groups').doc(group.id).collection('members').doc(currentUser.uid).set({
      uid: currentUser.uid,
      displayName: myName,
      photo: currentUser.photoURL || 'https://i.imgur.com/HeIi0wU.png',
      role: 'member',
      joinedAt: Date.now()
    });

    await db.collection('groups').doc(group.id).update({
      membersCount: firebase.firestore.FieldValue.increment(1)
    });

    showNotifToast(`✓ Joined "${group.name}"!`, 'success');
    switchTab('community');
    switchCommunitySubTab('groups');
    group.membersCount = (group.membersCount || 0) + 1;
    selectGroupChat(group);
  } catch (err) {
    console.error('Error joining group via invite link:', err);
    showNotifToast('Failed to join group: ' + err.message, 'error');
  }
}

// Expose groups list for forward feature
window._userGroups = window._userGroups || [];

// Consolidated Header Info Button Click Handler
function handleHeaderInfoClick() {
  if (currentChatMode === 'group' && selectedGroup) {
    if (typeof openGroupInfo === 'function') openGroupInfo();
  } else if (currentChatMode === 'channel' && selectedChannel) {
    if (typeof openChannelInfo === 'function') openChannelInfo();
  } else if (selectedUser) {
    if (typeof openChatInfo === 'function') openChatInfo();
  }
}
