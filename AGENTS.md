...
- **WhatsApp-style sticker ON a photo (the correct model):** stickers are NOT posted on their own — you pick a photo/video/text in the story composer, then tap a sticker button (`attachStorySticker('addYours'|'question')` in the `.story-sticker-bar`) to ATTACH a sticker onto that story. `pendingStorySticker = {type, text, id}` is shown as a removable chip (`.story-sticker-chip`). `shareStatus` saves it as `statusData.sticker = {type, text, id}` (and, for backward-compat, also writes the matching `promptId`/`promptText` for addYours or `questionId`/`questionText` for question). The story stays a normal image/video/text story with a URL. In the viewer, `renderStorySlide` renders a small `.story-sticker-overlay` card riding on the image (bottom-left); tapping it: addYours → `openAddYoursComposer(sticker.id, sticker.text)` so the viewer posts their OWN photo with the same prompt; question → `answerStoryQuestion(story)` (reads the question from `story.sticker.text`). `handleStoryContentClick` ignores `.story-sticker-overlay`/`.addyours-sticker` so taps don't toggle pause. Legacy standalone `type:"prompt"`/`type:"question"` imageless stories still render. **Don't re-add standalone ➕/❓ tabs that post the sticker alone** — that's the bug the user reported ("only the sticker posts on its own nothing else").
