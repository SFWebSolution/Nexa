...
- **"Ask a Question" stories:** a story type `type:"question"` with `questionId` + `questionText`, created via the ❓ tab (`switchStatusTab('question')`). Viewing renders a purple question overlay with a "💬 Type your answer" CTA; tapping calls `answerStoryQuestion(story)` which prompts the viewer and sends the answer to the asker as a `chats` doc carrying `storyReplyId`/`storyReplyUid` (so it renders via the existing story-reply preview card) + `storyQuestionId`/`storyQuestionText`/`storyQuestionAnswer`. **Gotcha:** `switchStatusTab` must map tab keys to exact element ids via a `tabIdMap` — the ➕ button's id is `tabAddYours` (capital Y), NOT `tabAddyours`. The old `getElementById("tab"+capitalize(tab))` produced `tabAddyours` → null → threw → the whole Add Yours button silently did nothing. Keep the map.
